...
try:
    ...
    print('Please install the Python scikit-learn package to use GLIMPS')
    raise
...
class GLIMPS(object):
    ...
    def _get_tr(self, traj, ref):
        ...
        if has_mdtraj:
            for i, x in enumerate(traj):
                trans[i], rot[i] = alignment.compute_translation_and_rotation(
                    x, ref)
        else:
            tr = ref.mean(axis=0)
            for i, x in enumerate(traj):
                tx = x.mean(axis=0)
                rotn, rssd = R.align_vectors(ref-tr, x - tx)
                bb = rotn.apply(x)
                trans[i] = (ref - bb).mean(axis=0)
                rot[i] = rotn.as_matrix()
    ...
    def transform(self, X):
        ...
        if n_dims < 2 or n_dims > 3:
            raise ValueError('Error: X must be an [n_atoms, 3]' +
                             ' or [n_samples, n_atoms, 3] array')
        one_frame = n_dims == 2
        if one_frame:
            X = np.array([X])
        if X.shape[2] != 3:
            raise ValueError('Error: X must be an [n_atoms, 3] or' +
                             ' [n_samples, n_atoms, 3] array')
        if X.shape[1] != self.pca_x.n_atoms:
            raise ValueError('Error: X contains {} atoms,'.format(X.shape[1]) +
                             ' was expecting {}'.format(self.pca_x.n_atoms))
    ...
    def inverse_transform(self, Y):
        ...
        n_dims = len(Y.shape)
        if n_dims < 2 or n_dims > 3:
            raise ValueError('Error: Y must be an [n_atoms, 3]' +
                             ' or [n_samples, n_atoms, 3] array')
        one_frame = n_dims == 2
        if one_frame:
            Y = np.array([Y])
        if Y.shape[2] != 3:
            raise ValueError('Error: Y must be an [n_atoms, 3] or' +
                             ' [n_samples, n_atoms, 3] array')
        if Y.shape[1] != self.pca_y.n_atoms:
            raise ValueError('Error: Y contains {} atoms,'.format(Y.shape[1]) +
                             ' was expecting {}'.format(self.pca_y.n_atoms))
...
class TrajGLIMPS(object):
    ...
    def __init__(self, chunksize=100, refine=True, x_valence=4, y_valence=4):
        if not has_mdtraj:
            print('Please install the Python MDTraj package to use these features')
            raise
...
class SolMapper(object):
    ...
    def fit(self, cg_traj, fg_traj):
        ...
        Train the mapper
        """
        nrf = fg_traj.topology.n_residues
        nrc = cg_traj.topology.n_residues
        self.n_merge = nrf // nrc
        ...
        while not done:
            ...
            d = cdist(fg_traj.xyz[0, rand_at:rand_at+1],
                      fg_traj.xyz[0, ::r_atoms])[0]

            n_clust = 10
            n_map = r_atoms * n_clust
            r_neighs = np.argsort(d)[:n_map] * r_atoms
            kmeans = KMeans(n_clusters=n_clust).fit(fg_traj.xyz[0, r_neighs])
            clusters = []
...
def _sol_cg(t_fg, cg_res, n_merge):
    ...
    r_atoms = t_fg.topology.residue(0).n_atoms  # number of atoms per residue
    t = t_fg.atom_slice(range(0, t_fg.n_atoms, r_atoms))
    # crds of every (n_merge-1)th 1st atoms:
    sample_xyz = t.xyz[:, ::(n_merge - 1)]
    ...
    for iframe in range(len(sample_xyz)):
        ...
        tmp_indx[:2 * n_cull:2] = -1  # remove 1 atom from each too-close pair
        ...
        for line in f.readlines():
            if line[0] == '#':
                ...
                if len(w) == 2:
                    ...
                    res_indices = [r.index for r in
                                   traj.topology.subset(atom_indices).residues]
    ...
    for i in range(len(trajfiles)):
        ...
        for j in range(1, len(indices[i])):
            if j != jlast + 1:
                ...
                selexpr = 'resid {} to {}'.format(jstart, jlast)
                sel = trajfiles[i].topology.select(selexpr)
