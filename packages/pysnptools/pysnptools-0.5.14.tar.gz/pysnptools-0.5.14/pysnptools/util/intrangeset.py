...
class IntRangeSet(object):
    ...
    **Methods and Operators**
    ...
    def __init__(self, *ranges_inputs):
        ...
        """
        if len(ranges_inputs) > 0 and isinstance(
            ranges_inputs[0], IntRangeSet
        ):  # Because we know self is empty, optimize for the case in which the first item is a IntRangeSet
    ...
    def __eq__(self, other):  # !!  'others' to ranges_input
        ...
        * ``a == b``
        ...
        for i, start in enumerate(self._start_items):
            if (
                start != other._start_items[i]
                or self._start_to_length[start] != other._start_to_length[start]
            ):
                return False
        return True
        ...
        * ``a >= b``
        ...
        for start_in, stop_in in IntRangeSet._static_ranges(*ranges_inputs):
            (
                ...
                index,
                contains,
            ) = self._best_start_length_index_contains(start_in)
    ...
    def _test():
        ...
        assert (
            IntRangeSet("-10:-4,-3,-2:2,1:6,7:13,13:16,14:17,20:26")
            == "-10:-4,-3:6,7:17,20:26"
        )
        assert (
        ...
        assert IntRangeSet("1:4").union("2,4,6", "1,6,-100") == IntRangeSet(
            "-100,1:5,6"
        )
        ...
        assert IntRangeSet("-2,1:4,20,25:100,101") & IntRangeSet(
            "1:101"
        ) == IntRangeSet("1:4,20,25:100")
        assert IntRangeSet("2:4,90:110") & IntRangeSet("1:100") == IntRangeSet(
            "2:4,90:100"
        )
        assert IntRangeSet("1:100") & IntRangeSet(
            "-2,1:4,20,25:100,100"
        ) == IntRangeSet("1:4,20,25:100")
        assert IntRangeSet("1:100") & IntRangeSet("2:4,90:110") == IntRangeSet(
            "2:4,90:100"
        )
        ...
        assert IntRangeSet("30:100,300") - IntRangeSet("0:67,100,200") == IntRangeSet(
            "67:100,300"
        )

        assert IntRangeSet("30:100,300") ^ IntRangeSet("0:67,99,200") == IntRangeSet(
            "0:30,67:99,200,300"
        )
        ...
        assert IntRangeSet("10:15,55:60") + IntRangeSet("58:61,100") == IntRangeSet(
            "10:15,55:61,100"
        )
        assert IntRangeSet("10:15,55:60") + "58:61,100" == IntRangeSet(
            "10:15,55:61,100"
        )
        ...
        assert (
            IntRangeSet("10:15,55:60").index("57,56:57") == "6:8"
        )  # returns the index of the start of the contiguous place where 57,"56-56" occurs
        ...
        assert IntRangeSet() is not None
        ...
        assert list(reversed(IntRangeSet("10:15,55:60"))) == list(
            reversed(list(IntRangeSet("10:15,55:60")))
        )
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, numbers.Integral):
            ...
            start_and_stop_generator = (
                self._two_index(start_index, stop_index)
                for start_index, stop_index in IntRangeSet._static_ranges(key)
            )
    ...
    def index(self, other):
        ...
        if isinstance(other, numbers.Integral):
            ...
            start_and_stop_index_generator = (
                (self._index_element(start), self._index_element(stop - 1) + 1)
                for start, stop in IntRangeSet._static_ranges(other)
            )
    ...
    def _index_element(self, element):
        ...
        if (
            index != len(self._start_items) and self._start_items[index] == element
        ):  # element is start value
        ...
        preceeding_lengths = (
            self._start_to_length[start] for start in preceeding_starts
        )
    ...
    def isdisjoint(self, ranges):
        ...
        """
        isempty_generator = (
            IntRangeSet(tuple)._binary_intersection(self).isempty
            for tuple in IntRangeSet._static_ranges(ranges)
        )
    ...
    def intersection(*ranges_inputs):
        ...
        * ``a.intersection(b)``
        ...
        IntRangeSet('4,6')
        """
        ranges_inputs = IntRangeSet._make_args_range_set(
            *ranges_inputs
        )  # generator to made every ranges a IntRangeSet
        ranges_inputs = sorted(
            ranges_inputs, key=lambda int_range_set: len(int_range_set._start_items)
        )  # sort so that IntRangeSet with smaller range_count is first
    ...
    def _binary_intersection(self, other):
        ...
        while True:
            start1, length1, index1, contains = other._best_start_length_index_contains(
                start0
            )
            ...
            if contains:
                if (
                    stop0 <= stop1
                ):  # All of range0 fits inside some range1, so add it the intersection and next look at the next range0
    ...
    def __sub__(
        self, *ranges_inputs
    ):  # !!could be made faster by being more direct instead of using complements
        """
    ...
    def __xor__(self, ranges):
        ...
        * ``a.symmetric_difference(b)``
        ...
        result = self - ranges
        diff_generator = (
            IntRangeSet(tuple) - self for tuple in IntRangeSet._static_ranges(ranges)
        )
    ...
    def remove(self, *ranges_inputs):
        ...
        for start_stop_tuple in IntRangeSet._static_ranges(*ranges_inputs):
            if start_stop_tuple not in self:
                raise KeyError()
        ...
        for iterable in iterables:
            if isinstance(iterable, numbers.Integral):
                ...
                assert (
                    ...
                    and isinstance(iterable[1], int)
                ), "Tuples must contain exactly two int elements that represent the start (inclusive) and stop (exclusive) elements of a range."
                ...
                assert (
                    ...
                    and start < stop
                    and step > 0
                ), "With slice, start and stop must be nonnegative numbers, stop must be more than start, and step, if given, must be at least 1"
                if step == 1:
                    yield start, stop
                else:
                ...
                if iterable == "":
                    ...
                    for range_string in iterable.split(","):
                        match = IntRangeSet._rangeExpression.match(
                            range_string
                        )  # LATER is there a good error message if it is not well formed?
                        if match is None:
                            raise Exception(
                                "The string is not well-formed. '{0}'".format(
                                    range_string
                                )
                            )
                ...
                raise Exception(
                    "Don't know how to construct a IntRangeSet from '{0}'".format(
                        iterable
                    )
                )
    ...
    def _internal_add(self, start, length=1):  # !! should it be "length" or "stop"
        ...
        while stop >= next:
            ...
            self._start_to_length[previous] = (
                new_stop - previous
            )  # ItemToLength[previous] + ItemToLength[next]
    ...
    def _best_start_length_index_contains(self, element):
        ...
        if (
            index != len(self._start_items) and self._start_items[index] == element
        ):  # element is the start element of some range
            ...
            )  # we already know element is greater than start
    ...
    def _internal_isub(self, start_in, length_in=1):
        # !! should it be "length" or "stop"?
        assert length_in > 0, "Length must be greater than zero"
        assert len(self._start_items) == len(self._start_to_length)
        stop_in = start_in + length_in

        # return the range that has the largest start and for which start<=element
        start0, length0, index0, contains0 = self._best_start_length_index_contains(
            start_in
        )
        if length_in > 1:
            start1, length1, index1, contains1 = self._best_start_length_index_contains(
                stop_in - 1
            )
        else:
        ...
        if not contains0:  # unchanged
            ...
            if not contains1:  # unchanged
                ...
                self._delete_ranges(
                    index0 + 1, index1 + 1
                )  # delete any middle and the last ranges
            else:  # changed
                assert index0 < index1, "real assert"
                self._shorten_last_range(
                    stop_in, start1, length1, index1
                )  # shorten last range
            ...
            if not contains1:  # unchanged
                self._delete_ranges(
                    index0, index1 + 1
                )  # delete start range and any middle ranges
            elif start1 + length1 == stop_in:  # deleted
                self._delete_ranges(
                    index0, index1 + 1
                )  # delete start range and any middle ranges and last range
            else:  # changed
                assert index0 <= index1, "real assert"
                self._shorten_last_range(
                    stop_in, start1, length1, index1
            ...
            # Is the end of last range unchanged, changed, or deleted?
            if not contains1:  # unchanged
                self._shorten_first_range(
                    start_in, start0, length0
                )  # shorten first range
                ...
                self._delete_ranges(
                    index0 + 1, index1 + 1
                )  # delete any middle ranges and last range
            else:  # changed
