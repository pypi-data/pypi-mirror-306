...
def snp_gen(
    ...
    for population_index, freq_pop in enumerate([freq_pop_0, 1.0 - freq_pop_0]):
        ...
        snps_parents = _generate_snps(
            ancestral, fst, int(iid_solo_count * freq_pop), sid_count
        )
        snp_list.append(snps_parents)
        snp_list.append(
            _generate_kids(
                parent_snps=snps_parents,
    ...
    snp_list.append(
        _generate_kids(
            parent_snps=np.concatenate(snp_list),
    ...
    if not label_with_pop:
        iid = np.array(
            [
                ["f_{0}".format(iid_index), "i_{0}".format(iid_index)]
                for iid_index in range(val.shape[0])
            ],
            dtype=str,
        ).reshape(-1, 2)
    else:
        ...
        iid0 = [
            ["0", str(iid_index)]
            for iid_index in range(len(snp_list[0]) + len(snp_list[1]))
        ]  # parents and children of pop 0
        iid1 = [
            ["1", str(iid_index)]
            for iid_index in range(len(snp_list[2]) + len(snp_list[3]))
        ]  # parents and children of pop 1
        iid2 = [
            ["2", str(iid_index)] for iid_index in range(len(snp_list[4]))
        ]  # children with parents in any pop
        iid = np.array(iid0 + iid1 + iid2, dtype=str).reshape(-1, 2)

    sid = np.array(
        ["snp_{0}".format(sid_index) for sid_index in range(val.shape[1])], dtype="str"
    )
    ...
    assert (
        len(sid) == 0 or chr_count > 0
    ), "chr_count must be at least 1 (unless sid_count is 0)"
    sid_per_chrom = int(np.ceil(float(len(sid)) / max(1, chr_count)))
    pos = np.array(
        list(
            [
                1 + sid_index // sid_per_chrom,
                1 + sid_index % sid_per_chrom,
                1 + sid_index % sid_per_chrom,
            ]
    ...
    snpdata = SnpData(
        ...
        val=val,
        pos=pos,
        parent_string="snp_gen(fst={0}, dfr={1}, iid_count={2}, sid_count={3}, maf_low={4}, maf_high={5}, seed={6}, sibs_per_family={7}, freq_pop_0={8})".format(
            fst,
            dfr,
            iid_count,
            sid_count,
...
def _generate_snps(ancestral, fst, iid_count, sid_count):
    ...
    if fst == 0.0:
        ...
        alpha = np.random.beta(
            ancestral * (1.0 - fst) / fst,
            (1.0 - ancestral) * (1.0 - fst) / fst,
            sid_count,
        )

    # generate from population frequencies
    snps = np.zeros(
        (iid_count, sid_count), dtype="int8"
    )  # .zeros not .empty because will be adding 1's to it
...
def _generate_kids(
    parent_snps, family_count, sibs_per_family
):  # !!! should it be sibs, kids, or children
    """
    ...
    for copy_index in range(2):  # "2" for diploid
        sample = parent_snps[
            parent_permutation[
                copy_index * family_count : (copy_index + 1) * family_count
            ],
            :,
        ]  # sample each allele
        for kid_index in range(sibs_per_family):
            rand = np.random.random((family_count, sid_count))
            snps[kid_index * family_count : (kid_index + 1) * family_count][
                rand < 0.5 * sample
            ] += 1
    return snps
...
def _generate_phenotype(snp_data, causals, genetic_var, noise_var, seed=None):
    ...
    if seed is not None:
        np.random.seed(
            int(seed % 2147483647)
        )  # !!!Better to use numpy.random.RandomState instead (look for other places in code, too) #old maxint
    ...
    try:
        ...
        causal_idx = np.random.choice(
            np.arange(snp_data.sid_count if num_causal > 0 else 1),
            size=num_causal,
            replace=False,
        )
    ...
    X_causal = 1.0 / np.sqrt(X_causal.shape[1]) * X_causal
    W = (
        np.random.randn(num_causal, num_phenotypes) * np.sqrt(genetic_var) + mean
...
class TestGenerate(unittest.TestCase):
    ...
    def gen_and_compare(self, output_file, **kwargs):
        ...
        bed = Bed(
            self.currentFolder + "/../../tests/datasets/generate/" + output_file,
            count_A1=False,
        )
    ...
    def test_gen2b(self):
        ...
        gen_snpdata = self.gen_and_compare(
            "gen2b.bed",
            fst=0.1,
            dfr=0.5,
            iid_count=200,
        ...
        ref_snpdata = Bed(
            self.currentFolder + "/../../tests/datasets/generate/gen2.bed",
            count_A1=False,
        ).read()
        assert (
            gen_snpdata != ref_snpdata
        ), "Expect different seeds to produce different results"
    ...
    def test_pheno1(self):
        ...
        gen_snpdata = SnpData(
            ...
            val=_generate_phenotype(
                some_snp_data, 10, genetic_var=0.5, noise_var=0.5, seed=5
            ).reshape(-1, 1),
        )
        # SnpNpz.write(r'c:\deldir\pheno1.snp.npz',gen_snpdata)
        ref_snpdata = SnpNpz(
            self.currentFolder + "/../../tests/datasets/generate/pheno1.snp.npz"
        ).read()
        assert gen_snpdata == ref_snpdata
            ...
            for sid_count in [0, 10, 5, 3, 2, 1]:
                for chr_count in [30, 10, 5, 3, 2, 1, 0]:
                    if chr_count == 0 and sid_count > 0:
                        continue  # not break
                    logging.debug(
                        "{0}, {1}, {2}".format(iid_count, sid_count, chr_count)
                    )
                    snpdata = snp_gen(
                        fst=0.1,
                        dfr=0.5,
