...
class _DibLib(object):
    def __init__(self, unique_name, dib_path, dir_path, dib_name):
        ...
        self.dibs_pattern = re.compile(r"^{0}_.*\.txt$".format(dib_name))
        if dib_name == "dibs":  # could make two subclasses, but this seems simpler
            ...
            raise Exception(
                "Don't know what test goes with dib_name of '{0}'".format(dib_name)
            )

    def tree_copy(self, priority, dibs_time, try_index):
        copy_count = len(
            [f for f in self.dir_path.walk() if PeerToPeer._copy_main_pattern.match(f)]
        )  # count main + copies
        logging.info(
            "PeerToPeer priority is {0} and copy_count is {1}".format(
                priority, copy_count
            )
        )
    ...
    def only_first_goes(self, priority, dibs_time, try_index):
        ...
        if priority == 0:
            logging.info(
                "Priority was > 0, but there is now a p2p file to copy, so the wait is over"
            )
            return "fixed"
        for f in self.dir_path.walk():
            if (
                PeerToPeer._copy_main_pattern.match(f)
                and self.dir_path._simple_getmtime(f) > dibs_time
            ):
                logging.info(
                    "Priority > 0, but there is now a p2p file to copy, so the wait is over"
                )
    ...
    def wait_for_turn(self):
        ...
        time.sleep(
            1.0
        )  # Sleep one second to (hopefully) see dibs made at exactly the same time.
        dibs_time = self.dib_path.getmtime(self.dibs_fn)
        sleep_time = 5.0
        for try_index in self.ever():
            priority = self.get_priority(
                dibs_time
            )  # Count number of remaining dibs files that were created before mine. That is my priority.
            ...
            sleep_time = min(
                sleep_time * random.uniform(1.0, 1.2), 60
            )  # wait 0 to 10% longer, but no more than a minute
    ...
    def get_priority(self, dibs_time):
        priority = 0
        sequence = (
            file for file in self.dib_path.walk() if self.dibs_pattern.match(file)
        )
        for other_dib in sequence:
            ...
            if (
                other_time is None
                or other_time < dibs_time
                or (other_time == dibs_time and other_dib < self.dibs_fn)
            ):
                priority += 1
        return priority
