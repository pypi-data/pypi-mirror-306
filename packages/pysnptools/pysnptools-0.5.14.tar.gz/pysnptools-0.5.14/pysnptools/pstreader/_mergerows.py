...
class _MergeRows(PstReader):  # !!!why does this start with _
    def __init__(self, reader_list, cache_file=None, skip_check=False):
        ...
        self._repr_string = "_MergeRows({0})".format(
        ...
        if cache_file is not None:
            if not os.path.exists(cache_file):
                ...
                np.savez(
                    ...
                    _row_count_list=self._row_count_list,
                )
            else:
    ...
    def _run_once(self):
        ...
        for reader_index, reader in enumerate(self.reader_list):
            if reader_index % 10 == 0:
                logging.info(
                    "_MergeRows looking at reader #{0}: {1}".format(
                        reader_index, reader
                    )
                )
            if not self.skip_check:
                assert np.array_equal(
                    col, reader.col
                ), "Expect columns to be the same across all files"
                np.testing.assert_equal(
                    col_property, reader.col_property
                ...
                assert (
                    len(row_set) == size_before + reader.row_count
                ), "Expect rows to be distinct in all files"
        ...
        if len(reader_and_row_index_list) == 0:
            ...
            logging.info(
                "Starting read from {0} subreaders".format(
                    len(reader_and_row_index_list)
                )
            )
            if order == "A" or order is None:
                order = "F"
            val = np.empty(
                (len(row_index), col_index_or_none_count), dtype=dtype, order=order
            )
            ...
            logging.info(
                "Ended read from {0} subreaders".format(len(reader_and_row_index_list))
            )
            return val
