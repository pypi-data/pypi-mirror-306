...
class PstNpz(PstReader):
    ...
    **Methods beyond** :class:`.NpzReader`
    ...
    def __init__(self, filename):
        ...
        super(
            PstNpz, self
        ).__init__()  # We don't send "file name" up because we know about super doesn't want it.
    ...
    def _run_once(self):
        ...
        with np.load(
            self._filename, allow_pickle=True
        ) as data:  # !! similar code in epistasis
            if (
                len(data.keys()) == 2 and "arr_0" in data.keys()
    ...
    def _read(
        ...
        with np.load(
            ...
            if (
                len(data.keys()) == 2 and "arr_1" in data.keys()
            ):  # for backwards compatibility
                val = data["arr_1"]
            else:
                val = data["val"]

        val, _ = self._apply_sparray_or_slice_to_val(
            val,
            row_index_or_none,
            col_index_or_none,
            order,
