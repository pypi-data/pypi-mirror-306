...
class _PstSubset(PstReader):
    ...
    def __repr__(self):
        s = "{0}[{1},{2}]".format(
            self._internal,
            _PstSubset.static_nice_string(self, self._row_indexer),
            _PstSubset.static_nice_string(self, self._col_indexer),
        )
        return s
    ...
    def _read(
        ...
        if hasattr(self._internal, "_read_accepts_slices"):
            assert (
                self._internal._read_accepts_slices
            ), "If an object has the _read_accepts_slices attribute, it must have value 'True'"
            composed_row_index_or_none = _PstSubset.compose_indexer_with_indexer(
                self._internal.row_count, self._row_indexer, self.row_count, row_indexer
            )
            composed_col_index_or_none = _PstSubset.compose_indexer_with_indexer(
                self._internal.col_count, self._col_indexer, self.col_count, col_indexer
            )
            val = self._internal._read(
                composed_row_index_or_none,
                composed_col_index_or_none,
                order,
                dtype,
                force_python_only,
                view_ok,
    ...
    def _run_once(self):
        ...
        self._row = self._internal.row[
            self._row_indexer
        ]  # !!! would be nice if these four calls were replaced by calls to, e.g. "def _col_index(self,col_indexer)", etc that could be overridden (by for example Pairs)
        self._col = self._internal.col[self._col_indexer]
        if self._row.dtype == self._col.dtype and np.array_equal(
            self._row, self._col
        ):  # When an object is square, keep the row and col the same object.
            self._col = self._row
    ...
    def static_nice_string(self, some_slice):
        if isinstance(some_slice, slice):
            return _PstSubset._slice_format[
                (
                    some_slice.start is not None,
                    some_slice.stop is not None,
                    some_slice.step is not None,
                )
            ].format(some_slice.start, some_slice.stop, some_slice.step)
