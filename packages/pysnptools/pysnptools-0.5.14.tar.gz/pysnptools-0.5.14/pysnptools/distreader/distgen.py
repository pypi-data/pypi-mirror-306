...
class DistGen(DistReader):
    ...
    **Constructor:**
    ...
    def _run_once(self):
        ...
        self._col_property = np.zeros(
            ((self._sid_count), 3)
        )  # Must be zero (not empty) because will leave _col_property[:,1] unchanged
    ...
    def _read(
        ...
        val = np.empty(
            (row_index_count, len(col_index), 3), order=order, dtype=dtype
        ...
        for ii, i in enumerate(
            ...
            val[:, a, :] = (
                ...
                else pstutil.sub_matrix(
                    batch_val, row_index_or_none, b, num_threads=num_threads
                )
            )
...
class TestDistGen(unittest.TestCase):
    def test1(self):
        ...
        ref = DistNpz(
            ...
            + "/../../tests/datasets/distgen.dist.npz"
        ).read()
