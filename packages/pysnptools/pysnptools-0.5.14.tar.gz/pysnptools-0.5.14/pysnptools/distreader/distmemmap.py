...
class TestDistMemMap(unittest.TestCase):
    def test1(self):
        ...
        distreader2.val[:, :, :] = [
            ...
            [[0, 1.0, 0], [0, 0.75, 0.25], [0.5, 0.5, 0]],
        ]
        assert np.array_equal(
            distreader2[[1], [1]].read(view_ok=True).val, np.array([[[0, 0.75, 0.25]]])
        )
        distreader2.flush()
        assert isinstance(distreader2.val, np.memmap)
        assert np.array_equal(
        ...
        assert np.array_equal(
            distreader3[[1], [1]].read(view_ok=True).val, np.array([[[0, 0.75, 0.25]]])
        )
