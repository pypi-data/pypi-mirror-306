...
from Xclient.asyncio_utils import asyncio_get_model_config, asyncio_wait_for_model_ready
from Xclient.exceptions import (
...
from Xclient.warnings import NotSupportedTimeoutWarning
from Xclient.model_config.triton_model_config import TritonModelConfig
...
class SequenceClient(ModelClient):
    """Sequence client for decoupled model deployed on the Triton Inference Server."""
    ...
    def step(self, *inputs, **named_inputs):
        _verify_inputs_args(inputs, named_inputs)
        ...
        if self.is_batching_supported:
            if inputs:
                ...
                named_inputs = {name: data[np.newaxis, ...] for name, data in named_inputs.items()}
        # print('inputs:',named_inputs['Boundary_conditions'].shape,named_inputs['Node_coordinates'].shape)
        if self._seq_start:
            self._seq_start = False
            result = self._infer(inputs or named_inputs,sequence_start=True) 
        else:
            result = self._infer(inputs or named_inputs,sequence_start=False) 
        return self._debatch_result(result)

    def end(self, *inputs, **named_inputs):
        _verify_inputs_args(inputs, named_inputs)
        if self.is_batching_supported:
            if inputs:
                inputs = tuple(data[np.newaxis, ...] for data in inputs)
                ...
                named_inputs = {name: data[np.newaxis, ...] for name, data in named_inputs.items()}
        if self._seq_start:
            result = self._infer(inputs or named_inputs,sequence_start=True,sequence_end=True) 
        else:
            result = self._infer(inputs or named_inputs,sequence_start=False,sequence_end=True) 
        # reset the sequence
    ...
    def _infer(self, inputs: _IOType, sequence_start=False, sequence_end=False):
        # if not self._lock.acquire(blocking=False):
        #     raise PyTritonClientInferenceServerError("Inference is already in progress")
        # print(inputs)
        inputs_wrapped, outputs_wrapped = self._create_request(inputs)
        ...
        try:
            ...
            response = self._infer_client.infer(
                ...
                outputs=outputs_wrapped,
                request_id=self._next_request_id,
                # correlation_id=self._next_request_id,
