...
from pydantic import BaseModel, field_serializer, AfterValidator, PlainSerializer, WithJsonSchema
...
from mailoxy.dmr import NotesInfo, PlayModeEnum, ActivityCodeEnum, ItemKindEnum
from mailoxy.other import DivingFish
...
def calculate_scoreRank(achievement: float) -> int:
    if achievement > 101.0000 or achievement < 0:
        raise ValueError("Invalid achievement value")
    ...
    for i in range(len(achievement_list)):
        if achievement >= achievement_list[i]:
            return len(achievement_list) - 1 - i
...
class MusicDetail(BaseModel):
    ...
    def to_diving_fish(self) -> DivingFish:
        return DivingFish(
            ...
            fs=self.syncStatus.name if self.scoreRank.value else "",
...
class MusicDetailRecord(BaseModel):
    ...
    def scr_base(self) -> int:
        return ((self.tapCriticalPerfect + self.tapPerfect) * 500 + self.tapGreat * 400 + self.tapGood * 250 +
                ...
                (self.slideCriticalPerfect + self.slidePerfect) * 1500 + self.slideGreat * 800 + self.slideGood * 500 +
                (self.breakCriticalPerfect + self.breakPerfect) * 2500 + self.breakGreat * 2000 + self.breakGood * 1000)
...
class UserActivity(BaseModel):
    ...
    def __UpsertPlayList(self, activityDetail: ActivityDetail):
        self.playList = [x for x in self.playList if x.id != activityDetail.id and not self.isToday(x.sortNumber)]
    ...
    def _MusicAchievement(self, acode: int, musicId: int, difficulty: int, level: int):
        if len([x for x in self.playList if x.id // 10 == acode // 10 and self.isToday(x.sortNumber)]) == 0:
            self.__addPlayList(ActivityDetail(kind=1, id=acode, param1=musicId, param2=difficulty, param3=level))
            return
        tempPlayList = [x for x in self.playList if x.id != acode and not self.isToday(x.sortNumber) and
                        x.id < acode or (x.id == acode and x.param3 < level)]
        if len(tempPlayList) < len(self.playList):
            self.playList = tempPlayList
            self.__addPlayList(ActivityDetail(kind=1, id=acode, param1=musicId, param2=difficulty, param3=level))

    def music_result(self, music_detail: MusicDetailRecord):
