...
def get_assests_path(dir_name: str, file_name: str) -> List[Path]:
    ...
    for part in listdir(base_path):
        ...
        part__path = part_dir / dir_name
        if not exists(part__path):
            continue
        for dirs in listdir(part__path):
            xml_path = part__path / dirs / file_name
            if exists(xml_path):
...
other_db = DBManager()\n\n""")
        ...
        #         if line_count <= 27:
        #             continue
        #         elif line == '\tpath: Path | None = None\n':
        #             continue
        #         elif line == 'class Parser:\n':
        #             break
        #         else:
        #             f.write(line)
