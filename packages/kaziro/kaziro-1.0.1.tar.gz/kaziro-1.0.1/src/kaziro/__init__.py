...
class Kaziro:
    ...
    class Order:
        ...
        def create_single_request(self, size: float, market_id: str) -> PlaceOrderResponse:
            order = OrderRequest(size=size, market_id=market_id, order_type="MARKET_REQUEST")
            return self.api.create_market_endpoint_v1_exchange_order_create_post([order])

        def create_single_reply(self, request_id: Optional[str], probability: float) -> PlaceOrderResponse:
            order = OrderRequest(request_id=request_id, probability=probability, order_type="MARKET_REPLY")
            return self.api.create_market_endpoint_v1_exchange_order_create_post([order])
