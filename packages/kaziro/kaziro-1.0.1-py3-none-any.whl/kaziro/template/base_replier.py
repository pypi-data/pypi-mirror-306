...
class BaseReplierTemplate:
    ...
    def start(self):
                    ...
                    if request_id and probability is not None and initializer and size:
                        ...
                        if validate_balance(probability, new_probability, size):
                            if validate_request(request_id):
                                create_order(request_id, new_probability)
                            else:
        ...
        def create_order(request_id: str, probability: float):
            try:
                response = self.kaziro.order.create_single_reply(request_id=request_id, probability=probability)
