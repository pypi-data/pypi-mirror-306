...
class DifferentialMorphologicalTree(torch.nn.Module):
    ...
    def __init__(self, trainset):
        ...
        for index, example in enumerate(trainset):
          ...
          if(trainset.isLoaded):
            ...
            #tree = ctl.ComponentTree(input, self.trainset.num_rows, self.trainset.num_cols, trainset.isMaxtree())
    ...
    def predict(self, batched_input):
        ...
        for input in batched_input:
            ...
            #tree = ctl.ComponentTree(input, self.trainset.num_rows, self.trainset.num_cols, self.trainset.isMaxtree())
            dic, attrs = ctl.Attribute.computerBasicAttributes(tree)
