...
def merge_fields(msg: str, msg_format: str, fields: list) -> dict:
    ...
    如果存在嵌套结构，将以递归方式进行合并。
    """
    parse_message = PARSERS.get(msg_format)
    if not fields:
        return parse_message(msg)
    # 将命令行传入的 `-a touser=xxx -a agentid=xxx`
    ...
    # 将 `{"event.task_id": "xxx"}`
    # 转化为 `{"event": {"task_id": xxx}}`
    unflat = unflatten(flat_dict, splitter="dot")
    result = recursive_update(
        default=parse_message(msg),
        custom=unflat
    )
    return result


def send_wx_msg(args) -> None:
    """
        发送消息到企业微信。
        ...
        参数 args 包含：
            - `message`: 字符串，待发送的消息内容。
            - `field`: 列表，附加字段以 "key=value" 格式添加到消息。
    ...
    try:
        ...
        corpapi.httpReq(
            url_type=WXWORK_SEND_APP_MESSAGE,
            params=message
        )
...
def main() -> None:
    """
        主函数，负责【初始化日志配置】，并根据参数【发送企业微信消息】。

        此函数首先根据提供的日志等级参数初始化日志系统。

        然后，它解析命令行参数，
        并调用 `async_send_wx_msg` 函数来处理消息发送的具体逻辑。
