...
"""
    ...
    在面向对象编程中，一个【抽象基类】（Abstract Base Class, ABC）
...
# 获取当前文件的绝对路径并将其作为系统路径加入 `sys.path`
sys.path.append(str(root_path))

# 在 py 代码中，PEP 8 是一个编码风格指南
# 其中 `E402` 错误意味着模块级别的导入语句没有出现在文件的顶部
...
class ApiException(Exception):
    """
        ...
        它的作用是在 API 调用过程中提供一种【标准化】的【错误处理】方法：
           ...
           - 通过在异常类中定义 `err_code` 和 `err_msg` 属性，
             ...
             因为第三方服务会返回特定的错误码和描述信息来表示不同类型的错误。
    ...
    def __init__(self, err_code: int, err_msg: str):
        ...
        :param err_code: 错误代码，一个【整数】表示特定的错误类型。
        :param err_msg: 错误信息，一个【字符串】描述错误的详细信息。
        """
...
class AbstractApis(ABC):
    """
        ...
        代码中定义的方法大多都是与【访问令牌】(Access Token)相关的操作。
    ...
    def get_token(self) -> str:
        """
            ...
            为了安全考虑，开发者请勿将 `access_token` 返回给前端，
            ...
            因此开发者，在使用业务接口前，要明确 `access_token` 的颁发来源，
            使用正确的 `access_token`。

            每个应用有【独立】的 secret，获取到的 `access_token` 只能【本应用使用】，
            所以每个应用的 `access_token` 应该分开来获取。

            开发者需要【缓存】 `access_token`，用于后续接口的调用
            （注意：不能频繁调用 `gettoken` 接口，否则会受到频率拦截）。

            当 `access_token` 失效或过期时，需要【重新获取】。

            `access_token` 的有效期通过返回的 `expires_in` 来传达，
            正常情况下为 7200 秒（2小时）。

            由于企业微信每个应用的 `access_token` 是彼此独立的，
            所以进行缓存时需要【区分应用】来进行存储。

            `access_token` 至少保留 512 字节的存储空间。

            企业微信可能会出于运营需要，提前使 `access_token` 失效，
            开发者应实现 `access_token` 失效时重新获取的逻辑。
    ...
    def httpReq(self, url_type: list or tuple, params: dict = None) -> dict:
        """
