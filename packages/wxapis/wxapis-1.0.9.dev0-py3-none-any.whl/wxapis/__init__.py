...
# -*- coding= utf-8 -*-
"""
    企业微信官方 API 文档提供了一套完整的接口，
    以支持开发者在企业微信平台上进行应用开发和数据交互。
...
def search_apis(kw=None):
    """
        在【当前模块】的 `__init__.py` 文件
        中定义的 API【端点】中进行搜索。

        该函数负责在预先定义的 API 端点集合中查找包含特定关键字的端点。
        ...
        注意:
            ...
            - 返回的列表中每个端点都是一个字典，格式为 `{ 端点名: 端点详情 }`。

            - 端点详情是一个包含 URL 路径和其他可能信息（如请求方法）的元组。

            - 如果搜索没有结果，会提供一条提示信息，并建议检查输入或拼写，
              或查阅官方文档。
