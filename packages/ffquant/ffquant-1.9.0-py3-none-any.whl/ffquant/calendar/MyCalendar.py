...
class MyCalendar:
    ...
    def isSymbolTradable(self, symbol: str, dt: str):
        ...
        if str(symbol).upper() == 'CAPITALCOM:HK50':
            closure_start_dt = dtime.replace(hour=5, minute=0, second=0, microsecond=0)
            closure_end_dt = dtime.replace(hour=6, minute=0, second=0, microsecond=0)
            return not (dtime >= closure_start_dt and dtime < closure_end_dt)
