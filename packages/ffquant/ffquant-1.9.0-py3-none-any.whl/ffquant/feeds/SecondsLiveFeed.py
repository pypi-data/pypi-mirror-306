...
class SecondsLiveFeed(bt.feeds.DataBase):
    ...
    def start(self):
        ...
        if self.p.backfill_size > 0:
            ...
            start_time = (end_time - timedelta(seconds=self.p.backfill_size * 5)).replace(second=0, microsecond=0)
    ...
    def _load(self):
        ...
        current_kline_time = (now.replace(second=0, microsecond=0) - timedelta(seconds=1 * 5)).astimezone(pytz.utc)
        ...
        if self.lines.datetime.idx == 0 or self.lines.datetime.datetime(-1).strftime('%Y-%m-%d %H:%M:%S') != current_kline_time_str:
            start_time = (now - timedelta(seconds=1 * 5)).replace(second=0, microsecond=0)
            end_time = now.replace(second=0, microsecond=0)
            start_time_str = start_time.strftime('%Y-%m-%d %H:%M:%S')
            end_time_str = end_time.strftime('%Y-%m-%d %H:%M:%S')
