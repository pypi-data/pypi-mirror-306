...
class Honeypot:
    ...
    def get_data_from_goplus(self, chain_id, token_id, should_update):
    ...
    @classmethod
    def get_data_from_honeypot(cls, chain_id, token_id, should_update):
        ...
        try:
            if response.status_code == 200:
                ...
                print(f"ERROR: In get_data_from_honeypot: {response.text}")
        except Exception as error:
    ...
    def get_data_from_quickintel(self, chain_id, token_id, should_update):
        if isinstance(chain_id, int):
            chain_id = id_to_chain.get(chain_id)
        ...
        if proxy:
            response = self.__session.post(url=url, proxies=proxy, data=json.dumps(payload), headers=headers)
            try:
                if response.status_code == 200:
                    data = response.json()
                    ...
                    print(f"ERROR: In get_data_from_quickintel: {response.text}")
            except Exception as error:
