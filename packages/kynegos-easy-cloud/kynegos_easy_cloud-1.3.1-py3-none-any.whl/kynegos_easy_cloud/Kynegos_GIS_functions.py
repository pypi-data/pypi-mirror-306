...
def process_gdf_for_bigquery(gdf, projected_crs_epsg=25830, swap_xy = False):
    ...
    def swap_coordinates(geom):
        if geom.is_empty:
            ...
            return LineString([(point.y, point.x) for point in geom.coords])
        elif geom.geom_type == 'Polygon':
            return Polygon([(point.y, point.x) for point in geom.exterior.coords])
        else:
            # Para Multi geometrías o tipos más complejos
            return geom.__class__([swap_xy(part) for part in geom])
