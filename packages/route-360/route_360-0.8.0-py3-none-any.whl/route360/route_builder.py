...
class RouteBuilder:
    def __init__(self,
        ...
        """
        config_path = pkg_resources.resource_filename('route360', 'config.json')
        ...
        self.loss_funct = loss_funct if loss_funct is not None else config.get("loss_funct")
        ...
        # MISTRAL jAlfXjYl3WSM5l1DqrxwENTxpsYWErPh
        # api_token='sk-None-YW6cZapcAda1tHICkMc0T3BlbkFJ9YEDXeYJWjN8uGyVGTJN'
        ...
        | |_) / _ \| | | | __/ _ \| || |_ __) |
        |  _ < (_) | |_| | ||  __/|__   _/ __/ 
        |_| \_\___/ \__,_|\__\___/   |_||_____|
                                                
        Low Latency, High Accuracy, Custom Query routing for Humans and Agents.
        """
    ...
    def _split_synthetic_data(self, synthetic_df, min_samples_per_label):
        ...
        # self.logger.info(user_samples.groupby('label').size())
        # self.logger.info(synthetic_samples.groupby('label').size())
        ...
        for label in synthetic_df['label'].unique():
            ...
            # self.logger.info(f"synthetic_samples_for_label, {synthetic_samples_for_label}")
            ...
            # self.logger.info(f"for {label} needed is {remaining_samples_needed}")
            ...
            for label in tqdm(labels, desc="Generating synthetic data"):
                ...
                try:
                    ...
                    if self.llm_provider == "ollama":
                        ...
                        time.sleep(1)
                    self.logger.info(f"Total for {label} - {len(examples)}")
    ...
    def _generate_adversarial_samples(
        ...
        # prompt += f"\nGenerate {number_of_synthetic_samples} or more adversarial i.e. lexically similar, realistic queries that are based on the samples above that sound like they belong in one of the above labels, BUT DO NOT fit into any of the provided labels above. Do not repeat the queries. A syntactically correct, well-formed JSON List as response is enough. No explanations or footnotes of any sort are needed."                        
        # prompt += f"\nGenerate {number_of_synthetic_samples} or more adversarial, realistic queries that are closely related to the labels above in the {domain} domain but do not fit any of the labels mentioned above. These queries should sound like they belong but do not correspond to any of the provided labels. Do not repeat the example queries. A syntactically correct, well-formed JSON List as response is enough. No explanations or footnotes of any sort are needed."                
    ...
    def _generate_additional_invalid_routes(self, prefix):
        ...
        if self.invalid_routes:
            ...
            add_invalid_routes_file = pkg_resources.resource_filename('route360', 'data/outliers_v1.csv')
    ...
    def _shuffle_dataset(self, dataset: Dataset) -> Dataset:
        ...
        """
        # return dataset.shuffle(seed=random.randint(0, 10000))
    ...
    def _calibrate_classifer(self, output_dir, val_text_embeddings, val_labels):
        try:
            ...
            classifier_path = os.path.join(output_dir, "route360_model", "model_head.pkl")
            ...
            self.logger.info(f"Confidence before/after calibration: {before_conf:.3f}/{after_conf:.3f}")
            
            calibrated_path = os.path.join(output_dir, "route360_model", "model_head_calibrated.pkl")
            joblib.dump(calibrated_model, calibrated_path)
            self.logger.info("Calibrated head saved successfully")
            self.logger.info(calibrated_model.classes_)
    ...
    def _train_and_evaluate(self, train_dataset, 
        ...
        if not self.only_oos_head:
            ...
            trainer.model.save_pretrained(os.path.join(output_dir, "route360_model"))    
        else:
            parts = model_name.split('/', 3)    
            output_dir =  '/'.join(parts[:3])

        contamination_pct = self.expected_oos_proportion
        # contamination_pct = 0.1 # auto
        ...
        if not self.only_oos_head:
            ...
            outlier_detector = OutlierDetector(contamination=contamination_pct, 
                                            output_dir=os.path.join(output_dir, "route360_model"), 
        ...
        vectordb = VectorDB()
        index_file_path = os.path.join(output_dir, "route360_model")
        ...
        # classifier_head = joblib.load(output_dir + "/route360_model/" + "model_head.pkl")
        # class_labels = classifier_head.classes_
        # y_val = np.array([np.where(class_labels == label)[0][0] for label in test_dataset["label"]])
        # X_val = trainer.model.model_body.encode(test_dataset["text"])
        # calibrated_classifier = CalibratedClassifierCV(base_estimator=classifier_head, method='sigmoid')
        # calibrated_classifier.fit(X_val, y_val)

        # joblib.dump(calibrated_classifier, output_dir + "/route360_model/" + "model_head_calibrated.pkl")
        # self.logger.info("Calibrated head saved.")
        ...
        if not self.skip_eval:
            ...
            for idx, model_path in enumerate(all_models, 1):
                ...
                model_dir = os.path.join(output_dir, "route360_model")
    ...
    # def _export_onnx(self, route360_model_path):
    #     """
    #     Exports the model to ONNX format in both FP32 and FP16 (if possible).
    #     """
        
    #     route360_model_path = os.path.join(route360_model_path, "route360_model")
    #     onnx_save_path = os.path.join(route360_model_path, "model_body.onnx")
    #     onnx_save_path_fp16 = os.path.join(route360_model_path, "model_body_fp16.onnx")
    #     quantized_model_path = os.path.join(route360_model_path, "model_body_quantized.onnx")
        
    #     # Export FP32 model
    #     setfit_model = SetFitModel.from_pretrained(route360_model_path, trust_remote_code=True)
    ...
    #             keep_io_types=True
    #         )
    #         onnx.save(fp16_model, onnx_save_path_fp16)
    #         self.logger.info(f"Successfully exported FP16 model to {onnx_save_path_fp16}")
            
    #         self._quantize_onnx_model(onnx_save_path_fp16, quantized_model_path)
    #         self.logger.info(f"Quantized FP16 model saved to {quantized_model_path}")
            
    #     except Exception as e:
    #         self.logger.warning(f"FP16 conversion failed: {str(e)}. Falling back to FP32 quantization.")
    #         self._quantize_onnx_model(onnx_save_path, quantized_model_path)
    #         self.logger.info(f"Quantized FP32 model saved to {quantized_model_path}")
    ...
    def build_routes(self):
        ...
        if not self._assess_data(train_dataset, self.min_samples):
            self.logger.error("Your train slice doesn't have enough samples, We recommend a minimum of 12 samples per route. Add more samples or choose ask route360 to generate synthetic data")
            sys.exit(0)

        train_dataset = self._shuffle_dataset(train_dataset)
        if eval_dataset:
            eval_dataset = self._shuffle_dataset(eval_dataset)
        self.logger.info(f"Training Route360 model using {self.model_name} for {self.epochs} epochs...")
        ...
        self.logger.info("Thank you for using Route360! May all your queries find their way.")


# route1 = Route("alarm", ['Snooze the alarm for 20 mins.', 'Set alarm to go off every 30 seconds for 20 minutes'])
# route2 = Route("reminder", ['Remind me to look for a dress for the wedding on Friday', 'Remind me to get dog food at 4:30 pm'])
# route3 = Route("weather", ['Whats the high for the next 3 days?', 'what is the temperature high and low for today'])
# build_request = RouteBuilderRequest([route1, route2, route3])
...
#             lr=2e-5,
#             loss_funct = "PairwiseArcFaceLoss",
#             min_samples=12,
#             batch_size =16,
...
#             skip_eval = False,
#             add_additional_invalid_routes = True,
