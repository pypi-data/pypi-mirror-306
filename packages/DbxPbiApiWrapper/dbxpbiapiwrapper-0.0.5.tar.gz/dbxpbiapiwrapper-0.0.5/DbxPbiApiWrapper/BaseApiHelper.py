    ...
    class PbiApiHandler:
        ...
        def getGroup(self, tokenObject: BaseValueObjects.TokenResponseObject):
            ...
            if (len(groupValueObject)) != 1:
                print(
                    "Expected to find {self.workspaceName} group, but the group does not exists."
                )
                return None
        ...
        def getDataset(
            ...
            if (len(datasetValueObject)) != 1:
                print(
                    f"Expected to find {self.datasetName} dataset, but the dataset does not exists."
                )
                return None
        ...
        def refreshInProgress(
            ...
            datasetValueObject = list(
                filter(lambda y: (y.status == "Unknown"), datasetObject.value)
            )
        ...
        def refreshStatus(
            ...
            datasetObject = BaseValueObjects.RootRefreshStatus.from_dict(jsonstring)
            datasetValueObject = list(
                filter(lambda y: (y.requestId == pushedRequestId), datasetObject.value)
            )
            if (len(datasetValueObject)) != 1:
