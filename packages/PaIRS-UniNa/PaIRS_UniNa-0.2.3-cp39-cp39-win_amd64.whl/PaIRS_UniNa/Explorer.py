...
class ProcessTree(PaIRSTree):
    ...
    def process_loop(self,paths,processType,ind,name0,i,opts):
        ...
        for j in range(len(opts)):
            ...
            ind_slave[3]=j
            if opts[j] in (0,2):
                ind_new=self.gui.copy_pars(ind_slave,ind_master,FlagNew=True,FlagSet=False)
                if opts[j]==2:
...
class PaIRS_Explorer(gPaIRS_Tab):         
    ...
    def __init__(self,parent=None,widgets=[]):
        ...
        self.processActioBar = ActionButtonBar(buttonData=processActionButtons,globalButtons=processGlobalActionButtons,buttonSize=processActionButtonSize)
        self.processTree_layout.addWidget(self.processActioBar)
        self.binActioBar = ActionButtonBar(buttonData=processActionButtons,globalButtons=processGlobalActionButtons,buttonSize=processActionButtonSize)
        self.processTree_layout.addWidget(self.binActioBar)
        ...
        self.processActioBar.additionalButtonBars={'globals': [self.processButtonBar], 'items': [self.stepButtonBar]}
        self.binActioBar.additionalButtonBars={'items': [self.stepButtonBar]}

        self.processTree = ProcessTree(self,processActionBar=self.processActioBar,widgets=widgets,Explorer=self)
        self.binTree = ProcessTree(self,restoreTree=self.processTree,processActionBar=self.binActioBar,widgets=widgets,Explorer=self)
