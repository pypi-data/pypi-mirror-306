    ...
    def __init__(self,flagDebug=False,app=None):
        ...
        class RCLbar:
            buttons={0: self.ui.button_reset_step,
                     1: self.ui.button_copy_step,
                     2: self.ui.button_link_step}
            buttonData={0: {'name': 'Reset step'}, 
                        1: {'name': 'Copy step from...'}, 
                        2: {'name': 'Link step to...'}}
        self.RCLbar=RCLbar()
        self.ui.Explorer.processActioBar.additionalButtonBars['items'].append(self.RCLbar)
        ...
        if len(self.processTree.itemList):
            for k,i in enumerate(self.processTree.itemList[0]):
                ...
                if itemWidget:
                    ...
                    if switch:
                        ITE:ITEpar=i[0]
                        switch.setVisible(ITE.flagRun<=0 and ITE.Process!=ProcessTypes.cal)
...
#*************************************************** Link and copy
    ...
    def copy_link_action(self,button:QPushButton,fun=lambda ind_slave, ind_master:None, FlagExcludeLinked=False, menuTitle='Copy step from...',icon=None,message='No process step available!'):
            ...
            for process in project[0]:
                for step in process[1:]:
                    ...
                    if process[0].FlagQueue and step.active and step.flagRun<=0 and step.Step!=StepTypes.cal and step.OptionDone!=0: 
                        ...
                        CAL.FlagCalVi=False
                    self.ui.Explorer.updateItemWidget(step)
            ...
            for ITEs_tree in ITEs_project_ind:
                ...
                for ITEs in ITEs_tree_ind:
                    ...
                    for ITE in ITEs_ind:
                        ...
                        indITE=ITE.ind
                        if  ind is not None and indITE!=ind:
                            pass
                        for w in self.tabWidgets: 
                            w:gPaIRS_Tab
