...
#*************************************************** Adjusting parameters
    def adjustOUTpar(self):
        ...
        if os.path.exists(self.OUTpar.path):
    ...
    def checkOUTpar(self,ind=None):
        ...
        OUT.OptionDone=1 if OUT.OptionValidPath==1 and OUT.OptionValidSubFold in (1,-1) and OUT.OptionValidRoot==1 and not OUT.FlagWarnCR else 0 if OUT.OptionValidPath==0 or OUT.OptionValidSubFold==0 or OUT.OptionValidRoot==0 or OUT.FlagWarnCR else -1
    ...
    def checkCommonRegion(self,ind=None):
        ...
        OUT.warnCR=''
        if OUT.Step in (StepTypes.disp,StepTypes.spiv):
