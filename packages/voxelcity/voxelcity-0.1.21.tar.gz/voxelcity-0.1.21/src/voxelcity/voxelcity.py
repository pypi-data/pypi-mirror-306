...
def create_3d_voxel(building_height_grid_ori, building_min_height_grid_ori, land_cover_grid_ori, dem_grid_ori, tree_grid_ori, voxel_size, land_cover_source, **kwargs):
    ...
    building_min_height_grid = np.flipud(building_min_height_grid_ori.copy())
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            for k in building_min_height_grid[i, j]:
                ...
                voxel_grid[i, j, ground_level+building_min_height+1:ground_level+building_height+1] = -3
...
def get_voxelcity(rectangle_vertices, building_source, land_cover_source, canopy_height_source, dem_source, meshsize, **kwargs):
    ...
    if kwargs["remove_perimeter_object"]:
        w_peri = int(kwargs["remove_perimeter_object"] * building_height_grid.shape[0] + 0.5)
        h_peri = int(kwargs["remove_perimeter_object"] * building_height_grid.shape[1] + 0.5)
