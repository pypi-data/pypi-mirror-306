                    ...
                    if event is not None:
                        ...
                        if origin == "agent":
                            compute_step = AgentComputeStep(
                                event_order=len(event.agent_compute_steps) + 1,
                            ...
                            compute_step = EnvironmentComputeStep(
                                event_order=len(event.environment_compute_steps) + 1,
                                compute_began=time.time(),
                                compute_ended=time.time(),
                                compute_input=inputs,
                                compute_output=None
                        ...
                        if compute_step:
                            logger.info(f"Logged {origin} input variables: {inputs}")
                    else:
                        logger.warning("Event is None; cannot log input variables")
                
                # Execute the function
                result = func(self, *args, **kwargs)
                
                # Log output variables
                if log_vars_output and event is not None:
                    outputs = {}
                    ...
                    if compute_step:
                        logger.info(f"Logged {origin} output variables: {outputs}")
                elif log_vars_output:
                    logger.warning("Event is None; cannot log output variables")
