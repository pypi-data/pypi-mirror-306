...
def trace_system(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs):
            # Create event if either:
            # 1. manage_event is "create"
            # 2. manage_event is "lazy_end" and no event exists
            should_create_event = (
                manage_event == "create" or
                (manage_event == "lazy_end" and 
                 (not hasattr(_local, "active_events") or 
                  event_type not in getattr(_local, "active_events", {})))
            )
            ...
            if manage_event in ["end", "lazy_end"]:
                current_event = get_current_event(event_type)
                current_event.closed = time.time()
                if hasattr(_local, "system_id"):
                    event_store.add_event(_local.system_id, current_event)
