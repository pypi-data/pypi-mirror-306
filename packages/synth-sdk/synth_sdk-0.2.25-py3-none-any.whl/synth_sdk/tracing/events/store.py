...
class EventStore:
    ...
    def add_event(self, system_id: str, event: Event):
        ...
        self.logger.debug(f"Event details: opened={event.opened}, closed={event.closed}, partition={event.partition_index}")
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Got system trace with {len(system_trace.partition)} partitions")
                ...
                self.logger.debug(f"Added event to partition {event.partition_index}. Total events: {len(current_partition.events)}")
            finally:
                self._lock.release()
