...
class ComputeStep:
    ...
    def to_dict(self):
        # Filter out non-serializable objects from compute_input
        serializable_input = []
        if isinstance(self.compute_input, list):
            for item in self.compute_input:
                if hasattr(item, '__class__'):
                    # Skip the TestAgent object
                    continue
                serializable_input.append(item)
        else:
            serializable_input = self.compute_input
