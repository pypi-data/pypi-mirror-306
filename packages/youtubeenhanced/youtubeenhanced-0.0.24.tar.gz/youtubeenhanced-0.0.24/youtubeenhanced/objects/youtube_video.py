...
class YoutubeVideo:
    ...
    def is_available(self):
        """
        This method returns True if this video is available (and able to obtain the
        information and download it) or False if not. If no 'duration' parameter,
        not available.
        """
    ...
    def get_hottest_scenes(self, scenes_number: int, scene_duration: float):
        ...
        'end' attributes.
        """
        if not scenes_number:
            raise Exception('No "scenes_number" provided.')
        
        # TODO: Check that 'scenes_number' is a valid and positive number
        
        if not scene_duration:
            raise Exception('No "scene_duration" provided.')
        
        # TODO: Check that 'scene_duration' is a valid and positive number

        if not self.heatmap:
            raise Exception('No heatmap available in the provided YoutubeVideo.')
