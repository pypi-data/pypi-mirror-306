...
LastEditTime: 2024-11-03 18:57:41
...
def download_task(var, time_str, lon_min, lon_max, lat_min, lat_max, depth, level, store_path, dataset_name, version_name):
    '''
    # 并行下载任务
    # 这个函数是为了并行下载而设置的，是必须的，直接调用direct_download并行下载会出问题
    '''
    direct_download(var, lon_min, lon_max, lat_min, lat_max, time_str,
                    depth, level, store_path, dataset_name, version_name)


def download(var, time_s, time_e, lon_min=0, lon_max=359.92, lat_min=-80, lat_max=90,  depth=None, level=None, store_path=None, dataset_name=None, version_name=None, num_workers=None):
    '''
    Description:
            ...
            if num_workers is None or num_workers <= 1:
                ...
                with ThreadPoolExecutor(max_workers=num_workers) as executor:
                    futures = [executor.submit(download_task, var, time_str, lon_min, lon_max, lat_min, lat_max,
                                               depth, level, store_path, dataset_name, version_name) for time_str in time_list]
