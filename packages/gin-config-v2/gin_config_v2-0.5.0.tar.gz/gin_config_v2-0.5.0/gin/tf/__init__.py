...
def _ensure_tf_install():  # pylint: disable=g-statement-before-imports
  ...
  # Minimum required version for TF2
  min_tf2_version = "2.0.0"
  
  tf_version = packaging.version.Version(tf.__version__)
  
  if tf_version < packaging.version.Version(min_tf2_version):
    raise ImportError(
        "This version of Gin-Config requires TensorFlow "
        "version >= {required}; Detected an installation of version {present}. "
        "Please upgrade TensorFlow to proceed.".format(
            required=min_tf2_version,
...
# Import TF2.x callback
from gin.tf.utils import GinConfigCallback

__all__ = ['GinConfigCallback']