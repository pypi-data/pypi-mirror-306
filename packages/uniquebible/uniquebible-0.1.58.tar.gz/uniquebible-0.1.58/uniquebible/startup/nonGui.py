...
def run_api_client_mode():
    ...
    def multiturn_api_output(apiCommandSuggestions=None):
        ...
        # make key bindings available in config to allow futher customisation via plugins
        config.key_bindings = prompt_shared_key_bindings
        ...
        while not command.lower() in (".quit",):
            # User command input
            command = config.main_prompt_session.prompt(
                promptIndicator,
                style=style,
    ...
    try:
        ...
        if command:
            ...
            run_api_client_mode(apiCommandSuggestions=apiCommandSuggestions)
