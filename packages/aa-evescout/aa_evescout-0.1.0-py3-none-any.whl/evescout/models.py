...
class SignaturePinger(models.Model):
    ...
    def ping_new_sig(self, signature: "SignatureSystem"):
        ...
        e = Embed(
            title=f"New signature from {signature.origin_system_name()} detected in {signature.system.name}",
        )
        e.set_author(
            name="EVE-scout",
    ...
    def __str__(self):
        return f"pinger {self.system.name}"
...
class SignatureSystem(models.Model):
    ...
    def distance_from_pinger(self, pinger: SignaturePinger) -> (int, float):
        ...
        """
        if route := pinger.system.route_to(self.system):
            gate_jumps = len(route) - 1  # -1 to avoid counting the initial system
            light_years = meters_to_ly(pinger.system.distance_to(self.system))
    ...
    def __str__(self):
        return f"{self.origin} > {self.system}"
