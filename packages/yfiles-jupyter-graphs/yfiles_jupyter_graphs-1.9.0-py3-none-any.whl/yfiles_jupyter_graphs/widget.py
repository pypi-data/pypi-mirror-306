...
class GraphWidget(DOMWidget):
    ...
    def get_node_cell_mapping(self):
        ...
        or None of not set.
        """
        return self._get_attribute_by_name('_node_cell_mapping')
    ...
    def del_node_cell_mapping(self):
        ...
        """
        self.__safe_delete_mapping('_node_cell_mapping')
    ...
    def _reset_group_nodes(self):
        if len(self._group_nodes) > 0:
            group_node_ids = set(map(lambda node_dict: node_dict.get('id'), self._group_nodes))
            self._nodes = [node for node in self._nodes if node['id'] not in group_node_ids]
    ...
    def get_node_parent_group_mapping(self):
        """Getter for the node parent node mapping property to create a nested graph hierarchy.
                ...
                """
        return self._get_attribute_by_name('_node_parent_group_mapping', 'default')
    ...
    def del_node_parent_group_mapping(self):
        """Deleter for the node parent node mapping property.
                ...
                """
        self.__safe_delete_mapping('_node_parent_group_mapping')

    node_parent_group_mapping = property(get_node_parent_group_mapping, set_node_parent_group_mapping, del_node_parent_group_mapping)
    ...
    def default_element_label_mapping(self, index: int, element: TDict):
        ...
        if self._data_importer == 'neo4j':
            item_label = GraphWidget.get_neo4j_item_text(element) or item_label
    ...
    def _get_attribute_by_name(
        ...
        elif fallback_to is not None and hasattr(self, fallback_to + attribute_name):
        ...
        def wrapped(index: int, element: TDict):
            ...
            if key == 'label':
                ...
                if isinstance(value, (str, int, float)):
                    ...
                    self._group_nodes.append({'id': parent_id, 'properties': {'label': label}})
                    # assign as this node's parent
                    element['parentId'] = parent_id

                elif isinstance(value, dict):
                    # for dicts, the user is required to provide a "label" property
                    if not 'label' in value or not isinstance(value.get('label'), (str, int, float)):
                    ...
                    parent_id = NEW_GROUP_ID_PREFIX + label
                    # store new group node
                    self._group_nodes.append({'id': parent_id, 'properties': dict(value.items())})
                    # assign as this node's parent
                    element['parentId'] = parent_id
    ...
    def _apply_parent_group_mapping(self):
        ...
        self._nodes = self._nodes + self._group_nodes
