...
def _generate_output_lines(scope, indent=0):
    def limit_entry_length(entry: str):
        if len(entry) > 10000:
            entry = f"{entry[0:5000]}\n  ...\n{entry[-5000:]}"
        return entry
    ...
    for entry in scope.entries:
        if isinstance(entry, Scope):
            ...
            if "requirements" in entry:
                ...
                if not ok:
                    yield " " * indent + "requirement failed: " + limit_entry_length(str(req))
            elif "assertion_failure" in entry:
                yield " " * indent + "assertion failed: " + limit_entry_length(entry["assertion_failure"])
