...
class OptionsValidator(BaseValidator):
    ...
    def validate(self, value):
        ...
        if value not in self.options:
            allowed_options = " | ".join(self.options)
            raise ValidationError(self.error_message or f"Value must be one of: {allowed_options}")
...
class EndsWithValidator(BaseValidator):
    ...
    def validate(self, value):
        ...
        if not any(value.endswith(ext) for ext in self.allowed_extensions):
...
class FloatValidator(BaseValidator):
    ...
    def validate(self, value):
        ...
        if not isinstance(value, float):
            raise ValidationError(self.error_message or "Value must be a float")
