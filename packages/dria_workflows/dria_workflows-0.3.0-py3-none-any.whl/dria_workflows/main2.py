import logging
from dria_workflows.workflows.interface import StackPage, Entry
...
def workflow(query: str, documents_info: List[str]) -> Workflow:
    """
    ...
    builder = WorkflowBuilder(query=query, documents_info=documents_info)
    ...
    builder.set_return_value(["output", "output"])
    builder.set_tools(["scraper", "duckduckgo"])
    return builder.build()


wf = workflow(query="x", documents_info=["y"]).model_dump_json(
    exclude_none=True, exclude_unset=True, indent=2
)
validate_workflow_json(wf)
