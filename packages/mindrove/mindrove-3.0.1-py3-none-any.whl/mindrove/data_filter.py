...
from mindrove.utils import check_memory_layout_row_major
...
class DataFilter(object):
    ...
    @classmethod
    def perform_bandpass(cls, data, sampling_rate: int, center_freq: float,
    ...
    @classmethod
    def perform_bandstop(cls, data, sampling_rate: int, center_freq: float,
                         band_width: float, order: int, filter_type: int, ripple: float) -> None:
    ...
    @classmethod
    def perform_wavelet_transform(cls, data, wavelet: str, decomposition_level: int) -> Tuple:
    ...
    @classmethod
    def perform_wavelet_denoising(cls, data, wavelet: str, decomposition_level: int) -> None:
    ...
    def get_csp(cls, data, labels) -> Tuple:
        ...
        if not (len(labels) == data.shape[0]):
            raise MindRoveError('Invalid number of elements in array <labels>', MindroveExitCodes.INVALID_ARGUMENTS_ERROR.value)
    ...
    def perform_fft(cls, data, window: int):
        ...
        if (not is_power_of_two(data.shape[0])):
            raise MindRoveError('data len is not power of 2: %d' % data.shape[0],
    ...
    def get_psd_welch(cls, data, nfft: int, overlap: int, sampling_rate: int, window: int) -> Tuple:
        ...
        if (not is_power_of_two(nfft)):
            raise MindRoveError('nfft is not power of 2: %d' % nfft, MindroveExitCodes.INVALID_ARGUMENTS_ERROR.value)
