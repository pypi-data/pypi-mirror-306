    ...
    try:
        if data:
            ...
            with open(files, "rb") as file_data:
                file = {"file": file_data}
                response = requests.post(url, params=params, files=file, timeout=timeout)
                file_data.close()
        else:
...
def make_delete_request(url, path_params=None, query_params=None, timeout=DEFAULT_TIMEOUT):
    """
