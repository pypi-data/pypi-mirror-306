...
from src.rpa_executor_icodefun.cli import RpaHubConfig
...
class TaskCollector:
    def __init__(self, config: RpaHubConfig, **kwargs):
        self.config = config
        self.xxl_biz_api = XXL_BIZ(config, **kwargs)
    ...
    def register(self, app: PyxxlRunner, task_info: TaskInfo):
        def inner_task():
            suite_path = task_info.suite_base_path.joinpath(task_info.suite_name)
        ...
        job_id = f"{task_info.suite_name}.{task_info.task_name}"
        self.xxl_biz_api.job_registry(job_id)
        app.register(name=job_id)(inner_task)
