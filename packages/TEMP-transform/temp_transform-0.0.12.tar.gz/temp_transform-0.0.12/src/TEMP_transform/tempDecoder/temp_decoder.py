...
class TEMPParser(tpg.VerboseParser):
    ...
    def surface_temps(self, x):
        ...
        temps['dewpoint'] = self.convert_dewpoint(st.group('dewpoint'))
    ...
    def convert_dewpoint(self, temp):
        if temp == '//':
            return temp
        else:
            value = int(temp)
    ...
    def iso_temps(self, x):
        ...
        iso_temps['dewpoint'] = self.convert_dewpoint(iso_temp.group('dewpoint'))
    ...
    def surface_sig_temps(self, x):
        ...
        surface_temps['dewpoint'] = self.convert_dewpoint(st.group('dewpoint'))
    ...
    def sig_lvl(self, x):
        ...
        if self.current_section == 6:
            ...
            sig_lvls.append({'indicator': sig_lvl.group('indicator'), 'pressure': pressure,
                             'temperatures': {'air': self.convert_air_temp(sig_lvl.group('data')[0:2], sig_lvl.group('data')[2]),
                                              'dewpoint': self.convert_dewpoint(sig_lvl.group('data')[3:])}
                                              })
    ...
    def trop_temps(self, x):
        ...
        trop_temps['dewpoint'] = self.convert_dewpoint(trop_temp.group('dewpoint'))
