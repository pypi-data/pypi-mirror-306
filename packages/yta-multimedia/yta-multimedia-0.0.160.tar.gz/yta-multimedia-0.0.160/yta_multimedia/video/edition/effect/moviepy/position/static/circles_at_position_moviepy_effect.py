...
class CirclesAtPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    @classmethod
    def apply(cls, video: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[MoviepyPosition, CoordinateCorner, CoordinateCenter] = MoviepyPosition.RANDOM_INSIDE):
        return cls.apply_over_video(video, BasePositionMoviepyEffect.get_black_background_clip(video.duration), position)

    @classmethod
    def apply_over_video(cls, video: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], background_video: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[MoviepyPosition, CoordinateCorner, CoordinateCenter] = MoviepyPosition.RANDOM_INSIDE):
        ...
        effect = video.set_position(lambda t: circular_movement(t, position[0], position[1])).set_start(0).set_duration(video.duration)
