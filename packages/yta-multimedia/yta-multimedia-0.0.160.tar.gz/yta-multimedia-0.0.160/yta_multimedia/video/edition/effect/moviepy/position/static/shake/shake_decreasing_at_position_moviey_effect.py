...
class ShakeDecreasingAtPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    by time.
    """
    def apply(self, position: Union[MoviepyPosition, CoordinateCorner, CoordinateCenter] = MoviepyPosition.RANDOM_INSIDE):
        """
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[MoviepyPosition, CoordinateCorner, CoordinateCenter] = MoviepyPosition.RANDOM_INSIDE):
        """
        ...
        if not isinstance(position, MoviepyPosition) and not isinstance(position, CoordinateCenter) and not isinstance(position, CoordinateCorner):
            raise Exception('Provided "position" is not a valid MoviepyPosition, CoordinateCenter or CoordinateCorner.')
        ...
        effect = self.video.set_position(lambda t: shake_decreasing_movement(t, position[0], position[1])).set_start(0).set_duration(self.video.duration)
