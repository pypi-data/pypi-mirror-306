...
from yta_general_utils.checker.type import variable_is_number_between
from yta_general_utils.file.handler import rename_file
...
def rescale_video(video: Union[VideoFileClip, str], output_width: int = 1920, output_height: int = 1080, output_filename: Union[str, None] = None):
    ...
    videoclip_rescaled = video.crop(x_center = floor(width / 2), y_center = floor(height / 2), width = new_width, height = new_height)
...
def resize_video(video: Union[VideoFileClip, str], width: int = None, height: int = None, output_filename: Union[str, None] = None):
    ...
    if not variable_is_number_between(width, 1, 1920) or not variable_is_number_between(height, 1, 1920):
