...
class ShakeAtPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    """
    def apply(self, position: Union[MoviepyPosition, CoordinateCenter, CoordinateCorner] = MoviepyPosition.RANDOM_INSIDE):
        """
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[MoviepyPosition, CoordinateCorner, CoordinateCenter] = MoviepyPosition.RANDOM_INSIDE):
        ...
        effect = self.video.set_position(lambda t: shake_movement(t, position[0], position[1])).set_start(0).set_duration(self.video.duration)
