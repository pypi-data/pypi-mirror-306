...
def position_video_in(video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[MoviepyPosition, CoordinateCenter, CoordinateCorner]):
    """
    ...
    if not isinstance(position, MoviepyPosition):
        if not isinstance(position, tuple) and len(position) != 2:
            raise Exception('Provided "position" is not a valid MoviepyPosition enum or (x, y) tuple.')
...
"""
def get_moviepy_position(video, background_video, position: Union[MoviepyPosition, CoordinateCenter, CoordinateCorner]):
    """
    ...
    if not isinstance(position, MoviepyPosition) and not isinstance(position, CoordinateCenter) and not isinstance(position, CoordinateCorner):
        raise Exception('Provided "position" is not MoviepyPosition, CoordinateCenter or CoordinateCorner.')
    
    position_tuple = None

    if isinstance(position, MoviepyPosition):
