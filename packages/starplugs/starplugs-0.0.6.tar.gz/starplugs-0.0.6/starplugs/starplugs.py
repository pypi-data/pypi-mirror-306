...
def dtree():
  ...
  # Tabular description of Decision Tree structure
  def get_decision_tree_rules(model, X, y):
      ...
      for sample_id in range(len(X)):
          ...
          for node_id in node_indicator.indices[node_indicator.indptr[sample_id]:node_indicator.indptr[sample_id + 1]]:
              if leaf_id[sample_id] == node_id:
                  continue
              if (X.iloc[sample_id, model.tree_.feature[node_id]] <= model.tree_.threshold[node_id]):
                  threshold_sign = "<="
              else:
                  threshold_sign = ">"
              rule.append(f"{X.columns[model.tree_.feature[node_id]]} {threshold_sign} {model.tree_.threshold[node_id]:.2f}")
          rules.append({
              "Sample": sample_id,
              "Decision Path": " AND ".join(rule),
              "Predicted Target": model.predict(X.iloc[[sample_id]])[0]
          })
      return rules

  # Generate and display the decision tree rules with predicted target values
  rules = get_decision_tree_rules(model, X_train, y_train)
  rules_df = pd.DataFrame(rules)
  print("\nTabular Decision Tree Rules (Sample Paths with Predicted Target):")
  print(rules_df.head())
