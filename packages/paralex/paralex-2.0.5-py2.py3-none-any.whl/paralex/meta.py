...
def paralex_factory(title, tables, basepath, **kwargs):
    ...
    Args:
        ...
        tables (dict): dictionary of table names to resource information
        basepath (Path): Absolute path where the metadata file will be stored.
        **kwargs:
    ...
    default_package = _get_default_package(basepath=basepath)
    ...
    for table in tables:
        if resource_name_pattern.match(table) is None:
            ...
            raise ValueError(f"Table name {repr(table)} is invalid. "
                             ...
                             "{"
                             f" ... '{suggested}': {tables[table]} ... "
                             "} ... )")
        table_infos = tables[table]
        table_infos["name"] = table
        table_contents = pd.read_csv(table_infos["path"])
        ...
        new_table_fields = list(table_infos["schema"]["fields"])
        for field in table_infos["schema"]["fields"]:
            if field["name"] not in cols:
                new_table_fields.remove(field)
        table_infos["schema"]["fields"] = new_table_fields
