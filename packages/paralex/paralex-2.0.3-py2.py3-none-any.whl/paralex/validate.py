...
class ParalexValidator(object):
    ...
    def has_iso_code(self):
        if "languages_iso639_2" not in self.p.custom:
            return (False, "Package is missing a languages_iso639_2 list. This is mandatory from Paralex v.2.0.0", "Add a list of 3-letter language codes")
        codes =  self.p.custom["languages_iso639_2"]
        if not codes:
            return (False, "Package has an empty languages_iso639_2 list. This is mandatory from Paralex v.2.0.0", "Add a list of 3-letter language codes")
        for code in codes:
            if not is_valid639_2(code):
                return (False, f"{code} is not a valid ISO 639-2 code", "Use only valid ISO 639-2 codes")
        return (True, "languages_iso639_2 list is present and valid")
    ...
    def tags_are_defined(self):
        taggable_tables = {"forms", "lexeme", "frequencies"} & self.standard_tables
