...
from bgutils.mongoUtil import mongoUtil
from flask import jsonify, json
from kafka import KafkaProducer, KafkaConsumer

from views.spider.BaseSpider import BaseSpider
...
def init_kafak():
    ...
    consumer = KafkaConsumer(
        ...
        value_deserializer=lambda m: loads(m.decode('utf-8'))  # 假设消息是JSON格式
    )
...
def getKafkaMsg(current_user):
    ...
    try:
        # 非阻塞方式
        messages = consumer.poll(timeout_ms=int(KAFKA_POLL_INTERVAL * 1000))
        if messages:
...
def dealKafkaMsg():
    try:
        ...
        # 阻塞方式
        for message in consumer:
            datastr=message.value
            # message是一个ConsumerRecord对象，包含消息的元数据（如偏移量、主题、分区等）和值
            print(f"Partition: {message.partition}, Offset: {message.offset}, Value: {message.value}")
            message = datastr
            topic = datastr['topic']

            db.process_item(message, topic)
