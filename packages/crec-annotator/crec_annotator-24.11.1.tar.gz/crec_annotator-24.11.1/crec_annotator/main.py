...
def editor_interface(content, filepath):
    ...
    # 1. Updated RCV extraction with optional 'Leg.'
    rcv_pattern = re.compile(
        r'\[.*?(?:Roll\s*No\.?|Rollcall\s*Vote\s*No\.).*?(\d+)(?:\s*(Leg\.))?\]',
        flags=re.IGNORECASE
    )

    # Extract RCVs as tuples (number, leg)
    rcv_matches = rcv_pattern.findall(content)

    # Initialize rcv_tags with unique RCV keys
    for rcv_num, leg in rcv_matches:
        rcv_key = f"{rcv_num} {'Leg.' if leg else ''}".strip()
    ...
    def add_rcv():
        rcv_num = simpledialog.askstring("Add RCV", "Enter Roll Call Vote number:")
        if rcv_num:
            leg = messagebox.askyesno("RCV Suffix", "Does this RCV have a 'Leg.' suffix?")
            rcv_key = f"{rcv_num} {'Leg.' if leg else ''}".strip()
            if rcv_key not in rcv_tags:
                rcv_tags[rcv_key] = set()
                rcv_listbox.insert(tk.END, rcv_key)
            else:
                messagebox.showinfo("Duplicate RCV", "This RCV number already exists.")
    ...
    def clear_tags():
        ...
        # Updated regex pattern to match both opening and closing tags with optional 'Leg.'
    ...
    tk.Button(rcv_frame, text="Add", command=add_rcv).pack(side=tk.LEFT, padx=5)
    ...
    def find_rcv(rcv):
        ...
        pattern = rf'\[.*?(?:Roll\s*No\.?|Rollcall\s*Vote\s*No\.).*?{escaped_rcv}.*?\]'
        content = text_widget.get("1.0", tk.END)
