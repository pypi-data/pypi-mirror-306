...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1294:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1293(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1294()))
    ...
    def _arrow1305(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1304(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1303(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1302(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1295(__unit: None=None) -> FSharpList[FsCell]:
                    ...
                    def _arrow1301(__unit: None=None) -> IEnumerable_1[FsCell]:
                        def _arrow1300(__unit: None=None) -> IEnumerable_1[FsCell]:
                            def _arrow1299(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1298(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1297(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        def _arrow1296(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1296))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1297))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1298))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1299))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1300))

                    return append(_arrow1295(), delay(_arrow1301))

                return to_list(delay(_arrow1302))

            return map_1(_arrow1303, dc)

        return append(singleton_1(headers), delay(_arrow1304))

    return transpose(to_list(delay(_arrow1305)))
