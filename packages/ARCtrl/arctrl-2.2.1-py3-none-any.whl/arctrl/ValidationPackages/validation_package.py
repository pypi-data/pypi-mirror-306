...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2961(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2960(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2958(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2957(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2957))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2958))

            return append(singleton("{"), delay(_arrow2960))

        return join("\n", to_list(delay(_arrow2961)))
...
ValidationPackage_reflection = _expr2971
