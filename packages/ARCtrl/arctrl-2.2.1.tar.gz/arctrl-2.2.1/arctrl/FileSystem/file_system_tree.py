...
class FileSystemTree(Union):
    ...
    def try_get_child_by_name(name: str) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow427(fst: FileSystemTree) -> FileSystemTree | None:
    ...
    def contains_child_with_name(name: str) -> Callable[[FileSystemTree], bool]:
        def _arrow428(fst: FileSystemTree) -> bool:
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow430(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow429(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow429))

        file_paths: Array[str] = to_array(delay(_arrow430))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow431(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr433:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow432(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr433()), None)
            if parent.tag == 1:
                def _arrow435(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow434(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow434))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow435)))
        ...
        class ObjectExpr438:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow437(x: Array[str], y: Array[str]) -> bool:
                    def _arrow436(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow436, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr438()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def to_file_paths(remove_root: bool | None=None) -> Callable[[FileSystemTree], Array[str]]:
        def _arrow439(root: FileSystemTree) -> Array[str]:
    ...
    def filter_files(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow440(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter_folders(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow442(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow448(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def Union(self, other_fst: FileSystemTree) -> FileSystemTree:
        this: FileSystemTree = self
        def _arrow453(__unit: None=None) -> Array[str]:
        ...
        class ObjectExpr455:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow454(x: str, y: str) -> bool:
                    return x == y
        ...
        paths: Array[str] = Array_distinct(_arrow453(), ObjectExpr455())
...
FileSystemTree_reflection = _expr465
