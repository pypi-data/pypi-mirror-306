...
class EvaluateApi():
    ...
    def check_x_total_value_update_dict(self, check_func, p, host, token, api, value_column_name="DEFAULT_VALUE", over_column_name="DEFAULT_COLUMN_NAME", results={}):
        ...
        count = self.get_total_count(
            host, token, api, p['path_with_namespace'], value_column_name)
        if count is not None:
            ...
            print(
                f"No {value_column_name} retrieved for project: {p.get('id')} - {p.get('path_with_namespace')}")
        return flag
    ...
    def check_full_stats(self, url, project, my_dict, headers={}):
        ...
        print(f"Listing full stats for project '{project_path}'")
    ...
    def get_registry_size(self, pid, path_with_namespace, source, token):
        ...
        if total_size is not None and total_size > 0:
            ...
            print(
                f"WARNING: Unable to retrieve project '{path_with_namespace}' registry size by GraphQL queries. Falling back to REST calls")
            registry_hashes = {}
            for registry_repo in self.gitlab_api.list_all(source, token, self.proj_registries_url(pid)):
                ...
                if error:
                    print(
                        f"Project '{path_with_namespace}' container registry inaccessible or disabled ({error}). SKIPPING...")
                    continue
                ...
                for registry_tags in self.gitlab_api.list_all(source, token, self.proj_registries_tags_url(pid, rid)):
                    ...
                    repo_details = safe_json_response(self.gitlab_api.generate_get_request(
                        source, token, self.proj_registries_tag_details_url(pid, rid, tid)))
