...
cdef class PyModule_f32:
    ...
    def barcodes(self, int degree, basepoints = None, num=100, box = None,threshold = False):
        ...
        if (len(box[0]) != 2) and (basepoints is None):
            ...
            num=len(basepoints)
        cdef vector[One_critical_filtration[float]] cbasepoints = _py2v1c_f32(basepoints)
    ...
    def euler_char(self, points:list|np.ndarray) -> np.ndarray:
        ...
        if type(points) is np.ndarray:
            ...
            if len(points.shape) == 1:
                points = [points]
        cdef vector[One_critical_filtration[float]] c_points = _py2v1c_f32(points)
...
cdef class PyModule_f64:
    ...
    def barcodes(self, int degree, basepoints = None, num=100, box = None,threshold = False):
        ...
        if (len(box[0]) != 2) and (basepoints is None):
            ...
            num=len(basepoints)
        cdef vector[One_critical_filtration[double]] cbasepoints = _py2v1c_f64(basepoints)
    ...
    def euler_char(self, points:list|np.ndarray) -> np.ndarray:
        ...
        if type(points) is np.ndarray:
            ...
            if len(points.shape) == 1:
                points = [points]
        cdef vector[One_critical_filtration[double]] c_points = _py2v1c_f64(points)
        # cdef One_critical_filtration temp
        # for point in points:
        #     temp.clear()
        #     for truc in point:
