...
class BaiJiaNewsSchema(NewsReleaserSchema):
  ...
  def create_fill_atom(self):
    ...
    atoms = [
      ...
      # activty
      #self.atom_factory.createRollin('to activity','div[class^=index-module]>.cheetah-public:nth-of-type(1)',{'x':0,'y':150}),
      #self.atom_factory.createClickable('activity','div[class^=index-module]>.cheetah-public:nth-of-type(1)'),
    ]
  ...
  def __get_text_atom(self):
    atoms = [
      ...
      # the value must be the material key 
      self.atom_factory.createTextArea('texts','body','material_body_text',1),
  ...
  def create_richtext_atom(self):
    ...
    self.richtext_atom = self.atom_factory.createRichText('richtext','no parent sel',atoms)

  def create_preview_atom(self):
    atoms = [
      self.atom_factory.createPause('wait images to upload',10),
    ]
