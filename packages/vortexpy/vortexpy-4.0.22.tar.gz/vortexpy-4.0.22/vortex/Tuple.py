...
class _TupleToPlainJsonMixin:
    ...
    def restfulJsonDictToTupleWithValidation(
        ...
        def convert(fieldName: str, value):
            ...
            # If a value is not provided, but is required
            #     Get the tupleField,
            #     if it has and has a defaultValue
            #         Then assign the default value
            if value is None and not (type(None) in get_args(typeHint)):
                tupleField = getattr(TupleClass, fieldName, None)
                if (
                    isinstance(tupleField, TupleField)
                    and tupleField.defaultValue is not None
                ):
                    value = tupleField.defaultValue

                # Cleanup, this is a long function
                del tupleField
