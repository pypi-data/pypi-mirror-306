...
class GroundnessRetrievalScore(BaseModel):
    """Binary scores for relevance checks"""
...
class GroundednessChecker:
    ...
    Attributes:
        ...
        target (str): 평가 대상 ('retrieval-answer' 또는 'question-answer')
    """
    ...
    def create(self):
        ...
        if self.target == "retrieval-answer":
            llm = self.llm.with_structured_output(GroundnessRetrievalScore)
            output_parser = PydanticOutputParser(
                pydantic_object=GroundnessRetrievalScore
            )
