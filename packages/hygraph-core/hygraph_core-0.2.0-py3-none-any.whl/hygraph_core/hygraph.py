    ...
    def group_by(self, *groupings):
        ...
        for grouping in groupings:
            if callable(grouping):
                ...
                # If grouping is an alias, create a function that returns the element's ID associated with the alias
                self.groupings.append(lambda result, alias=grouping: result[alias].getId())
            else:
        ...
        for group_key, group_items in grouped_results.items():
            ...
            for aggregation in self.aggregations:
                ...
                # Separate handling for time series and static properties
                    if  elements:
                        try:
                            temporal_property = elements[0].get_temporal_property(property_name)
                            if temporal_property and isinstance(temporal_property, TimeSeries):
                                # Time series aggregation
                                time_series_list = [
                                    element.get_temporal_property(property_name)
                                    for element in elements if element.get_temporal_property(property_name)
                                ]
                                if time_series_list:
