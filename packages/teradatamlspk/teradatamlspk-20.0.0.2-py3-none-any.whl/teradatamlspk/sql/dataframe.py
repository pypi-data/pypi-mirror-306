...
class DataFrame:
    ...
    persist = lambda self, StorageLevel = StorageLevel.MEMORY_AND_DISK_DESER: self
    ...
    def dtypes(self):
        ...
        return self._data._td_column_names_and_types
    ...
    def summary(self, *statistics):
        ...
        if not statistics:
            summary_df = DataFrame(data=self._summary(statistics=["count", "mean", "std", "min", "percentile", "max"]))
        else:
            percentile = []
            statistic = []
            ...
            summary_df = DataFrame(data=self._summary(percentile=percentile, statistics=statistic))
    ...
    def cube(self, *cols):
        ...
        if len(columns) == 0:
            return self
        return DataFrame(self._data.cube(columns))
    ...
    def rollup(self, *cols):
        ...
        if len(columns) == 0:
            return self
        return DataFrame(self._data.rollup(columns))
