...
from teradatasqlalchemy.types import INTEGER, DATE, FLOAT, NUMBER, INTERVAL_DAY_TO_SECOND, INTERVAL_YEAR_TO_MONTH, VARCHAR, TIMESTAMP, BIGINT, CHAR
from teradatamlspk.sql.types import StringType
from teradatamlspk.sql.dataframe_utils import DataFrameUtils as df_utils
from teradataml.dataframe.functions import call_udf as tdml_call_udf
...
log = lambda arg1, arg2: Column(tdml_column=_get_tdml_col(arg2).log(_get_tdml_col(arg1)))
log10 = lambda col: Column(tdml_column=_get_tdml_col(col).log10())
log1p = lambda col: Column(tdml_column=_get_tdml_col(col+1).ln())
log2 = lambda col: Column(tdml_column=_get_tdml_col(col).log(2.0))
...
to_timestamp_ntz = lambda col, format=None:  Column(tdml_column=_get_tdml_col(col).to_timestamp(format=(_format_date_string(format) if format else 'YYYY-MM-DD HH24:MI:SS.FF6')))
...
from_utc_timestamp = lambda timestamp, tz: Column(tdml_column=_get_tdml_col(timestamp).
           concat("", _get_tdml_col(timestamp).
                  ...
                  to_char('TZH:TZM')).
           cast(TIMESTAMP).
           cast(TIMESTAMP(timezone=True), timezone = _get_tdml_col(tz)).
           to_char('YYYY-MM-DD HH24:MI:SS.FF6').
           cast(TIMESTAMP))
...
reverse = lambda col : Column(tdml_column=_get_tdml_col(col).reverse())
...
parse_url = lambda url, pathToExtract, key=None: Column(tdml_column=_get_tdml_col(url).parse_url(_get_tdml_col(pathToExtract)))
...
to_binary = unknown
to_char = lambda col, format: Column(tdml_column=_get_tdml_col(col).to_char(_format_date_string(format) if isinstance(_get_tdml_col(col)._type, (DATE, TIMESTAMP)) else format))
to_number = lambda col, format: Column(tdml_column=_SQLColumnExpression(func.to_number(_get_tdml_col(col).expression, format)))
to_varchar = lambda col, format: Column(tdml_column=_get_tdml_col(col).to_char(_format_date_string(format) if isinstance(_get_tdml_col(col)._type, (DATE, TIMESTAMP)) else format))
    ...
    if f is None:
        def wrapper(f):
            def func_(*args):
                ...
                args = [arg._tdml_column.compile().replace('"','') if isinstance(arg, Column) else arg for arg in args]
...
def call_udf(name, *cols, **kwargs):
    ...
    quotechar = kwargs.pop('quotechar', None)
    # Convert Column object to string.
    cols = [col._tdml_column.compile().replace('"','') if isinstance(col, Column) else col for col in cols]
