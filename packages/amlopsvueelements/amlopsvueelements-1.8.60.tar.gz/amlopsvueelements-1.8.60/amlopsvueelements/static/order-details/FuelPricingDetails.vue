...
type Props = {
  order: IOrder | null;
  isLoading: boolean;
  isLoadingUpdateOrderPricing: boolean;
};
...
const { orderPricing, updateOrderPricing, updateOrderRoi } = toRefs(orderReferenceStore);

const onPriceChange = useDebounceFn(async (value: any) => {
  orderPricing.value!.fuel_pricing.client.unit_price_amount = value;
  await updateOrderPricing.value(props.order!.id!);
  await updateOrderRoi.value(props.order!.id!);
}, 1000);
