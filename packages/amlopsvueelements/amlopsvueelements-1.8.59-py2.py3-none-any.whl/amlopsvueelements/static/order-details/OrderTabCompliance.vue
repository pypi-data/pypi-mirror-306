    ...
    <div class="compliance-step bg-white w-full border border-transparent rounded-md">
      <div class="compliance-step-header flex justify-between py-[1rem] px-[0.75rem]">
        ...
        <div class="flex items-center gap-[1rem]">
          <Loading v-if="isLoadingRequestManagementApproval" />
          <Button
            v-if="isRequestManagementApprovalShown"
            class="button flex items-center gap-2"
            @disabled="isLoadingRequestManagementApproval ? 'true' : 'false'"
      ...
      <div
        ...
        :class="{ 'items-center': proceedComplianceQuery.loading }"
      >
        <Loading v-if="proceedComplianceQuery.loading" />
        <div
          v-if="proceedComplianceQuery.data && !proceedComplianceQuery.loading"
        ...
        <div
          ...
          :style="{
            backgroundColor: proceedComplianceQuery.data.compliance_status.background_color,
            color: proceedComplianceQuery.data.compliance_status.text_color
          }"
        >
          {{ proceedComplianceQuery.data.compliance_status.status }}
        </div>
      </div>
    </div>
    <div class="compliance-step bg-white w-full border border-transparent rounded-md">
...
import { useFetchRequestManagementApproval } from '@/services/order/fetchers';
...
const { proceedComplianceQuery } = useOrderReferenceStore();

const { callFetch: fetchRequestManagementApproval, loading: isLoadingRequestManagementApproval } =
  useFetchRequestManagementApproval({
    onSuccess: () => {
      props.order.id && proceedComplianceQuery.callFetch(props.order.id);
    }
  });

const isRequestManagementApprovalShown = computed(
  () =>
    proceedComplianceQuery.data?.order_approval?.is_auto_approved === false &&
    proceedComplianceQuery.data?.order_approval?.is_approved === null &&
    proceedComplianceQuery.data?.order_approval?.requested_at === null
);

const onRequestManagementApproval = async () => {
  if (!props.order.id) return;

  await fetchRequestManagementApproval(props.order.id);
};

watch([() => orderStore.order?.id, () => orderStore.currentStep], ([orderId, step]) => {
  const isCurrentStep = step === 2;

  if (orderId && isCurrentStep) proceedComplianceQuery.callFetch(orderId);
});
</script>
