...
class AICoreV2Client:
    ...
    def from_env(profile_name: str=None,
        ...
        **kwargs: check the parameters of the class constructor
        """
        env_credentials = fetch_credentials(profile=profile_name)

        # if cert_url is present in the fetched credentials, rename it to auth_url
        if 'cert_url' in env_credentials.keys():
            env_credentials['auth_url'] = env_credentials['cert_url']
            del env_credentials['cert_url']

        env_cred_keys = list(env_credentials.keys())
        for k in env_cred_keys:
            if kwargs.get(k) is not None:
                del env_credentials[k]
