...
from .jsonformat import JsonFormat  # default serialization format
...
class Application(Composite):
    ...
    _app_id = "masterpiece"  # default application id
    ...
    def save_configuration(cls) -> None:
        ...
        try:
            ...
            cls.log_error(f"No configuration file {filename} found")
            ...
            if issubclass(format_class, Format):
                ...
                for name, ctor in MasterPiece.factory().items():
                    ...
                    with open(filename, "r", encoding="utf-8") as f:
                        format = format_class(f)
                        format.load_configuration(ctor)
                        cls.log_error(f"Configuration file {filename} loaded")
        except FileNotFoundError:
            cls.log_error(f"No configuration file {filename} found")
    ...
    def parse_args(cls):
        ...
        cls._argmaestro = ArgMaestro()
        for _c, clazz in MasterPiece.factory().items():
    ...
    def init_app_id(cls, app_id: str = "myapp") -> None:
        ...
        if cls._init:
            cls.log_info(
                "--init specified, class configuration files will be created on exit"
            )
            atexit.register(cls.save_configuration)
    ...
    def deserialize(self) -> None:
        ...
        if self.serialization_file != "" and self.serialization_format != "":
            ...
            self.warning(f"No serialization file or format specified")
    ...
    def serialize(self) -> None:
        ...
        if self.serialization_file != "":
            ...
            self.warning("Cannot serialize, format or filename not configured")
