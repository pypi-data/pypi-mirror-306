...
class PlugMaster(MasterPiece):
    ...
    def find_class_by_name(self, name) -> Union[Type[MasterPiece], None]:
    ...
    def instantiate_class_by_name(
        ...
        if entry is not None:
            ...
            if isinstance(obj, Plugin):
                ...
                plugin.install(app)
                self.info(
                    f"Plugin {obj.name}:{str(type(plugin))} plugged in to {app.name}"
                )
            elif isinstance(obj, MasterPiece):
                app.add(obj)
                self.info(f"Object {obj.name}:{str(type(obj))} added into {app.name}")
            else:
    ...
    def install(self, app: Composite) -> None:
        ...
        for name, entry in self.plugins.items():
            ...
            if isinstance(obj, Plugin):
                ...
                plugin.install(app)
                self.info(
                    f"Plugin {name} {obj.name}:{str(type(plugin))} added to {app.name}"
                )
            elif isinstance(obj, MasterPiece):
                app.add(obj)
                self.info(f"Object {obj.name}:{str(type(obj))} installed to {app.name}")
            else:
                self.error(f"{type(obj)} is not a MasterPiece, skipped")
