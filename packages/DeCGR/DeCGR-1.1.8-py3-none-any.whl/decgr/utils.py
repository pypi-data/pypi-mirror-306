...
def runFitHiC(path, control_dict, chrom_list, chro_lens, resolution):
    ...
    os.system('python {0}/decgr/fithic/HiCPro2FitHiC.py -i {1} -b {2} -o {0}/tmp -r {3}'.format(
        path, path + '/tmp/fithic.matrix', path + '/tmp/fithic.bed', str(resolution)))
    os.system(
        'python {0}/decgr/fithic/fithic_copy.py -i {0}/tmp/fithic.interactionCounts.gz -f {0}/tmp/fithic.fragmentMappability.gz -o {0}/tmp -r {1} -x All'
