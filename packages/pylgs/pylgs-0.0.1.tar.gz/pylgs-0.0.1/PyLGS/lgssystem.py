...
class LGSSystem(ParametricObject):
    ...
    directory = Path('/Users/simon/Development/PyLGS/pylgs/LGSSystems/')
    block_names = [
    ...
    def __init__(
        ...
        for name in self.block_names:
            setattr(
                self, 
                name,
                LincombOperator.from_file(self.directory/system/name).partial_assemble(self.fixed_params).with_(name=name)
            )
        ...
        self.initial = self.Flux.source.from_xarray(
            LincombOperator.from_file(
                self.directory/self.system/'b'
            ).partial_assemble(
                Mu(BeamTransitRatePerS=1)
            ).matrix.rename(
                {'Density matrix (range)': 'Density matrix (source)'}
            )
        )
...
def names(cls:LGSSystem)->list: 
    """The names of all defined atomic systems."""
    return [x.name for x in cls.directory.iterdir() if x.is_dir() and not x.name.startswith('.')]
