...
class MangoLeftMenuButton(QPushButton):
    def __init__(
        ...
        self._set_bg_color = THEME.dark_one
    ...
    def paintEvent(self, event):
        ...
        if self._is_active:
            # DRAW BG BLUE
            p.setBrush(QColor(THEME.context_color))
            p.drawRoundedRect(rect_blue, 8, 8)

            # BG INSIDE
            p.setBrush(QColor(THEME.bg_one))
            ...
            # DRAW TEXT
            p.setPen(QColor(THEME.text_active))
            ...
            # DRAW BG BLUE
            p.setBrush(QColor(THEME.dark_four))
            p.drawRoundedRect(rect_blue, 8, 8)
        ...
        # NORMAL BG
            ...
            if self._is_toggle_active:
                # BG INSIDE
                p.setBrush(QColor(THEME.dark_three))
                p.drawRoundedRect(rect_inside, 8, 8)

                # DRAW TEXT
                p.setPen(QColor(THEME.text_foreground))
                p.drawText(rect_text, Qt.AlignVCenter, self.text())  # type: ignore

                # DRAW ICONS
                if self._is_toggle_active:
                    self.icon_paint(p, self._icon_path, rect_icon, THEME.context_color)
                else:
                    self.icon_paint(p, self._icon_path, rect_icon, self._set_icon_color)
            else:
                # BG INSIDE
    ...
    def icon_active(self, qp, image, width):
        ...
        painter = QPainter(icon)
        painter.setCompositionMode(QPainter.CompositionMode_SourceIn)  # type: ignore
        painter.fillRect(icon.rect(), THEME.bg_one)
    ...
    def change_style(self, event):
        if event == QEvent.Enter:  # type: ignore
            if not self._is_active:
                ...
                self._set_bg_color = THEME.dark_three
            self.repaint()
            ...
            if not self._is_active:
                self._set_icon_color = THEME.context_color
                self._set_bg_color = THEME.dark_four
            self.repaint()
