...
class TableBase(QueryObjectBase, Aliasable, ABC):
    ...
    def __init__(self, alias: str | None = None):
        ...
        self._columns_map: dict[str, Column] = {}  # Keys is identifier, meaning alias if it has one, or real name
...
class Select(QueryBuilderBase):
    ...
    def join_relation(self, model: type[ModelInterface], relation_name: str, join_type: JoinTypes = JoinTypes.INNER):
        try:
    ...
    def having(self, *conditions):
        if self._having:
            raise QueryBuilderException("HAVING already set")  # Expected to be set once with the group by
    ...
    def invert_order_by(self):
        ...
        self._order_by = [col.term if isinstance(col, Descending) else Descending(col) for col in self._order_by]
        return self
    ...
    def render(self, build_context: BuildContext) -> str:  # Make sure to build the SQL in top-down order so SQL Server paramters get injected in order
        # Validating
        if not self._select:
    ...
    def render(self, build_context: BuildContext) -> str:
        ...
        if self._on_conflict is not None:
            ...
            if self._on_conflict == 'update':
                res += 'UPDATE SET\n'
                res += ',\n'.join(f'"{cn}" = EXCLUDED."{cn}"'for cn in self._column_names if cn not in self._primary_keys)
