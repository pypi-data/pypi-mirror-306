...
class TableControls(TypedDict):
    ...
    offset: int
    descending: bool
    order_by: list[ColumnOrder]


def decode_order_by(model: type[BaseModel], column_orders: list[ColumnOrder]) -> list[Column | Descending]:
    new_order_by = []
...
def load_from_model_query(
        ...
        order_by : Sequence[Term] | None = None,
...
def resolve_type_loader_factory(model_list: list[type[BaseModel]]):
    ...
    async def resolve_type_loader(_, info, type_name: str, controls: TableControls):
        gqltype = info.schema.get_type(type_name)
        ...
        q = load_from_model_query(
            ...
            order_by=decode_order_by(model, controls['order_by']),
...
def simple_table_resolver_factory[T: BaseModel](model: type[T], query_modifier: Callable[[Select], Select] | None = None):
    async def simple_table_resolver(_, info, controls: TableControls) -> list[T]:
        q = load_from_model_query(
            model,
            filters=controls['filters'],
            limit=controls['limit'],
