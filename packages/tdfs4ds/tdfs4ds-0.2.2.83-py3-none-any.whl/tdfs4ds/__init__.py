...
def augment_source_with_features(source_schema, source_name, entity_id, feature_selection, output_view_schema,
    ...
    for feature_store_view, list_features in locations.items():
        ...
        for feature in list_features:
            ...
            # Construct the LEFT JOIN clause for each feature with specific feature ID and version
            query_join.append(
                f"""
                LEFT JOIN (CURRENT VALIDTIME SELECT A.*, A.FEATURE_VALUE AS {feature[2]} FROM {feature_store_view} A WHERE FEATURE_ID = {feature[0]} AND FEATURE_VERSION = '{feature[1]}') {table_name}
                ON {on_clause}
                """
            )
