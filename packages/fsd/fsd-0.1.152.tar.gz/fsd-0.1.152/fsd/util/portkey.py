...
class AzureModel(BaseModel):
    ...
    async def coding_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"AzureModel coding_prompt failed: {str(e)}")
            raise
    ...
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"AzureModel prompt failed: {str(e)}")
            raise
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        try:
            ...
            logger.error(f"AzureModel stream_prompt failed: {str(e)}")
            raise
...
class BedrockModel2(BaseModel):
    ...
    async def coding_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"BedrockModel2 coding_prompt failed: {str(e)}")
            raise
    ...
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"BedrockModel2 prompt failed: {str(e)}")
            raise
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        try:
            ...
            logger.error(f"BedrockModel2 stream_prompt failed: {str(e)}")
            raise
    ...
    def generate_image(self, prompt: str, model: str):
        try:
            ...
            logger.error(f"BedrockModel2 generate_image failed: {str(e)}")
            raise
...
class BedrockModel(BaseModel):
    ...
    async def coding_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"BedrockModel coding_prompt failed: {str(e)}")
            raise
    ...
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"BedrockModel prompt failed: {str(e)}")
            raise
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        try:
            ...
            logger.error(f"BedrockModel stream_prompt failed: {str(e)}")
            raise
    ...
    def generate_image(self, prompt: str, model: str):
        try:
            ...
            logger.error(f"BedrockModel generate_image failed: {str(e)}")
            raise
...
class GeminiModel(BaseModel):
    ...
    async def coding_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"GeminiModel coding_prompt failed: {str(e)}")
            raise
    ...
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        try:
            ...
            logger.error(f"GeminiModel prompt failed: {str(e)}")
            raise
...
class DalleModel(BaseModel):
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        try:
            ...
            logger.error(f"DalleModel generate_image failed: {str(e)}")
            raise
    ...
    async def prompt(self, 
        ...
        while len(tried_models) < len(self.MODEL_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for prompt")
                ...
                logger.error(f"{model_type} model is taking too long to respond, switching to another model")
    ...
    async def arch_prompt(self, 
        ...
        while len(tried_models) < len(self.Architect_MODEL_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for architecture")
                model = getattr(self, f"{model_type}_model")
                ...
                logger.error(f"{model_type} model is taking too long to respond for architecture, switching to another model")
                tried_models.add(model_type)
            except Exception as e:
                tried_models.add(model_type)
                if "429" in str(e):
    ...
    async def coding_prompt(self, 
        ...
        while len(tried_models) < len(self.CODING_MODEL_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for coding")
                model = getattr(self, f"{model_type}_model")
                ...
                logger.error(f"{model_type} model is taking too long to respond for coding, switching to another model")
                tried_models.add(model_type)
            except Exception as e:
                tried_models.add(model_type)
                if "429" in str(e):
    ...
    async def stream_prompt(self, 
        ...
        while len(tried_models) < len(self.STREAM_MODEL_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for streaming")
                model = getattr(self, f"{model_type}_model")
                ...
                logger.error(f"{model_type} model is taking too long to respond for streaming, switching to another model")
                tried_models.add(model_type)
            except Exception as e:
                tried_models.add(model_type)
                if "429" in str(e):
    ...
    async def explainer_stream_prompt(self, 
        ...
        while len(tried_models) < len(self.STREAM_EXPLAINER_MODEL_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for explainer streaming")
                model = getattr(self, f"{model_type}_model")
                ...
                logger.error(f"{model_type} model is taking too long to respond for explainer streaming, switching to another model")
                tried_models.add(model_type)
            except Exception as e:
                tried_models.add(model_type)
                if "429" in str(e):
    ...
    async def arch_stream_prompt(self, 
        ...
        while len(tried_models) < len(self.ARCH_STEAM_WEIGHTS):
            try:
                ...
                logger.info(f"Currently using {model_type} model for architecture streaming")
                model = getattr(self, f"{model_type}_model")
                ...
                logger.error(f"{model_type} model is taking too long to respond for architecture streaming, switching to another model")
                tried_models.add(model_type)
            except Exception as e:
                tried_models.add(model_type)
                if "429" in str(e):
