...
class AIGateway:
    ...
    def _select_model(self, weights, exclude=None):
        ...
        # Calculate target ratios based on weights
        total_usage = sum(self.model_usage_count[model] for model in weights)
        if total_usage == 0:
            # For initial selection, use weights directly
            ...
            # Select based on weights for first usage
            ...
            # Calculate current usage ratios and compare with target weights
            available_models = [model for model in weights if model not in (exclude or set())]
            if not available_models:
                raise ValueError("No available models to choose from")
            
            # Calculate how far each model's usage is from its target ratio
            ratio_diffs = []
            ...
            # Select the model that's furthest below its target ratio
        ...
        for current_weights in weights_to_try:
            while len(tried_models) < len(current_weights):
                ...
                try:
                    ...
                    logger.debug(f"Error in generating image with {model_type}, trying next model: {str(e)}")
