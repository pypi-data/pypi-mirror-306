    ...
    async def get_coding_request(self, file, techStack):
        ...
        try:
            ...
            while retries < max_retries:
                response = await self.ai.coding_prompt(
                    self.conversation_history, self.max_tokens, 0.2, 0.1
                )
                ...
                if lines and "> REPLACE" in lines[-1]:
                    ...
                    continuation_prompt = (
                        "The previous response was cut off before completing the SEARCH/REPLACE block. "
                        "Please continue from where you left off without overlapping any content. "
                        "Ensure the continuation ends with '>>>>>>> REPLACE'."
                    )
                    self.conversation_history.append({"role": "user", "content": continuation_prompt})
                    retries += 1

            # After max retries, return the content and log an error
            logger.error(
                "The `CodingAgent` encountered an error while getting coding request: Response incomplete after maximum retries."
            )
            return content

        except Exception as e:
            logger.error("The `CodingAgent` encountered an error while getting coding request")
            logger.error(f"{e}")
            raise
