...
class TokenController(metaclass=MetaServiceFWDI):
    ...
    def post(user_repository:UserRepositoryFWDI = Depends(), form_data:OAuth2PasswordRequestForm = Depends(), jwt_tools:JwtToolsFWDI = Depends())->Token:
        TokenController.__log__(f"Request auth from :{form_data}","info")

        user = JwtServiceFWDI.authenticate_user(user_repository.get_all(), form_data.username, form_data.password)
        TokenController.__log__(f"Result auth from :{user}","info")
        ...
        TokenController.__log__(f"Create token :{access_token}","info")
