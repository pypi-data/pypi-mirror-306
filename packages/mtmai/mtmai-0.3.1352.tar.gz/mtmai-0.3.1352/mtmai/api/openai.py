...
from mtmai.agents.chat_profiles.phi_hello import PhiHelloAgent
...
async def chat_completions(
    ...
    if chat_profile == "assistant":
        # assistant_agent = AssistantGraph()
        # return StreamingResponse(
        #     assistant_agent.run_graph(
        #         messages=messages, user_id=str(user.id), params=params
        #     ),
        #     media_type="text/event-stream",
        # )
        ...
        return StreamingResponse(
            phi_data_hello.stream_messages(
                messages=messages, user_id=str(user.id), params=params
            ),
            media_type="text/event-stream",
        )
