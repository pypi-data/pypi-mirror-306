...
def build_app():
    ...
    # LOG.info("api agent protocol")
    # from mtmai.forge.sdk.routes.agent_protocol import base_router

    # api_router.include_router(base_router, prefix="/skyvern", tags=["skyvern"])

    # LOG.info("api myhelper")
    # from mtmai.forge.sdk.routes.myhelper import myhelper_router

    # api_router.include_router(myhelper_router, prefix="/myhelper", tags=["myhelper"])
    # LOG.info("api websocket")
    # from mtmai.forge.sdk.routes.streaming import websocket_router

    # api_router.include_router(websocket_router, prefix="/streams", tags=["streams"])
...
async def serve():
    try:
        ...
        LOG.info("server starting.", host="0.0.0.0", port=port, server_url=server_url)
