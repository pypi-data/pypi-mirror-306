...
from mtmlib.github import git_clone_repos, git_commit_push
...
def release_npm():
    ...
    bash(f"bun run turbo build")

    for package in npm_packages:
        npm_patch_version(package)
    bash(f"bun run changeset publish --no-git-tag")
...
async def dp_cfpage():
    ...
    if re.search(r'runtime\s*=\s*["\']nodejs["\']', pre_content):
        new_content = re.sub(r'runtime\s*=\s*"nodejs"',
                             r'runtime="edge"', pre_content)
