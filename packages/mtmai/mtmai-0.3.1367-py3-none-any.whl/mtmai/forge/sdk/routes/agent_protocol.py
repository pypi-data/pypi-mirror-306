...
from mtmai.exceptions import StepNotFound
from mtmai.forge import app
from mtmai.forge.prompts import prompt_engine
from mtmai.forge.sdk.api.aws import aws_client
from mtmai.forge.sdk.api.llm.exceptions import LLMProviderError
from mtmai.forge.sdk.artifact.models import Artifact, ArtifactType
from mtmai.forge.sdk.core import skyvern_context
from mtmai.forge.sdk.core.permissions.permission_checker_factory import (
    PermissionCheckerFactory,
)
from mtmai.forge.sdk.core.security import generate_skyvern_signature
from mtmai.forge.sdk.db.enums import OrganizationAuthTokenType
from mtmai.forge.sdk.executor.factory import AsyncExecutorFactory
from mtmai.forge.sdk.models import Organization, Step
from mtmai.forge.sdk.schemas.organizations import (
    GetOrganizationAPIKeysResponse,
    GetOrganizationsResponse,
    OrganizationUpdate,
)
from mtmai.forge.sdk.schemas.task_generations import (
    GenerateTaskRequest,
    TaskGeneration,
    TaskGenerationBase,
)
from mtmai.forge.sdk.schemas.tasks import (
    CreateTaskResponse,
    OrderBy,
    SortDirection,
    Task,
...
from mtmai.forge.sdk.services import org_auth_service
from mtmai.forge.sdk.settings_manager import SettingsManager
from mtmai.forge.sdk.workflow.exceptions import (
...
from mtmai.forge.sdk.workflow.models.yaml import WorkflowCreateYAMLRequest
from mtmai.webeye.actions.actions import Action
...
async def get_task(
    ...
    if task_obj.status == TaskStatus.failed and (
        ...
        if latest_step.output and latest_step.output.action_results:
            ...
            failure_reason += str(
                [
                    f"[{ar.exception_type}]: {ar.exception_message}"
                    for ar in latest_step.output.action_results
                ]
            )
...
@base_router.get("/workflows/runs", response_model=list[WorkflowRun])
...
class WorkflowCreateYAMLRequestWithYaml(BaseModel):
    yaml: str
...
async def create_workflow(
    req: WorkflowCreateYAMLRequestWithYaml,
    current_org: Organization = Depends(org_auth_service.get_current_org),
) -> Workflow:
    analytics.capture("skyvern-oss-agent-workflow-create")
    try:
        workflow_yaml = yaml.safe_load(req.yaml)
...
class WorkflowUpdateYAMLRequestWithYaml(BaseModel):
    yaml: str
...
async def update_workflow(
    workflow_permanent_id: str,
    request: WorkflowUpdateYAMLRequestWithYaml,
    current_org: Organization = Depends(org_auth_service.get_current_org),
) -> Workflow:
    ...
    try:
        workflow_yaml = yaml.safe_load(request.yaml)
    except yaml.YAMLError:
        raise HTTPException(status_code=422, detail="Invalid YAML")
