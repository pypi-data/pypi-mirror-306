...
from intelliw.utils.intelliwapi.context import header_ctx
    ...
    async def response_process(self, data, func, need_feature):
        ...
        try:
            ...
            if emsg is None:
                ...
                resp = message.APIResponse('013-501-015002', "api", emsg, result)
        except Exception as e:
            logger.error(traceback.format_exc())
            msg = f"API服务处理推理数据错误 {e}"
            resp = message.APIResponse('013-501-015002', "api", msg, msg)
        return resp, False
...
async def middleware_header(request: FARequest, call_next):
...
async def set_config(request: FARequest):
    try:
        ...
        logger.error(traceback.format_exc())
        resp = {'code': '013-501-015002', 'message': str(e)}
    return FAResponse(content=str(resp), media_type="application/json")
