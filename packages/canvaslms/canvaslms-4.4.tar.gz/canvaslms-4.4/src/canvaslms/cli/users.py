...
def add_user_option_wo_depends(parser, required=False):
    ...
    parser.add_argument("-u", "--user", metavar="user_regex", help=help, **options)
...
def add_group_category_option_wo_depends(parser):
    ...
    parser.add_argument(
        ...
        required=False,
        help="Filters groups only from the group categories matching " "category_regex",
    )
...
def process_group_options(canvas, args):
    ...
    return list(filter_groups(course_list, args.group))
