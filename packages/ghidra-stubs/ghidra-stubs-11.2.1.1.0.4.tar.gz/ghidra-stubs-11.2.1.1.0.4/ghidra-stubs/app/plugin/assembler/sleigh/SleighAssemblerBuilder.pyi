...
class SleighAssemblerBuilder(ghidra.app.plugin.assembler.sleigh.AbstractSleighAssemblerBuilder, ghidra.app.plugin.assembler.AssemblerBuilder):
    """
    An AssemblerBuilder capable of supporting almost any SleighLanguage
     ...
     production precedes the is keyword. The machine-code constraints and production
     ...
     op=(0,3);regD=(4,7);imm8=(8,15). In little endian, this would indicate a 2-byte
     token:
     ...
     Thus, this constructor is assigned the pattern 0101...., which handles
     op=5. regD and imm8 remain as operands. The operands of
     ...
     examine the op field before parsing regD.
     ...
     are not actually separated by spaces. For example, in the  file, there is JMP, and
     ...
     are rather short. The pre-compute part of this process is implemented in #buildGrammar()
     and #buildParser(). Parsing is then encapsulated in AssemblyParser.
     ...
     constraint is op == 5 || op == 6. Thus, overall, it is possible a single prototype
     ...
     with the mnemonic production ^instruction, though. These "pure recursive"
     ...
     [instruction] => [instruction] has associated with it all pure recursive
     ...
     AssemblyTreeResolver#resolveRootRecursion(AssemblyResolutionResults).
