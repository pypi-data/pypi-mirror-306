...
from bomiot.server.core.utils import get_job_id
...
class SchedulerManager(Thread):
    ...
    def _remove_deprecated_jobs(self, job, force=False):
        ...
        :return:
        """
        if not job.modified and not force:
            return
        ...
        realtime_job_ids = list(self.realtime_job())
        deprecated_job_ids = [
    ...
    def _add_or_modify_new_jobs(self, job, force=False):
        ...
        job_func = importlib.import_module(f'{job.executor}')
        # if job doesn't exist, add it. otherwise replace it
        self.scheduler.add_job(job_func, job.trigger, id=job_id, replace_existing=True, **configuration)
