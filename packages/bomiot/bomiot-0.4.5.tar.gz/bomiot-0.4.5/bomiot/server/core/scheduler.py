...
def sm_send_success_signal_handler(sender, **kwargs):
    ...
    if model_data == 'JobList':
        ...
        if job_id not in current_job:
            current_job.append(job_id)
            data = kwargs['msg'].get('data', '')
            try:
                job_existed = JobList.objects.get(job_id=job_id,
                                                  module_name=inspect.getmodule(sender).__name__,
                                                  func_name=sender.__name__)
                if data.get('description', ''):
                    job_existed.description = data.get('description', '')
                job_existed.trigger = data.get('trigger')
                job_existed.configuration = data.get('configuration', '')
                job_existed.save()
            except:
...
class SchedulerManager(Thread):
    ...
    def realtime_jobs(self):
        ...
        :return:
        """
        list_jobs = JobList.objects.all()
        list_jobs_ids = list(map(lambda obj: obj.id, list_jobs))
        deprecated_job_ids = [
            job_id for job_id in current_job if not job_id in list_jobs_ids]
        if deprecated_job_ids:
            # remove deprecated jobs
            for job_id in deprecated_job_ids:
                job_delete = JobList.objects.get(job_id=job_id)
                job_delete.delete()
