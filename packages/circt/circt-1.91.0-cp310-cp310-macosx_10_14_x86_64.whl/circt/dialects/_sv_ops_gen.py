...
class AlwaysOp(_ods_ir.OpView):
  ...
  def __init__(self, events, clocks, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_557')) else
      _ods_ir.AttrBuilder.get('anonymous_557')(events, context=_ods_context))
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
...
def macro_ref(macro_name, inputs, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MacroRefOp(macroName=macro_name, inputs=inputs, loc=loc, ip=ip))
