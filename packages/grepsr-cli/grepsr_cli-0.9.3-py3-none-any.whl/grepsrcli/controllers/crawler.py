...
class Crawler(Controller):
    ...
    def create(self):
        ...
        if self.app.pargs.init:
            ...
            if template == 'node':
                ...
                if lang_type_index == '2':
                    packages_to_install.append('typescript')
        ...
        if template == 'vc':
            ...
            render_boilerplate(path.join("node_ts", "package.json.jinja2"), data, path.join(plugin_path, 'package.json'))
        else:
    ...
    def package_install(self):
        ...
        if plugin_path:
            ...
            self.app.log.info(f'Installing package `{", ".join(final_packages)}`')
            test.node_pkg_install(' '.join(final_packages))
        else:
            self.app.log.error(f"plugin: {plugin_name} not found for crawler type: {crawler_type}")
            self.app.exit_code = 127
