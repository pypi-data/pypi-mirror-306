...
def createInitialContext(team: ObjectId, task: ObjectId=None, project: ObjectId=None, client: ObjectId=None, plan: ObjectId=None, deal: ObjectId=None) -> (dict):
    ...
    if client:
        ...
        if context["client"].get("contacts"):
            context["client"]["contacts"] = { c['role']: c['contact'] for c in context["client"].get("contacts", []) if c.get('contact') and c.get('role') }
        if context["client"].get("external"):
            context["client"]["external"] = { c['role']: c['contact'] for c in context["client"].get("external", []) if c.get('contact') and c.get('role') }
        if context["client"].get("assignees"):
            context["client"]["assignees"] = { c['role']: c['user'] for c in context["client"].get("assignees", []) if c.get('user') and c.get('role') }
