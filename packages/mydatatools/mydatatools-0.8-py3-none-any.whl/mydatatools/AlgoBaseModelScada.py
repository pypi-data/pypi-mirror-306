...
class AlgoBaseModelScada(object):
    ...
    def get_args(self, data_date_range='D', alarm_date_range=None):
        ...
        if data_date_range.endswith('D'):
            match = re.match("(\d+)", data_date_range)
            num = match.group(0) if match else 1
            start_date_default = (today - datetime.timedelta(days=int(num))).strftime('%Y-%m-%d 00:00:00')
            end_date_default = today.strftime('%Y-%m-%d 00:00:00')
        elif data_date_range == 'M':
            start_date_default = last_month_start_date.strftime('%Y-%m-%d 00:00:00')
            end_date_default = last_month_end_date.strftime('%Y-%m-%d 23:59:59')
        elif data_date_range == '30D':
            start_date_default = (today - datetime.timedelta(days=30)).strftime('%Y-%m-%d 00:00:00')
            end_date_default = today.strftime('%Y-%m-%d 00:00:00')
        else:
