...
class DataBase:
    ...
    async def delete_after_time(self):
        try:
            ...
            # Вычисляем условие для устаревших записей
            expiration_hours = UrlMapping.expiration  # Получите значение expiration для каждой записи
            expiration_condition = UrlMapping.date + func.interval(f'{expiration_hours} hours') <= current_time

            # Получаем устаревшие записи
            stmt = select(UrlMapping).filter(expiration_condition)
            result = await self.session.execute(stmt)
            expired_mappings = result.scalars().all()

            if expired_mappings:
                # Удаляем устаревшие записи
                delete_stmt = delete(UrlMapping).where(expiration_condition)
                await self.session.execute(delete_stmt)
                await self.session.commit()

                # Логируем информацию о удалённых записях
