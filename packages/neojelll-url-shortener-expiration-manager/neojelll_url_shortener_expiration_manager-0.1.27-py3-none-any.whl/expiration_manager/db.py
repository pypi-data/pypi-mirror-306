...
class DataBase:
    ...
    async def delete_after_time(self):
        ...
        # Получаем устаревшие записи, где дата + период истечения < текущее время
            stmt = select(UrlMapping).filter(
                UrlMapping.date + (UrlMapping.expiration * text("INTERVAL '1 hour'")) <= current_time  # 3600 секунд в часе
            )
            ...
            # Удаляем устаревшие записи
                delete_stmt = delete(UrlMapping).where(
                    UrlMapping.date + (UrlMapping.expiration * text("INTERVAL '1 hour'")) <= current_time
                )
