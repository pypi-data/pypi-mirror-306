...
project = pysylph.__name__
author = re.match('(.*) <.*>', pysylph.__author__).group(1)
year = datetime.date.today().year
copyright = '{}, {}'.format("2024" if year==2024 else "2024-{}".format(year), author)

# extract the semantic version
semver = semantic_version.Version.coerce(pysylph.__version__)
version = str(semver.truncate(level="patch"))
release = str(semver)

# patch the docstring of `pysylph` so that we don't show the link to redirect
# to the docs (we don't want to see it when reading the docs already, duh!)
doc_lines = pysylph.__doc__.splitlines()
...
html_theme_options = {
    ...
    "icon_links": [
        ...
        {
            "name": "PyPI",
            "url": "https://pypi.org/project/pysylph",
...
htmlhelp_basename = pysylph.__name__
