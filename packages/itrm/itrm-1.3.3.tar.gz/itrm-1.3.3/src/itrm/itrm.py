...
class Config:
    ...
    location will be printed to the terminal. Subsequently, you can get the
    location of the configuration file by "print(itrm.Config.where())".
    """
    ...
    def __init__(self):
        ...
        the config.ini file. Rewrite the config.ini file if it was missing or
        incomplete. """
        ...
        # Define the color map, the foreground command, and the gray color.
        Config.darks, Config.lites, Config.frgnd, Config.gray \
                = set_color_map(Config.cmap)
    ...
    def where():
        """ Return the location of the config.ini file if it exists. """
    ...
    def what():
        """ Return the global configuration parameter values. """
...
class Sets:
    ...
    def nonfinite(self): # Sets method
        """
        Replace the data sets with markers for non-finite values. The markers are
...
class Canvas:
    ...
    def __init__(self, cols, rows, uni, cmap=None, mode=0): # Canvas method
        ...
        self.border = None
        self.blank = None
        if (cmap is not None) and (cmap.lower() != Config.cmap):
            self.darks, self.lites, self.frgnd, self.gray \
                    = set_color_map(cmap.lower())
        else:
...
class View:
    ...
    def zoom_in_indices(self, sets): # View method
        ...
        for j in range(sets.J):
            ...
            elif xk_hi <= self.x_lo_lin: # both on the left
                # If there are more valid x values to the right,
                if k_hi < k_max:
                    xj_right = xj[k_hi+1:k_max+1] # not including xk_hi
                    ...
                    self.k_lo[j] = self.k_hi[j] # right-most on the left
            elif xk_lo >= self.x_hi_lin: # both on the right
                # If there are more valid x values to the left,
                if k_lo > k_min:
                    xj_left = xj[k_min:k_lo] # not including xk_lo
...
def table(matrix, head=None, left=None, width=10, sep="  ", uni=None):
    ...
    if isinstance(matrix, (str, float, int)):
        ...
        raise TypeError("table: matrix must be a list!")
