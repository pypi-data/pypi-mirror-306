...
def pplg_request_certificate_info(api_token, tenant_id, cert_type, digest_b64):
...
def pplg_request_user_info(api_token, tenant_id):
...
async def proceed_with_signing(
        ...
        signature, certificate, other_certs, 
...
async def full_procedure(input_file, output_file, api_token, tenant_id, cert_type):
    print('full_procedure')
    field_name = 'Signature1'
    
    # 0. Get user_info for reason
    _, user_info = pplg_request_user_info(api_token, tenant_id)
    ...
    _, signing_response = pplg_request_certificate_info(api_token, tenant_id, cert_type, digest_b64)
    ...
    other_certs = [
        x509.Certificate.load(base64.b64decode(cert_b64))
        for cert_b64 in signing_response['other_certs']
    ]
...
def sign(input_file, output_file, api_token, tenant_id, cert_type):
    print('sign')
    print(input_file, output_file, api_token, tenant_id, cert_type)
    asyncio.run(full_procedure(input_file, output_file, api_token, tenant_id, cert_type))
