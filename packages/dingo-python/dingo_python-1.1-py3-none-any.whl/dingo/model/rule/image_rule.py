...
class ImageDuplicateDetection(BaseRule):
...
@Model.rule_register('QUALITY_INEFFECTIVENESS', [])
class ImageTextSimilarity(BaseRule):
    custom_config = DynamicRuleConfig(threshold=0.17)
    ...
    def eval(cls, input_data: MetaData) -> ModelRes:
        ...
        from dingo.model.rule.common.image_util import download_similar_tool
        ...
        if cls.custom_config.refer_path is None:
            similar_tool_path = download_similar_tool()
        else:
            similar_tool_path = cls.custom_config.refer_path[0]
        ...
        if average_score < cls.custom_config.threshold:
            res.error_status = True
            res.type = 'QUALITY_INEFFECTIVENESS'
            res.name = cls.__name__
...
if __name__ == '__main__':
    ...
    tmp = ImageDuplicateDetection().eval(data)
    print(tmp)