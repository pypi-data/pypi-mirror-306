...
@Model.llm_register('transformers_api')
class TransformersApi(BaseLLM):
    model = None
    tokenizer = None

    dynamic_config = DynamicLLMConfig(prompt_id="CONTEXT_RELEVANCE_PROMPT")
    ...
    def call_api(cls, input_data: MetaData) -> ModelRes:
        ...
        while attempts < 3:
            try:
                response = cls.generate_words(get_prompt(cls.dynamic_config) % input_data.content)
