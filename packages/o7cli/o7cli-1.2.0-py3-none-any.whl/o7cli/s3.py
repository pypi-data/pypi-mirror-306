...
class S3(Base):
    ...
    def load_buckets(self):
        ...
        for bucket in self.buckets:
            ...
            # Policies
            bucket["Policies"] = self.get_bucket_policies(bucket=bucket["Name"])
            bucket["IsSSL"] = self.verify_ssl_requests_only(
                bucket=bucket["Name"], statements=bucket["Policies"]
            )
    ...
    def load_bucket(self):
        ...
        self.bucket_info = {}
        self.bucket_info["Policies"] = self.get_bucket_policies(bucket=bucket)
        self.bucket_info["IsSSL"] = self.verify_ssl_requests_only(
            bucket=bucket, statements=self.bucket_info["Policies"]
        )
    ...
    def display_buckets(self):
        ...
        params = TableParam(
            columns=[
                ...
                ColumnParam(title="Region", type="str", data_col="Region"),
                ColumnParam(title="SSL", type="str", data_col="IsSSL"),
            ]
        )
        print()
    ...
    def display_bucket(self):
        ...
        if self.bucket_info["IsSSL"]:
            print(o7t.format_normal("SSL Request is enforced"))
        else:
            print(o7t.format_alarm("SSL Request is not enforced"))
    ...
    def menu_folder(self):
        ...
        obj.add_option(
            o7m.Option(
                key="r", name="Raw", callback=lambda: pprint.pprint(self.objects)
            )
        )
...
if __name__ == "__main__":
    ...
    the_s3.load_bucket_extra("appstream2-36fb080bb8-ca-central-1-649732530399")
    the_s3.load_bucket_extra(
        "apistack-apibasicservicedownloadbucket31ba7d2a-jg2n5e641xqe"
    )
