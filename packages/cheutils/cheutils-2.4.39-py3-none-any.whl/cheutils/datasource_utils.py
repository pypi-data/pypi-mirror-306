...
class DBTool(object):
    ...
    def __mssql_pyodbc_creator(self, *args, **kwargs):
        try:
            ...
            try:
                ...
                LOGGER.error('An error occured while connecting to the database: {}'.format(tb))
    ...
    def __get_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish default connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_pyodbc_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish pyodbc connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_pymssql_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish pymssql connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_psycopg2_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish psycopg2 connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_pymysql_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish pymysql connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_mysqlclient_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish mysqldb connection {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def __create_mysqlconnector_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish mysqlconnector connection = {}', traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
    ...
    def read_db_table(self, db_table='', index_col=None, coerce_float=False, parse_dates=None,
        ...
        try:
            ...
            tb = ex.__traceback__
            LOGGER.error('FAILURE: {}', traceback.print_tb(tb))
            ...
            with self.__get_connection() as connection:
                ...
                if query_string is None:
                    ...
                    try:
                        ...
                        tb = normEx.__traceback__
                        LOGGER.error('Optimized read failure = {}', traceback.print_tb(tb))
                        raise DBToolException(normEx).with_traceback(tb)
                else:
                    LOGGER.debug('Optimized read ...')
                    try:
                        ...
                        tb = normEx.__traceback__
                        LOGGER.error('Optimazed read failure = {}', traceback.print_tb(tb))
                        raise DBToolException(normEx).with_traceback(tb)
    ...
    def bulk_insert(self, data_df: pd.DataFrame, db_table: str, primary_keys: list, truncate=False):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing bulk insert: {}', traceback.print_tb(tb))
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failed: executing bulk_insert to table: {} - {}', db_table, traceback.print_tb(tb))
...
class DBToolFactory(object):
    ...
    def get_tool(self, ds_key, verbose=False) -> DBTool:
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing get_tool for datasource key: {} - {}', ds_key, traceback.print_tb(tb))
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('DBToolFactory encountered a problem configuring datasource with key = {}, {}', ds_key, traceback.print_tb(tb))
            raise DBToolException(err).with_traceback(tb)
...
class DSWrapper(object):
    ...
    def apply_to_datasource(self, data_df, ds_config=None):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing apply_to_datasource: {}', traceback.print_tb(tb))
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failure: {}', traceback.print_tb(tb))
