...
class DBTool(object):
    ...
    def __mssql_pyodbc_creator(self, *args, **kwargs):
        try:
            ...
            try:
                ...
                LOGGER.error('An error occured while connecting to the database: {}', traceback.print_tb(sys.exc_info()[2]))
    ...
    def __get_connection(self, autocommit=True):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failure to establish default configured connection = {}', traceback.print_tb(sys.exc_info()[2]))
            raise DBToolException(err).with_traceback(tb)
    ...
    def execute_query(self, query: str) -> None:
        ...
        try:
            ...
            LOGGER.error('FAILURE: {}', traceback.print_tb(sys.exc_info()[2]))
            ...
            with self.__get_connection() as connection:
                ...
                if query_string is None:
                    ...
                    try:
                        ...
                        tb = err.__traceback__
                        LOGGER.error('Optimized read failure = {}', traceback.print_tb(sys.exc_info()[2]))
                        raise
                else:
                    LOGGER.debug('Optimized read ...')
                    try:
                        ...
                        tb = err.__traceback__
                        LOGGER.error('Optimazed read failure = {}', traceback.print_tb(sys.exc_info()[2]))
                        raise
        ...
        with self.__get_connection() as connection:
            with connection.begin():
                try:
                    ...
                    msg = LOGGER.error("DB error: {}", traceback.print_tb(sys.exc_info()[2]))
                    tb = err.__traceback__
                    raise DBToolException(err).with_traceback(tb)
                except Exception as err:
                    tb = err.__traceback__
        ...
        with self.__get_connection() as connection:
            with connection.begin():
                try:
                    ...
                    tb = err.__traceback__
                    LOGGER.error("DB error: {0}", traceback.print_tb(sys.exc_info()[2]))
                    raise DBToolException(err).with_traceback(tb)
                except Exception as err:
                    tb = err.__traceback__
    ...
    def bulk_insert(self, data_df: pd.DataFrame, db_table: str, primary_keys: list, truncate=False):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing bulk insert: {}', traceback.print_tb(sys.exc_info()[2]))
            raise
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failed: executing bulk_insert to table: {} - {}', db_table, traceback.print_tb(sys.exc_info()[2]))
...
class DBToolFactory(object):
    ...
    def get_tool(self, ds_key, verbose=False) -> DBTool:
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing get_tool for datasource key: {} - {}', ds_key, traceback.print_tb(sys.exc_info()[2]))
            raise
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('DBToolFactory encountered a problem configuring datasource with key = {}, {}', ds_key, traceback.print_tb(sys.exc_info()[2]))
            raise DBToolException(err).with_traceback(tb)
...
class DSWrapper(object):
            ...
            if chunksize is None:
                ...
                for data_df in data_read:
                    ...
                    try:
                        ...
                        if is_raw:
                            LOGGER.warning('WARNING: Processing a raw-templated file: {}', traceback.print_tb(sys.exc_info()[2]))
                        else:
                            raise DSWrapperException(ignore)
                    # do any additional processing
                    if dropna_cols is not None:
    ...
    def apply_to_datasource(self, data_df, ds_config=None):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing apply_to_datasource: {}', traceback.print_tb(sys.exc_info()[2]))
            raise
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failure: {}', traceback.print_tb(sys.exc_info()[2]))
    ...
    def __save_to_datafile(self, data_df, ds_config=None):
        ...
        if is_csv:
            try:
                ...
                LOGGER.error('FAILED attempt to save data to csvfile: {}, {}', path_to_data_file, traceback.print_tb(sys.exc_info()[2]))
            ...
            try:
                ...
                LOGGER.error('FAILED attempt to save data to excelfile: {}', path_to_data_file, traceback.print_tb(sys.exc_info()[2]))
                raise DSWrapperException(ex)