...
from .tools.macros_data import MacroData, IdeToTest
...
class MaestroMacros(MaestroMacroManager):
    ...
    def on_page_context(self, _ctx, page:Page, *, config:MkDocsConfig, nav:Navigation):
        ...
        if self.is_page_with_something_to_insert(page):
            ...
            if self.is_test_ides_page(page):
            ...
            if page.url in self._pages_with_run_macro:
                m = self._pages_with_run_macro[page.url]
                chunks.append(
                    f'<span id="{ m.storage_id }" class="{ HtmlClass.py_mk_hidden }">'
                        f'run: { m.storage_id }'
                    '</span>'
                )
    ...
    def is_test_ides_page(self, page:Optional[Page]=None):
        """
