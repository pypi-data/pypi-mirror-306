...
def _logger(name, flush=False, top_level=True, parallel_log_config=None):
    ...
    if logger.name == "_extract_timeseries" and not top_level and parallel_log_config:
        # Only QueueHandler will be in handler list
        logger.handlers.clear()
        if "queue" in parallel_log_config: logger.addHandler(QueueHandler(parallel_log_config["queue"]))
        if "level" in parallel_log_config: logger.setLevel(parallel_log_config["level"])
    ...
    if not default_handlers and not (parallel_log_config or logger.name == "_extract_timeseries" and top_level):
