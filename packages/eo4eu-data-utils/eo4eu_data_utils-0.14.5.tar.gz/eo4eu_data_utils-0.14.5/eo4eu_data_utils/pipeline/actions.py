...
class Filter(Action):
    def __init__(self, predicates: list[Any], select_by: str = "_selector"):
...
class Switch(Action):
    def __init__(self, cases: list[Any,Action], select_by: str = "_selector"):
        self._cases = cases
        self._select_by = select_by
...
class Branch(Action):
    def __init__(self, predicate: Any, action: Action, select_by: str = "_selector"):
...
def _prepare_predicate(
    ...
    try:
        ...
        if isinstance(predicate, str):
            ...
            return functools.partial(
                lambda path, regex, attr_name: invert ^ (
                    regex.fullmatch(str(path.attr(attr_name))) is not None
                ),
                regex = re.compile(predicate.replace(".", "\\.").replace("*", ".*")),
                attr_name = attr
            )
        return functools.partial(
            lambda path, predicate, attr_name: path.attr(attr_name) == predicate,
            predicate = predicate,
            attr_name = attr
        )
    except Exception:
