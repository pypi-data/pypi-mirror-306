...
class Swarms:
    def __init__(self, vectr_base: VectrBase, host: str, port: int, bootstrap_host: str = None, bootstrap_port: int = None):
    ...
    async def initialize(self):
        node = KademliaNode(host=self.host, port=self.port)
        await node.start()
        if self.bootstrap_host and self.bootstrap_port:
            await node.bootstrap(self.bootstrap_host, self.bootstrap_port)

        message_broker = MessageBroker(node)
        vector_db = VectorDB(self.vectr_base, node)
        
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
            raise ValueError("ANTHROPIC_API_KEY environment variable is not set")
        
        anthropic_llm = AnthropicLLM(api_key=api_key)

        self.coordinator = MultiAgentCoordinator(message_broker, node, vector_db, anthropic_llm)

    async def run(self, task: dict):
        if not self.coordinator:
            await self.initialize()
        await self.coordinator.add_task(task)
        await self.coordinator.run()

    async def run_rag_workflow(self, query: str):
        if not self.coordinator:
            await self.initialize()
        return await self.coordinator.run_rag_workflow(query)

    async def analyze_task(self, task: dict):
        required_skills = self.task_analyzer.identify_required_skills(task)
        complexity = self.task_analyzer.estimate_complexity(task)
        dependencies = self.task_analyzer.identify_dependencies(task)
        estimated_duration = self.task_analyzer.estimate_duration(task)
    ...
    async def create_custom_agent(self, agent_type: str, agent_id: str, **kwargs):
        if not self.coordinator:
            await self.initialize()
        return await self.coordinator.create_agent(agent_type, agent_id, **kwargs)
