...
Timer::Timer(const Callback& callback, float tickRate, bool loop, bool absolute) :
  _callback(callback),
  _loop(loop),
  _absolute(absolute),
  _active(true)
...
void Timer::restart(TimerManager* timerManager) {
  if (timerManager == nullptr)
    throw std::invalid_argument("Invalid TimerManager object");

  _active = true;
  timerManager->addTimer(shared_from_this());
}
...
void Timer::pythonBindings(py::module& m) {
  py::class_<Timer, std::shared_ptr<Timer>>(m, "Timer")
    .def(py::init<>())
    .def(py::init<const Callback&, float, bool, bool>())
...
void TimerManager::pythonBindings(py::module& m) {
  py::class_<TimerManager, std::shared_ptr<TimerManager>, Component>(m, "TimerManager")
    ...
    .def("addTimer", py::overload_cast<const Callback&, float, bool>(&TimerManager::addTimer))
