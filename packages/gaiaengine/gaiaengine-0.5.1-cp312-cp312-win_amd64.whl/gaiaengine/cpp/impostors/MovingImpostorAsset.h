...
/** One instance that will load the textures and metadata used commonly by
  * ImpostorAnimationHandler instances.
  */
class MovingImpostorAsset : public ImpostorAsset {
public:
  MovingImpostorAsset(const std::vector<std::string>& assetPaths);
  MovingImpostorAsset(const std::vector<std::string>& assetPaths, const std::vector<float>& animDurations);
  MovingImpostorAsset(const std::vector<std::string>& assetPaths, const std::vector<float>& animDurations, const std::vector<float>& replayDelays);
