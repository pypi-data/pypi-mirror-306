...
Plot::Plot(UIManager* uiManager, const std::string& title, int flags):
  UIElement(uiManager),
  _title(title),
  _flags(flags)
{
...
void Plot::buildFrame() {
  ...
  if (ImPlot::BeginPlot(_title.c_str(), _xAxisTitle.empty() ? nullptr : _xAxisTitle.c_str(), _yAxisTitle.empty() ? nullptr : _yAxisTitle.c_str(), glm::vec2(-1, -1), ImPlotFlags_NoTitle)) {
...
void Plot::pythonBindings(py::module& m) {
  py::class_<Plot, std::shared_ptr<Plot>, UIElement>(m, "Plot")
    .def(py::init<UIManager*, const std::string&>())
    .def(py::init<UIManager*, const std::string&, int>())
