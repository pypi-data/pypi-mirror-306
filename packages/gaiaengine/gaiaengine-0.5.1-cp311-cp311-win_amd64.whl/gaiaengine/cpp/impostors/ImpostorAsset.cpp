...
std::vector<std::shared_ptr<SDL_Palette>> ImpostorAsset::Palettes;

std::shared_ptr<SDL_Surface> ImpostorAsset::loadSpriteSheet(const std::string& texturePath, const std::string& infoFilePath, std::vector<SpriteInfo>& currentSprites) {
...
std::shared_ptr<SDL_Surface> ImpostorAsset::loadSLP(const std::string& filePath, std::vector<SpriteInfo>& currentSprites) {
...
std::shared_ptr<SDL_Palette> ImpostorAsset::selectPalette(uint32_t paletteID) {
  switch (paletteID) {
  case 0x00:
  case 0x10:
  case 0x18:
...
void ImpostorAsset::processSLPDrawingCommands(std::shared_ptr<SDL_Surface> surface, std::shared_ptr<SDL_Palette> palette, std::ifstream& input, int currentPixelX, int currentPixelY) {
...
ImpostorAsset::ImpostorAsset(const std::string& assetPath):
  ImpostorAsset(std::vector<std::string>{ assetPath })
{}

ImpostorAsset::ImpostorAsset(const std::vector<std::string>& assetPaths):
  _assetPaths(assetPaths)
{}

ImpostorAsset::ImpostorAsset(const Color& color, const glm::ivec2& size) :
  _fallbackColor(color),
  _fallbackSize(size)
{}

void ImpostorAsset::load() {
...
glm::vec4 ImpostorAsset::getTexRectangle(int textureID, int spriteID) const {
...
void ImpostorAsset::pythonBindings(py::module& m) {
  py::class_<ImpostorAsset, std::shared_ptr<ImpostorAsset>>(m, "ImpostorAsset")
    .def(py::init<>())
    .def(py::init<Color, glm::ivec2>())
    .def(py::init<const std::string&>())
    .def(py::init<const std::vector<std::string>&>())
    .def("isLoaded", &ImpostorAsset::isLoaded)
    .def("load", &ImpostorAsset::load)
    .def("textureExists", &ImpostorAsset::textureExists)
    .def_readwrite("assetPaths", &ImpostorAsset::_assetPaths)
    .def_readwrite("unitID", &ImpostorAsset::_unitID);
}