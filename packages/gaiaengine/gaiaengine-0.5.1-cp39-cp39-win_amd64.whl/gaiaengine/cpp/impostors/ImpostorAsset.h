...
// Lazily loaded impostor asset reference
// Gets loaded when asked or when an impostor is created using it
class ImpostorAsset {
public:
  ImpostorAsset() = default;
  ImpostorAsset(const Color& color, const glm::ivec2& size);
  ...
  ImpostorAsset(const std::string& assetPath);
  ImpostorAsset(const std::vector<std::string>& assetPaths);
  ...
  inline bool textureExists(int textureID) const { return textureID >= 0 && textureID < _textureArray->getCount() && _textureArray->getTexSize(textureID).x > 0; }
