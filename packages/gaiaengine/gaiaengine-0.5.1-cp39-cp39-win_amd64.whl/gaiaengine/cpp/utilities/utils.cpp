...
void ut::convertToPositiveRectangle(glm::ivec4& rect) {
  rect.z += rect.x;
  rect.w += rect.y;
...
bool ut::getRectanglesIntersect(glm::ivec4 a, glm::ivec4 b) {
  convertToPositiveRectangle(a);
  convertToPositiveRectangle(b);
...
std::string ut::textFileToString(const std::string& path) {
  ...
  if (ops == nullptr) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to load file %s SDL Error: %s\n", path.c_str(), SDL_GetError());
    return "";
  }
