...
from .aoe_moving_impostor import AoEMovingImpostor
...
class AoELoader:
    ...
    def __init__(self, scenario_file, **kwargs):
        ...
        self.create_impostor = AoELoader.create_impostor
        if 'create_impostor' in kwargs:
            self.create_impostor = kwargs['create_impostor']
    ...
    def init_window(self, window):        
        if not hasattr(window, "terrain") or window.terrain is None:
            window.terrain = window.create(AoETerrain, self._map_data)
        else:
            window.terrain.reset(self._map_data)

        if not isinstance(window.camera, gaia.HeightmapCamera):
            window.camera = gaia.HeightmapCamera(window.terrain)
            window.event_manager.create(gaia.HeightmapCameraInput, window.camera, window.timer_manager)

        if not hasattr(window, "impostor_manager") or window.impostor_manager is None:
            window.impostor_manager = window.create(gaia.ImpostorManager, window.terrain)
            window.impostor_manager.baseImpostorSizeFactor = 0.01
        else:
            for impostor in window.impostor_manager.getImpostors():
                impostor.delete()
        ...
        for key, unit_info in self._unit_data.items():
            ...
            try:
                ...
                [self.create_impostor(window.impostor_manager, asset, gaia.Vec2(y, x), rotation) for
                x, y, rotation
                in zip(it, it, it)]
            except KeyError:
    ...
    @staticmethod
    def create_impostor(impostor_manager, asset, position, rotation):
        class_to_create = AoEMovingImpostor if isinstance(asset, gaia.MovingImpostorAsset) else gaia.Impostor
        return impostor_manager.create(class_to_create, asset, position)
    ...
    def get_asset_from_unit_id(unit_id):
        ...
        if AoELoader.is_moving_unit(unit_id):
            asset = gaia.MovingImpostorAsset(slp_file_list, AoELoader.ANIM_DURATIONS[str_unit_id], AoELoader.REPLAY_DELAYS[str_unit_id])
        else:
            asset = gaia.ImpostorAsset(slp_file_list)
