...
class AoEMovingImpostor(gaia.MovingImpostor):
    def __init__(self, *args):
        ...
        self.onTargetOrDirectionSet.bind(gaia.Callback(self.start_moving_animation))
        self.onStoppedMoving.bind(gaia.Callback(self.start_idle_animation))
    ...
    def decay(self):
        if self.isAnimationLoaded(AoEAnim.DECAY):
            self.startAnimation(AoEAnim.DECAY, gaia.Callback(self.delete))
        else:
            self.delete()
    ...
    def die(self):
        if not self.is_dead:
            ...
            if self.isAnimationLoaded(AoEAnim.DIE):
                self.startAnimation(AoEAnim.DIE, gaia.Callback(self.decay))
            else:
                self.decay()