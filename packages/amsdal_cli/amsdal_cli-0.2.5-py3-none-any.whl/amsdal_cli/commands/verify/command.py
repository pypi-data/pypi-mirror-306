...
def verify_command(
    ...
    *,
    ...
    print(rich_info('Syntax checking...'), end=' ')
    ...
    if errors:
        for error in errors:
            print(rich_error(f'File: {error.file_path.resolve()}: {error.message}'))
            print(rich_error(str(error.details)))
    ...
    print(rich_info('Build models checking...'), end=' ')
    with tempfile.TemporaryDirectory() as _temp_dir:
        ...
        try:
            ...
            print(rich_error(f'Failed: {ex} - {traceback.format_exc()}'))
