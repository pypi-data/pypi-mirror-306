...
def init_app_context(
    ...
    *,
    ...
    if not cli_config.exists():
        print(rich_error(f'The directory "{app_path.resolve()}" does not contain AMSDAL application.'))
    ...
    with cli_config.open('rt') as config_file:
        try:
            ...
            print(rich_error(f'The config file "{cli_config.resolve()}" is corrupted.'))
