...
def new_command(
    ...
    if not output_path.exists():
        print(rich_error(f'The output path "{output_path.resolve()}" does not exist.'))
        raise typer.Exit

    if output_path.is_file():
        print(rich_error(f'The output path "{output_path.resolve()}" is not a directory.'))
        raise typer.Exit
    ...
    if output_path.exists():
        if output_path.is_file():
            print(rich_error(f'The path "{output_path.resolve()}" is not a directory.'))
            raise typer.Exit

        if any(output_path.iterdir()):
            print(rich_error(f'The directory "{output_path.resolve()}" is not empty.'))
            raise typer.Exit
    ...
    print(rich_success(f'The application is successfully created in {output_path.resolve()}'))
