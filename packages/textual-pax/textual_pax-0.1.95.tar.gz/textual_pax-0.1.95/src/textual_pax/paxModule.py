...
import pandas as pd
import os
from .tui_exceptions import TerminalNotFoundError, IPnotWhitelisted
...
def get_terminal_properties(serial_numbers):
  """
  Retrieves terminal properties for a list of serial numbers.

  Args:
    serial_numbers: A list of serial numbers.
    pax_terms: An instance of the PaxTerms class.

  Returns:
    A list of dictionaries, each containing terminal properties.
  """

  results = []
  for serialNo in serial_numbers:
    paxterm = PaxTerms(serialNo)
    terminal_properties = paxterm.termtool.get(serialNo[:3], {})
    if terminal_properties:
      terminal_properties["serialNo"] = serialNo
      results.append(terminal_properties)
      if terminal_properties.get("hasAccessory"):
        accessory_info = terminal_properties.get("accessory", {})
        terminal_properties["accessoryName"] = accessory_info.get("name")
  return results
...
async def buildRequest(session,operation:str,**kwargs):
    ...
    async with method(term.fullurl,json=term.body,headers=headers) as resp:
        ...
        if term_data['businessCode'] == 13108:
