...
from lion_core.funcs.func.lcall import alcall
from lion_core.generic import LogManager, Node, Pile, Progression
from lion_core.models import FieldModel
from lion_core.protocols.operatives.action import ActionResponseModel
from lion_core.protocols.operatives.operative import NewModelParams, Operative
...
class Branch(Node):
    ...
    def to_df(self, *, progress: Progression = None) -> pd.DataFrame:
        if progress is None:
            return self.msgs.messages.to_df(columns=MESSAGE_FIELDS)
        msgs = [
    ...
    async def operate(
        ...
        **kwargs,
        ...
        if (
            invoke_actions
            and hasattr(response_model, "action_required")
            and response_model.action_required
            and hasattr(response_model, "action_requests")
