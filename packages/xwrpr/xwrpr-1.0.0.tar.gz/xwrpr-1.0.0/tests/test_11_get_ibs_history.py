    ...
    with caplog.at_level(log_level):
        ...
        try:
            ...
            with pytest.raises(ValueError):
                ibs_history = xtb.getIbsHistory(start = datetime.now()-timedelta(days = 2), end = datetime.now()+timedelta(days = 1))
            logger.debug("Checking failure conditions: start > end")
            with pytest.raises(ValueError):
                ibs_history = xtb.getIbsHistory(start = datetime.now(), end = datetime.now()-timedelta(days = 2))
