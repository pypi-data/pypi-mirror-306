...
def calculate_verbosity(level: int):
    if level >= 2:
...
def create_console_handler(verbosity: int, console_format: str):
...
def create_file_handler(log_filepath: str, file_format: str):
...
def create_queue_handler(queue: queue.Queue[logging.LogRecord]):
    handler = logging.handlers.QueueHandler(queue)
    return handler


def create_queue_listener(queue: queue.Queue[logging.LogRecord], *handlers: logging.Handler):
...
def setup_logger(verbosity: int, log_filepath: str, console_format: str, file_format: str):
