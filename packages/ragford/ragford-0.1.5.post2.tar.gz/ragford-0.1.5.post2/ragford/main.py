...
def main():
    ...
    PLAYHT_DEFAULT_VOICE = os.getenv('PLAYHT_DEFAULT_VOICE')
    ...
    playht_model = PlayHTModel(user_id=PLAYHT_USER_ID, api_key=PLAYHT_API_KEY, voice=PLAYHT_DEFAULT_VOICE)
    ...
    async def speak_command(ctx: lightbulb.Context):
        ...
        # Generate audio using PlayHT model with specified audio_path
        try:
            # Define a temporary path to save the audio file
            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp_file:
                audio_file_path = tmp_file.name
    
            # Generate audio and save it directly to the temporary file
            playht_model.predict(text=text_to_speak, audio_path=audio_file_path)
