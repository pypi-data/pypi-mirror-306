...
SINGLE_QUOTES_MAP = str.maketrans({"‘": "'", "’": "'", "‛": "'", "❛": "'", "❜": "'"})
DOUBLE_QUOTES_MAP = str.maketrans(
    {
        "“": '"',
        "”": '"',
        "„": '"',
...
def get_quotes(text: str) -> list[int]:
    """
...
def clear_subquotes(text: str) -> str:
    ...
    """
    return re.sub("''", "'", text.translate(SINGLE_QUOTES_MAP))


def set_simple_quotes(text: str, simplify_everything: bool = False) -> str:
    """
    ...
    """
    res = re.sub('""', '"', text.translate(DOUBLE_QUOTES_MAP))
    if simplify_everything:
        return clear_subquotes(res)
    return res
...
def requoter(input_text: str, use_simple: bool = False) -> str:
    ...
    """
    input_text = set_simple_quotes(input_text, use_simple)
    if use_simple:
        return input_text

    positions = get_quotes(input_text)
