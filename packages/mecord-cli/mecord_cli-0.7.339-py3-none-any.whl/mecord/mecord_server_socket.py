...
class MecordTaskExecutorThread(Thread):
    ...
    def idlePower(self, ext=''):
        if self.mtype:
            ...
            num = (self.max_counter - self.cur_counter) if self.low_cur_counter >= 0 else 0
        if self.ex_flag and self.task_queue.qsize() == 0 and num == 0:
            self.cur_counter = 0
    ...
    def webService(self):
        ...
        async def nextTask():
            try:
                ...
                return {'msg': 'success', 'cur_counter': self.cur_counter}
            except Exception as e:
