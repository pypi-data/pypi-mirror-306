...
"""
Vishesh: This module provides functions to solve the Traveling Salesman Problem (TSP) using the Held-Karp algorithm.

Functions:
    tsp(graph, start):
        Solves the TSP for a given graph and starting city using dynamic programming.
        ...
        Returns:
            tuple: A tuple containing the minimum cost, the path taken, and a memoization dictionary for the paths.

    show_path(graph, start_city):
        Visualizes the solution to the TSP using matplotlib and networkx.
        
        Parameters:
            graph (list of list of int): The adjacency matrix representing the graph.
            start_city (int): The starting city.
