...
CHALK_SKIP_PRECISE_UNDERSCORE_SOURCE_COLUMN = os.getenv("CHALK_SKIP_PRECISE_UNDERSCORE_SOURCE_COLUMN", "") == "1"
"""
    ...
    def __init__(self):
        ...
        if current_frame is not None:
            definition_location = UnderscoreDefinitionLocation(
                ...
                column=1,  # Currently, this is just a guess, due to lack of reliable column info directly on Python frame
            )

            # Attempt to get the exact AST node to get a more-precise caller location
            # for error reporting purposes:
            if not CHALK_SKIP_PRECISE_UNDERSCORE_SOURCE_COLUMN:
