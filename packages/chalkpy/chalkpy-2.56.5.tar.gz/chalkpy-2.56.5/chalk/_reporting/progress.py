...
class ProgressService:
    ...
    def handle_explainer_text_if_erring(self):
        if self.explainer_text:
            # FIXME -- why are we modifying a private variable?
            assert self.caller_method is not None
            self.explainer_text._text = [  # pyright: ignore[reportPrivateUsage]
    ...
    def poll_report(
        ...
        while datetime.now() < must_receive_next_report_by and datetime.now() < completion_deadline:
            time.sleep(0.5)
            batch_report = self.client.get_batch_report(self.operation_id, self._environment_id, computer_id)
    ...
    def await_operation(
        self, computer_id: int, must_fail_on_resolver_error: bool, timeout: float | timedelta | None
    ) -> None:
        ...
        if self.main_progress is not None:
            ...
            main_task = self.main_progress.tasks[main_task_id]
        else:
            main_task_id = None
            main_task = None
        ...
        with ChalkLive(self.enclosure_panel, auto_refresh=True):
            for batch_report in self.poll_report(
                computer_id, timeout, completion_timeout=self.client.default_job_timeout
            ):
