    ...
    *,
    ...
    if feature_name not in class_features:
        ...
        raise _UnderscoreValidationError(
            f"The underscore expression '{repr(underscore)}' is invalid because the feature class '{class_namespace.namespace}' does not have a feature named '{underscore._chalk__attr}'"
        )
...
def _supplemental_validate_underscore_expression(
    ...
    *,
    ...
    if isinstance(underscore, UnderscoreAttr) and isinstance(underscore._chalk__parent, UnderscoreRoot):
        if underscore._chalk__attr in ("chalk_window"):
    ...
    if isinstance(underscore, UnderscoreAttr):
        ...
        if isinstance(parent_result, _ScalarExpr):
            raise _UnderscoreValidationError(
                f"The underscore expression '{repr(underscore)}' is invalid because '{underscore._chalk__parent}' is a feature value, not a namespace, so a feature called '{underscore._chalk__attr}' cannot be extracted from it"
            )
    ...
    if isinstance(underscore, UnderscoreBinaryOp):
        left = _supplemental_validate_underscore_expression(
            state=state,
            class_namespace=class_namespace,
            underscore=underscore._chalk__left,
        )
        right = _supplemental_validate_underscore_expression(
            state=state,
            class_namespace=class_namespace,
            underscore=underscore._chalk__right,
        )

        for side_name, side_result, side_underscore in [
            ("left", left, underscore._chalk__left),
            ("right", right, underscore._chalk__right),
        ]:
            if side_result is not None and isinstance(side_result, _NamespaceExpr):
                raise _UnderscoreValidationError(
                    f"The underscore expression '{repr(underscore)}' is invalid because the {side_name} input '{side_underscore}' is a feature namespace '{side_result.namespace}' which cannot be used as a scalar value"
                )
