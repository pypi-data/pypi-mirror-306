...
from ct_redis.cache import MyCtRedis, configure_redis_pool


def init_cache(app: Flask, logger: logging.Logger) -> MyCtRedis:
    """
    Initializes the MyCtRedis library and attaches it to the Flask app instance for centralized Redis caching.

    This function sets up a Redis caching extension for the application, allowing cached values to be
    stored and retrieved using the MyCtRedis client. It adds the MyCtRedis instance to the app’s extensions
    ...
    ```python
    from flask import Flask
    from ct_redis import init_cache
    import logging
    ...
    --------
    MyCtRedis
        The initialized MyCtRedis instance, configured for the application.
    """
    ...
    redis_instance = MyCtRedis(app.config['REDIS_CACHE'], logger,
