...
class MyCtRedis:
    """
    MyCtRedis provides caching functionality with Redis, utilizing a decorator-based caching approach.
    ...
    def __init__(self, is_cache_enabled, logger, redis_pool, expire_time=60):
        """
        Initializes the MyCtRedis instance.
    ...
    def cache1(self, template_cache_key: str, field: str, expire_time: Optional[int] = None,
    ...
    def commit(self, response):
        ...
        queued commands when the response has a successful (200) status code.
        This ensures that commands are committed only for successful requests,
        helping maintain data consistency and avoid unnecessary Redis writes.
        ...
        """
        try:
            if 'pipe' in g and response.status_code == 200:
                g.pipe.execute()
        except Exception as error:
            self.logger.info(f"Failed to commit changes to Redis: {e}")
