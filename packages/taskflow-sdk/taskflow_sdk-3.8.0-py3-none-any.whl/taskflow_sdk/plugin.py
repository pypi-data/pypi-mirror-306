...
hookspec = pluggy.HookspecMarker("taskflow_sdk")
...
def plugin_manager() -> pluggy.PluginManager:
    ...
    _plugin_manager = pluggy.PluginManager("taskflow_sdk")
    _plugin_manager.add_hookspecs(sys.modules["taskflow_sdk.plugin"])
...
def _discover_plugins(manager: pluggy.PluginManager) -> None:
    try:
        # taskflow_sdk_plugin is a custom package
        # which extends existing functionality on taskflow action server via plugins
        import taskflow_sdk_plugins

        taskflow_sdk_plugins.init_hooks(manager)
