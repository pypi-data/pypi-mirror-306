...
from taskflow_sdk.constants import (
    DEFAULT_ENCODING,
...
from taskflow_sdk.exceptions import (
    FileIOException,
...
def check_version_compatibility(taskflow_version: Optional[Text]) -> None:
    """Check if the version of taskflow and taskflow_sdk are compatible.
    ...
    taskflow and taskflow_sdk.

    Args:
        taskflow_version: A string containing the version of taskflow that
                        is making the call to the action server.

    Raises:
        Warning: The version of taskflow version unknown or not compatible with
                    this version of taskflow_sdk.
    """
    # Check for versions of Taskflow that are too old to report their version number
    if taskflow_version is None:
        warnings.warn(
            f"You are using an old version of taskflow which might "
            f"not be compatible with this version of taskflow_sdk "
            f"({taskflow_sdk.__version__}).\n"
            ...
            f"both taskflow and taskflow_sdk."
        )
        return

    taskflow = taskflow_version.split(".")[:-1]
    sdk = taskflow_sdk.__version__.split(".")[:-1]

    if taskflow != sdk:
        warnings.warn(
            f"Your versions of taskflow and "
            f"taskflow_sdk might not be compatible. You "
            f"are currently running taskflow version {taskflow_version} "
            f"and taskflow_sdk version {taskflow_sdk.__version__}.\n"
