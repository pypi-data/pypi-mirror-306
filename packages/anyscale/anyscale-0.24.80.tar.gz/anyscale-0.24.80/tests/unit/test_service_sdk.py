...
from anyscale.utils.workspace_notification import WorkspaceNotificationAction
...
class TestDeploy:
    ...
    def test_basic(
        ...
        assert fake_client.rolled_out_model == ApplyServiceModel(
            ...
            config=ExternalAPIServiceConfig(
                ...
                protocols=Protocols(grpc=GrpcProtocolConfig(enabled=False)),
            ),
        )
    ...
    def test_grpc_options_enabled(
        ...
        assert fake_client.rolled_out_model == ApplyServiceModel(
            ...
            config=ExternalAPIServiceConfig(
                access=AccessConfig(use_bearer_token=True),
                protocols=Protocols(
                    grpc=GrpcProtocolConfig(enabled=True, service_names=["hello."])
                ),
            ),
        )

    def test_grpc_options_no_service_names(
        self,
        service_sdk_with_fakes: Tuple[
            ServiceSDK, FakeAnyscaleClient, TestLogger, FakeTimer
        ],
    ...
    def test_in_place_during_rollout(
        ...
        service = fake_client.get_service(
            name="test-service-name", cloud=None, project=None
        )
    ...
    def test_workspace_notification_is_sent(
        ...
        service_id = fake_client.get_service(
            "test-service-name", cloud=None, project=None
        ).id
        assert len(fake_client.sent_workspace_notifications) == 1
        ...
        assert fake_client.sent_workspace_notifications[0].body.startswith(
            "Service 'test-service-name' deployed (version ID: "
        )
...
class TestStatus:
    ...
    def test_status_with_projects(
        ...
        fake_client.register_project_by_name(
            name=fake_client.DEFAULT_PROJECT_NAME,
            project_id=fake_client.DEFAULT_PROJECT_ID,
        )
        fake_client.register_project_by_name(name=project, cloud=cloud)

        service_id = sdk.deploy(
            ServiceConfig(
                applications=[{"import_path": "main:app"}],
                name=name,
                cloud=cloud,
        ...
        # deploy another service with the same name, under the default project
        sdk.deploy(
            ServiceConfig(
                applications=[{"import_path": "main:app"}], name=name, cloud=cloud
            )
        )

        # ensure that the project filter works
        status: ServiceStatus = sdk.status(name=name, cloud=cloud, project=project)
        assert status.id == service_id
        assert status.primary_version.config.project == project  # type: ignore
...
class TestRollback:
    def test_not_found(
        ...
        with pytest.raises(
            ...
            sdk.rollback(name="does-not-exist")
    ...
    def test_max_surge_percent(
        ...
        sdk.rollback(name="test-name", max_surge_percent=50)
