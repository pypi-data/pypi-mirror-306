...
def _get_mesh(mesh_type):
    ...
    mesh_fine_path = (
        parent_dir / f"meshes/mesh_{str(0.001).replace('.', '_')}__width_{10}.msh"
    )

    mesh_default_path = (
        parent_dir / f"meshes/mesh_{str(0.01).replace('.', '_')}__width_{10}.msh"
    )

    mesh_wide_path = (
        parent_dir / f"meshes/mesh_{str(0.05).replace('.', '_')}__width_{20}.msh"
    )
