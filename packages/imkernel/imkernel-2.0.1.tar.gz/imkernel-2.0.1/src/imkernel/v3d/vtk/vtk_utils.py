...
# region new
def show_obj_stl(file_path: str, color: str = None, show_edges: bool = False):
...
def show_obj_jupyter(file_path: str, color: str = None):
    mesh = pv.read(file_path)
    plotter = pv.Plotter()
    plotter.add_mesh(mesh, color=color, show_edges=True)
    # plotter.add_bounding_box()  # 添加包围盒
    # plotter.add_axes()  # 添加坐标轴
    # plotter.camera.zoom(1.5)  # 调整相机视角
    #
    # # 添加光照效果
    ...
    #
    # # 设置背景渐变
    # plotter.set_background('white', 'lightblue')
