...
from experimentalTreatingIsiPol.docConfig import _alpha, _beta, _gamma, _delta, _epsilon, _zeta, _eta
...
class SeveralMechanicalTestingFittingLinear():
    ...
    def __init__(self, docConfig: str, archive_name: str,
        ...
        basta executar a função `print_machines`:

        ```python
        from experimentalTreatingIsiPol.machines import print_machines
        print_machines() 
        ```
        Obs.: futuramente, o nome desse argumento irá mudar. Faz mais sentido pensar em um nome de argumento neutro, e criar 
        uma tabela, ou dicionário, para tradução do que significa. Mas isso irá exigir muitos testes e refatoração.
        ...
        + *materialType* (str) : Tipo de material ensaiado. Irá influenciar na forma que ele interpreta a falha. 
        OBS.: Isso deverá mudar, pois tudo deve ficar subentendido pela norma utilizada.
        + *direction* (str) : Direção de ensaio. Isso influencia a forma de apresentação final dos dados. Para aperecer, por exemplo, $\nu_{12}$ ou $\nu_{21}$, por exemplo.
        + *calculus_method* (str) : Método de cálculo das propriedades mecânicas, ou também, a norma utilizada. 
        ...
        + *x_min* & *x_max* : Valor de input customizado para consideração da região inicial linear da curva. Caso não tenha sido utilizada nenhuma norma, eles serão utilizados 
        para o cálculo das respectivas propriedades mecânicas. 
    ...
    def __verifications(self, docConfig: str, archive_name :  str, 
        ...
        if testType == 'flexural' and docConfig not in ['_beta','_zeta']:
            raise Exception('Para o teste  de flexão, utilizar o docConfig = _beta')
        
        if testType == 'shear' and docConfig !='_eta':
            raise Exception('Para o teste  de cisalhamento, utilizar o docConfig = _eta')
        
        if testType == 'compression' and docConfig !='_beta': # confirmar depois
