...
from initialize import get_fdc, get_auth, is_initialized # todo .
from utilities.feature_data_renamings import vi_data_renaming, climate_data_renaming, soil_data_renaming, \
    forecast_data_renaming, historical_yield_data_renaming  # todo .

GREENHUB_REST_API_URL: str = 'https://us-central1-digitalyieldmonitoringplatform.cloudfunctions.net'
...
def _fetch_data(file_path: str) -> Optional[pd.DataFrame]:
    ...
    res = requests.get(GREENHUB_REST_API_URL + '/getFeatureData', params=params, headers=auth_headers)
...
def get_vi_data(
    ...
    if start_year > datetime.now().year or end_year > datetime.now().year:
        raise ValueError(f'The start_year and end_year must be less than or equal to the current year.')
    if start_year < 2000 or end_year < 2000:
        print(f'Attention: There is no VI data available before 2000.'
    ...
    if missing_years:
        print(f"Attention: no VI data found for {', '.join([str(i) for i in sorted(missing_years)])} {WARNING_COLOR_PRINT}")
...
def get_climate_data(
    ...
    if start_year > datetime.now().year or end_year > datetime.now().year:
        raise ValueError(f'The start_year and end_year must be less than or equal to the current year.')
    if start_year < 2000 or end_year < 2000:
        print(f'Attention: There is no climate data available before 2000.'
              f'\nFor more information please checkout the official greenhub.ai documentation. {WARNING_COLOR_PRINT}')
...
def get_180d_forecast_data(
    ...
    if time_aggregation not in TIME_RESOLUTIONS:
        raise ValueError(f'The given time_aggregation is not known. '
                         f'These time_aggregations are available: {", ".join(TIME_RESOLUTIONS)}.')
    if start_year < 2000:
        raise ValueError(f'There is no climate forecast data available before 2000. '
                         f'For more information please checkout the official '
                         f'greenhub.ai documentation. {WARNING_COLOR_PRINT}')
    if start_month < 1 or start_month > 12:
        raise ValueError(f'The given start_month is not between 1 (january) and 12 (december).')

    # construct all data files paths needed for the requested vi data
    feature_files_paths = [f'featureData/{country.upper()}/forecast/{spatial_resolution}/{time_aggregation}/'
...
def get_historical_yield_data(
    ...
    if year < 2000:
        print(f'Attention: There is no historical yield data available before 2000.'
              f'\nFor more information please checkout the official greenhub.ai documentation. {WARNING_COLOR_PRINT}')

    # construct data file path needed for the requested historical yield data
    feature_files_paths = [f'historicalData/{country}/{crop}/yield/{year}/{spatial_resolution}']
