...
class LLM(Chat):
    ...
    def chat(self,
        ...
        response = self.completions.create(
            ...
            messages=map(to_send, (messages or self.messages) + [message, system]),
            **kwargs
        )
    ...
    def stream(self,
        ...
        response = self.completions.create(
            model=model or self.model,
            messages=map(to_send, (messages or self.messages) + [message, system or self.system]),
            stream=True,
            **kwargs
        )
