...
class MainWindow(QMainWindow):
    ...
    def init_file_menu(self):
        ...
        open_layout = QAction('Open Layout With Dataset...', file_menu)
        open_layout.triggered.connect(self.open_layout)
        file_menu.addAction(open_layout)

        apply_layout = QAction('Apply Layout To Current Dataset...', file_menu)
    ...
    def open_layout(self, *args, fname=None):
        with self.app.server.updates_paused():
            if fname is None:
                fname = QFileDialog.getOpenFileName(
                    self, "Open Layout...", "", "Layout File (*.yml)", "")[0]
            if not fname:
                logger.info("No file provided, not opening layout")
                return False
            logger.info("Opening layout from '%s'", fname)
            self.app.prefs.add_recent_layout(fname)
            with open(fname) as f:
                layout = yaml.safe_load(f.read())
            try:
                if layout['dataset']['live']:
                    self.tabs.setCurrentIndex(0)
                    self.app.server.live_data.deserialise(layout['dataset'])
                else:
                    fd = self.open_dataset(fname=layout['dataset']['filename'])
                    fd.deserialise(layout['dataset'])
                tab = self.tabs.currentWidget()
                ...
                error = QErrorMessage(self)
                error.setModal(True)
                error.showMessage("Error while restoring layout: " + str(e))
                return False
            else:
                return True
