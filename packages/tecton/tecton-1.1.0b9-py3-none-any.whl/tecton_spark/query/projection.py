...
@attrs.frozen
class MultiRtfvCalculationSparkNode(SparkExecNode):
    input_node: SparkExecNode
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        calculation_sql_exprs = [expr(col.get_sql(with_alias=True)) for col in calculation_query_terms]

        return input_df.select("*", *calculation_sql_exprs)
