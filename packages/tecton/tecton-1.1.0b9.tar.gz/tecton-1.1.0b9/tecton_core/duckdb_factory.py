...
class DuckDBConfig:
    memory_limit: Optional[str] = None  # Use string to specify memory size, e.g., '1GB'
    num_threads: Optional[int] = None  # Number of threads for DuckDB


def create_connection(duckdb_config: DuckDBConfig = DuckDBConfig()) -> "DuckDBPyConnection":
    """
    ...
    duckdb_memory_limit = (
        duckdb_config.memory_limit
        if duckdb_config.memory_limit is not None
    ...
    num_duckdb_threads = (
        duckdb_config.num_threads if duckdb_config.num_threads is not None else conf.get_or_none("DUCKDB_NTHREADS")
    )
