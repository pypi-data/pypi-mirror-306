...
from tecton_core.specs.feature_view_spec import RealtimeFeatureViewSpec
...
@attrs.frozen
class MultiRtfvCalculationNode(QueryNode):
    """
    Evaluates the calculation features in multiple RTFVs.
    """
    ...
    def as_str(self) -> str:
        ...
        return f"Evaluate multiple calculation features in '{fdw_names}'"
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        for fdw, namespace in self.feature_definition_namespaces:
            ...
            for calc in typing.cast(RealtimeFeatureViewSpec, fdw.fv_spec).calculation_features:
                output_column = f"{namespace}{sep}{calc.name}" if self.use_namespace_feature_prefix else calc.name
                calculation_query_term = calc.root.to_query_term().as_(output_column)
                calculation_query_terms.append(calculation_query_term)
        return (
            self.func.query().with_(input_query, uid).from_(AliasedQuery(uid)).select(Star(), *calculation_query_terms)
        )
