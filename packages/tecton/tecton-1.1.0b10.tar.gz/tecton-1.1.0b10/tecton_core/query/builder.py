...
from tecton_core.query.nodes import MultiRtfvCalculationNode
...
def _dacs_are_rtfv_and_have_calculations(dacs: List[FeatureDefinitionAndJoinConfig]) -> bool:
    if not all(d.feature_definition.is_rtfv for d in dacs):
        return False
    odfv_specs = [cast(RealtimeFeatureViewSpec, d.feature_definition.fv_spec) for d in dacs]
    return any(spec.calculation_features for spec in odfv_specs)
...
def _build_odfv_subtree(
    ...
    # TODO (lee): currently this routes all rtfvs with calcs to the calculations node, and co-occurring features like
    #   attributes are broken. Will add this compatibility in an upcoming PR.
    if _dacs_are_rtfv_and_have_calculations(odfv_dacs):
        newtree = MultiRtfvCalculationNode(
            dialect=dialect,
            compute_mode=compute_mode,
