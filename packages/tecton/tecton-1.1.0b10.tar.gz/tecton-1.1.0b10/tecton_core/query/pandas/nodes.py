...
class PandasFeatureViewPipelineNode(ArrowExecNode):
        ...
        if pipeline_node.HasField("transformation_node"):
            ...
            if secrets:
                if not context.secret_resolver:
                    ...
                    raise TectonValidationError(msg)
                resolved_secrets = context.secret_resolver.resolve_map(secrets)
    ...
    @staticmethod
    def _get_resources(
        resource_provider_specs: Mapping[str, specs.ResourceProviderSpec], execution_context: ExecutionContext
    ) -> Dict[str, Any]:
        resources = {}
        for key, resource_provider_spec in resource_provider_specs.items():
            ...
            if resource_provider_spec.secrets:
                ...
                function_args["context"] = ResourceProviderContext(
            ...
            try:
                ...
                msg = f"Evaluating Resource '{resource_provider_spec.name}' failed with exception."
                raise UserCodeError(msg) from exc
        return resources
