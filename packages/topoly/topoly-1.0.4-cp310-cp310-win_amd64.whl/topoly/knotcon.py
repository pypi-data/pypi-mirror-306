...
class Poly2D(Poly):
    ...
    def mul_val(val1, val2):
        ...
        x1,y1 = arr1.shape
        x2,y2 = arr2.shape
        new_arr = np.zeros([x1+x2-1,y1+y2-1], dtype=int)
        col0 = col01 + col02
        row0 = row01 + row02
        for j in range(y1):
            for i in range(x1):
    ...
    def str2pol(poly):
        ...
        arr = np.zeros([len(rowlen),left+right+1], dtype=int)
        for j in range(len(poly)):
            for i in range(len(poly[j])):
...
class HOMFLYPT(Poly2D):
    ...
    def __add__(self, other):
        ...
        new_val = self.mul_val(new_val, invs[self.inv].str2pol('-1 [0] -1|[[0]]'))
...
class Kauffman_polynomial(Poly2D):
    ...
    def __add__(self, other):
        new_name = super().__add__(other)
        new_val = self.mul_val(self.val, other.val)
        new_val = self.mul_val(new_val, invs[self.inv].str2pol('1 [0] 1|[[-1]]'))
        return invs[self.inv](new_name, new_val)
    def __mul__(self, other):
        new_name = super().__mul__(other)
        new_val = Poly2D.mul_val(self.val, other.val)
