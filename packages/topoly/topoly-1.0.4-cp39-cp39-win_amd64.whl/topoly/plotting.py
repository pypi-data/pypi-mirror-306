...
class KnotMap:
    def __init__(self, protlen, protstart=0, patches=1, missing_residues=[], broken_chain=[], rasterized=False,
        ...
        #print(patches)
        self.legend_gs = gridspec.GridSpec(nrows=patches, ncols=1, left=0.80, right=0.98, bottom=0.25, top=0.85, wspace=0.15,hspace=0.15)
    ...
    def add_patch(self, WRdata, palette=Colors.Knots, k=0, length=0):
        ...
        #subscript={"8_20|3_1#3_1":r"$8_{20}|3_{1}\#3_{1}$","3_1#3_1":r"$3_{1}\#3_{1}$","3_1#5_1":r"$3_{1}\#5_{1}$","3_1#5_2": r"$3_{1}\#5_{2}$"}


        #print("knot_name", knot_name)
        ...
        def change_subscript(knot_name):
            ...
            new_knot_name = knot_name
    
            if "_" in knot_name:
                L = knot_name.split("|")
                for i in range(len(L)):
                    L[i] = "\#".join([change_one(x) for x in L[i].split("#")])

                new_knot_name = "|".join(L)
                new_knot_name = r"$"+new_knot_name+"$"
                
            return new_knot_name


        # subscript
        new_knot_name = change_subscript(knot_name)
        '''
        new_knot_name = knot_name
        if "_" in knot_name:
            kn = knot_name.split("_")

            if knot_name not in subscript.keys():
                new_knot_name=r'${{{}}}_{{{}}}$'.format(kn[0],kn[1])
                print(new_knot_name)
            else:
                new_knot_name = subscript[knot_name]
        '''
        ...
        if knot_name == "-3":
            ...
            label_name = palette['name'] + " " + new_knot_name
        ...
        if (length>3):
            axins = inset_axes(self.axes[self.patches_counter], width="100%", height="30%")
            cbar = self.fig.colorbar(t, cax=axins, orientation='horizontal')
            cbar.ax.set_title(label_name, fontsize=7.5)
            if k == 0:
                cbar.set_ticks([])
        else:
            ip = InsetPosition(self.axes[self.patches_counter], [-0.8, 0.05, 1, 1])
            self.axes[self.patches_counter].set_axes_locator(ip)
            axins = inset_axes(self.axes[self.patches_counter], width="20%", height="100%")
            cbar = self.fig.colorbar(t, cax=axins,ticks=ticks)
...
class Reader:
    def __init__(self, input_data, cutoff=0.48, min_one_dim=2, min_spot=4, knot=''):
        ...
        if type(input_data) is dict:
            ...
            try:
                indices = self.read_dictionary_input(ast.literal_eval(input_data))
