...
from topoly import homfly, import_structure
...
algorithms_links = {'HOMFLY-PT': homfly}
algorithms_knots = {'HOMFLY-PT': homfly}
...
unknowns = {'9_32': 'X[1,4,2,5];X[13,18,14,1];X[3,9,4,8];X[9,3,10,2];'
            ...
            '-9_32': 'X[1,4,2,5];X[13,18,14,1];X[3,9,4,8];X[9,3,10,2];'
                     'X[7,15,8,14];X[15,11,16,10];X[5,12,6,13];X[11,17,12,16];X[17,7,18,6]'}
...
if sys.platform.startswith("darw"):
    pytest.skip("skipping tests on mac", allow_module_level=True)
...
def prepare_link_polynomials_codes():
    ...
    for algorithm in algorithms_links.keys():
        ...
        for code in link_codes:
            ...
            polynomials[algorithm][code] = algorithms_links[algorithm](data, run_parallel=False, parallel_workers=1)
