...
class User:
    def __init__(self, user_id: int, is_bot: bool = False, first_name: str = None, last_name: str = None,
        ...
        :param last_name: Фамилия пользователя (может быть отсутствовать)
        :param username: Имя пользователя (username) на платформе Telegram (может быть отсутствовать)
        :param language_code: Код языка, используемого пользователем
        :param is_premium: Является ли пользователь премиум-аккаунтом
        :param added_to_attachment_menu: Добавлен ли пользователь в меню вложений
        :param can_join_groups: Может ли пользователь присоединяться к группам
        :param can_read_all_group_messages: Может ли пользователь читать все сообщения в группе
        :param supports_inline_queries: Поддерживает ли пользователь инлайн-запросы
        :param can_connect_to_business: Может ли пользователь подключиться к бизнес-аккаунту
        :param has_main_web_app: Имеет ли пользователь основное веб-приложение
        '''
        self.id = user_id
        self.is_bot = is_bot
...
class Chat:
    def __init__(self, chat_id: int, chat_type: str, title: str = None, username: str = None,
        ...
        :param chat_type: Тип чата (например, private, group, supergroup, channel)
        :param title: Заголовок чата (только для групп, супер-групп и каналов, может быть отсутствовать)
        :param username: Имя пользователя (username) чата (может быть отсутствовать)
        :param first_name: Имя пользователя (только для приватных чатов, может быть отсутствовать)
        :param last_name: Фамилия пользователя (может быть отсутствовать)
        :param is_forum: Указывает, является ли чат форумом (может быть отсутствовать)
        '''
        self.id = chat_id
        self.type = chat_type
        self.title = title
...
class ChatMember:
    def __init__(self, user: 'User', status: str, **kwargs: dict):
        ...
        :param user: Объект User, представляющий участника чата
        :param status: Статус участника в чате (например, creator, administrator, member, restricted, kicked)
        :param kwargs: Дополнительные параметры в зависимости от статуса
        '''
        self.user = user
        self.status = status
...
class ChatPermissions:
    def __init__(
        ...
        :param can_send_messages: Разрешение на отправку текстовых сообщений
        :param can_send_media_messages: Разрешение на отправку медиафайлов (фото, видео и т.д.)
        :param can_send_polls: Разрешение на отправку опросов
        :param can_send_other_messages: Разрешение на отправку стикеров и GIF
        :param can_add_web_page_previews: Разрешение на добавление превью веб-страниц
        :param can_change_info: Разрешение на изменение информации о чате
        :param can_invite_users: Разрешение на приглашение участников
        :param can_pin_messages: Разрешение на закрепление сообщений
        :param can_manage_topics: Разрешение на управление темами (доступно в группах с темами)
        '''
...
class PhotoSize:
    def __init__(self, file_id: str, file_unique_id: str, width: int, height: int, file_size: int = None):
        ...
        :param file_unique_id: Уникальный идентификатор файла, неизменный для разных чатов
        :param width: Ширина фотографии
        :param height: Высота фотографии
        :param file_size: Размер файла (опционально)
        '''
...
class Photo:
    def __init__(self, file_id: str, file_unique_id: str, width: int, height: int):
        ...
        :param file_id: Уникальный идентификатор файла
        :param file_unique_id: Уникальный идентификатор файла
        :param width: Ширина фотографии
        :param height: Высота фотографии
        '''
        self.file_id = file_id
...
class Audio:
    def __init__(self, file_id: str, file_unique_id: str, duration: int, performer: str = None, title: str = None, thumbnail: dict = None):
        ...
        :param file_id: Уникальный идентификатор файла
        :param file_unique_id: Уникальный идентификатор файла
        :param duration: Длительность аудиофайла в секундах
        :param performer: Исполнитель (опционально)
        :param title: Название аудиофайла (опционально)
        :param thumbnail: Миниатюра фотографии аудиофайла (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
...
class Voice:
    def __init__(self, file_id: str, file_unique_id: str, duration: int, mime_type: str = None, file_size: int = None, thumbnail: dict = None):
        ...
        :param mime_type: MIME-тип файла (опционально)
        :param file_size: Размер файла (опционально)
        :param thumbnail: Миниатюра фотографии (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
        self.duration = duration
...
class Video:
    def __init__(self, file_id: str, file_unique_id: str, duration: int, width: int, height: int, thumbnail: dict = None):
        ...
        :param file_id: Уникальный идентификатор файла
        :param file_unique_id: Уникальный идентификатор файла
        :param duration: Длительность видеофайла в секундах
        :param width: Ширина видео
        :param height: Высота видео
        :param thumbnail: Миниатюра фотографии видео (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
        self.duration = duration
...
class VideoNote:
    def __init__(self, file_id: str, file_unique_id: str, duration: int, length: int, thumbnail: dict = None, file_size: int = None):
        ...
        :param file_id: Уникальный идентификатор файла
        :param file_unique_id: Уникальный и неизменный идентификатор файла
        :param duration: Длительность видео в секундах
        :param length: Длина стороны видеозаметки (в пикселях)
        :param thumbnail: Миниатюра фотографии видеозаметки (опционально)
        :param file_size: Размер файла (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
...
class Animation:
    def __init__(self, file_id: str, file_unique_id: str, width: int, height: int, duration: int, 
        ...
        :param thumbnail: Миниатюра фотографии анимации (опционально)
        :param file_name: Имя файла (опционально)
        :param mime_type: MIME-тип анимации (опционально)
        :param file_size: Размер файла (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
...
class Dice:
    def __init__(self, emoji: str, value: int):
        ...
        :param emoji: Эмодзи кости
        :param value: Выпавшее значение кости
        '''
        self.emoji = emoji
        self.value = value
...
class Sticker:
    def __init__(self, file_id: str, file_unique_id: str, width: int, height: int, 
        ...
        :param is_animated: Является ли стикер анимированным
        :param is_video: Является ли стикер видео-стикером
        :param thumbnail: Миниатюра стикера (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
        self.width = width
...
class Document:
    def __init__(self, file_id: str, file_unique_id: str, file_name: str = None, 
        ...
        :param file_id: Уникальный идентификатор файла
        :param file_unique_id: Уникальный идентификатор файла
        :param file_name: Имя файла документа (опционально)
        :param mime_type: MIME-тип документа (опционально)
        :param file_size: Размер файла (опционально)
        :param thumbnail: Миниатюра документа (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
        self.file_name = file_name
...
class File:
    def __init__(self, file_id: str, file_unique_id: str, file_size: int, file_path: str = None):
        ...
        :param file_path: Путь к файлу на сервере (опционально)
        '''
        self.file_id = file_id
        self.file_unique_id = file_unique_id
        self.file_size = file_size
...
class WebhookInfo:
    def __init__(self, url: str = None, has_custom_certificate: bool = None, 
        ...
        :param has_custom_certificate: Наличие кастомного SSL-сертификата
        :param pending_update_count: Количество ожидающих обновлений
        :param last_error_date: Дата последней ошибки
        :param last_error_message: Сообщение последней ошибки
        :param max_connections: Максимальное количество подключений
        :param allowed_updates: Список типов разрешенных обновлений
        '''
        self.url = url
...
class InputFile:
    def __init__(self, file_path: str):
        ...
        :param file_path: Путь к файлу на локальном компьютере
        '''
        self.file_path = file_path
...
class InputMedia:
    def __init__(self, media: str, caption: str = None, mode: str = None, caption_entities: list = None):
        ...
        :param media: Строка, содержащая URL медиа контента, который будет отправлен
        :param caption: Строка, содержащая текст к медиа контенту
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        '''
        self.media = media
...
class InputMediaPhoto(InputMedia):
    def __init__(self, media: str, caption: str = None, mode: str = "Markdown", 
        ...
        :param media: Строка, содержащая URL фото, которое будет отправлено
        :param caption: Строка, содержащая текст к фото
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        :param show_caption_above_media: Булевое значение, указывающее, нужно ли отображать подпись над фото
        :param has_spoiler: Булевое значение, указывающее, является ли фото спойлером
        '''
...
class InputMediaVideo(InputMedia):
    def __init__(self, media: str, caption: str = None, mode: str = "Markdown", 
        ...
        :param media: Строка, содержащая URL видео, которое будет отправлено
        :param caption: Строка, содержащая текст к видео
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        :param show_caption_above_media: Булевое значение, указывающее, нужно ли отображать подпись над видео
        :param width: Целое число, определяющее ширину видео
        :param height: Целое число, определяющее высоту видео
        :param duration: Целое число, определяющее длительность видео в секундах
        :param supports_streaming: Булевое значение, указывающее, поддерживает ли видео потоковую передачу
        :param has_spoiler: Булевое значение, указывающее, является ли видео спойлером
        '''
        super().__init__(media, caption, mode, caption_entities)
        self.type = 'video'
        self.show_caption_above_media = show_caption_above_media
...
class InputMediaAnimation(InputMedia):
    def __init__(self, media: str, caption: str = None, mode: str = "Markdown", 
        ...
        :param media: Строка, содержащая URL анимации, которая будет отправлена
        :param caption: Строка, содержащая текст к анимации
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        :param show_caption_above_media: Булевое значение, указывающее, нужно ли отображать подпись над анимацией
        :param width: Целое число, определяющее ширину анимации
        :param height: Целое число, определяющее высоту анимации
        :param duration: Целое число, определяющее длительность анимации в секундах
        :param has_spoiler: Булевое значение, указывающее, является ли анимация спойлером
        '''
        super().__init__(media, caption, mode, caption_entities)
        self.type = 'animation'
        self.width = width
...
class InputMediaAudio(InputMedia):
    def __init__(self, media: str, caption: str = None, mode: str = "Markdown", caption_entities: list = None,
        ...
        :param media: Строка, содержащая URL аудиофайла, который будет отправлен
        :param caption: Строка, содержащая текст к аудиофайлу
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        :param duration: Целое число, определяющее длительность аудиофайла в секундах
        :param performer: Строка, содержащая имя исполнителя аудиофайла
        :param title: Строка, содержащая название аудиофайла
        '''
        super().__init__(media, caption, mode, caption_entities)
        self.type = 'audio'
        self.duration = duration
...
class InputMediaDocument(InputMedia):
    def __init__(self, media: str, caption: str = None, mode: str = "Markdown", 
        ...
        :param media: Строка, содержащая url документа или файл, который будет отправлен
        :param caption: Строка, содержащая текст к документу
        :param mode: Строка, указывающая тип разметки текста
        :param caption_entities: Список объектов MessageEntity, описывающих форматирование текста
        :param disable_content_type_detection: Булевое значение, указывающее, нужно ли отключать 
    ...
    def to_dict(self) -> dict:
        ...
        :return: Словарь с данными для отправки документа
        '''
        data = super().to_dict()
        data['type'] = self.type
...
class CallbackQuery:
    def __init__(self, callback_query_data: dict):
        ...
        :param callback_query_data: Словарь с данными callback-запроса
        '''
...
class MessageEntity:
    def __init__(self, type: str, offset: int, length: int, url: str = None,
        ...
        :param type_: Тип сущности ('mention', 'hashtag', 'bot_command', 'url', и т.д.)
        :param offset: Смещение в тексте, где начинается сущность
        :param length: Длина сущности в тексте
        :param url: URL, связанный с сущностью (для типа 'text_link')
        :param user: Пользователь, связанный с сущностью (для типа 'text_mention')
        :param language: Код языка (для типа 'pre')
        '''
        self.type = type
        self.offset = offset
        self.length = length
...
class Message:
    def __init__(self, message_id: int, chat: 'Chat', from_user: 'User', text: str = None, 
        ...
        :param chat: Чат, в котором было отправлено сообщение
        :param from_user: Пользователь, который отправил сообщение (опционально)
        :param text: Текст сообщения (опционально)
        :param date: Время отправки сообщения в формате UNIX (опционально)
        :param reply_to_message: Сообщение, на которое дан ответ (опционально)
        :param content_type: Тип содержимого сообщения (текст, фото, аудио и т.д.)
        :param photo: Список объектов Photo, если сообщение содержит фото (опционально)
        :param audio: Объект Audio, если сообщение содержит аудиофайл (опционально)
        :param video: Объект Video, если сообщение содержит видео (опционально)
        :param video_note: Объект VideoNote, если сообщение содержит видеозаметку (опционально)
        :param voice: Объект Voice, если сообщение содержит голосовое сообщение (опционально)
        :param animation: Объект Animation, если сообщение содержит анимацию (опционально)
        :param dice: Объект Dice, если сообщение содержит бросок кубика (опционально)
        :param sticker: Объект Sticker, если сообщение содержит стикер (опционально)
        :param document: Объект Document, если сообщение содержит документ (опционально)
        :param new_chat_members: Список новых участников чата (опционально)
        :param new_chat_member: Новый участник чата (опционально)
        :param left_chat_member: Участник, покинувший чат (опционально)
        '''
...
class Bot:
    ...
    def reply_message(self, chat_id: Union[int, str], text: str, mode: str = "Markdown", disable_web_page_preview: bool = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_photo(self, chat_id: Union[int, str], photo: Union[str, bytes], caption: str = None, mode: str = "Markdown", disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
        '''Отправляет фото'''
        method = 'sendPhoto'
        if not chat_id:
            raise ValueError("chat_id не должен быть None")
    ...
    def reply_audio(self, chat_id: Union[int, str], audio: Union[str, bytes], caption: str = None, mode: str = "Markdown", duration: int = None, performer: str = None, title: str = None, thumb: Union[str, bytes] = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
        '''Отправляет аудио'''
        method = 'sendAudio'
        if not chat_id:
            raise ValueError("chat_id не должен быть None")
    ...
    def reply_document(self, chat_id: Union[int, str], document: Union[str, bytes], caption: str = None, mode: str = "Markdown", disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
        '''Отправляет документ'''
        method = 'sendDocument'
        if not chat_id:
            raise ValueError("chat_id не должен быть None")
    ...
    def reply_video(self, chat_id: Union[int, str], video: Union[str, bytes], duration: int = None, width: int = None, height: int = None, caption: str = None, mode: str = "Markdown", supports_streaming: bool = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
        '''Отправляет видео'''
        method = 'sendVideo'
        if not chat_id:
            raise ValueError("chat_id не должен быть None")
    ...
    def reply_video_note(self, chat_id: Union[int, str], video_note: Union[str, bytes], duration: int = None, length: int = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_animation(self, chat_id: Union[int, str], animation: Union[str, bytes], duration: int = None, width: int = None, height: int = None, caption: str = None, mode: str = "Markdown", disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
        '''Отправляет анимацию'''
    ...
    def reply_voice(self, chat_id: Union[int, str], voice: Union[str, bytes], caption: str = None, mode: str = "Markdown", duration: int = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_location(self, chat_id: Union[int, str], latitude: float, longitude: float, live_period: int = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_venue(self, chat_id: Union[int, str], latitude: float, longitude: float, title: str, address: str, foursquare_id: str = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_contact(self, chat_id: Union[int, str], phone_number: str, first_name: str, last_name: str = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_sticker(self, chat_id: Union[int, str], sticker: Union[str, bytes], disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def reply_dice(self, chat_id: Union[int, str], emoji: str, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Markup = None) -> Optional['Message']:
    ...
    def stop_poll(self, chat_id: Union[int, str], message_id: int, reply_markup: Markup = None) -> bool:
    ...
    def copy_message(self, chat_id: Union[int, str], from_chat_id: Union[int, str], message_id: int, caption: str = None, disable_notification: bool = None, mode: str = "Markdown", reply_markup: Markup = None) -> bool:
        '''Копирует сообщение'''
        method = 'copyMessage'
        if not chat_id:
            raise ValueError("chat_id не должен быть None")
...
#Редакт чего-то
    def edit_message_text(self, chat_id: Union[int, str], message_id: int, text: str, inline_message_id: str = None, mode="Markdown", reply_markup: Markup = None) -> bool:
    ...
    def edit_message_caption(self, chat_id: Union[int, str], message_id: int, caption: str, inline_message_id: str = None, mode: str = "Markdown", show_caption_above_media: bool = False, reply_markup: Markup = None) -> bool:
    ...
    def edit_message_media(self, chat_id: Union[int, str], message_id: int, media: Union[str, bytes, InputMediaPhoto, InputMediaAnimation, InputMediaVideo, InputMediaAudio, InputMediaDocument], inline_message_id: str = None, reply_markup: Markup = None) -> bool:
    ...
    def edit_message_reply_markup(self, chat_id: Union[int, str], message_id: int, reply_markup: Markup, inline_message_id: str = None) -> bool:
...
#Получение апдейтов
    def get_updates(self, timeout: int = 20, allowed_updates: Union[list, str] = None, long_polling_timeout: int = 30) -> list:
    ...
    def always_polling(self, interval: int = 1, timeout: int = 20, long_polling_timeout: int = 30, allowed_updates: Union[list, str] = None, restart_on_error: bool = True):
...
#Чат инфа
    def get_chat(self, chat_id: Union[int, str]) -> Optional['Chat']:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при получении чата: {response}")
            return None
    ...
    def get_chat_administrators(self, chat_id: Union[int, str]) -> Union['ChatMember', list]:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при получении администраторов: {response}")
            return []
    ...
    def get_chat_members_count(self, chat_id: Union[int, str]) -> int:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при получении количества участников: {response}")
            return 0
    ...
    def get_chat_member(self, chat_id: Union[int, str], user_id: int) -> Optional['ChatMember']:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при получении информации о пользователе: {response}")
            return None
    ...
    def set_chat_photo(self, chat_id: Union[int, str], photo: Union[str, bytes, InputFile]) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при удалении фотографии: {response}")
            return False
    ...
    def set_chat_title(self, chat_id: Union[int, str], title: str) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при установке названия: {response}")
            return False
    ...
    def set_chat_description(self, chat_id: Union[int, str], description: str) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при установке описания: {response}")
            return False
    ...
    def leave_chat(self, chat_id: Union[int, str]) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при выходе из чата: {response}")
            return False
...
# Административные команды
    def kick_chat_member(self, chat_id: Union[int, str], user_id: int, until_date: float = time.time()) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при удалении участника: {response}")
            return False
    ...
    def ban_chat_member(self, chat_id: Union[int, str], user_id: int, until_date: float = time.time(), revoke_messages: bool = False) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при блокировке участника: {response}")
            return False
    ...
    def unban_chat_member(self, chat_id: Union[int, str], user_id: int, only_if_banned: bool = False) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при разблокировки участника: {response}")
            return False
    ...
    def mute_user(self, chat_id: Union[int, str], user_id: int, duration: int = 3600) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при блокировке отправки сообщений участником: {response}")
            return False
    ...
    def unmute_user(self, chat_id: Union[int, str], user_id: int) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при разблокировке отправки сообщений участником: {response}")
            return False
    ...
    def restrict_chat_member(self, chat_id: Union[int, str], user_id: int, permissions: ChatPermissions, until_date: float = time.time()) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при изменении разрешений участника: {response}")
            return False
    ...
    def promote_chat_member(self, chat_id: Union[int, str], user_id: int, can_change_info: bool = False, can_post_messages: bool = False, can_edit_messages: bool = False, can_delete_messages: bool = False, can_invite_users: bool = False, can_restrict_members: bool = False, can_pin_messages: bool = False, can_promote_members: bool = False) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при изменении прав участника: {response}")
            return False
    ...
    def set_chat_permissions(self, chat_id: Union[int, str], permissions: ChatPermissions) -> bool:
        ...
        if response and 'result' in response:
            ...
            print(f"Ошибка при изменении прав для всех участников чата: {response}")
            return False
