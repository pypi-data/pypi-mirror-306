...
from .utils.choice import build_user_choice
from .client import OldJiraServerClient, OldJiraServerSetupClient

logger = logging.getLogger("sentry.integrations.old_jira_server")

DESCRIPTION = """
This is a fork of official Jira Server integration, simple uses basic authentication by login and password.
...
class InstallationForm(forms.Form):
    ...
    username = forms.CharField(
        ...
        widget=forms.TextInput(attrs={"placeholder": "sentry_service_account"}),
    )
...
class OldJiraServerIntegration(IssueSyncIntegration):
    """
    ...
    def get_link_issue_config(self, group, **kwargs):
        ...
        autocomplete_url = reverse(
            "sentry-extensions-oldjiraserver-search", args=[org.slug, self.model.id]
        )
        for field in fields:
        ...
        default_comment = "Linked Sentry Issue: [{}|{}]".format(
            ...
            absolute_uri(group.get_absolute_url(params={"referrer": "old_jira_server"})),
        )
        fields.append(
            {
                "name": "comment",
    ...
    def search_url(self, org_slug):
        return reverse("sentry-extensions-oldjiraserver-search", args=[org_slug, self.model.id])
    ...
    def get_projects(self, cached=True):
        ...
        try:
            ...
            logger.info(
                "old_jira_server.get_projects.error",
                extra={
        ...
        if len(jira_projects) == 0:
            logger.info(
                "old_jira_server.get_projects.no_projects",
                extra={
                    "integration_id": self.model.id,
                    "organization_id": self.organization_id,
                },
    ...
    def get_create_issue_config(self, group: Group | None, user: RpcUser, **kwargs):
        ...
        kwargs = kwargs or {}
        kwargs["link_referrer"] = "old_jira_server_integration"
        ...
        try:
            ...
            logger.info(
                "old_jira_server.get_create_issue_config.unauthorized",
        ...
        for field in fields:
            if field["name"] == "priority":
                ...
                try:
                    ...
                    logger.info(
                        "old_jira_server.get_create_issue_config.no-matching-reporter",
                        extra={
                            "integration_id": self.model.id,
                            "organization_id": self.organization_id,
    ...
    def sync_assignee_outbound(
        ...
        if user and assign:
            ...
            if jira_user is None:
                ...
                logger.info(
                    "old_jira_server.assignee-not-found",
                    extra={
        ...
        try:
            ...
            # TODO(jess): do we want to email people about these types of failures?
            logger.info(
                "old_jira_server.failed-to-assign",
                extra={
    ...
    def sync_status_outbound(self, external_issue, is_resolved, project_id, **kwargs):
        ...
        try:
            ...
            logger.warning(
                "old_jira_server.status-sync-fail",
                extra={
                    "organization_id": external_issue.organization_id,
                    "integration_id": external_issue.integration_id,
                    "issue_key": external_issue.key,
...
class OldJiraServerIntegrationProvider(IntegrationProvider):
    key = "old_jira_server"
    name = "Old Jira Server"
    metadata = metadata
    integration_cls = OldJiraServerIntegration
    ...
    def build_integration(self, state):
        ...
        webhook_secret = sha1_text(install["password"]).hexdigest()

        hostname = urlparse(install["url"]).netloc
        external_id = "{}:{}".format(hostname, install["username"])[:64]
    ...
    def create_webhook(self, external_id, webhook_secret, install):
        client = OldJiraServerSetupClient(
            install["url"], install["username"], install["password"]
        )
        try:
            client.create_issue_webhook(external_id, webhook_secret)
        except ApiError as err:
            logger.info(
                "old-jira-server.webhook.failed",
