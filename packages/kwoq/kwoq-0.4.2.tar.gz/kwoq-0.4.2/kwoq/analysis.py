...
class EconometricAnalysis:
    ...
    def task1_specification(self) -> Dict[str, Any]:
        ...
        # Сначала находим лучшую нелинейную модель
        best_nonlinear_r_squared = 0
        best_nonlinear_name = None
        best_nonlinear_model = None
        best_nonlinear_results = None
        ...
        for name, model in models.items():
            if model is not None:  # Проверяем, что модель существует
                ...
                # Если это нелинейная модель и она лучше предыдущей нелинейной
                if name != 'Линейная' and r_squared > best_nonlinear_r_squared:
                    best_nonlinear_r_squared = r_squared
                    best_nonlinear_name = name
                    best_nonlinear_model = model
                    best_nonlinear_results = result
                # После нахождения лучшей нелинейной модели проверяем условие 2%
        if best_nonlinear_name and (best_nonlinear_r_squared - linear_r_squared) > 0.02:
            best_model_name = best_nonlinear_name
            best_r_squared = best_nonlinear_r_squared
            best_model = best_nonlinear_model
            best_results = best_nonlinear_results
        else:
            # Оставляем линейную модель
            best_model_name = 'Линейная'
            best_r_squared = linear_r_squared
            best_model = linear_model
        ...
        for i in range(len(significant_mask)):
            ...
            if t_stats[i] <= t_critical:
                ...
                if len(significant_columns) > 0:
                    if len(self.X.shape) > 1:  # Check if X is 2D
                        X_significant = self.X[:, significant_columns]
                    else:
                        X_significant = self.X[significant_columns]

                    if len(X_significant.shape) > 1 or significant_mask[0]:  # Check if there are any remaining terms
    ...
    def task4_heteroskedasticity(self) -> Dict[str, Any]:
        ...
        if self.model_name == 'Полином 2 степени':
            plt.scatter(self.X[:, 1], self.residuals, alpha=0.5)
        else:
