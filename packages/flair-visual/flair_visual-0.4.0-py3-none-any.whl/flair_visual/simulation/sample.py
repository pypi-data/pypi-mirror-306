...
class AtomLossCircuitSampler(Generic[CircuitType]):
    ...
    def __post_init__(self, loss_prob_calculator: SurvivalProbabilityCalculator):
        ...
        assert len(self.measure_tags) > 0, "No measurement tags found in gate events"
    ...
    def clean_circuit(self) -> CircuitType:
        # atoms with 0 survival probability are lost and do not participate in any gate events
        active_qubits = (self.survival_probs > 0).cumprod(axis=0).astype(bool)
