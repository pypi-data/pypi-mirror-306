...
from pg_common import log_info, log_error, start_coroutines, base64_decode, base64_encode, \
    GLOBAL_DEBUG, fernet_decrypt, fernet_encrypt
...
from .define import ENV_NEEDS_BODY_MIDDLEWARE, ENV_NEEDS_CHECK_SESSION, ENV_CHECK_SESSION_IGNORE_URI, \
    ...
    SESSION_EXPIRED_TIME, Container, ENV_CONTAINER_KEY
...
class _CustomBodyResponder:
    ...
    async def receive_with_msg(self):
        ...
        if _body:
            ...
            _container_key = config.get_conf(ENV_CONTAINER_KEY, "_container_")
...
if config.get_conf(ENV_NEEDS_CHECK_SESSION, default=True):
    ...
    async def http_inspector(request, call_next):
        ...
        _user = json.loads(fernet_decrypt(_token.encode(), _cfg[_game]['fernet_key']))
        if 'last_login' not in _user:
            return PlainTextResponse("FORBIDDEN4", status_code=status.HTTP_403_FORBIDDEN)

        _now = int(time.time())
        if _now - _user['last_login'] >= SESSION_EXPIRED_TIME:
            return PlainTextResponse("UNAUTHORIZED", status_code=status.HTTP_401_UNAUTHORIZED)

        _user['last_login'] = _now
        request.scope[config.get_conf(ENV_CONTAINER_KEY, "_container_")].user = _user
        response = await call_next(request)
        # response.headers[_header_key] = " ".join([_game, rsa_encrypt2(json.dumps(_user), _cfg[_game]['pub_key'])])
        response.headers[_header_key] = " ".join([_game, fernet_encrypt(json.dumps(_user).encode(), _cfg[_game]['fernet_key'])])
        return response
...
async def http_log_inspector(request, call_next):
    ...
    request.scope[config.get_conf(ENV_CONTAINER_KEY, "_container_")] = _container
    response = await call_next(request)
    _process_time = time.perf_counter() - _start_time
    _container.log.update({"process_time": _process_time})
