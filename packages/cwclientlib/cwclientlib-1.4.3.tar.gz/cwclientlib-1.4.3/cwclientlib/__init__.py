...
def cwproxy_for(instance, **kw):
    ...
    if instance not in cfg:
        ...
        if len(instances) == 1:
            ...
            raise ValueError(
                "Found %r configuration entries for %r" % (len(instances), instance)
            )
    if instance not in cfg:
    ...
    request_mech = cfg.get("request-mech", "rqlcontroller")
    url = cfg.get("url")
    if not url:
    ...
    proxy_class = CWApiProxy if request_mech == "api" else CWProxy
    proxy_cls = kw.pop("proxycls", proxy_class)
