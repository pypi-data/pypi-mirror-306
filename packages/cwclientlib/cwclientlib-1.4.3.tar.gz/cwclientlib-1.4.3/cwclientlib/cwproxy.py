...
class CWProxy:
    ...
    @property
    def _domain(self):
        return urlparse.urlparse(self._base_url).netloc

    @property
    def _cookies(self):
        return self._requests_session.cookies

    def _get_csrf_tokens_cached(self):
        return [
            cookie
    ...
    def _get_logs_for_csrf_tokens_cached(self) -> str:
        parameters = []
        for cookie in self._get_csrf_tokens_cached():
            parameters.append(
    ...
    def _get_csrf_token_domained_with_possible_paths(self):
        paths = getattr(self, "CSRF_TOKEN_COOKIE_POSSIBLE_PATHS", ("/",))
        if not paths:
            return
        cookies = self._cookies
        domain = self._domain
        for path in paths:
            value = cookies.get(self.CSRF_TOKEN_COOKIE_NAME, domain=domain, path=path)
            if value:
                value = value.strip()
                if value:
                    return value
    ...
    @property
    def _csrf_token_cached(self):
        cookies = self._cookies
        try:
            return cookies.get(self.CSRF_TOKEN_COOKIE_NAME)
        except CookieConflictError:
            domain = self._domain
            try:
                return cookies.get(self.CSRF_TOKEN_COOKIE_NAME, domain=domain)
                ...
                parameters = self._get_logs_for_csrf_tokens_cached()
                if parameters:
                    error_message += (
    ...
    def _grab_csrf_token(self):
        self._requests_session.get(self._base_url, allow_redirects=False)
        value = self._csrf_token_cached
        if not value:
    ...
    def _csrf_token(self):
        cached_value = self._csrf_token_cached
        if cached_value:
            return cached_value
        return self._grab_csrf_token()
...
class CWApiProxy(CWProxy):
    ...
    def _convert_api_response_to_legacy(
        self, posted: requests.models.Response
    ) -> requests.models.Response:
        """Returns the same response as the legacy CWProxy"""
        json_data = posted.json()

        if "result_sets" in json_data:
            result_sets = json_data["result_sets"]

            data_content = []
            for element in result_sets:
                data_content.append(element["rows"])

            posted._content = json.dumps(data_content).encode()

        return posted

    def _convert_resolve_query_reference(self, query: Union[dict, None]) -> dict:
