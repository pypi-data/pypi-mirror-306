...
class Scheduler:
    ...
    def __init__(
        ...
        *,
        ...
        metering_throttle_action: Optional[MeteringThrottleAction] = None,
            ...
            with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
                ...
                for stage in OperationStage:
                    if stage != OperationStage.COMPLETED:
                        jobs_by_stage[cast(int, stage.value)] = 0

                for bucket, value in results:
                    jobs_by_stage[bucket] = cast(int, value)

                metrics["jobs"] = jobs_by_stage
