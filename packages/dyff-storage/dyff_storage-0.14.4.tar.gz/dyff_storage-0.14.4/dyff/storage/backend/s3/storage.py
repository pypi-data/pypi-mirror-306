            ...
            if path.is_file():
                if path.is_symlink():
                    ...
                    if not target.is_absolute():
                        # Target is relative to the link file (more accurately,
                        # its parent directory).
                        #
                        # Note that the result could be un-normalized
                        # (i.e. /foo/../bar), but we can't call .resolve()
                        # because it won't handle symlinks-to-symlinks correctly
                        # (when restored, all the links would point directly to
                        # the final target).
                        target = path.parent / target
                    symlinks.append(
                        {
                            "link": str(path.relative_to(source_path)),
                            # Note that this will raise for targets outside
                            # the directory tree (which is a good thing)
                            "target": str(target.relative_to(source_path)),
