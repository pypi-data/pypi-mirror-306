    ...
    def download_recursive(self, source: str, destination: str) -> None:
        ...
        try:
            with open(Path(destination) / ".dyff" / "symlinks.json", "r") as fin:
                symlinks: list[dict[str, str]] = json.load(fin)["symlinks"]
            for entry in symlinks:
                link = Path(destination) / entry["link"]
                target = Path(destination) / entry["target"]
                if not target.exists() or not target.is_file():
                    raise ValueError(
                        f"symlink target does not exist or is not a file: {link} -> {target}"
                    )
