...
from .helpers import send_reading_summary
...
def upsert_timeseries(df: pandas.DataFrame, api_inputs: ApiInputs, is_local_time: bool = True,
    ...
    def add_carbon_column(df, cost_carbon_calculation):
        ...
        if len(cost_carbon_calculation.carbon_calculations_exp) > 0:
            for item in cost_carbon_calculation.carbon_calculations_exp:
                ...
                mask = df['ObjectPropertyId'] == str(item.object_property_id).lower()
    ...
    def create_meta_column(df, slice_columns, save_slices, cost_carbon_calculation):
        ...
        if save_slices:
            ...
            df['Meta'] = df.apply(lambda row: json.dumps({'Cost': row['Cost'], 'Carbon': row['Carbon']}), axis=1)
            df = df.drop(columns=slice_columns)
        
        return df

    cost_carbon_calculation = CostCarbonCalculation(api_inputs=api_inputs, df=data_frame)
    data_frame = create_meta_column(data_frame, slice_columns, save_additional_columns_as_slices, cost_carbon_calculation)
    ...
    if (send_notification == True):
        ...
        def _upload_sensor_latest_value(api_input, data_frame, data_feed_file_status_id, container):
            ...
            try:
                ...
                latest_indices = data_frame.groupby('ObjectPropertyId')[
                    'Timestamp'].idxmax()
                latest_sensor_value_df = data_frame.loc[latest_indices, [
