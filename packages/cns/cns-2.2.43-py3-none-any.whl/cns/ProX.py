...
class ProX(cns.sqlX, cns.SQLstrX):
    ...
    # ----------------- tmpl --------------------------24.10.27
    tmpl['select'] = {'头部':'select', '模板':"{连接2} `{值}`",  '连接':','} # ok
    tmpl['col'] = { '模板': "{连接2}{符}{值}{符}", '连接':','}     # ok     '符s':'检查', '符i':'插入' 
    tmpl['col2'] = { '模板': "{连接2}{值}",  '连接':','}  # ok
    tmpl['strin'] = { '模板': "{连接2}'{值}'",  '连接':','} # ok
    tmpl['sum'] = { '模板': "{连接2}{名字}(`{值}`) as '{值}'",  '连接':','} # ok
    tmpl['max'] = { '模板':"{连接2}{名字}(`{值}`) as '{值}'",  '连接':','} # ok
    ...
    tmpl['un2'] =  { '模板': "{连接2}SELECT\n*\nFROM `{前缀}{值}{后缀}`",'连接':'UNION ALL \n', '头部':'SELECT\n*\nFROM(', '尾部':')t1 ORDER BY 1', '前缀':'', '后缀':''} #ok
    tmpl['group_by'] = { '模板': "{连接2}{计数}", '连接':',', '头部':' GROUP BY '} # ok
    

    tmpl['增数据'] = {'头部':'INSERT INTO `{表名}` (', '模板': "{连接2}`{值}`", '连接':',', '尾部':')'}  # ok
    tmpl['删数据'] = {'模板':'DELETE FROM `{{表名}}` WHERE {值};'}   # ok
    tmpl['改数据'] = {'头部':'UPDATE `{{表名}}` SET ', '模板':"{连接2}{值}", '连接':',', '尾部':';'}
    
    tmpl['建表'] = {'头部':'CREATE TABLE {表名} (', '模板':"{连接2}`{值}` {{{值}_数据类型}}" , '连接':',', '尾部':'\n)'}    
    tmpl['删表'] = {'模板':"DROP TABLE IF EXISTS `{值}`;"}

    tmpl['alter'] = {'模板':"ALTER TABLE {符}{前缀}{值}{后缀}{符} {字段}; ", '前缀':'', '后缀':''}
    tmpl['column'] = {'模板':"{连接2}{指令} COLUMN {符}{值}{符} {数据类型}", '连接':',', '符s':'`', '符i':'`' }
    tmpl['增字段'] = {'头部':'ALTER TABLE {表名}', '模板':"{连接2}ADD `{值}` {数据类型}", '连接':','}   # '模板':"{连接2} ADD `{值[0]}` {值[1]}"
    tmpl['删字段'] = {'头部':'ALTER TABLE {表名}', '模板':"{连接2}DROP COLUMN `{值}`", '连接':','}
    tmpl['改字段'] = {'头部':'ALTER TABLE {表名}', '模板':"{连接2}CHANGE COLUMN {值[0]} {值[1]} {值[2]} ", '连接':','}  # [ ['原字段名', '新字段名', '数据类型'], ]
    tmpl['增索引'] = {'头部':'ALTER TABLE {表名}', '模板':"{连接2}ADD INDEX {值} (`{值}`)", "连接":','}
    tmpl['删索引'] = {'头部':'ALTER TABLE {表名}', '模板':"{连接2}DROP INDEX {值}", "连接":','}
...
if __name__ == '__main__':
    ...
    names1 = ['23年实际','24年实际_1_4月','24年实际_5月','24年实际_6月','24年实际_7月','24年实际_8月','24年实际_9月v2']  # 实际
    names2 = ['24年预算_1_7月v1','24年预算_8月v1', '24年预算_9_12月v1']  # 预算
    表名清单 = names1+names2


    修改字段清单 = '''
    `实际_自营_广告-收入`  `实际_整体_广告-收入`
    '''
    ...
    #print( 生成SQL('DROP', 删除字段清单, 表名清单,'') )  8经营中心
    print( 生成SQL('CHANGE', 修改字段清单, 表名清单,'') )
    #print( 生成SQL('ADD', 添加字段清单, 表名清单,'varchar(12) DEFAULT NULL') )
