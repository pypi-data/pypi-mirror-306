...
class ProX(cns.sqlX, cns.SQLstrX):
    ...
    tmpl['改类型'] = {'头部':'ALTER TABLE `{表名}`', '模板':"{连接2}MODIFY COLUMN {前缀}{符}{值}{符}{后缀}", '连接':',', '前缀':'', '后缀':'', '符s':'`', '符i':'`' }  # price DECIMAL(6,2)
...
'''
    #--- 单表多字段
    sql = X.sum("1,2,3")
    sql = X.改类型(字段清单, {'后缀':' varchar(12) DEFAULT NULL'})
    #print(sql)

    字段清单2 = [
    `实际_自营_广告-收入`  varchar(12) DEFAULT NULL.
    `实际_整体_广告-收入`  varchar(12) DEFAULT NULL,
