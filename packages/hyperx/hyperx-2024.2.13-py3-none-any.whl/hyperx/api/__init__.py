...
class ZoneAnalysisSolverResult(ZoneAnalysisResult):
	...
	def Create_ZoneAnalysisSolverResult(limitUltimate: types.LimitUltimate, loadCaseId: int, scenarioId: int | None, margin: Margin, analysisDefinition: AnalysisDefinition, analysisObjectTag: Enum, analysisObjectIndex: int, analysisDetails: AnalysisDetailCol):
		return ZoneAnalysisSolverResult(_api.ZoneAnalysisSolverResult(_types.LimitUltimate(limitUltimate.value), loadCaseId, scenarioId, margin._Entity, analysisDefinition._Entity, analysisObjectTag._Entity, analysisObjectIndex, analysisDetails._Entity))

	@property
	def AnalysisObjectTag(self) -> Enum:
		result = self._Entity.AnalysisObjectTag
		thisClass = type(result).__name__
		givenClass = Enum
		for subclass in Enum.__subclasses__():
	...
	def GetFamilyObjectUID(self, familyId: types.BeamPanelFamily, conceptId: int) -> types.FamilyObjectUID | None:
		return types.FamilyObjectUID(self._Entity.GetFamilyObjectUID(_types.BeamPanelFamily(familyId.value), conceptId))
...
class IdEntityCol(Generic[T], ABC):
	...
	def IdEntityColList(self) -> tuple[IdEntity]:
		idEntityColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(IdEntity)}
		for idEntityCol in self._Entity:
			if type(idEntityCol).__name__ in subclasses.keys():
				thisType = subclasses[type(idEntityCol).__name__]
				idEntityColList.append(thisType(idEntityCol))
			else:
				raise Exception(f"Could not wrap item in IdEntityCol. This should not happen.")
		return tuple(idEntityColList)
...
class IdNameEntityCol(IdEntityCol, Generic[T]):
	...
	def IdNameEntityColList(self) -> tuple[T]:
		idNameEntityColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(T)}
		for idNameEntityCol in self._Entity:
			if type(idNameEntityCol).__name__ in subclasses.keys():
				thisType = subclasses[type(idNameEntityCol).__name__]
				idNameEntityColList.append(thisType(idNameEntityCol))
			else:
				raise Exception(f"Could not wrap item in IdNameEntityCol. This should not happen.")
		return tuple(idNameEntityColList)
...
class FailureCriterion(IdNameEntity):
	...
	def LimitUltimate(self, value: types.LimitUltimate | None) -> None:
		self._Entity.LimitUltimate = value if value is None else _types.LimitUltimate(value.value)
...
class LoadProperty(AssignableProperty):
	...
	def AssignTo(self, entityIds: tuple[int], category: types.FamilyCategory) -> PropertyAssignmentStatus:
		...
		entityIdsList = MakeCSharpIntList(entityIds)
		entityIdsEnumerable = IEnumerable(entityIdsList)
		return PropertyAssignmentStatus[self._Entity.AssignTo(entityIdsEnumerable, _types.FamilyCategory(category.value)).ToString()]
...
class EntityWithAssignablePropertiesCol(IdNameEntityCol, Generic[T]):
	...
	def EntityWithAssignablePropertiesColList(self) -> tuple[T]:
		entityWithAssignablePropertiesColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(T)}
		for entityWithAssignablePropertiesCol in self._Entity:
			if type(entityWithAssignablePropertiesCol).__name__ in subclasses.keys():
				thisType = subclasses[type(entityWithAssignablePropertiesCol).__name__]
				entityWithAssignablePropertiesColList.append(thisType(entityWithAssignablePropertiesCol))
			else:
				raise Exception(f"Could not wrap item in EntityWithAssignablePropertiesCol. This should not happen.")
		return tuple(entityWithAssignablePropertiesColList)
...
class HyperFea:
	...
	def RunIterations(self, numberOfIterations: int, startWithSizing: bool, stressReportFormat: types.StressReportFormat = None) -> None:
		...
		:raises ``System.InvalidOperationException``:
		'''
		return self._Entity.RunIterations(numberOfIterations, startWithSizing, stressReportFormat if stressReportFormat is None else _types.StressReportFormat(stressReportFormat.value))
	...
	def AddLayer(self, item1 = None, item2 = None, item3 = None, item4 = None) -> StiffenerLaminateLayer:
		...
		if (isinstance(item1, tuple) or isinstance(item1, list) or isinstance(item1, set)) and item1 and any(isinstance(x, types.StiffenerLaminateLayerLocation) for x in item1) and isinstance(item2, str) and (isinstance(item3, float) or isinstance(item3, int)) and (isinstance(item4, float) or item4 is None or isinstance(item4, int)):
			...
			if item1 is not None:
				for x in item1:
					if x is not None:
						locationsList.Add(_types.StiffenerLaminateLayerLocation(x.value))
...
class FolderCol(IdNameEntityCol[Folder]):
	...
	def FolderColList(self) -> tuple[Folder]:
		folderColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(Folder)}
		for folderCol in self._Entity:
			if type(folderCol).__name__ in subclasses.keys():
				thisType = subclasses[type(folderCol).__name__]
				folderColList.append(thisType(folderCol))
			else:
				raise Exception(f"Could not wrap item in FolderCol. This should not happen.")
		return tuple(folderColList)
...
class ZoneJointContainerCol(IdNameEntityCol, Generic[T]):
	...
	def ZoneJointContainerColList(self) -> tuple[T]:
		...
		subclasses = {x.__name__: x for x in _all_subclasses(T)}
		for zoneJointContainerCol in self._Entity:
			if type(zoneJointContainerCol).__name__ in subclasses.keys():
				thisType = subclasses[type(zoneJointContainerCol).__name__]
				zoneJointContainerColList.append(thisType(zoneJointContainerCol))
			else:
				raise Exception(f"Could not wrap item in ZoneJointContainerCol. This should not happen.")
		return tuple(zoneJointContainerColList)
...
class Application:
	...
	def __init__(self, application: _api.Application):
		...
		ValidateDatabaseVersion(self.Version)
	@property
...
class SizingMaterialCol(IdEntityCol[SizingMaterial]):
	...
	def AddSizingMaterial(self, materialId: int) -> bool:
		...
		:return: ``False`` if the material does not exist or is already in the list of sizing materials.
		'''
		...
	...
	def RemoveSizingMaterial(self, name: str) -> bool:
		...
		:return: ``False`` if the material does not exist or is not in the list of sizing materials.
		'''
		...
...
class LoadPropertyUserFeaRowCol(IdNameEntityCol, Generic[T]):
	...
	def LoadPropertyUserFeaRowColList(self) -> tuple[T]:
		loadPropertyUserFeaRowColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(T)}
		for loadPropertyUserFeaRowCol in self._Entity:
			if type(loadPropertyUserFeaRowCol).__name__ in subclasses.keys():
				thisType = subclasses[type(loadPropertyUserFeaRowCol).__name__]
				loadPropertyUserFeaRowColList.append(thisType(loadPropertyUserFeaRowCol))
			else:
				raise Exception(f"Could not wrap item in LoadPropertyUserFeaRowCol. This should not happen.")
		return tuple(loadPropertyUserFeaRowColList)
...
class LoadPropertyUserGeneralRowCol(IdNameEntityCol, Generic[T]):
	...
	def LoadPropertyUserGeneralRowColList(self) -> tuple[T]:
		loadPropertyUserGeneralRowColList = []
		subclasses = {x.__name__: x for x in _all_subclasses(T)}
		for loadPropertyUserGeneralRowCol in self._Entity:
			if type(loadPropertyUserGeneralRowCol).__name__ in subclasses.keys():
				thisType = subclasses[type(loadPropertyUserGeneralRowCol).__name__]
				loadPropertyUserGeneralRowColList.append(thisType(loadPropertyUserGeneralRowCol))
			else:
				raise Exception(f"Could not wrap item in LoadPropertyUserGeneralRowCol. This should not happen.")
		return tuple(loadPropertyUserGeneralRowColList)
...
class Environment(ABC):
	...
	def InstallLocation(self) -> str:
		return self._Entity.InstallLocation
...
class HelperFunctions(ABC):
	...
	def GetFamilyObjectForObjectId(familyId: types.BeamPanelFamily, objectId: int) -> types.FamilyObjectUID:
		return types.FamilyObjectUID(_api.HelperFunctions.GetFamilyObjectForObjectId(_types.BeamPanelFamily(familyId.value), objectId))
