...
def migrate(models=[], custom_user_model=None, initial_admin_data=None):
    if custom_user_model:
        ...
        print("Setting user model to:", custom_user_model)  # Отладочная строка

    user_model = get_user_model() or User
    DarkHandler.darkfream.admin.register_model(user_model)
    print("Migrate user model:", user_model)  # Отладочная строка
    ...
    # If initial_admin_data provided, update default values
    if initial_admin_data and isinstance(initial_admin_data, dict):
        # Ensure password and is_admin are not overridden
