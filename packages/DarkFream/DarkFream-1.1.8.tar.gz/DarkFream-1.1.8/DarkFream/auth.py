...
class AdminAuth:
    def __init__(self, app):
        ...
        print("AdminAuth user model:", self.user_model)  # Отладочная строка
...
class Auth:
    ...
    def __init__(self, app):
        self.app = app
        self.user_model = get_user_model() or User
        print("Auth user model:", self.user_model)  # Отладочная строка
    ...
    def get_current_user(self, session):
        user_id = session.get('user_id')
        if user_id:
            return self.user_model.get_or_none(self.user_model.id == user_id)
        return None
