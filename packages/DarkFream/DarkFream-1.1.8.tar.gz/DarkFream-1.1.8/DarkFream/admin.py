...
class DarkAdmin:
    def __init__(self, app):
        ...
        print("Admin user model:", self.user_model)  # Отладочная строка
                    ...
                    for field_name, field in model.get_fields():
                        if field_name in data['data']:
                            if isinstance(field, ForeignKeyField):
                                ...
                                # Берем последнее значение из списка значений
                                values = data['data'].get(field_name, ['0'])
                                value = bool(int(values[-1]))  # берем последний элемент списка
