    ...
    def compile_latex(
        ...
        try:
            if docker_ready:
                ...
                relative_file = file.name  # On utilise juste le nom du fichier
                current_path = os.path.abspath(os.getcwd())
                
                # Adaptation du chemin pour Windows
                if os.name == 'nt':
                    current_path = current_path.replace('\\', '/')
                    # Si le chemin commence par C:, le convertir en format /c/...
                    if ':' in current_path:
                ...
                cmd = [
                    "docker",
                    "run",
                    "-i",  # Retiré le -t pour éviter les problèmes sur certains systèmes
                    "--rm",
                    "-v",
                    f"{current_path}:/data",
                ...
                cmd = [
                    ...
                    "-interaction=nonstopmode",
                    "-shell-escape",
                    str(file.name)  # Utilisation du nom de fichier relatif
                ]
