...
def report_ec4_composited_beam(steel: SteelMember_EC, shearconn: ShearConnector_EC, slab: SlabMember_EC, leng: GirderLength, load: UnitLoads) -> ResultBytes:
    dll = load_dll()
...
def calc_ec4_composited_beam(steel: SteelMember_EC, shearconn: ShearConnector_EC, slab: SlabMember_EC, leng: GirderLength, load: UnitLoads) -> dict:
    dll = load_dll()
    json_data_list = [steel.json(), shearconn.json(), slab.json(), leng.json(), load.json(), ctypes.c_void_p(0), ctypes.c_void_p(0)]
...
if __name__ == "__main__":
    res = report_ec4_composited_beam(SteelMember_EC(), ShearConnector_EC(), SlabMember_EC(), GirderLength(), UnitLoads())
    save_result_to_json(res, 'report_result.json')
    print(res)