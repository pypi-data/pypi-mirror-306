...
from moapy.data_pre import MemberForce, PMOptions, AxialForceOpt, AngleOpt, DgnCode, Lcom, Moment
...
def test_report_rc_cracked_stress():
    res = wgsd_oapi.report_rc_cracked_stress(Material(), Geometry(), DgnCode(), Lcom(name="lcom", f=MemberForce(Mx=Moment(value=10.0, unit=enUnitMoment.Nmm))))
    return res
