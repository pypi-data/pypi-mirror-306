...
class ConcreteMaterial:
	...
	Concrete Material Input Class
 	...
 	Args:
		grade (float): Concrete Grade
	"""

	grade: float = dataclass_field(default=25, metadata={"description": "Concrete Grade"})

	def __post__init__(self):
		if self.grade < 0:
			raise ValueError("Concrete Grade must be greater than 0")


@auto_schema(title="Concrete Properties", description="Concrete Properties")
def concrete_properties(gradeConc: ConcreteMaterial):
	"""concrete_properties

	Args:
		gradeConc (ConcreteMaterial): Concrete Material Input Class

	Returns:
		str: Concrete Material Properties
	"""    
...
@auto_schema(title="Stress-Strain Curve", description="Stress-Strain Curve")
