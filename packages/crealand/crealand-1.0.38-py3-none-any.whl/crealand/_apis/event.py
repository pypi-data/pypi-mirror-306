...
#对象进入/离开判定区域事件
@_utils.check_type
def onAreaObjectEvent(runtime_id:Union[int,float],action:str,area_id:Union[int,float],cb:Any):
...
#分类进入/离开判定区域事件
@_utils.check_type
def onAreaClassEvent(config_id:str,action:str,area_id:Union[int,float],cb:Any):
    def cb_wrapper(err,data):
        if err is None:
            cb()
        else:
