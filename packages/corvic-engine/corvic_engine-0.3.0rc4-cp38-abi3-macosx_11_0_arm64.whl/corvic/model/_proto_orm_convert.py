...
def proto_to_orm(
    ...
    match proto_obj:
        ...
        case models_pb2.Pipeline():
            return _pipeline_proto_to_orm(proto_obj)


def _pipeline_merge_old_orm(new_orm: orm.Pipeline, old_orm: orm.Pipeline):
    new_input_ids = {val.resource.id for val in new_orm.inputs}
    for input in old_orm.inputs:
        if input.resource.id not in new_input_ids:
            new_orm.inputs.append(input)

    new_output_ids = {val.source.id for val in new_orm.outputs}
    for output in old_orm.outputs:
        if output.source.id not in new_output_ids:
            new_orm.outputs.append(output)


def merge_old_orm(new_orm: _Orm, old_orm: _Orm | None) -> None:
    if old_orm is None:
        return
    ...
    match new_orm:
        ...
        case orm.Agent():
            return
        case orm.Pipeline():
            _pipeline_merge_old_orm(new_orm, cast(orm.Pipeline, old_orm))
