...
class Predictor:
    ...
    def load_model_with_retry(self, model_path):
        """
        저장된 pickle 모델을 로드합니다. 예외 발생 시 모듈 경로 설정 후 재시도합니다.
        """
        try:
    ...
    def load_model(self, model_path):
        ...
        with open(model_path, 'rb') as f:
            # map_location으로 장치를 지정하여 CPU에서 모델을 로드할 수 있도록 함
            model = pickle.load(f, encoding='latin1', map_location=self.device)
        return model
