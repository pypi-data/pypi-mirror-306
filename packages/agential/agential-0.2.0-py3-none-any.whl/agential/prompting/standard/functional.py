"""Standard prompting functional module."""
...
def accumulate_metrics(steps: List[List[StandardStepOutput]]) -> Dict[str, Any]:
    """Accumulate total metrics from a list of StandardStepOutput objects.
    ...
    Args:
        steps (List[List[StandardStepOutput]]): A list of a list of StandardStepOutput objects representing individual steps.
    ...
    for step in steps:
        for warming_step in step:
            total_prompt_tokens += warming_step.answer_response.prompt_tokens
            total_completion_tokens += warming_step.answer_response.completion_tokens
            total_tokens += (
                warming_step.answer_response.prompt_tokens
                + warming_step.answer_response.completion_tokens
            )
            total_prompt_cost += warming_step.answer_response.prompt_cost
            total_completion_cost += warming_step.answer_response.completion_cost

            total_cost += (
                warming_step.answer_response.prompt_cost
                + warming_step.answer_response.completion_cost
            )
            total_prompt_time += warming_step.answer_response.prompt_time
