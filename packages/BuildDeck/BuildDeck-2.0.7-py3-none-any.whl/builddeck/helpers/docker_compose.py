...
def generate_docker_compose(services, environment, additional_services=None):
    ...
    with open(compose_file, 'w') as file:
        yaml.dump(compose, file, default_flow_style=False, sort_keys=False, indent=4, Dumper=InlineListDumper)

    logger.info(f"📝 Docker Compose file for {environment} environment generated as {compose_file}")
    return compose_file


def run_docker_compose(services, environment, is_build, additional_services):
    # Get the dynamic Docker Compose file
    compose_file = generate_docker_compose(services, environment, additional_services)
