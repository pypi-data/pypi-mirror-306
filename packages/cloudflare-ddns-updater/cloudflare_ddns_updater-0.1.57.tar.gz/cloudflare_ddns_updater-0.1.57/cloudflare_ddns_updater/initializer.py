...
def print_cron_job_interval():
    ...
    with open(CONFIG_FILE_PATH, 'r') as json_file:
        config_data = json.load(json_file)
        force_interval = config_data["FORCE_IP"]
        if force_interval == 0:
            return
    # Check cron interval
    cron = CronTab(user=True)
    for job in cron:
        if job.comment == cron_comment:
            ...
            if interval == "":
                print(f"Script currently runs once a minute "
                      f"and IP address update is forced every {force_interval} days.")
                return
...
def manage_cron_job():
    ...
    cron.write()
    print(f"Successfully set new interval to {cron_interval} minutes.")
...
def main():
    ...
    parser.add_argument('--logs', action='store_true', help="Changes the amount of logs produced")
