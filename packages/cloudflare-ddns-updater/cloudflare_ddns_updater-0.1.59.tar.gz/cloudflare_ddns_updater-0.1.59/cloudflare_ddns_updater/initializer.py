...
def log_quantity():
    ...
    print("Log level 'full' logs everything.")
    print("Log level 'minimal' logs only errors and IP address changes.")
    if log_level == "full":
...
def get_user_token():
    # First, attempt to turn off terminal echo
    if sys.stdin.isatty():
        try:
            ...
            try:
                # Disable echo by changing the settings
                termios.tcsetattr(fd, termios.TCSADRAIN, old_settings)
                # If this works, proceed with getpass-style input
                print("Enter your API token (input hidden):", end='', flush=True)
                token = input()  # Using input here instead of getpass to keep full control
                print("\nToken received.")
            finally:
                # Restore terminal settings regardless of success/failure
                termios.tcsetattr(fd, termios.TCSADRAIN, old_settings)
        except termios.error:
            # If setting terminal attributes fails, fall back to visible input
            token = input("Enter your API token (input visible): ")
    else:
        # Non-interactive environment (such as redirected I/O), so just use visible input
        token = input("Enter your API token (input visible): ")
