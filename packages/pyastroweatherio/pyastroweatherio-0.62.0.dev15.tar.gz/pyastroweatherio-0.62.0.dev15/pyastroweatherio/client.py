...
class AstroWeather:
    ...
    async def _get_atmosphere(
        self, details_seventimer, details_metno
    ) -> AtmosphereData | None:
    ...
    async def _get_condition(
        ...
        cloudcover_high = details_metno.get("cloud_area_fraction_high")
        cloudcover_medium = details_metno.get("cloud_area_fraction_medium")
        cloudcover_low = details_metno.get("cloud_area_fraction_low")
        fog = details_metno.get("fog_area_fraction")
        ...
        if self._experimental_features:
            ...
            fog2m = (
                await self._atmosphere.calculate_fog_density(
                    temp2m, rh2m, dewpoint2m, wind_speed
                )
                * 100
            )
        else:
    ...
    async def _get_forecast_data(
        self, forecast_type, hours_to_show
    ) -> List[ForecastData]:
        ...
        forecast_time = now.replace(minute=0, second=0, microsecond=0).replace(
            microsecond=0, tzinfo=timezone.utc
        )
        if self._test_datetime is not None:
            forecast_time = self._test_datetime.replace(
        ...
        for metno_index, datapoint in enumerate(self._weather_data_metno):
            datapoint_time = datetime.strptime(
                datapoint.get("time"), "%Y-%m-%dT%H:%M:%SZ"
            ).replace(microsecond=0, tzinfo=timezone.utc)
            ...
            atmosphere_data = await self._get_atmosphere(
                details_seventimer, details_metno
            )
            ...
            td = TimeDataModel(
                {
                    ...
                    "forecast_time": datetime.strptime(
                        datapoint.get("time"), "%Y-%m-%dT%H:%M:%SZ"
                    ).replace(microsecond=0, tzinfo=timezone.utc),  # timestamp
                }
            )
            ...
            for index in range(
                ...
                if (
                    ...
                    try:
                        ...
                        _LOGGER.error(
                            f"Failed to parse nightly conditions model data: {item}"
                        )
                        _LOGGER.error(ve)
