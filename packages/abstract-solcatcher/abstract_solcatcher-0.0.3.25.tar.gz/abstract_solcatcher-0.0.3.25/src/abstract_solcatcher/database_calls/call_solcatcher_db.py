...
def get_from_table(method):
    return [table for table in get_tables if table.get('tableName').lower() == make_single(method).lower()]
dbName = 'solcatcher'
env_path=get_env_path()
env_mgr = envManager(dbName,insert_list_file,env_path)
...
class clientMgr(metaclass=SingletonMeta):
    def __init__(self):
        if not get_tableshasattr(self, 'initialized'):  # Prevent reinitialization
