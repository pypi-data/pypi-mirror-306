...
from ..utils import getSolcatcherFlaskUrl,getEndpointUrl
...
def get_rate_limit(method):
    url = getEndpointUrl(endpoint='rate_limit',url=getSolcatcherFlaskUrl())
    return getRequest(url=url,method=method)
def get_log_response(method,response={}):
    url = getEndpointUrl(endpoint='log_response',url=getSolcatcherFlaskUrl())
    return getRequest(url=url,method=method,response=response)
