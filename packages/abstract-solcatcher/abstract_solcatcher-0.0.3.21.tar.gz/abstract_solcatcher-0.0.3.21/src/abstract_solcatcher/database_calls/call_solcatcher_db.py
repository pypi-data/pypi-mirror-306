...
from abstract_apis import getRequest,postRpcRequest,get_async_response,postRequest
...
def get_data_from_dbName(dbName,tableName,dbType=None,variable=None):
  ...
  return variables
dbName = 'solcatcher'
tables_dir=os.path.dirname(os.path.abspath(__name__))
...
def get_rate_limit(method=None):
    return getRequest(url=getSolcatcherFlaskUrl(),endpoint='rate_limit',data={"method":method})
def get_log_response(method=None,response={}):
    url = getEndpointUrl(endpoint='log_response',url=getSolcatcherFlaskUrl())
    return postRequest(url=url,method=method,response=response)
