...
class OptSymFunctionSinToSum( optFunctionBase.OptFunctionBase ):
  ...
  def optimize( self, elem, action ):
    ...
    elemSym = symexpress3.SymFormulaParser( "sum( n, 0 ,20, exp( n, -1 ) * exp( 2 * n + 1, x ) / factorial( 2 * n + 1 ) )" )
...
def Test( display = False):
  ...
  if str( symNew ).strip() != "sum( n,0,20, exp( n,(-1) ) *  exp( 2 * n + 1,pi * 1 * 4^^-1 ) *  factorial( 2 * n + 1 )^^-1 )":
