...
class TestComputeNestCli(unittest.TestCase):
    ...
    def test_import_command_dockerfile_private(self):
        file_path = '/Users/liuyuhang/PycharmProjects/computenest-cicd/tests/service/test-cli-output/config.yaml'
        print(os.environ)
        access_key_id = os.environ.get("ALIBABA_CLOUD_ACCESS_KEY_ID")
        access_key_secret = os.environ.get("ALIBABA_CLOUD_ACCESS_KEY_SECRET")
        with open(file_path, 'r') as stream:
        ...
        version_name = ''
        update_artifact = 'False'
        zip_file_path = "service/authorizer-main.zip"
        extract_path = os.path.join(os.getcwd(), "authorizer-main")  # 解压到当前工作目录下的文件夹
