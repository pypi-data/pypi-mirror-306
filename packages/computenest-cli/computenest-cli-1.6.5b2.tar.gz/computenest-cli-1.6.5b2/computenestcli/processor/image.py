...
class ImageProcessor:
    ...
    @Util.measure_time
    def process_acr_image(self, acr_image_name, acr_image_tag, file_path, docker_repo_url, build_type, build_args: []):
        logger.info(f"Starting process_acr_image with {locals()}")
        ...
        if build_type == constant.DOCKER_REPO_TYPE:
            ...
            print(file_path)
            commands = [
                f"docker build {build_args_str} -t {acr_image_name}:{acr_image_tag} . --progress plain",
                f"docker login {repository_name} --username={username} --password={password}",
        ...
        merged_command = commands
        try:
            Util.run_cli_commands(merged_command, file_path)
        except Exception as e:
            raise e
