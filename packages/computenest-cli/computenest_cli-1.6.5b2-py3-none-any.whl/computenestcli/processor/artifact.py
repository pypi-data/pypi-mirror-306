...
class ArtifactProcessor:
    ...
    def _create_acrimage_from_acrimage_builder(self, service_config, artifact_config, file_path_config):
        # logger.info(
        #     f"service_config: {service_config}, artifact_config: {artifact_config}, file_path_config: {file_path_config}")
        try:
            ...
            logger.info(f"acr_image_builder:{acr_image_builder}")
            ...
            if build_type == constant.DOCKER_REPO_TYPE:
                ...
                image_processor.process_acr_image(acr_image_name, acr_image_tag, file_path, docker_repo_url, build_type, build_args)
                repo_id = self._get_repo_id(artifact_type, acr_image_name)
            elif acr_image_builder.get(constant.DOCKER_FILE_PATH):
                docker_file_path = os.path.join(os.path.dirname(file_path), acr_image_builder[constant.DOCKER_FILE_PATH])
                logger.info("file_path: {}".format(docker_file_path))
                image_processor.process_acr_image(acr_image_name, acr_image_tag, docker_file_path,
                                                   None, constant.DOCKER_FILE_TYPE, build_args)
                logger.info("Image build successful and pushed to the target repository.")
                repo_id = self._get_repo_id(artifact_type, acr_image_name)
            else:
            ...
            logger.exception("An error occurred while trying to open the file.")
            raise
    ...
    def process(self, data_config, file_path_config, update_artifact='', version_name=''):
        ...
        for artifact_config in artifact_dict.values():
            ...
            artifact_config[UPDATE_ARTIFACT] = update_artifact == TRUE
