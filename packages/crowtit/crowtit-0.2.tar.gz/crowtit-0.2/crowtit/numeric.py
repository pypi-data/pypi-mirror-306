...
def _kde(
    ...
    text = f'missing values: count: {missing_values_count}, ratio: {missing_value_ratio} %'

    if ax is None:
        fig.subplots_adjust(top=0.85)
        ax = fig.add_subplot()

    # ax.set_title(f"Kernel Density Estimation. \nINFO: {text}", fontsize=font_size)
    sns.histplot(data=series, ax=ax, kde=True, color=color)
    # sns.kdeplot(series + 1, ax=ax, log_scale=log_scale, warn_singular=False, color=color)
    # sns.histplot()
