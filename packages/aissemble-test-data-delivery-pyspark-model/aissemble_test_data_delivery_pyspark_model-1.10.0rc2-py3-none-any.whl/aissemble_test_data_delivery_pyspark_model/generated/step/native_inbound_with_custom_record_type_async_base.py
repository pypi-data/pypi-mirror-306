...
def aissemble_encrypt_simple_aes(plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundWithCustomRecordTypeAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./krausening/base')
...
def aissemble_encrypt_with_vault_key(key, plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundWithCustomRecordTypeAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./config')
            os.environ['KRAUSENING_BASE'] = 'krausening/base/'
...
class NativeInboundWithCustomRecordTypeAsyncBase(AbstractPipelineStep):
    """
    Performs scaffolding asynchronous processing for NativeInboundWithCustomRecordTypeAsync. Business logic is delegated to the subclass.

    GENERATED CODE - DO NOT MODIFY (add your customizations in NativeInboundWithCustomRecordTypeAsync).

    Generated from: templates/data-delivery-pyspark/asynchronous.processor.base.py.vm
    """

    logger = LogManager.get_instance().get_logger('NativeInboundWithCustomRecordTypeAsyncBase')
    step_phase = 'NativeInboundWithCustomRecordTypeAsync'
    bomIdentifier = "Unspecified NativeInboundWithCustomRecordTypeAsync BOM identifier"
    ...
    async def execute_step(self, inbound: CustomRecord) -> None:
        """
        Executes this step.
        """
        start = time_ns()
        NativeInboundWithCustomRecordTypeAsyncBase.logger.info('START: step execution...')
        ...
        stop = time_ns()
        NativeInboundWithCustomRecordTypeAsyncBase.logger.info('COMPLETE: step execution completed in %sms' % ((stop - start) / 1000000))
        


    @abstractmethod
    async def execute_step_impl(self, inbound: CustomRecord) -> None:
        """
        This method performs the business logic of this step,
        and should be implemented in NativeInboundWithCustomRecordTypeAsync.
        """
        pass
    ...
    def check_and_apply_encryption_policy(self, inbound: CustomRecord) -> None:
        ...
        NativeInboundWithCustomRecordTypeAsyncBase.logger.info('Checking encryption policies')
        ...
        for key, encrypt_policy in retrieved_policies.items():
            ...
            if encrypt_policy.encryptPhase:
                if self.step_phase.lower() == encrypt_policy.encryptPhase.lower():
                    ...
                    NativeInboundWithCustomRecordTypeAsyncBase.logger.info('Encryption policy does not apply to this phase: ' + self.step_phase)
    ...
    def apply_encryption_to_dataset(self, inbound: CustomRecord, fields_to_update: List[str], algorithm: str) -> CustomRecord:
        ...
        '''
        NativeInboundWithCustomRecordTypeAsyncBase.logger.info('applying encryption')
