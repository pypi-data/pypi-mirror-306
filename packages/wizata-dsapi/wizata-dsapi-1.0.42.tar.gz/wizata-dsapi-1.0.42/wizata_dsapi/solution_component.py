...
class SolutionComponent(ApiDto):
    ...
    def to_json(self, target: str = None):
        ...
        if self.default_from is not None and verify_relative_datetime(
            self.default_from
        ):
    ...
    def from_json(self, obj):
        ...
        if "defaultFrom" in obj.keys() and obj["defaultFrom"] is not None:
            self.default_from = obj["defaultFrom"]
        if "defaultTo" in obj.keys() and obj["defaultTo"] is not None:
