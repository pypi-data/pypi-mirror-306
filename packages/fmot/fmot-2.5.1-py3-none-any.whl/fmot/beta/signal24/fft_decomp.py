...
class FFTwiddle(nn.Module):
    def __init__(
        ...
        self.gmac_todd = GMACv2(act_precision, bits_headroom=bits_headroom)
        self.add = GMACv2(
            act_precision, torch.tensor([1, 1]), bits_headroom=bits_headroom
        )
        self.sub = GMACv2(
    ...
    def forward(self, even_real, even_imag, odd_real, odd_imag):
        ...
        todd_real = self.gmac_todd(
            [self.twid_real, self.neg_twid_imag], [odd_real, odd_imag], []
        )
        # todd_imag = self.twid_real * odd_imag + self.twid_imag * odd_real
        todd_imag = self.gmac_todd(
            [self.twid_real, self.twid_imag], [odd_imag, odd_real], []
        )
