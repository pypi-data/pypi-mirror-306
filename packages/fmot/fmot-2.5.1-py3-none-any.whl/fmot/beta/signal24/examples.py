...
class ExampleModelB(nn.Module):
    """
    Example Overlap-Add system with higher-precision STFT/iSTFT with STFT pre-normalization.
    The STFT pre-normalization further improves the quantization performance.
    ...
    Important changes:
    - Uses fmot.beta.signal24.NormedSTFT / fmot.beta.signal24.NormedISTF instead of
        fmot.nn.STFT / fmot.nn.ISTFT
    - NormedSTFT and NormedISTFT have different input/output signatures than the standard
        STFT / ISTFT layers. The code below provides detailed in-line comments to highlight the
        differences.
    - Uses fmot.beta.signal24.ComplexMultiply to apply the complex mask (this ensures that the
        masked STFT coefficients remain in higher numerical precision)
    """
    ...
    def __init__(
        ...
        self.stft = NormedSTFT(
            ...
            weight_precision=weight_precision,
            window_fn=window_fn,
            max_norm_db=max_norm_db,  # maximum pre-normalizing gain, in dB. 35 is a good value.
        )
    ...
    def forward(self, audio: Tensor):
        """
        NormedSTFT returns two tuples:
         - The first tuple (re_normed, im_normed, inv_norm) contains higher-precision
            and normalized STFT coefficients. The `inv_norm` value is
            a tensor of shape (batch, num_frames, 1) containing an inverse
            normalizing gain
         - The second tuple (re, im) contains int16 STFT coefficients at absolute intensity
            levels. These are suitable for direct consumption by a DNN.
            They are related to the first tuple as:

                re = cast(re_normed * inv_norm, int16)
                im = cast(im_normed * inv_norm, int16)

        In this example, we pass the absolute-intensity int16 (re, im) coefficients
        directly into the DNN.
        """
        (re_normed, im_normed, inv_norm), (re, im) = self.stft(audio)
        ...
        # apply complex mask to the higher-precision/normalized STFT coefficients
        re_normed, im_normed = self.complex_mul(mask_re, mask_im, re_normed, im_normed)

        """
        NormedISTFT takes as input normalized re/im components along with the 
        inverse-normalizing gain provided by the NormedSTFT.
        
        Internally, the inverse-normalizing gain is applied after taking the inverse DFT
        to ensure that the output signal is at the correct intensity.
        """
        out = self.istft(re_normed, im_normed, inv_norm)
        return out
...
def test_ola_quantization(model: nn.Module, hop_length: int, plot=False):
    ...
    return qsnr_fp, qsnr_quant, qsnr_fqir, quant_fqir_mse
...
if __name__ == "__main__":
    ...
    for act_precision in [int16, int24]:
        for weight_precision in [int8, int16]:
            ...
            qsnr_fp, qsnr_quant, qsnr_fqir, mismatch = test_ola_quantization(
    ...
    for act_precision in [int16, int24]:
        for weight_precision in [int8, int16]:
            ...
            print(
                f"With {act_precision} x {weight_precision} and NormedSTFT/ISTFT:"
                f"\n\tF.P. reconstruction SNR: {qsnr_fp:.1f}dB"
                f"\n\tQAT reconstruction SNR: {qsnr_quant:.1f}dB"
                f"\n\tFQIR reconstruction SNR: {qsnr_fqir:.1f}dB"
                f"\n\tFQIR-QUANT mismatch: {mismatch:.1f}"
