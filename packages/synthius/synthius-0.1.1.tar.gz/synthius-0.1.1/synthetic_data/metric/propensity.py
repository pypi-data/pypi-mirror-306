...
class PropensityScore(BaseMetric):
    ...
    def __init__(
        ...
        *,
        ...
        self.real_data: pd.DataFrame = self.load_data(real_data_path)
    ...
    def evaluate(self: PropensityScore, synthetic_data_path: Path) -> pd.DataFrame:
        ...
        autogluon_score = self.compute_autogluon_score(train_data, test_data, model_subdir=f"{model_name}")
        xgboost_score = self.compute_xgboost_score(train_data, test_data)
        hist_gradient_boosting_score = self.compute_hist_gradient_boosting_score(train_data, test_data)

        model_name = synthetic_data_path.stem

        logger.info("Propensity Score for %s Done.", model_name)

        result = {
            "Model Name": model_name,
            "Autogluon": autogluon_score,
            "XGBoost": xgboost_score,
            "HistGradientBoosting": hist_gradient_boosting_score,
        }
    ...
    def pivot_results(self: PropensityScore) -> pd.DataFrame:
        ...
        df_melted = df_results.melt(
            id_vars=["Model Name"],
            value_vars=["Autogluon", "XGBoost", "HistGradientBoosting"],
            var_name="Metric",
            value_name="Value",
        )
