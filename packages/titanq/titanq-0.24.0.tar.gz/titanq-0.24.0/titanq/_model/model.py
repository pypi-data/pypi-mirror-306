...
class Model:
    ...
    def set_objective_expression(self, expr: MathObject, target=Target.MINIMIZE):
        """
        ...
        """
        if not isinstance(expr, MathObject):
            raise TypeError(f"invalid input of type {type(expr).__name__}")

        weights = expr.generate_weights(self._variables)
        bias = expr.generate_bias(self._variables)
    ...
    def add_constraint_from_expression(self, equation: Equation):
        ...
        if not isinstance(equation, Equation):
            raise TypeError(f"invalid input of type {type(equation).__name__}")

        mask, bounds = equation.generate_constraint(self._variables)
