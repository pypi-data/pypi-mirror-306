...
class PowerTraceConstraint(Constraint):
    ...
    class Conversion(ConstraintConversion):
        ...
        def convert(cls, con, options):
            ...
            from ..expressions.algebra import block, rsoc, I
            ...
            if con.relation == Constraint.LE and num > den:
                ...
                if n == 1:
                    lis = [rhs]*den  + [x]*(pown - num) + [I(n)]*(num - den)
                else:
                    lis = [v[0]]*den + [x]*(pown - num) + [I(n)]*(num - den)
                ...
                if n == 1:
                    lis = [rhs] * den + [x] * num + [I(n)] * (pown - num - den)
                else:
                    lis = [v[0]] * den + [x] * num + [I(n)] * (pown - num - den)
                ...
                if n == 1:
                    ...
                    P.add_constraint(
                        block([[lis[0], I(n)], [I(n), lis[1]]]) >> 0)
                    P.add_constraint(v[0].tr <= rhs)
            elif con.relation == Constraint.GE:
                pown = cls._np2(den)

                if n == 1:
                    lis = [x]*num + [rhs]*(pown - den)  + [I(n)]*(den - num)

                else:
                    lis = [x]*num + [v[0]]*(pown - den) + [I(n)]*(den - num)
