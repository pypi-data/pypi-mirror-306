...
class Problem(Valuable):
    ...
    def type(self):
        ...
        quantum = [
            expressions.QuantumEntropy,
            expressions.NegativeQuantumEntropy,
            expressions.QuantumConditionalEntropy,
            expressions.QuantumKeyDistribution,
            constraints.QuantRelEntropyConstraint,
            constraints.QuantCondEntropyConstraint,
            constraints.QuantKeyDistributionConstraint]
        complex = [
            constraints.ComplexAffineConstraint,
            constraints.ComplexLMIConstraint]
        ...
        if objective is None:
            ...
            elif C.issubset(set(linear + sdp + quantum)):
                base = "Quantum Relative Entropy Program"
            ...
            if C.issubset(set(linear + [constraints.LogSumExpConstraint])):
                base = "Geometric Program"
        elif isinstance(objective, tuple(quantum)):
            if C.issubset(set(linear + sdp + quantum)):
                base = "Quantum Relative Entropy Program"
        elif isinstance(objective, expressions.WeightedSum):
            if all(isinstance(expr, tuple(linear + quantum))
                   for expr in objective.expressions):
                if C.issubset(set(linear + sdp + quantum)):
                    base = "Quantum Relative Entropy Program"
    ...
    def check_current_value_feasibility(self, tol=1e-5, inttol=None):
        ...
        for constraint in all_cons:
            ...
            psd_constraints = (
                constraints.LMIConstraint,
                constraints.OpRelEntropyConstraint,
                constraints.MatrixGeoMeanEpiConstraint,
                constraints.MatrixGeoMeanHypoConstraint,
            )

            if isinstance(constraint, psd_constraints) or hack:
