...
def import_cbf(filename):
    ...
    while True:
        ...
        if lsplit and lsplit[0] in data_keywords:
            ...
            if lsplit[0] in ("OBJFCOORD", "FCOORD") and (
                "PSDVAR" not in seen_blocks
            ):
                raise Exception("missing PSDVAR block")
            if lsplit[0] in ("OBJACOORD", "ACOORD", "HCOORD") and (
                "VAR" not in seen_blocks
            ):
                raise Exception("missing VAR block")
            if lsplit[0] in ("DCOORD", "HCOORD") and (
                "PSDCON" not in seen_blocks
            ):
