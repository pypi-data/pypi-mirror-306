...
qe       = Fn("S({})");     """Quantum entropy glyph."""
qre      = Fn("S({}||{})");     """Quantum relative entropy glyph."""
...
# Matrix geometric mean glyphs.
geomean  = Op("{} # {}", 3, assoc=False)
"""Matrix geometric mean glyph."""
wgeomean = Op("{} #_{} {}", 3, assoc=False)
...
def clever_dotp(left, right, complexLHS, scalar=False):
    """Describe an inner product in a clever way.

    :param bool complexRHS: Whether the right hand side is complex.
    """
    lconj = glyphs.conj(left) if complexLHS else left

    # Detect scalar multiplication.
    if scalar:
        if left == right:
            if complexLHS:
                return glyphs.squared(glyphs.abs(left))
            else:
                return glyphs.squared(left)
        else:
            return glyphs.clever_mul(lconj, right)

    # Detect identities.
    if from_glyph(lconj, glyphs.idmatrix):
        return glyphs.trace(right)
    if from_glyph(right, glyphs.idmatrix):
        return glyphs.trace(lconj)

    # Detect sums.
    if from_glyph(lconj, glyphs.matrix) and lconj.operands[0] in (1, "1"):
        return glyphs.sum(right)
    if from_glyph(right, glyphs.matrix) and right.operands[0] in (1, "1"):
        return glyphs.sum(lconj)

    # Fall back to the inner product glyph.
