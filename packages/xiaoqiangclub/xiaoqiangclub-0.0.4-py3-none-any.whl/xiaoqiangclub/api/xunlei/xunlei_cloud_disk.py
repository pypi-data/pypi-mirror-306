...
class XunleiCloudDisk(XunleiBase):
    ...
    async def __yunpan_get_dir_info(self, parent_folder_id: str = "", return_raw_data: bool = False) -> Optional[
        List[Dict]]:
        """
        获取云盘文件夹信息：文件/目录id等
    ...
    async def __yunpan_file_manage(self, old_path: Union[str, List[str]], new_path: str, task_type: str) -> \
        ...
        if not (path_info_list and new_path_info):
            return None
        ...
        if json_data:
            if task_type == "batchDelete":
                ...
                self.log.info(f"云盘 {old_path} 移动/复制到 {new_path} 成功（有可能是部分成功，请自行查看云盘）！")
            return True
        else:
            if task_type == "batchDelete":
    ...
    async def yunpan_file_delete_forever(self, path: Union[str, List[str]]):
        """
        永久删除云盘文件或文件夹
    ...
    async def yunpan_file_delete(self, path: Union[str, List[str]], delete_forever: bool = False):
        """
        删除云盘文件或文件夹
        ...
        if delete_forever:
            ...
            return await asyncio.gather(*[move_to_recycle_bin(one_path) for one_path in path])

    async def yunpan_create_folder(self, folder_name: str, target_folder: str = None) -> Optional[Dict]:
        """
        云盘新建文件夹

        :param target_folder: 目标文件夹，例如：我的转存，默认为None：新建在根目录
        ...
        if target_folder is None:
            ...
            parent_folder_info = await self.__yunpan_get_resource_info(target_folder)
        ...
        target_folder = target_folder or '根目录'
        if response.status_code == 200:
            await asyncio.sleep(1)  # 等待服务器创建成功
            self.log.info(f"在云盘 {target_folder} 目录下新建文件夹 {folder_name} 成功！")
            ...
            self.log.error(
                f"在云盘 {target_folder} 目录下创建文件夹 {folder_name} 失败：{json_data.get('error_description')}")
            return None
    ...
    async def yunpan_create_download_task(self, download_url: str, target_folder: str,
                                          ...
                                          sub_file_index: Optional[List[Union[int, str]]] = None) -> bool:
        """
        ...
        :param rename: 注意：当下载的链接是一个多文件资源，只会修改文件夹名称。
        :param download_same_name: 当遇到同名文件或文件夹时是否继续下载，继续下载会自动在文件名后面添加 "(1)"、"(2)" 等后缀
        ...
        if not rename and file_count < 2:
            rename = download_url_info.get('task_name')

        if (not download_same_name) and await self.yunpan_file_in_folder(rename, target_folder):
            self.log.error(f'文件已存在，跳过：{download_url}')
            return False
        ...
        if response.status_code == 200:
            ...
            if rename and file_count < 2:
                task_id = json_data.get('task', {}).get('id')
                ...
                for _ in range(60):  # 循环等待5分钟云添加完成
                    ...
                    # 获取所有任务
                    yunpan_tasks = await self.yunpan_get_tasks_history()

                    if not yunpan_tasks:
                        self.log.error(f'{download_url} 重命名 {rename} 失败！')
                        return True

                    url_name = None
                    for task in yunpan_tasks.get('tasks'):
                        if task.get('id') == task_id:
                            url_name = target_folder + '/' + task.get('name')  # 初始名称
                            break

                    self.log.debug(
                        f'等待云添加 {download_url} 到 {target_folder} 的任务完成，已等待 {(_ + 1) * 5} 秒...')
                    if await self.yunpan_rename(url_name, rename):
                        break
        else:
            self.log.error(
...
{parts_xml}
    ...
    async def yunpan_upload_folder(self, local_folder_path: str, yunpan_save_path: str,
        ...
        # 创建根目录
        folder_info = await self.yunpan_create_folder(root_dir, yunpan_save_path)
        if not folder_info:
            return
        ...
        async def create_folder_with_limit(folder_name, new_folder):
            async with semaphore:
                return await self.yunpan_create_folder(folder_name, new_folder)
    ...
    async def yunpan_get_all_share_link(self, return_raw_data: bool = False, only_useful: bool = False,
        ...
        :param save_path: 迅雷云盘分享数据文件要存放的文件夹路径（文件名自动生成），默认为None，不保存。
        :return:
        """
        share_links = []
        next_page_token: str = ''
    ...
    async def __get_or_create_folder_id(self, parent_path: str, yp_save_root_path: str) -> Optional[str]:
        ...
        for part in path_parts:
            ...
            if not folder_info:
                folder_info = await self.yunpan_create_folder(part, os.path.dirname(current_path))
                if not folder_info:
