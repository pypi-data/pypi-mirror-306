...
# 新版hao6v网站链接
HAO6V_NEW_URLS = ["https://www.xb6v.com", "https://www.66s6.net", "https://www.66s6.cc", "https://www.66ss.org"]
        ...
        return [
            {
                ...
                "detail_url_path": li.css("a::attr(href)").get()  # 详情页链接
            }
            for li in ul.css("li")
        ]
...
async def download_ranking_list(mode: int = 0, only_red: bool = False) -> List[dict]:
    ...
    for li in lis:
        ...
        movie_data['detail_url_path'] = li.css('a::attr(href)').get()
        if mode != 2:
...
async def get_new_detail_url_data(new_detail_url: str,
    ...
    """
    获取 新版6v 详情页的数据
    :param new_detail_url: 新版6v 详情页链接列表
    ...
    if get_magnets or only_return_magnets:
        ...
        for a in selector.xpath('//*[@id="post_content"]/table/tbody/tr/td/a'):
            magnet_list.append({
                "title": title + a.xpath('./text()').get().strip(),  # 标题
...
async def get_old_detail_url_data(old_detail_url: str,
    ...
    :param old_detail_url: 旧版6v 详情页链接
...
async def get_list_page_data(url: str, only_red: bool = True, get_page_num: int = 1, get_all: bool = False,
    ...
    for li in lis:
        ...
        m['detail_url_path'] = li.css('a::attr(href)').get()
...
async def __parse_search_page(url: str = None, selector: Selector = None, only_detail_links: bool = False,
    ...
    for table in tables:
        ...
        movie_info['detail_url_path'] = detail_url
...
async def search_movie(search_query: str, only_detail_links: bool = False,
