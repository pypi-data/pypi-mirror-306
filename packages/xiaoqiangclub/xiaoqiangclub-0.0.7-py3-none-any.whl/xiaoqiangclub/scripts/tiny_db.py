...
class TinyDBManager:
    ...
    def insert(self, data: Union[Dict, List[Dict]], table_name: str = 'default',
               skip_existing: bool = False) -> Union[str, List[str]]:
        """
        向指定表中插入数据
        ...
        try:
            ...
            raise ValueError(f"插入数据时出现错误：{repr(e)}")
    ...
    def query(self, conditions: Dict, table_name: str = 'default') -> List[Dict]:
        """
        查询表中符合条件的数据
        ...
        :return: 符合条件的字典列表
        """
        log.debug(f"查询表 {table_name}，查询条件: {conditions}")
        try:
            table = self.db.table(table_name)
            ...
            raise ValueError(f"查询数据时出现错误：{repr(e)}")

    def update(self, query: Dict, new_data: Union[Dict, List[Dict]], table_name: str = 'default') -> List[int]:
        """
        更新符合条件的数据
        ...
        try:
            ...
            raise ValueError(f"更新数据时出现错误：{repr(e)}")
