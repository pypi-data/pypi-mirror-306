...
class TinyDBManager(TinyDB):
    ...
    def update_data(self, query: Dict, new_data: Union[Dict, List[Dict]], table_name: str = None) -> Optional[
        List[int]]:
        """
        更新符合条件的数据
        ...
        """
        log.debug(f"更新操作，查询条件: {query}，新的数据: {new_data}, 表: {table_name}")
        try:
            if table_name:
                # 更新指定表的数据
                table = self.table(table_name)
                ...
                combined_condition = self._combine_conditions(conditions_list)
                result = self.__perform_update(table, combined_condition, new_data)
    ...
    @staticmethod
    def __perform_update(table, combined_condition, new_data):
        """
        执行具体更新操作，支持字典和字典列表
        """
