...
class ModuleInstaller:
    ...
    def install(self, package: str, mirror: str = 'https://pypi.tuna.tsinghua.edu.cn/simple') -> None:
        """
        安装指定的模块
        ...
        subprocess.check_call(
            [sys.executable, '-m', 'pip', 'install', '-i', mirror, '-U', package])
    ...
    def verify_installation(self) -> None:
        ...
        for install_name, import_name in self.module_mapping.items():
            try:
                ...
                raise ImportError(f"{import_name} 安装失败，请手动安装：pip install {install_name}")
