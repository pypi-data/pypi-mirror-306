...
def install(package):
    subprocess.check_call(
        [sys.executable, '-m', 'pip', 'install', '-i', 'https://pypi.tuna.tsinghua.edu.cn/simple', '-U', package])
...
for install_name, import_name in module_mapping.items():
    try:
...
# 验证安装
if is_installed:
    for package in module_mapping.values():
        try:
            module = importlib.import_module(package)
        except ImportError:
            raise ImportError(f"{package} 未安装，请使用 pip install {package} 安装！")
...
class AutoGUI:
    def __init__(self, target: Optional[str] = None, auto_start: bool = False, sleep_time: int = 1,
                 start_wait_time: int = 3, start_safe_mode: bool = False):
        """
        初始化 AutoGUI 类
        # 注意：这里需要安装 pip install -i https://pypi.doubanio.com/simple opencv-python Pillow pywin32

        :param target: 要打开的文件、程序或网址，可选参数
        :param auto_start: 是否自动打开目标，默认为False
        :param sleep_time: 每一步操作后等待的时间，默认为1秒
        :param start_wait_time: 等待程序启动的时间，默认为3秒，当auto_start为True时生效。
        :param start_safe_mode: 是否启用安全模式：按 Esc 键强制终止任务！默认为False，注意：当主程序正常运行结束后，监听程序需要手动按 Esc 停止！
        """
        ...
        if self.target and self.target.startswith(('http://', 'https://')):
            self.title = self.get_url_title(self.target)
            if auto_start:
                # 打开网址
                webbrowser.open(self.target)
        elif self.target and os.path.exists(self.target):
            self.title = os.path.splitext(os.path.basename(self.target))[0]  # 使用文件名作为窗口标题
            if auto_start:
            ...
            if auto_start:
                ...
                self.title = None
                log.info('未设置启动对象，如需启动对象，请手动调用 start方法。')

        if auto_start and start_wait_time > 0:
            time.sleep(start_wait_time)

        # 启动安全模式
        if start_safe_mode:
            log.info('已启用安全模式，按下 Esc 键，可强制终止程序！')
            # 鼠标、键盘、剪贴板的监听对象
    ...
    def safe_mode(self, key):
        ...
        if key == keyboard.Key.esc:  # 键盘esc被释放，停止监听
            ...
            self.show_windows_popup('警告', '您已强制终止任务！')
            # 取消置顶
            self.finish()
    ...
    def get_image_center_pos(self, image_path: str, **kwargs) -> Optional[tuple]:
        """
        获取图像中心位置
    ...
    def search_image(self, image_path: str, click_left: bool = True, clicks: int = 1,
        ...
        if not os.path.exists(image_path):
            ...
            self.show_message("图片不存在", f"图片 {image_path} 不存在！\n 程序将退出。")
            # 退出程序
            self.finish()
        ...
        while True:
            ...
            if wait_forever:
                ...
                if start_time + 30 * n < time.time():  # 每30秒显示一次提示信息
                    n += 1
                    self.show_windows_popup("未找到图片", f"未找到 {image_path}，\n退出请按 Esc 键。")
                continue
            ...
            if fail_out:
                self.show_message("无法定位图片", f"没有找到 {image_path}！\n程序将退出。", )
                # 退出程序
                self.finish()
        ...
        while True:
            ...
            if wait_forever:
                ...
                if start_time + 30 * n < time.time():  # 每30秒提醒一次
                    n += 1
                    self.show_windows_popup("图片未出现", f"没有等到 {image_path} 出现，\n退出请按 Esc 键。")
                continue

            if fail_out:
                self.show_message("图片未出现", f"没有等到 {image_path} 出现！\n程序将退出。")
                # 退出程序
                self.finish()
    ...
    def wait_for_image_to_disappear(self, image_path: str, confidence: float = 0.6, grayscale: bool = False,
        ...
        while True:
            ...
            if wait_forever:
                ...
                if start_time + 30 * n < time.time():  # 每30秒提醒一次
                    n += 1
                    self.show_windows_popup("图片未消失", f"没有等到 {image_path} 消失，\n退出请按 Esc 键。")
                continue

            if fail_out:
                self.show_message("图片未消失", f"没有等到 {image_path} 消失！\n程序将退出。")
                # 退出程序
                self.finish()
    ...
    def get_url_title(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            if not title:
                self.show_message("无法获取标题", "请检查网络连接或网页是否正确")
                return None
    ...
    def show_message(self, title: str, message: str, button: str = 'OK'):
        """
        显示提示消息
        注意：此方法会阻塞程序执行，直到用户点击按钮

        :param title: 消息标题
        :param message: 消息内容
        :param button: 按钮文本
        """
        log.info(f'显示窗口消息：{message}')
        pyautogui.alert(text=message, title=title, button=button)

    def show_windows_popup(self, message: str, title: str = '消息', duration: int = 3,
                           logo_path: str = None, use_thread=True):
        """
        显示气泡消息

        :param message: 气泡消息的内容
        :param title: 气泡消息的标题
        :param duration: 气泡消息显示时间（秒），默认为3秒
        :param logo_path: 气泡消息的图标路径
        :param use_thread: 是否使用单线程运行，默认为 True。注意：如果为 False，会阻塞主线程。
        """
        log.info(f'显示气泡消息：{message}')
        # 判断系统
        if 'Windows' in platform.system():
            try:
                show_custom_info(message, title=title, display_time=duration * 1000, logo_path=logo_path,
                                 use_thread=use_thread)
            except Exception as e:
                log.error(f'显示气泡消息失败: {e}')
                raise  # 重新抛出异常，或处理异常
        else:
            raise NotImplementedError('只兼容Windows系统')

    def finish(self, delay: int = 3):
        """
        结束程序

        :param delay: 延迟退出时间（秒），默认为3秒
        """
        self.show_windows_popup(
            f'任务 {self.title} 结束，共耗时 {round(time.time() - self.start_time, 2)} 秒。\n程序将在 5 秒后自动退出，感谢使用！',
            '任务结束', duration=delay, use_thread=True)
