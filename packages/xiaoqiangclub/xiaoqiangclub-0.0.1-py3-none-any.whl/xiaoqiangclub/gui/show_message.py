...
def set_icon(window: tk.Tk, logo_path: str = None) -> None:
    ...
    try:
        if os.path.isfile(logo_path):
            ...
            img = img.convert("RGBA")  # 确保图像为RGBA格式
            ...
            img.thumbnail((32, 32), Image.LANCZOS)  # 使用LANCZOS重采样
            photo = ImageTk.PhotoImage(img)
...
def show_input(prompt: str, title: str = '请输入', confirm_text: str = "确认", cancel_text: str = "取消",
               default_value: str = None, logo_path: str = Base64Images.logo_png) -> str:
    """
    显示自定义输入对话框，允许用户输入文本。
...
def show_custom_info(message: str, title: str = '消息', display_time: int = 3000,
                     ...
                     logo_path: str = Base64Images.logo_png, prompt_tone: bool = 'default',
