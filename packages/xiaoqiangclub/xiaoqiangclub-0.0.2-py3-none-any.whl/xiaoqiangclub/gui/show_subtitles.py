...
class ShowSubtitles:
    def __init__(self, font_family: str = "Arial", font_size: int = 24, font_color: str = "white",
                 bg_color: str = "black", bg_opacity: float = 0.8, position: Union[str, Tuple[int, int]] = "bottom",
                 duration: int = 3000, caption_width: Optional[int] = None, fade_in: bool = True,
                 fade_out: bool = True):
        """
        显示字幕
    ...
    def __hide_message(self) -> None:
        """ 隐藏消息并检查队列 """
        self.__fade_out()
        self.root.after(1000, self.__process_queue)  # 处理下一条消息
    ...
    def __split_text_into_lines(self, text: str) -> Tuple[int, str]:
        ...
        sentences = text_splitter(text, keep_symbols=True)
        lines = []
        current_line = ""
        ...
        for sentence in sentences:
            ...
            if width <= self.caption_width - 40:  # 如果当前行宽度小于最大宽度（考虑内边距）
    ...
    def show_message(self, message: str, duration: Optional[int] = None) -> None:
        """
        显示消息
        ...
        try:
            ...
            self.queue.put((formatted_message, duration))
        except Exception as e:
            log.error(f"Error splitting text: {e}")
