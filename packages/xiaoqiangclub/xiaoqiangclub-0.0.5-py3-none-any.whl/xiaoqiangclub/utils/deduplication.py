...
class Deduplication:
    ...
    def __init__(self, storage_file: Optional[str] = None, lazy_save: bool = False,
        ...
        :param lazy_save: 是否采用懒惰保存策略，默认为 False。如果设置为 True，则 save_interval 和 save_count 参数生效。注意：为了防止数据丢失，请确保在程序退出前执行save_processed_data/save_processed_data_async 方法，保存数据。
    ...
    def is_processed(self, data: Any) -> bool:
        """
        检查数据是否已处理
    ...
    async def save_processed_data_async(self) -> None:
        """
        将已处理的数据集合保存到文件中（如果设置了文件名）
        """
        if self.storage_file:
    ...
    async def add_data_async(self, data: Any) -> bool:
        """
        ...
        if data_hash not in self.processed_data:
            ...
            if self.lazy_save:
                if (self.save_interval and time.time() - self.last_save_time >= self.save_interval) or \
                        (self.save_count and self.data_added_since_last_save >= self.save_count):
                    await self.save_processed_data_async()  # 使用异步保存
                    self.last_save_time = time.time()  # 更新上次保存时间
                    if self.save_count:
                        self.data_added_since_last_save = 0  # 重置计数器
            else:
                await self.save_processed_data_async()
            return True
        return False
