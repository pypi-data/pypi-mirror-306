...
class DtaServiceHeader:
    ...
    def __init__(self, project: str, authorization: str) -> None:
    ...
    def get(self) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: A dictionary with keys "Authorization" and
                            ...
                            authorization token and project identifier.
        """
        return {
...
class DtaSecrets:
    ...
    def get_all(self, project: str | None = None,
        ...
        get_data = self.__ask("/retrieve/all")
    ...
    def get(self,
            ...
            version: int | None = None) -> str | int | None:
        """
        ...
        get_data = self.__ask(
            f"/retrieve/{secret_name}/versions/{self.get_version(version)}"
        )
        if get_data.status_code == 200:
    ...
    def __ask(self, resource: str, payload: dict = {}) -> Any:
        ...
        if self.is_production:
            resource += "/production"
        secret_response = requests.request("GET", service_url + resource,
                                           headers=headers, data=payload)
        return secret_response

    def __service_builder(self) -> Tuple[Text, Dict]:
        """
        Builds the service URL and headers for the DTA service.

        Returns:
            Tuple[Text, Dict]: A tuple containing the DTA service URL and the
                               headers.
        """
        headers = DtaServiceHeader(project=self.dta_project,
                                   authorization=self.authorization)
        service_url = os.environ.get("DTA_SECRET_URL")
