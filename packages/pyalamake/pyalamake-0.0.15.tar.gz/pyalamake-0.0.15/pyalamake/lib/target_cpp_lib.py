...
class TargetCppLib(TargetBase):
    ...
    def compiler(self, val):
        self._cxx = val
        if self._cxx == 'gcc':
            self._compile_opts = self._compile_opts.replace('-std=c++20', '')
    ...
    def _gen_lib(self):
        ...
        for file in self.sources:
            ...
            if self._lib_type == 'shared':
                fpic = '-fPIC '
            self._writeln(f'\t{self._cxx} -MMD {fpic} -c {self._inc_dirs} {self._compile_opts} {file} -o {obj}')
            self._objs += f'{obj} '
            build_deps += f'{file} '
    ...
    def _gen_shared_library(self):
        ...
        lib_name = f'lib{self.target}.so'
        lib = f'{svc.gbl.build_dir}/{lib_name}'
        self._writeln(f'{lib}: {self._objs}')
        self._writeln(f'\t{self._cxx} -MMD --shared -fPIC {self._objs} {self._link_paths} {self._libs} -o {lib}')
        self._writeln('')
    ...
    def _gen_static_library(self):
        ...
        lib = f'{svc.gbl.build_dir}/{lib_name}'
        self._writeln(f'{lib}: {self._objs}')
        self._writeln(f'\tar rcs {lib} {self._objs} {self._link_paths} {self._libs}')
        self._writeln('')
        ## see baseclass for definition of self.target
        self.add_clean(lib_name)
