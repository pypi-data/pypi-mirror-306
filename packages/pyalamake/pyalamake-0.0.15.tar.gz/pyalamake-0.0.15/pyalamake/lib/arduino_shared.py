...
class ArduinoShared:
    ...
    def __init__(self):
        ...
        if svc.gbl.os_name == 'macos':
            self.arduino_dir = '~/Library/Arduino15'
            self.arduino_dir = os.path.expanduser(self.arduino_dir)
        else:
        ...
        if svc.gbl.os_name == 'macos':
            self.avrdude_dir = '/opt/homebrew/etc'
        else:
            self.avrdude_dir = '/usr/share/arduino/hardware/tools'

        ## additional include directories for the arduino library
        if svc.gbl.os_name == 'macos':
            ## see above
            self.core_includes = [
                f'{self.arduino_dir}/packages/arduino/hardware/avr/1.8.6/cores/arduino',
                f'{self.arduino_dir}/packages/arduino/hardware/avr/1.8.6/variants/standard',
            ]
        else:
            ## see above
            self.core_includes = [
                f'{self.arduino_dir}/hardware/arduino/avr/cores/arduino',
                f'{self.arduino_dir}/hardware/arduino/avr/variants/standard',
            ]
