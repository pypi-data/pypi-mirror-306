...
class TargetArduino(TargetBase):
    ...
    def _gen_app(self):
        ...
        for file in self.sources:
            ...
            self._writeln(
                f'\t{self._shared.cpp} {self._shared.cpp_opts} {self._compile_opts} {self._inc_dirs} {file} -o {obj}')
    ...
    def _gen_upload(self):
        ...
        self._writeln(
            f'\t{self._shared.avrdude} -v {avrdude_args} -D -Uflash:w:{self._hexfile}:i -Ueeprom:w:{self._eepfile}:i')
        self._writeln('')
