...
def main():
    ...
    for module_name in modules:
        ...
        for name, obj in _alias_configs(module_name, args.ignore_abc):
...
def _should_export(obj: typing.Any, ignore_abc: bool):
    # If this is a `TypeAliasType`, resolve the actual type.
    ...
    def _is_config_type(obj: typing.Any, ignore_abc: bool):
        try:
            # If this is a `TypeAliasType`, resolve the actual type.
            obj = _unwrap_type_alias(obj)
            ...
            if typing.get_origin(obj) is typing.Annotated:
                return _is_config_type(typing.get_args(obj)[0], ignore_abc)
            ...
            if typing.get_origin(obj) in (types.UnionType, typing.Union):
                return all(_is_config_type(t, ignore_abc) for t in typing.get_args(obj))
            ...
            if isinstance(obj, typing.TypeVar):
                if obj.__bound__ is not None:
                    return _is_config_type(obj.__bound__, ignore_abc)
                if obj.__constraints__:
                    return all(
                        _is_config_type(c, ignore_abc) for c in obj.__constraints__
                    )
    ...
    if _is_config_type(obj, ignore_abc):
        return True
...
def _module_configs(module_name: str, ignore_abc: bool):
    ...
    for _, cls in inspect.getmembers(module, inspect.isclass):
        if _should_export(cls, ignore_abc):
            yield cls


def _alias_configs(module_name: str, ignore_abc: bool):
    # Import the module
    module = importlib.import_module(module_name)
    ...
    for name, obj in inspect.getmembers(module):
        if _should_export(obj, ignore_abc):
            yield name, obj
