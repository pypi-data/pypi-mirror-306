...
class Ticket(
    ...
    upgrade_notes = dd.RichTextField(_("Resolution"), blank=True, format="plain")
    ...
    waiting_for = models.CharField(_("Waiting for"), max_length=200, blank=True)
    ...
    def as_paragraph(self, ar, **kwargs):
        ...
        if self.assigned_to:
            more.append(
                format_html(_("assigned to {}"), ar.obj2htmls(self.assigned_to))
            )
        if len(more):
