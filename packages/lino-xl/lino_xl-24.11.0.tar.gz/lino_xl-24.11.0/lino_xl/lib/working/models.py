...
@dd.background_task(every_unit="weekly", every=1, saturday=True, start_time="04:00")
def send_weekly_report(ar):
    tpl = rt.get_template("working/weekly_report.eml")
    recipients = [obj.user.email for obj in rt.models.working.Contract.objects.all()]
    ar.permalink_uris = True
    ar.renderer = dd.plugins.memo.front_end.renderer
    context = ar.get_printable_context()
    body = tpl.render(**context)
    sender = settings.SERVER_EMAIL
    subject = _("Weekly activity {site}").format(
