...
class Sessions(dd.Table):
    ...
    def sum_duration(cls, user, sd, ed):
        ...
        sar = cls.request(user=user, param_values=pv)
        rv = Duration()
        for s in sar:
...
class ActiveContracts(Contracts):
    ...
    def comments_last_week(cls, obj, ar=None):
        return count_comments(obj.user, dd.today(-7), dd.today(-1))

    @dd.virtualfield(dd.IntegerField(_("Comments\nlast month")))
    def comments_last_month(cls, obj, ar=None):
        return count_comments(obj.user, dd.today(-30), dd.today(-1))


def count_comments(user, sd, ed):
    pv = dict(start_date=sd, end_date=ed)
    qs = rt.models.comments.Comments.request(user=user, param_values=pv)
    return qs.get_total_count()
