...
from nlpbridge.persistent.db_crud import CRUDRouter, CRUDNode, CRUDEdge, CRUDTemplate
...
class RouterManager:
    ...
    async def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
        ...
        print(f'meta_template_id: {router.meta_template_id}')

        # Add basic info template
        if router.meta_template_id != None:
            meta_template_id = int(router.meta_template_id)
            basic_info_template = await TemplateManager.get_templates([meta_template_id])[0]
            print(f'basic info: {basic_info_template.content}') # debug
            for tmp in templates:
                tmp.content = f'\n{basic_info_template.content}\n{tmp.content}'
                print(f'template info: {tmp.content}') # debug
