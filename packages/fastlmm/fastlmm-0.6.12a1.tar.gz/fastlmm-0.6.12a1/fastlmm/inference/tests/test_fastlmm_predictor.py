...
class TestFastLMM(unittest.TestCase):
    ...
    def test_api(self):
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            cov_train, unit_trained = (
                self.covariate_whole[train_idx, :]
                .read()
            ...
            K_train = SnpKernel(
                self.snpreader_whole[train_idx, :], Unit(), block_size=100
            )
            (
                train_train_kernel,
                snp_trained,
                kernel_trained,
            ) = K_train._read_with_standardizing(
            ...
            whole_kernel = (
                SnpKernel(self.snpreader_whole, Unit(), block_size=100)
                .read()
                .standardize()
            )
            train_kernel = whole_kernel[train_idx].read(order="A", view_ok=True)
            ...
            fastlmm1 = FastLMM(
                snp_standardizer=SS_Identity(), kernel_standardizer=KS_Identity()
            )
            fastlmm1.fit(
                K0_train=train_kernel, X=self.covariate_whole, y=self.pheno_whole
            )  # iid intersection means we won't really be using whole covar or pheno
            predicted_pheno, covar = fastlmm1.predict(
                K0_whole_test=whole_test_kernel, X=self.covariate_whole, count_A1=False
            )
        ...
        fastlmm2 = FastLMM()
        fastlmm2.fit(
            K0_train=self.snpreader_whole[train_idx, :],
            X=self.covariate_whole,
            y=self.pheno_whole[train_idx, :],
        )  # iid intersection means we won't really be using whole covar
        predicted_pheno, covar = fastlmm2.predict(
            K0_whole_test=self.snpreader_whole[test_idx, :],
            X=self.covariate_whole,
            count_A1=False,
        )
    ...
    def test_notebook1(self):
        ...
        training = bed[
            :400, :
        ]  #!!!later: the learning code doesn't like it if there are two instances of bed[:400] that are not "is -equal"
        fastlmm2 = FastLMM(GB_goal=2).fit(
            K0_train=training, X=cov[:400, :], y=pheno[:400, :]
        )

        # Predict on training data:
        predicted_score, covariance = fastlmm2.predict(
            K0_whole_test=training, X=cov[:400, :], count_A1=False
        )

        assert np.array_equal(
            pheno.iid[:400], predicted_score.iid
        ), "for plots to make sense, the iids must be in the order"
        if do_plot:
            plt.plot(
                pheno.val[:400, :], predicted_score.val, "b.", [-5, 5], [-5, 5], "-r"
            )
            plt.errorbar(
                pheno.val[:400, :],
                predicted_score.val,
                yerr=np.sqrt(np.diag(covariance.val)),
                fmt=".",
            )
        ...
        predicted_score, covariance = fastlmm2.predict(
            K0_whole_test=bed[400:500, :], X=cov[400:500, :], count_A1=False
        )

        assert np.array_equal(
            pheno.iid[400:500], predicted_score.iid
        ), "for plots to make sense, the iids must be in the order"
        if do_plot:
            plt.plot(
                pheno.val[400:500, :], predicted_score.val, "b.", [-5, 5], [-5, 5], "-r"
            )
            plt.errorbar(
                pheno.val[400:500, :],
                predicted_score.val,
    ...
    def test_one(self):
        ...
        fastlmm1 = FastLMM(GB_goal=2).fit(
            K0_train=G0_train, X=covariate_train, y=pheno_train
        )
        ...
        predicted_pheno, covar = fastlmm2.predict(
            K0_whole_test=G0_test, X=covariate_test, count_A1=False
        )
    ...
    def test_str(self):
        ...
        fastlmm1 = FastLMM(GB_goal=2).fit(
            K0_train=G0_train, X=covariate_train, y=pheno_train, count_A1=False
        )
    ...
    def test_lr_no_K0(self):
        ...
        covariate_train3.val = np.array(
            [[float(num)] for num in range(covariate_train3.iid_count)]
        )
        pheno_train3 = self.pheno_whole[train_idx, :].read()
        np.random.seed(0)
        pheno_train3.val = (
            covariate_train3.val * 2.0
            + 100
            + np.random.normal(size=covariate_train3.val.shape)
        )  # y = 2*x+100+normal(0,1)
        ...
        predicted_pheno, covariance = fastlmm3.predict(
            K0_whole_test=KernelIdentity(pheno_train3.iid),
            X=covariate_train3,
            count_A1=False,
        )  # test on train
    ...
    def test_lr_as_lmm(self):
        ...
        for force_full_rank in [True, False]:
            # Learn model, save, load
            fastlmmx = FastLMM(GB_goal=2, force_full_rank=force_full_rank).fit(
                K0_train=covariate_train, X=None, y=pheno_train
            )
            ...
            if do_test_on_train:
                # Predict with model (test on train)
                predicted_pheno, covar = fastlmm.predict(
                    K0_whole_test=covariate_train, X=None, count_A1=False
                )  # test on train
            ...
            predicted_pheno, covar = fastlmm.predict(
                K0_whole_test=covariate_test, X=None, count_A1=False
            )  # test on train
    ...
    def test_lr2(self):
        ...
        first_name = None
        for name, K0_train, K0_whole_test in [
            (
                "Identity Kernel",
                KernelIdentity(self.snpreader_whole.iid[train_idx]),
                KernelIdentity(
                    self.snpreader_whole.iid, test=self.snpreader_whole.iid[test_idx]
                ),
            ),
            ...
            # Learn model, save, load
            fastlmmx = FastLMM(GB_goal=2).fit(
            ...
            if do_test_on_train:
                # Predict with model (test on train)
                predicted_pheno, covar = fastlmm.predict(
                    K0_whole_test=K0_train, X=covariate_train, count_A1=False
                )  # test on train
            ...
            # Predict with model (test on test)
            predicted_pheno, covar = fastlmm.predict(
                K0_whole_test=K0_whole_test, X=covariate_test, count_A1=False
            )  # test on train
    ...
    def test_str2(self):
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            KernelNpz.write(
                K0_train_filename, whole_kernel[train_idx].read(order="A", view_ok=True)
            )

        fastlmm1 = FastLMM(GB_goal=2).fit(
            K0_train=K0_train_filename, X=covariate_train, y=pheno_train
        )
        ...
        predicted_pheno, covar = fastlmm2.predict(
            K0_whole_test=whole_kernel[:, test_idx].read(order="A", view_ok=True),
            X=covariate_test,
            count_A1=False,
        )
    ...
    def _fasttwoK(self, force_low_rank, GB_goal):
        ...
        G1_train = SnpData(
            iid=G0_train.iid,
            sid=[item + "_1" for item in G0_train.sid],
            val=G0_train.read().val,
            pos=G0_train.pos,
            name="Different SNP names for {0}".format(G0_train),
        )
        covariate_train = self.covariate_whole[train_idx, :]
        pheno_train = self.pheno_whole[train_idx, :]

        logging.info("force_low_rank = {0}".format(force_low_rank))
        fastlmm1 = FastLMM(force_low_rank=force_low_rank, GB_goal=GB_goal).fit(
            K0_train=G0_train,
            K1_train=G1_train,
            X=covariate_train,
            y=pheno_train,
        ...
        G1_test = SnpData(
            iid=G0_test.iid,
            sid=[item + "_1" for item in G0_test.sid],
            val=G0_test.read().val,
            pos=G0_test.pos,
            name="Different SNP names for {0}".format(G0_test),
        )
        covariate_test = self.covariate_whole[test_idx, :]

        predicted_pheno, covar = fastlmm2.predict(
            K0_whole_test=G0_test,
            K1_whole_test=G1_test,
            X=covariate_test,
            count_A1=False,
        )
        ...
        output_file = self.file_name(
            "fasttwoK"
            + ("_force_low" if force_low_rank else "")
            + ("GB{0}".format(GB_goal) if GB_goal is not None else "")
        )
        Dat.write(output_file, predicted_pheno)
    ...
    def test_lowrank(self):
        ...
        mean_low, covar_low = (
            FastLMM(force_low_rank=True, GB_goal=2)
            .fit(
                K0_train=G0_train,
                y=pheno_whole[train_idx, :],
                X=self.covariate_whole[train_idx, :],
            )
            .predict(
                K0_whole_test=G0_test,
                X=self.covariate_whole[test_idx, :],
                count_A1=False,
            )
        )
        mean_full, covar_full = (
            FastLMM(force_full_rank=True, GB_goal=2)
            .fit(
                K0_train=G0_train,
                y=pheno_whole[train_idx, :],
                X=self.covariate_whole[train_idx, :],
    ...
    def test_twoK(self):
        ...
        fastlmm1 = FastLMM(GB_goal=2).fit(
            K0_train=G0_train, K1_train=G0_train, X=covariate_train, y=pheno_train
        )
        filename = self.tempout_dir + "/model_one.flm.p"
        pstutil.create_directory_if_necessary(filename)
        fastlmm2 = fastlmm1
    ...
    def test_lr(self):
        ...
        for force_full_rank, force_low_rank in [(True, False), (False, True)]:
            ...
            fastlmm3x = FastLMM(
                ...
                GB_goal=2,
            ).fit(K0_train=G0_train, X=covariate_train3, y=pheno_train3)
            filename = self.tempout_dir + "/model_lr.flm.p"
            pstutil.create_directory_if_necessary(filename)
            fastlmm3 = fastlmm3x

            # Predict with model (test on train)
            predicted_pheno, covar = fastlmm3.predict(
                K0_whole_test=G0_train, X=covariate_train3, count_A1=False
            )  # test on train
    ...
    def test_lmm(self):
        ...
        iid = [
            ["cid{0}P{1}".format(iid_index, iid_index // 250)] * 2
        ...
        K0a = KernelData(
            iid=iid, val=np.empty([iid_count, iid_count]), name="related by distance"
        )
        for iid_index0 in range(iid_count):
            for iid_index1 in range(iid_count):
                K0a.val[iid_index0, iid_index1] = (
                    1 if iid_index0 % 5 == iid_index1 % 5 else 0
                )
                if iid_index1 < iid_index0:
                    assert (
                        K0a.val[iid_index0, iid_index1]
                        == K0a.val[iid_index1, iid_index0]
                    )
        ...
        for iid_index in range(iid_count):
            K0b.val[iid_index, :] = 1 - np.abs(home - home[iid_index]) ** 0.1

        # make covar just numbers 0,1,...
        covar = SnpData(
            iid=iid, sid=["x"], val=np.array([[float(num)] for num in range(iid_count)])
        )
        covariate_train = covar[train_idx, :].read()
        covariate_test = covar[test_idx, :].read()

        for name, h2, K0 in [("clones", 1, K0a), ("line_world", 0.75, K0b)]:
            sigma2x = 100
            varg = sigma2x * h2
            vare = sigma2x * (1 - h2)
            ...
            p3 = (
                np.random.multivariate_normal(np.zeros(iid_count), K0.val)
                * np.sqrt(varg)
            ).reshape(-1, 1)
            ...
            for factor in [1, 100, 0.02]:
                ...
                v3 = np.var(p3)
                logging.debug(
                    "Original h2 of {0}. Generated h2 of {1}. Learned h2 of {2}".format(
                        h2, v3 / (v2 + v3), fastlmmx.h2raw
                    )
                )
                ...
                if do_test_on_train:
                    # Predict with model (test on train)
                    predicted_pheno, covar_pheno = fastlmm.predict(
                        K0_whole_test=K0_train, X=covariate_train, count_A1=False
                    )  # test on train
                    ...
                    predicted_pheno0, covar_pheno0 = fastlmm.predict(
                        K0_whole_test=K0_train[:, 0],
                        X=covariate_train[0, :],
                        count_A1=False,
                    )  # test on train #0
                    assert (
                        np.abs(predicted_pheno0.val[0, 0] - predicted_pheno.val[0, 0])
                        < 1e-6
                    ), "Expect a single case to get the same prediction as a set of cases"
                    assert (
                        np.abs(covar_pheno0.val[0, 0] - covar_pheno.val[0, 0]) < 1e-6
                    ), "Expect a single case to get the same prediction as a set of cases"

                # Predict with model (test on test)
                predicted_phenoB, covar_phenoB = fastlmm.predict(
                    K0_whole_test=K0_whole_test, X=covariate_test, count_A1=False
                )  # test on test
                output_file = self.file_name("lmmb_" + name)
                Dat.write(output_file, predicted_phenoB)
                covar2 = SnpData(
                    iid=covar_phenoB.row,
                    sid=covar_phenoB.col[:, 1],
                    val=covar_phenoB.val,
                )  # kludge to write kernel to text format
                ...
                if do_plot:
                    ...
                    pylab.xlim([-1, 10])
                    pylab.errorbar(
                        covariate_test.val, predicted, yerr, linestyle="None"
                    )
                    pylab.suptitle(
                        name
                        + ": test on test: test X to true target (green) and prediction (red)"
                ...
                predicted_phenoB0, covar_phenoB0 = fastlmm.predict(
                    K0_whole_test=K0_whole_test[:, 0],
                    X=covariate_test[0, :],
                    count_A1=False,
                )  # test on a single test case
                assert (
                    np.abs(predicted_phenoB0.val[0, 0] - predicted_phenoB.val[0, 0])
                    < 1e-6
                ), "Expect a single case to get the same prediction as a set of cases"
                assert (
                    np.abs(covar_phenoB0.val[0, 0] - covar_phenoB.val[0, 0]) < 1e-6
                ), "Expect a single case to get the same prediction as a set of cases"
                ...
                predicted_phenoC, covar_phenoC = fastlmm.predict(
                    K0_whole_test=K0_whole_some, X=covariate_some, count_A1=False
                )
    ...
    def test_snps(self):
        ...
        # Learn model, save, load
        fastlmm3x = FastLMM(GB_goal=2).fit(
            K0_train=G0_train, X=covariate_train3, y=pheno_train3
        )
    ...
    def test_kernel(self):
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            assert np.array_equal(
                K0_train.iid, covariate_train3.iid
            ), "Expect iids to be the same (so that early and late Unit standardization will give the same result)"
            assert np.array_equal(
                K0_train.iid, pheno_train3.iid
            ), "Expect iids to be the same (so that early and late Unit standardization will give the same result)"
        ...
        # Learn model, save, load
        fastlmm3x = FastLMM(GB_goal=2).fit(
            K0_train=K0_train, X=covariate_train3, y=pheno_train3
        )
        filename = self.tempout_dir + "/model_snps.flm.p"
        pstutil.create_directory_if_necessary(filename)
        fastlmm3 = fastlmm3x
        ...
        # Predict with model (test on train)
        predicted_pheno, covar = fastlmm3.predict(
            K0_whole_test=K0_train, X=covariate_train3, count_A1=False
        )  # test on train
        ...
        self.compare_files(
            predicted_pheno, "snps"
        )  # "kernel" and "snps" test cases should give the same results
    ...
    def test_kernel_one(self):
        ...
        pheno_train = self.pheno_whole[train_idx, :]
        assert np.array_equal(
            K0_train.iid, covariate_train.iid
        ), "Expect iids to be the same (so that early and late Unit standardization will give the same result)"
        assert np.array_equal(
            K0_train.iid, pheno_train.iid
        ), "Expect iids to be the same (so that early and late Unit standardization will give the same result)"
    ...
    def compare_files(self, answer, ref_base):
        ...
        for iid_index in range(reference.row_count):
            for sid_index in range(reference.col_count):
                ...
                assert (
                    abs(a_v - r_v) < 1e-4 or abs(a_v - r_v) / abs(r_v) < 1e5
