...
class _SnpWholeTest(KernelReader):
    ...
    def row(self):
        if not hasattr(self, "_row"):
            assert np.array_equal(
                self.train.sid, self.test.sid
            ), "Expect train and test to have same sid in same order"
            train_set = set(tuple(item) for item in self.train.iid)
            test_unique = [
                item2
                for item2 in (tuple(item) for item in self.test.iid)
                if item2 not in train_set
            ]
            self._row = np.r_[
                self.train.iid, np.array(test_unique, dtype="str").reshape(-1, 2)
            ]
        return self._row
    ...
    def __getitem__(self, iid_indexer_and_snp_indexer):
        ...
        assert not isinstance(
            row_index_or_none, str
        ), "row_index_or_none should not be a string"
        ...
        if np.array_equal(test.iid, iid):
            return SnpKernel(
                test, standardizer=self.standardizer, block_size=self.block_size
            )

        # case 3: Just re-reordering the iids
        if len(row_index_or_none) == self.row_count and (
            col_index_or_none is None or len(col_index_or_none) == self.col_count
        ):
            result = _SnpWholeTest(
                train=self.train,
                test=test,
                standardizer=self.standardizer,
                block_size=self.block_size,
    ...
    def _read(
        ...
        result = self[row_index_or_none, col_index_or_none]._read(
            row_index_or_none,
            col_index_or_none,
            order,
            dtype,
    ...
    def __repr__(self):
        s = "_SnpWholeTest(train={0},test={1},standardizer={2}".format(
            self.train, self.test, self.standardizer
        )
...
class _SnpTrainTest(KernelReader):
    ...
    def _read(
        ...
        if (
            ...
            or np.array_equal(row_index_or_none, np.arange(self.row_count))
            and col_index_or_none is None
            or np.array_equal(col_index_or_none, np.arange(self.col_count))
        ):
            ...
            if self.block_size is None or (
                self.train.sid_count <= self.block_size
                or self.train.sid_count <= self.train.iid_count + self.test.iid_count
            ):
                train_snps = self.train.read(
                    dtype=dtype, num_threads=num_threads
                ).standardize(self.standardizer, num_threads=num_threads)
                test_snps = self.test.read(
                    dtype=dtype, num_threads=num_threads
                ).standardize(self.standardizer, num_threads=num_threads)
                ...
                if order == "A":
                    order = "C"
                k_val = np.zeros(
                    [self.train.iid_count, self.test.iid_count],
                    dtype=dtype,
                    order=order,
                )
                ct = 0
                ...
                for start in range(0, self.train.sid_count, self.block_size):
                    ct += self.block_size
                    train_snps = (
                        self.train[:, start : start + self.block_size]
                        .read(dtype=dtype, num_threads=num_threads)
                        .standardize(self.standardizer, num_threads=num_threads)
                    )
                    test_snps = (
                        self.test[:, start : start + self.block_size]
                        .read(dtype=dtype, num_threads=num_threads)
                        .standardize(self.standardizer, num_threads=num_threads)
                    )
                    if order == "F":  # numpy's 'dot' always returns 'C' order
            ...
            raise Exception(
                "_SnpTrainTest currently only has code for reading all of train x test"
            )

    def __repr__(self):
        s = "_SnpTrainTest(train={0},test={1},standardizer={2}".format(
            self.train, self.test, self.standardizer
        )
        if self.block_size is not None:
            s += ",block_size={0}".format(self.block_size)
...
**Constructor:**
    ...
    def fit(
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            K0_train = _kernel_fixup(
                K0_train,
                iid_if_none=y.iid,
                standardizer=self.snp_standardizer,
                count_A1=count_A1,
            )
            K1_train = _kernel_fixup(
                K1_train,
                iid_if_none=y.iid,
                standardizer=self.snp_standardizer,
                count_A1=count_A1,
            ...
            X, covar_unit_trained = X.standardize(
                self.covariate_standardizer, block_size=block_size, return_trained=True
            )  # This also fills missing with the mean
            ...
            y0 = (
                y.read().val
            )  #!!!later would view_ok=True,order='A' be ok because this code already did a fresh read to look for any missing values

            from fastlmm.association.single_snp import (
                _Mixer,
            )  #!!!move _combine_the_best_way to another file (e.g. this one)
            ...
            self.G0_train = (
                K0_train.snpreader if isinstance(K0_train, SnpKernel) else None
            )  #!!!later expensive?
            self.G1_train = (
                K1_train.snpreader if isinstance(K1_train, SnpKernel) else None
            )  #!!!later expensive?
            return self
    ...
    def score(
        ...
        if not return_per_iid:
            ...
            if not return_mse_too:
                result = SnpData(
                    iid=y.iid, sid=["nLL"], val=np.empty((y.iid_count, 1)), name="nLL"
                )
                for iid_index in range(y.iid_count):
                    var = multivariate_normal(
                        mean=mean[iid_index], cov=covar[iid_index, iid_index]
                    )
    ...
    def _extract_fixup(kernel):
        assert (
            kernel.iid0_count >= kernel.iid1_count
        ), "Expect iid0 to be at least as long as iid1"
    ...
    def predict(
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            if self.mixer.do_g:
                ...
                vare = (1.0 - self.h2raw) * self.sigma2
                Ainv = LA.inv(
                    (1.0 / vare) * np.dot(self.G.T, self.G)
                    + (1.0 / varg) * np.eye(self.G.shape[1])
                )
                testAinv = np.dot(Gstar.test.val, Ainv)
                pheno_predicted = np.dot(X.val, self.beta) + (1.0 / vare) * np.dot(
                    np.dot(testAinv, self.G.T), self.y - np.dot(self.X, self.beta)
                )
                pheno_predicted = pheno_predicted.reshape(-1, 1)
                covar = np.dot(testAinv, Gstar.test.val.T) + vare * np.eye(
                    Gstar.test.val.shape[0]
                )
