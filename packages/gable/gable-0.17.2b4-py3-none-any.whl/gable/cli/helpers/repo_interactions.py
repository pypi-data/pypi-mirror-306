...
def get_git_repo_db(path: Union[os.PathLike, str], use_git_object_db=False) -> Repo:
    """Returns a gitpython Repo object for the given path. If the given path is not
    in a git repo, or the repo doesn't have an origin remote, an exception is
    raised."""
    try:
        if use_git_object_db:
            repo = Repo(path, search_parent_directories=True, odbt=GitDB)  # type: ignore
        else:
            repo = Repo(path, search_parent_directories=True)
    except InvalidGitRepositoryError:
        raise click.ClickException(
...
def clone_to_temp_dir(repo: Repo, branch: str) -> tuple[Repo, str]:
    """Clones the given repo to a temporary directory, and checks out the given branch.
    Returns the new Repo object."""
    temp_dir = tempfile.TemporaryDirectory().name
    temp_repo = repo.clone(temp_dir)
    temp_repo.git.checkout(branch)
    return temp_repo, temp_dir


def get_origin_head_default_branch(repo: Repo) -> str:
    """Returns the default branch of the git repository."""
    return repo.git.rev_parse("origin/HEAD", abbrev_ref=True).split("/")[-1]
