...
async def entrada_de_notas_207(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    ...
    try:
        ...
        try:
            app = Application().connect(class_name="TFrmNotaFiscalEntrada")
            main_window = app.window(class_name="TFrmNotaFiscalEntrada")
            main_window.set_focus()
            
            console.print("Tentando clicar no Botão OK...\n")
            ...
            try:
                ...
                if btn_ok.exists():
                    ...
                    console.print("Botão OK não encontrado, verificando se a nota foi incluída...")
                    retorno = await verify_nf_incuded()

            except Exception as e:
                console.print(f"Erro ao tentar interagir com o botão OK: {e}\n")
                retorno = await verify_nf_incuded()
