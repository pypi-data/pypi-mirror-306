...
async def worker_sleep(multiplier: int):
    ...
    console.log(
        f"Aguardando {(timeout_multiplicador * multiplier)} segundos...",
        style="bold yellow",
    )
...
async def itens_not_found_supplier(xml: str):
    ...
    try:
        try:
            ...
            console.print(f"Não foi possivel conectar a tela de aguarde + Mensagem..\n")
...
async def rateio_despesa(unidade_code: str)  -> RpaRetornoProcessoDTO:
    ...
    try:
        ...
        edit = panel_centro.child_window(class_name="TDBIEditCode")

        value_centro = int(unidade_code)+1000

        if edit.exists():
            edit.set_edit_text(value_centro)
            edit.type_keys("{TAB}")
        else:
            console.print(f"Campo tipo de despesas - Não foi encontrado...\n")
            return RpaRetornoProcessoDTO(
        ...
        return RpaRetornoProcessoDTO(
                sucesso=False,
                retorno="Erro ao processar tela de Informações para importação da Nota Fiscal Eletrônica para inserir o tipo de despesa, erro: {e}",
                status=RpaHistoricoStatusEnum.Falha,
            )
