...
class ManagedTrade:
	...
	"""
	def __init__(self, trade: Trade, template: Template, entryOrder: Order):
		self.trade = trade
...
class TradeManager():
	...
	def _onOrderStatusChanged(self, order: Order, status: OrderStatus):
		"""
		Handles the 
		"""
		...
		for managedTrade in self._trades:
			if managedTrade.entryOrder == order:
				...
				if status == OrderStatus.FILLED:
					...
					logger.debug(f'Entry order for trade {managedTrade.trade.id} was filled. Trade is opened now')
					...
					self._backgroundScheduler.add_job(self._createTakeProfitAndStop, id='CreateTakeProfitAndStop' + str(managedTrade.trade.id), args=[managedTrade])
	...
	async def _trackEntryOrder(self, managedTrade: ManagedTrade):
		...
		adjustedPrice = managedTrade.entryOrder.price + managedTrade.template.adjustmentStep
		logger.info('Adjusting entry order. Current Limit Price: {} Adjusted Limit Price: {}', managedTrade.entryOrder.price, adjustedPrice)
	...
	async def _createTakeProfitAndStop(self, managedTrade: ManagedTrade):
		...
		#logger.debug(f'Using Take Profit Percentage: {managedTrade.template.takeProfit} and Stop Loss Percentage: {managedTrade.template.stopLoss}')
		#takeProfitPrice = fillPrice * (managedTrade.template.takeProfit / 100)
		#limitPrice = OptionHelper.roundToTickSize(fillPrice - takeProfitPrice)
		# # Create Take Profit Order
		# takeProfitOrder = managedTrade.template.createTakeProfitOrder(managedTrade.entryOrder)
		# # Create Stop Loss Order
		# stopLossOrder = managedTrade.template.createStopLossOrder(managedTrade.entryOrder)
		# # Place Take Profit Order
		# self._backgroundScheduler.add_job(self._placeTakeProfitOrder, id='PlaceTakeProfitOrder' + str(managedTrade.trade.id), args=[takeProfitOrder, managedTrade])
		# # Place Stop Loss Order
		# self._backgroundScheduler.add_job(self._placeStopLossOrder, id='PlaceStopLossOrder' + str(managedTrade.trade.id), args=[stopLossOrder, managedTrade])