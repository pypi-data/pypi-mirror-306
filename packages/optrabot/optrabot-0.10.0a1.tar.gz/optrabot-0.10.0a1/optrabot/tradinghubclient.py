...
class TradinghubClient():
	...
	async def _poll(self):
		try:
			...
			if response.text != '\"\"' and response.text != '':
				...
				signalStrategy = response_data['strategy']
				logger.debug('Received Signal: {}', response_data['strategy'])
				templateProcessor = TemplateProcessor()
				triggeredTemplate = templateProcessor.determineTemplate(signalStrategy)
				...
				signalData = SignalData(timestamp=self._parseTimestamp(response_data['time']), close=response_data['close'], strike=0 )
				...
				if signalStrategy == '0DTEIronFly' or signalStrategy == '0DTELunchIF':
					signalData.strike = response_data['vwaptarget']
				else:
					signalData.strike = response_data['shortstrike']

				#templateProcessor.processTemplate(triggeredTemplate, signalData)
				...
				self.optraBot._backgroundScheduler.add_job(templateProcessor.processTemplate, args=[triggeredTemplate, signalData], id='processtemplate')
				return
	...
	# 	try:
	# 		fetch_url = self.hub_host + '/fetch_signal'
	# 		url_params = {'agentid': self._instanceId}
	# 		headers = {'X-API-Key': self._apiKey, 'X-Version': self.optraBot.Version}
	# 		if self._shuttingdown:
	# 			logger.debug("Client Session closed. Stop polling.")
	# 			return
