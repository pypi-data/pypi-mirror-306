...
from optrabot.broker.order import Order as BrokerOrder, OrderStatus
...
class BrokerConnector(ABC):
	...
	@abstractmethod
	async def placeOrder(self, order: BrokerOrder, template: Template):
		""" 
	...
	def _emitOrderStatusEvent(self, order: BrokerOrder, status: OrderStatus):
		""" 
		Emits the order status event
		"""
		self.orderStatusEvent.emit(order, status)
