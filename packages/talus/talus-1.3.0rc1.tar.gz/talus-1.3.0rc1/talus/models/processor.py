...
DEFAULT_DEAD_LETTER_EXCEPTIONS: tuple = (ValidationError,)
...
class RoutingKeyNotFoundMessageProcessor(MessageProcessorBase):
    dead_letter_exceptions: tuple[Type[Exception]] = format_dead_letter_exceptions(LookupError)

    def process_message(self, message: ConsumeMessageBase):
        """
        Process a message with an unknown routing key by logging and sending to the DLQ.
        """
        logger.error(f"Routing key not found: {message!r}")
        self.raise_dead_letter_exception(message)

    def raise_dead_letter_exception(self, message: ConsumeMessageBase) -> None:
        """
        Raise an exception that will be dead lettered.
        """
        raise LookupError(f"Routing key {message.routing_key} not found")
