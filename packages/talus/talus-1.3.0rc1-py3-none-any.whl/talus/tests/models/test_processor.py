...
from talus.models.processor import RoutingKeyNotFoundMessageProcessor
...
@pytest.fixture()
def routing_key_not_found_message_processor(
    consume_message_cls,
) -> RoutingKeyNotFoundMessageProcessor:
    """
    A message processor that raises a LookupError.
    """

    class RoutingKeyNotFoundProcessor(RoutingKeyNotFoundMessageProcessor):
        message_cls: Type[ConsumeMessageBase] = consume_message_cls
    ...
    yield RoutingKeyNotFoundProcessor()


def test_routing_key_not_found_processor(
    routing_key_not_found_message_processor, consumed_message, consumer
):
    """
    :given: A message processor that raises a LookupError.
    :when: The message processor is called.
    :then: The message processor should DLQ the message.
    """
    # given
    method, properties, body = consumed_message
    # when
    routing_key_not_found_message_processor(
        channel=consumer.channel, method=method, properties=properties, body=body
    )
    # then
    assert routing_key_not_found_message_processor._process_message_called
    assert not consumer.channel.get_waiting_message_count()  # no messages left in the queue
