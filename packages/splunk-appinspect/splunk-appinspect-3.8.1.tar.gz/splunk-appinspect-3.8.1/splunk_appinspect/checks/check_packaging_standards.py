...
class _VersionValidation:
    # this is necessary to trigger warnings in check_version_is_valid_semver
    fail_action: CheckMessage
    # (section, option, action_if_missing, action_if_invalid)
    settings_to_check: Iterable[Tuple[str, str, CheckMessage, CheckMessage]]
    ...
    def check_config(self, app: "App", config: "ConfigurationProxy") -> Generator[CheckMessage, Any, None]:
        ...
        for (
            ...
            if not section.has_option(option_name):
                ...
                yield action_if_missing(
                    f"A `{option_name}` attribute formatted as {self._valid_version_structure} is required in "
            ...
            if not self._is_valid_version_number(option.value):
                yield action_if_invalid(
                    ...
                    f"must be specified as {self._valid_version_structure}.",
        ...
        try:
            ...
            if not self._is_valid_version_number(manifest_version):
                yield self.failure_action(
                    ...
                    f"{self._valid_version_structure}.",
...
class CheckValidVersionNumber(Check):
    check = _VersionValidation.check
    check_config = _VersionValidation.check_config
    check_app_manifest = _VersionValidation.check_app_manifest
    _valid_version_structure = "`Major.Minor.Revision`"
    failure_action = FailMessage
    settings_to_check = (
        # (section, option, action_if_missing, action_if_invalid)
    ...
    def __init__(self):
        super().__init__(
            config=CheckConfig(
                name="check_valid_version_number",
    ...
    def _is_valid_version_number(version_number: str) -> bool:
        app_version_matcher = AppVersionNumberMatcher()
        ...
        return bool(app_version_matcher.match(version_number))
...
class CheckVersionIsValidSemver(Check):
    ...
    check_config = _VersionValidation.check_config
    check_app_manifest = _VersionValidation.check_app_manifest
    _valid_version_structure = "valid semver 2.0.0 string"
    failure_action = WarningMessage
    settings_to_check = (
        # (section, option, action_if_missing, action_if_invalid)
        ("id", "version", WarningMessage, WarningMessage),
        ("launcher", "version", WarningMessage, WarningMessage),
    )
