...
def check_basic_readme(app: "App", reporter: "Reporter") -> None:
    ...
    if not readmes:
        ...
        for readme in readmes:
            full_file_path = Path(app.app_dir, readme)
            try:
                with codecs.open(str(full_file_path), encoding="utf-8") as file:
                    contents = file.read()
                    if contents.strip():
                        break
                ...
                if full_file_path.stat().st_size > 0:
                    break
        else:
            reporter_output = f"README file(s) found but appear to be empty. File: {readme}"
...
def check_documentation_language(app: "App", reporter: "Reporter") -> None:
    ...
    for readme in readmes:
        try:
            ...
            with codecs.open(str(full_file_path), encoding="utf-8") as file:
                ...
                if lang != "en":
                    reporter_output = (
                        f'Language for README appears to be in "{lang}",'
            ...
            reporter_output = f"Could not detect language of README file. File: {readme}"
            reporter.manual_check(reporter_output, readme)


@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.MANUAL)
@splunk_appinspect.display(report_display_order=9)
def check_editing_and_proofreading(reporter: "Reporter") -> None:
...
def check_documented_included_open_source(reporter: "Reporter") -> None:
...
def check_search_acceleration(app: "App", reporter: "Reporter") -> None:
    """Check that use of
    ...
    if app.file_exists("default", "savedsearches.conf"):
        reporter.manual_check("App has a savedsearches.conf. Please verify that this file is documented.")
    else:
...
def check_custom_commands(app: "App", reporter: "Reporter") -> None:
...
@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.MANUAL)
def check_archived_files(app: "App", reporter: "Reporter") -> None:
    ...
    if archived_files:
        for _, file, _ in archived_files:
            reporter_output = f"Found archived {file}. Please make sure any archived files in the app are documented."
...
@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.MANUAL)
def check_outputs_documented(app: "App", reporter: "Reporter") -> None:
