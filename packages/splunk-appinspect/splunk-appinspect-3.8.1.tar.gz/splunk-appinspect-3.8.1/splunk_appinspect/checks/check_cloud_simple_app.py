    ...
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                ...
                "do not contain any subdirectories and only contain .meta files. "
        ...
        for directory, filename in iter(config_file_paths.items()):
            ...
            for external_command_stanza in external_command_stanzas:
                ...
                if script_filename_matches:
                    ...
                    if target_splunk_version >= Tags.SPLUNK_8_0:
                        if not external_command_stanza.has_option(
                            "python.version"
                        ) or external_command_stanza.get_option("python.version").value.lower() not in [
                            PYTHON_3_VERSION,
                            ...
                            external_command_stanza.get_option("python.version").value.lower() == PYTHON_LATEST_VERSION
                        ):
                            reporter_output = (
                                f" The `transforms.conf` stanza [{external_command_stanza.name}]"
...
def check_for_binary_files_without_source_code(app: "App", reporter: "Reporter") -> None:
    ...
    should be a source code provided with the same name. Or, there should be a declaration of what the
    binary file is all about in the app's README. Details for passing this check will be returned if you fail
    it."""
                ...
                if re.match(file_output_regex, file_output):
                    ...
                    if binary_name in source_name_pool:
                        reporter_output = (
                            ...
                            f" Binary file: {current_file_relative_path}"
                        ...
                        if not readme_find:
                            reporter_output = (
                                f"Binary file (Format: {file_output}) fail to"
                                ...
                                " (You might need create one) in your App's README."
...
@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.CLOUD, Tags.MANUAL, Tags.JAVA)
...
def check_for_shell(app: "App", reporter: "Reporter") -> None:
    ...
    for directory, file, _ in app.iterate_files(types=shell_types):
        file_path = Path(directory, file)
        file_full_path = app.get_filename(file_path)
        reporter_output = "A shell script was found and need to be verified."
        reporter.manual_check(reporter_output, file_name=file_path)
