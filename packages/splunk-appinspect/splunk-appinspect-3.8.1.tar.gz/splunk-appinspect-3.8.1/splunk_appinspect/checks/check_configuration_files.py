...
def check_no_default_stanzas(app: "App", reporter: "Reporter") -> None:
    ...
    def is_new_splunk_defined_conf(file_name: str) -> bool:
        return file_name in NEW_SPLUNK_DEFINED_CONFS
    ...
    for conf, section_name in conf_stanza_pairs:
        ...
        if is_new_splunk_defined_conf(conf.relative_path.name):
            reporter.warn(reporter_output, conf.relative_path, lineno)
        else:
            reporter.fail(reporter_output, conf.relative_path, lineno)
