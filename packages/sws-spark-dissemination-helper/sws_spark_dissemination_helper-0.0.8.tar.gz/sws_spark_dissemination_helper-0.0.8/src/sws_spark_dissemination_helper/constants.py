...
class IcebergTables:
    ...
    class __IcebergTable:
        def __init__(self, iceberg_id: str, path: str, csv_path: str):
            self.iceberg_id = iceberg_id
    ...
    def __init__(self, dataset_id: str, tag_name: str) -> None:
        self.__dataset_id = dataset_id
        self.__tag_name = tag_name

        self.BRONZE = self._create_iceberg_table("BRONZE")
        self.SILVER = self._create_iceberg_table("SILVER")

        # GOLD tables with specific suffixes
        self.GOLD_SDMX = self._create_iceberg_table("GOLD", "_sdmx_disseminated")
        self.GOLD_SWS_VALIDATED = self._create_iceberg_table("GOLD", "_sws_validated")
        self.GOLD_SWS_DISSEMINATED = self._create_iceberg_table(
            "GOLD", "_sws_disseminated"
        )

    def _create_iceberg_table(self, level: str, suffix: str = "") -> __IcebergTable:
        database = getattr(IcebergDatabases, f"{level}_DATABASE")
        scheme = getattr(IcebergDatabases, f"{level}_SCHEME")

        iceberg_id = f"{database}.{self.__dataset_id}{suffix}"
        path = f"{scheme}/{self.__dataset_id}{suffix}"
        csv_path = f"{CACHED_CSV_FOLDER}/{scheme}/{self.__dataset_id}{suffix}/{self.__tag_name}.csv"

        return self.__IcebergTable(iceberg_id, path, csv_path)
