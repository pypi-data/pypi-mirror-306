...
class RucioDB:
    ...
    def _merge_initial_sow(self):
        dsids = self.rucio_db["dsid"].unique()
        dsids = [dsid for dsid in dsids if not pd.isnull(dsid)]

        campaigns = self.rucio_db["campaign"].unique()
        campaigns = [c for c in campaigns if not pd.isnull(c)]

        for dsid in dsids:
            for campaign in campaigns:
                sel = (self.rucio_db["dsid"] == dsid) & (self.rucio_db["campaign"] == campaign)

                sum_initial_sow = self.rucio_db[sel]["initial_sow"].sum()
                sum_initial_sow_sq = self.rucio_db[sel]["initial_sow_sq"].sum()

                self.rucio_db.loc[sel, "initial_sow"] = sum_initial_sow
                self.rucio_db.loc[sel, "initial_sow_sq"] = sum_initial_sow_sq
