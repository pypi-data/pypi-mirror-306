...
def stop_worker(worker: subprocess.Popen, port_id: int, protocol: str, ipc_dir: str) -> int:
    """
    Stop a worker.
...
class PP_Worker:
    ...
    def __init__(self, parent_pid: int, port_id: int, protocol: str = "tcp", ipc_dir: str = "tmp"):
        """
        port_id: int: port to bind the 0mq socket
        protocol: str: data transfer protocol to use
    ...
    def _forward(self, data: dict):
        ...
        if pp_id not in self.cache.keys():
            ...
            return PP_Worker.err_reply(
                "PP_Worker: forward: Postprocessor is missing.",
        ...
        while action != PP_Worker.poison_pill:
            ...
            try:
                ...
                PP_Worker.logger.info(
                    "PP_Worker.run: json.JSONDecodeError: error in json.loads"
                )
                reply = PP_Worker.err_reply("PP_Worker.run: json.loads: bad json")
            except KeyError as e:
                PP_Worker.logger.info("PP_Worker.run: KeyError: most probably bad json")
                # most probably no 'action' in json
                reply = PP_Worker.err_reply(
                    f"PP_Worker.run: PP_Worker.run: KeyError: {e}"
                )
            except Exception as e:
                PP_Worker.logger.info("PP_Worker.run: Unknown exception")
                # unknown error
                reply = PP_Worker.err_reply(f"PP_Worker.run: {e}")
            finally:
    ...
    def err_reply(cls, err_msg: str, err_code=err_code_other) -> bytes:
        return msgpack.packb({PP_Worker.ret_val_key: "Err",
...
def set_mem_limit(memory_limit: int = -1):
    """
    Set memory usage limit
    memory_limit: int: process address space limit, in Mb
    """
    # set max size of Process Address Space
    if "linux" in sys.platform and memory_limit > 0:
        rc.setrlimit(rc.RLIMIT_AS, (memory_limit, memory_limit))  # type: ignore[attr-defined]
...
def run_worker(
    ...
    try:
        worker = PP_Worker(parent_pid=parent_pid, port_id=port_id, protocol=protocol, ipc_dir=ipc_dir)
        if worker.binded_addr != "":
            # binding success, report and start running
            ret_msg_0: str = json.dumps({PP_Worker.ret_val_key: "OK", PP_Worker.data_key: 0})
            sys.stderr.write(ret_msg_0 + "\n")
            sys.stderr.flush()
            worker.run()
        else:
            # binding failed, report and exit process
            ret_msg_1: str = json.dumps({PP_Worker.ret_val_key: "OK", PP_Worker.data_key: 1})
            sys.stderr.write(ret_msg_1 + "\n")
            sys.stderr.flush()
    except Exception as e:
        PP_Worker.logger.error(f"run_worker: PP_Worker constructor throws: {e}")
        ret_msg_2: str = json.dumps({PP_Worker.ret_val_key: "OK", PP_Worker.data_key: 2})
...
def start_worker():
    ...
    parser.add_argument("-d", "--parent_pid",
                        help="parent process pid; 0 if there is none",
                        default="0")
    parser.add_argument("-p", "--port_id",
                        help="port to be used for ZeroMQ socket",
                        default="5000")
    parser.add_argument("-r", "--protocol",
                        help="protocol used by ZeroMQ socket",
                        default="tcp")
    parser.add_argument("-i", "--ipc_dir",
                        help="directory to be used by ZeroMQ socket for ipc communication",
                        default="tmp")
    parser.add_argument("-m", "--memory_limit_MB",
                        help="process address space limit, in Mb",
                        default=str(MEMORY_LIMIT))
    parser.add_argument("-t", "--time_limit_sec",
