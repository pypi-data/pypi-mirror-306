        ...
        try:
            ...
            if chain_obj.chainId == 1:
                ...
                logs = await loop.run_in_executor(None, lambda: self.w3_client.eth.get_logs({
                    "topics": [[
                        "0x8c5be1e5ebec7d5bd14f714f7d1dd480f5e2ad69d3e34d956a1c8f8d2b44c0da",
                ...
                # Get latest block
                latest_block = await loop.run_in_executor(None, 
                    lambda: self.w3_client.eth.get_block('latest'))
                
                # Get logs
                logs = await loop.run_in_executor(None, 
                    lambda: self.w3_client.eth.get_logs({
                        'fromBlock': "latest",
                        'toBlock': "latest",
                    }))
                return logs
