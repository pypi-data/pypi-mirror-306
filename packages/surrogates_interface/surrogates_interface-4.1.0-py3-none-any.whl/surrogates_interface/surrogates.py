...
class TensorFlowModel(SurrogateModel):
    ...
    def _predict_model_output(self, x):
        ...
        y = self.model.predict(x, batch_size=x.shape[0], verbose=0)
        return y
...
class PyTorchModel(SurrogateModel):
    import torch
    import torch._dynamo  # fmt: skip
    torch._dynamo.config.suppress_errors = True
    import torch.func as torch_func  # fmt: skip

    # from torch.func import jacfwd, jacrev, jvp, vjp, vmap  # fmt: skip

    def __init__(self, *args, dtype=torch.float16, max_batch=50_000, **kwargs):
    ...
    def __to_numpy(self, _x):
        try:
            ...
            cp = np
        lalib = (
            cp if self.device == self.torch.device("cuda") else np
        )  # pragma: no cover
    ...
    def __to_tensor(self, _x):
        if len(_x.shape) < 2:
            _x = _x.reshape(1, -1)
        return self.torch.as_tensor(_x, device=self.device, dtype=self.dtype)
    ...
    def _save_model(self, filepath):
        self.torch.save(self.model.state_dict(), filepath)
    ...
    def _predict_model_jvp(self, x, dx):
        if not hasattr(self, "batched_jvp_func"):
            ...
            def single_jvp(_x, _v):  # fmt: skip
                return self.torch_func.jvp(g, (_x,), (_v,))
            self.batched_jvp_func = self.torch_func.vmap(single_jvp)
    ...
    def _predict_model_vjp(self, x, dy):
        if not hasattr(self, "batched_vjp_func"):
            def g(_x):  # fmt: skip
                return self.model(_x)
            def single_vjp(_x, _v):  # fmt: skip
                out, f = self.torch_func.vjp(g, _x)
                return out, f(_v)
            self.batched_vjp_func = self.torch_func.vmap(single_vjp)
    ...
    def _predict_model_output_and_jacobian_fwd(self, x):
        if not hasattr(self, "y_jac_fwd_func"):
            ...
            self.y_jac_fwd_func = self.torch_func.vmap(
                self.torch_func.jacfwd(g, has_aux=True)
            )
    ...
    def _predict_model_output_and_jacobian_rev(self, x):
        if not hasattr(self, "y_jac_rev_func"):
            def g(_x):  # fmt: skip
                result = self.model(_x)
                return (result, result)
            self.y_jac_rev_func = self.torch_func.vmap(
                self.torch_func.jacrev(g, has_aux=True)
            )
    ...
    def _predict_model_jacobian_sbatch(self, x):  # pragma: no cover
        ...
        if not hasattr(self, "y_jac_func"):
            def g(_x):  # fmt: skip
                result = self.model(_x)
                return (result, result)
            self.y_jac_func = self.torch_func.vmap(
                self.torch_func.jacrev(g, has_aux=True)
            )
