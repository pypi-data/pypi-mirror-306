...
class DomoAccount_Credential(dmacb.DomoAccount_Default):
    ...
    async def test_auths(self, backup_auth: dmda.DomoAuth = None):
        try:
            await self.test_token_auth()
        
        except dmde.DomoError as e:
            print(f"{self.name}: error generating token_auth: {e}")

        try:
            await self.test_full_auth()

        except dmde.DomoError as e:
            print(f"{self.name}: error generating full_auth: {e}")
...
async def set_target_user_password(self: DomoAccount_Credential,
    ...
    await target_user.reset_password(new_password, debug_api = debug_api)
    
    self.set_password(new_password)

    await self.update_config(debug_api = debug_api)
    
    return await self.test_full_auth()
...
async def reset_access_token(self, token_name = None, duration_in_days = 90, debug_api : bool = False):
    ...
    domo_access_token = next((dat for dat in (await target_user.get_access_tokens()) if dat.name == token_name), None)
    ...
    return await self.test_token_auth()
