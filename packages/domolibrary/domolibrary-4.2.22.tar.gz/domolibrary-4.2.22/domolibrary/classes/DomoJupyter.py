...
class DomoJupyterDataSource:
    dj_workspace: DomoJupyterWorkspace = field(repr=False)
    ...
    async def share_with_workspace_as_input_datasource(
        self,
        dj_workspace: DomoJupyterWorkspace = None,
...
class DJW_PermissionToAccountDenied(dmde.DomoError):
    def __init__( message, account_name):
        super().__init__(message, entity_id = account_name)
...
def read_domo_jupyter_account(
    ...
    try:
        ...
        if str(e).starswith('Permissions denied for workspace'):
            raise DJW_PermissionToAccountDenied(message = f"share account with user - {e}", account_name = account_name) from e

        if str(e).startswith('Failed to obtain workspace account properties for workspace'):        
            raise DJW_AccountInvalid_NotAddedToWorkspace(message = f"add account to workspace - {e}", account_name = account_name) from e
...
class DomoJupyterAccount:
    ...
    async def get_account(self, is_use_default_account_class: bool = True):
        ...
        except dmacc.GetAccount_NoMatch as e:
            self.is_exists = False
    ...
    def read_creds(
        ...
        if not (
            self.domo_account
            and isinstance(self.domo_account, dmacc.DomoAccount_Credential)
        ):
            return self.creds
...
async def add_input_dataset(
    ...
    djds = DomoJupyterDataSource(alias=domo_dataset.name, dataset_id=domo_dataset.id,dj_workspace = self)
...
async def get_accounts(
    ...
    async def _get_accounts(account_id, auth, props, session, debug_api):
        domo_account = await dmacc.DomoAccount.get_by_id(
