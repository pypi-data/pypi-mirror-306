...
def get_fc_hosts():  # noqa: ANN201
    ...
    if retcode != 0:
        return None
    # remove 'host' prefix
    return re.sub('host', '', output).split()
...
def t_wwpn_of_host(host=None):  # noqa: ANN001, ANN201
    ...
    for h in hosts:
        r_ports = get_fc_host_remote_ports(h)
        if r_ports:
            for r_port in r_ports:
...
def fc_target_id_of_htwwpn(t_wwpn=None, h_wwpn=None):  # noqa: ANN001, ANN201
    ...
    if not target_ids:
        logging.error(f'fc_target_id_of_htwwpn(): No FC target assigned to target WWPN {t_wwpn}')
        return None
...
def scsi_disk_of_htwwpn_wwid(h_wwpn, t_wwpn, wwid):  # noqa: ANN001, ANN201
    ...
    for scsi_disk_id in scsi_disk_ids:
        if re.match(r'%s:\d+$' % target_id, scsi_disk_id):  # noqa: UP031
...
def get_fc_host_rport_targets(host, r_port):  # noqa: ANN001, ANN201
    ...
    if retcode != 0:
        return None
    return re.sub('target', '', output).split()
