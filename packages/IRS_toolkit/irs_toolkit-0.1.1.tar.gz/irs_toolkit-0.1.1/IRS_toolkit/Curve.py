...
class Curve:
    ...
    def __init__(self, curve=None, date_curve=None, snowflake_instance=None):
        ...
        if curve is None:
            self.get_yield_curve_from_snowflake()
        else:
            self.curve = curve
    ...
    def get_yield_curve_from_snowflake(self):
        curve_data = self.snowflake_instance.query(
            "SELECT * FROM SANDBOX.QUANT_RISK.REFINITIV_ESTR_YIELD_CURVES"
        )
        curve_data = curve_data[curve_data["date"] == str(self.date.date())]

        if curve_data.empty:
            print("No data in snowflake for the date " + str(self.date.date()) + ".")

        curve_data = curve_data.T
        curve_data = curve_data.reset_index()

        curve_data.columns = ["Instrument", "Mid_geo"]
        curve_data = curve_data.iloc[1:]
        curve_data["Mid_geo"] = curve_data["Mid_geo"] / 100
        curve_data_ = curve_data[["Instrument", "Mid_geo"]]
        curve_data_.columns = ["Tenor", "Rate"]
        self.curve = curve_data_
    ...
    def monthly_avg_daily(
        self, start_date, end_date, frequency="D", relative_delta=relativedelta(days=0)
    ):
        """
