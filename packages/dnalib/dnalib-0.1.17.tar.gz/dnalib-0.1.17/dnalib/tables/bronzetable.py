...
class FireBaseLoader(Loader):
    ...
    def iterate_documents(self):
        ...
        while True:
            ...
            for doc in docs:
                ...
                yield doc                
            print(f"Firebase loaded {count} documents.", end="\r")            
    ...
    def load_incremental(self):
        ...
        for doc in self.iterate_documents():
            self.docs.append(doc.to_dict())
        # converts all documents to json string, make primitives string to avoid break schema
        self.df_raw = Utils.spark_instance().read.json(Utils.spark_instance().sparkContext.parallelize(self.docs).map(lambda x: json.dumps(x)), primitivesAsString=True)      
        # convert structs to string, it garantee that all fields are string, and avoid break schema 
...
class LoaderFactory:
    ...
    def create_loader(source_type, 
        ...
        if source_type.value == SourceType.SQLSERVER.value:
            ...
            return FireBaseLoader(source_path, loader_type, collection_name, order_by, page_size, init_load, end_load)
        else:
