...
import CloudVolume.BeamImager as BeamImager
import pandas as pd
import numpy as np
from tkinter import * 
...
class DataVisualizationApp:
    def __init__(self, master):
        ...
        self.density_label = tk.Label(self.density_frame, text="Number of Clusters:", width=22, bg = "#dec231")
        ...
        self.density_gmm_label = tk.Label(self.density_gmm_frame, text="Number of Clusters:", width=22, bg = "#dec231")
        ...
        self.edit_frame = tk.Frame(self.main_frame, bg = "#dec231")
        ...
        # Button to add cluster into result to save it as seperate point clouds awith .csv file with volume informations
        self.saving_clusters = []

        self.save_current_cluster = tk.Button(self.total_volume_frame, text="Adding choosen cluster", command=self.saving_clusters_point_cloud, width = 22, height= 3, bg = "#5cb85c")
        self.save_current_cluster.grid(row=0, column=0, padx=10, pady=5)
 
        # Button to remove the last line from saved clusters and csv file
        self.remove_last_line_button = tk.Button(self.total_volume_frame,text="Remove the last cluster", command=self.remove_last_cluster, width=22, height=3, bg="#5cb85c")
        self.remove_last_line_button.grid(row=1, column=0, padx=10, pady=5)
            
        # Button to save clusters to CSV
        self.save_to_csv_button = tk.Button(self.total_volume_frame,text="Save clusters and volumes", command=self.save_clusters_to_csv, width=22, height=3, bg="#5cb85c")
        self.save_to_csv_button.grid(row=2, column=0, padx=10, pady=5) 
        
        # Create a frame for the buttons and checkboxes
        self.button_frame = tk.Frame(self.main_frame)
        self.button_frame.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)

        #Initialize an empty DataFrame with four columns
        self.df_to_csv = pd.DataFrame(columns=['Cluster', 'id', 'volume', 'customized_volume'])
        
        self.inactive_clusters = [] # List for storing id of clusters which are saved in the list for csv file of result
        self.cluster_buttons = {}
        # Store cluster buttons to control their state
        self.cluster_buttons = {}
        ...
        # Initialize original list of cluster IDs
        self.original_cluster_ids = list(range(1, len(self.colors_clusters) + 1))  # IDs from 1 to 40
        ...
        for cluster_id, color in enumerate(self.colors_clusters, start=1):
            ...
            self.cluster_buttons[cluster_id] = button
        ...
        self.interpolation_frame.grid(row=12, column=0, pady=10)
        ...
        self.number_interpolated_point_entry = tk.Entry(self.interpolation_frame, width=15)
        ...
        self.interpolation_button.grid(row=3, column=0, sticky=tk.W)

        ########################
        # Field 9: Voxelization
        ########################
        ...
        self.voxel_size_entry = tk.Entry(self.interpolation_frame, width=15)
        ...
        self.Lower_Threshold_entry = tk.Entry(self.interpolation_frame, width=15)
        ...
        self.Upper_Threshold_entry = tk.Entry(self.interpolation_frame, width=15)
        ...
        self.Volume_button.grid(row=14, column=0, sticky=tk.W) 

        ####################
        # Save point cloud
        ####################
        self.save_frame = tk.Frame(self.interpolation_frame)
        self.save_frame.grid(row=15, column=0, padx=10, columnspan=2, pady = 37)

        self.number_points_current_pcd_label = tk.Label(self.save_frame, text=" Size Current Point Cloud", width=22)
        self.number_points_current_pcd_label.grid(row=16, column=0, sticky=tk.W)

        self.number_points_current_pcd = tk.Entry(self.save_frame,   width = 15)
        self.number_points_current_pcd.grid(row=16, column=1)

        self.number_points_previous_pcd_label = tk.Label(self.save_frame, text=" Size Previous Point Cloud", width=22)
        self.number_points_previous_pcd_label.grid(row=17, column=0, sticky=tk.W)

        self.number_points_previous_pcd = tk.Entry(self.save_frame,   width = 15)
        self.number_points_previous_pcd.grid(row=17, column=1)

        self.save_point_cloud = tk.Button(self.save_frame, text="Saving the Current Point Cloud as pcd", command=self.saving_point_cloud, width = 30, height= 3, padx=40)
        self.save_point_cloud.grid(row=18, column=0, columnspan=2)

        #Save data with ping and beam to be able load them later 
        self.save_data_frame = tk.Button(self.save_frame, text= "Saving the Current Point Cloud as csv", command= self.saving_df, width= 30, height= 3, padx=40)
        self.save_data_frame.grid (row= 19, column=0, columnspan = 2)

        # Button to Access Previous Point Cloud
        #self.count = 0
        self.access_previous_button = tk.Button(self.save_frame, text="Access the Previous Point Cloud", command=self.access_previous_point_cloud, width = 30, height= 3, padx=40)
        self.access_previous_button.grid(row=20, column=0, columnspan=2)

        #######################################################
        # Text box and saving and calculating volume for result
        #######################################################

        # Text Box for Information
        self.text_frame = tk.Frame(master)
        self.text_frame.grid(row=0, column= 2,rowspan =40)
        self.text_box = tk.Text( self.text_frame,height = 30, width = 90)
        self.text_box.grid(row=0, column= 0,rowspan =19, columnspan= 3)
        ...
        # Button to add cluster into result to save it as seperate point clouds awith .csv file with volume informations
        self.saving_clusters = []  
        self.label_pointcloud = tk.Label(self.text_frame, text="Cluster", width = 34, height= 2)
        self.label_pointcloud.grid(row=20, column=0) 
        self.text_box_clusters = tk.Text( self.text_frame,width = 30,height = 30 )
        self.text_box_clusters.grid(row=21, column= 0,rowspan =20)

        self.saving_clusters = []
        self.saving_volume_voxelized = []
        self.saving_volume_voxelized_custom = [] 

        # Button to save clusters to CSV
        self.lebel_volume = tk.Label(self.text_frame, text="Volume",  width = 34, height= 2)
        self.lebel_volume.grid(row=20, column=1)
        
        self.text_box_clusters_volume_noWeighted = tk.Text( self.text_frame, width = 30,height = 30)
        self.text_box_clusters_volume_noWeighted.grid(row=21, column= 1,rowspan =20)

        self.label_volume_customized = tk.Label(self.text_frame, text="Customized Volume",  width = 34, height= 2)
        self.label_volume_customized.grid(row=20, column=2)  

        self.text_box_clusters_volume_Weighted = tk.Text( self.text_frame, width = 30,height = 30)
        self.text_box_clusters_volume_Weighted.grid(row=21, column= 2,rowspan =20)
    ...
    def set_default_sea_weed_values(self):
        ...
        self.lower_threshold_entry.insert(0, str(self.default_low_threshold))
        self.default_high_threshold = -30
        self.upper_threshold_entry.insert(0, str(self.default_high_threshold))

        # Default percentage to remove data from the bottom
        self.default_percent = 0.15
        self.remove_bottom_entry.insert(0, str(self.default_percent))

        # Default parameters for outlier removal
        self.default_nb_neighbors = 1000
        self.nb_neighbors_entry.insert(0, str(self.default_nb_neighbors))
        self.default_std_ratio = 2.0
        self.std_ratio_entry.insert(0, str(self.default_std_ratio))
        self.default_nb_points = 1000
        self.nb_points_entry.insert(0, str(self.default_nb_points))
        self.default_radius = 0.5
        self.radius_entry.insert(0, str(self.default_radius))

        # Default number of clusters
        self.default_num_clusters = 2
        self.density_clusters_entry.insert(0, str(self.default_num_clusters))
        self.density_gmm_components_entry.insert(0, str(self.default_num_clusters))
        self.default_minClusterSize = 10000
        self.minClusterSize_entry.insert(0, str(self.default_minClusterSize))
        self.default_minNumSamples = 1000
        self.minNumSamples_entry.insert(0, str(self.default_minNumSamples))
        ...
        self.Upper_Threshold_entry.insert(0, str(self.default_Upper_Threshold))
        self.default_Lower_Threshold = 1
        self.Lower_Threshold_entry.insert(0, str(self.default_Lower_Threshold))

        self.default_number_interpolation = 10
        self.number_interpolated_point_entry.insert(0, str(self.default_number_interpolation))

        self.voxel_size_default = 0.15
        self.voxel_size_entry.insert(0, str(self.voxel_size_default))
    ...
    def saving_point_cloud(self):
        ...
        file_path = filedialog.asksaveasfilename(defaultextension=".pcd", filetypes=[("PCD files", "*.pcd"), ("All files", "*.*")])
        
        if isinstance(self.point_cloud_history[-1], pd.DataFrame):
            if file_path:
                BeamImager.save_pointcloud_with_raw_intensity_ascii_pcd(self.point_cloud_history[-1][["x","y","z", "intensity"]], file_path)

        else:    
        
            if file_path:
                self.df_filtered = BeamImager.pointcloud_to_df (self.point_cloud_history[-1])
                self.merged_df = BeamImager.merging_ping (self.df, self.df_filtered)
                BeamImager.save_pointcloud_with_raw_intensity_ascii_pcd(self.merged_df[["x","y","z", "intensity"]], file_path)
    ...
    def saving_df(self):
        ...
        if isinstance(self.point_cloud_history[-1], pd.DataFrame):
            if file_path:
                self.point_cloud_history[-1].iloc[:, :-3].to_csv(file_path, index =False, header=False)
    ...
    def upload_data(self):
        ...
        if file_path:
            ...
            if num_columns==4:
                ## Getting geometry and intensity of data
                self.x_data, self.y_data, self.z_data, self.intensity_data = BeamImager.getting_values_noping(self.df)
    ...
    def remove_most_frequent(self):
        ...
        self.point_cloud_history.append(self.df) 
        self.counter_pointcloud()

        #self.point_cloud, self.text = BeamImager.df_to_pointcloud(self.df)
        ## Print number of points
        #self.text = f"Number of points: {len(self.point_cloud.points)}"
        #self.print_info(self.text)
    ...
    def counter_pointcloud (self):
        
        if len(self.point_cloud_history) > 0 :
            if isinstance(self.point_cloud_history[-1], pd.DataFrame):
                self.number_points_current_pcd.delete(0, tk.END)
                self.number_points_current_pcd.insert(0, str(len(self.point_cloud_history[-1]))) 
            else: 
                self.number_points_current_pcd.delete(0, tk.END)
                self.number_points_current_pcd.insert(0, str(len(self.point_cloud_history[-1].points)))   

        if len(self.point_cloud_history) > 1 :
            if isinstance(self.point_cloud_history[-2], pd.DataFrame):
                self.number_points_previous_pcd.delete(0, tk.END)
                self.number_points_previous_pcd.insert(0, str(len(self.point_cloud_history[-2]))) 
            else: 
                self.number_points_previous_pcd.delete(0, tk.END)
                self.number_points_previous_pcd.insert(0, str(len(self.point_cloud_history[-2].points)))    
    ...
    def perform_Kmean_density_clustering(self):
        ...
        self.cluster_clouds, self.global_bounds, self.initialized = BeamImager.visualize_clusters(self.clustered_point_clouds, self.colors_clusters)
    ...
    def merge_clusters (self):
        ...
        for i in self.custom_indices[:len(self.result_clusters)]:
            
            position = self.custom_indices.index(i)
            
            # Check if the index is not in the list of indices to merge
            if i  not in indices:
                
                # If not in the list, append the cluster to the new list
                merged_clusters.append(self.result_clusters[position])
        # Merge the clusters specified by the indices list
        merged_cluster = o3d.geometry.PointCloud()
        for index in indices:
            position = self.custom_indices.index(index)
            print(f"{index} in position{position}")
            merged_cluster += self.result_clusters[position]
    ...
    def remove_clusters (self):
        ...
        remaining_clusters = []
        # Get which clusters in the result should remove
        indices = self.checked_clusters
        
        # Iterate through each index in the indices list
        for index in self.custom_indices[:len(self.result_clusters)]:
            ...
            if index  not in indices:
                ...
                position = self.custom_indices.index(index)
                # If not in the list, append the cluster to the new list
                remaining_clusters.append(self.result_clusters[position])
    ...
    def update_clusters (self):
        global initialized, global_bounds
    
        # Ensure global bounds are initialized only once
        if not self.initialized:
            BeamImager.initialize_global_bounds(self.result_clusters)  # Assuming self.result_clusters has the point clouds
            initialized = True
        ...
        self.custom_indices = [item for item in self.original_cluster_ids if item not in self.inactive_clusters]

        # Ensure custom_indices is truncated to match sorted_point_clouds
        #if len(self.custom_indices) >= len(sorted_point_clouds):
        #    self.custom_indices = self.custom_indices[:len(sorted_point_clouds)]     

        # changing enumerate into using zip function with customized list of number, which is hte one are not saved and inactivated
        for i, pc in zip(self.custom_indices, sorted_point_clouds):
            points = np.asarray(pc.points)    
            ax.scatter(points[:, 0], points[:, 1], points[:, 2], label=f'Cluster {i }', c=self.colors_clusters[i-1])
        ...
        for i, size in zip(self.custom_indices, sizes):
            txt_cluster = f'Cluster {i} (Size: {size} points)'
            print(txt_cluster)
            
        # Apply global bounds to ensure consistency in plot dimensions
        ax.set_xlim(self.global_bounds["min_x"], self.global_bounds["max_x"])
        ax.set_ylim(self.global_bounds["min_y"], self.global_bounds["max_y"])
        ax.set_zlim(self.global_bounds["min_z"], self.global_bounds["max_z"])
    ...
    def choose_cluster(self, id):
        ...
        position = self.custom_indices.index(id)

        self.print_info(self.text)
    
        if self.result_clusters is not None and 0 <= position <= len(self.result_clusters):
            clusters_original = BeamImager.visualize_selected_cluster(self.result_clusters, position)
            point_cloud = clusters_original[position]  
        ...
        if point_cloud is not None:
            ...
            self.point_cloud_to_save = point_cloud
    ...
    def saving_clusters_point_cloud(self):
        # Iterate over all the checked clusters
        for cluster_id in self.checked_clusters:
            
            # Set the current cluster point cloud to save
            self.choose_cluster(cluster_id)  # Assumes a method to fetch the point cloud by cluster ID
            print(self.point_cloud_to_save)
            self.id = cluster_id  # Update the current cluster ID
            print(self.id)

            self.point_cloud = self.point_cloud_to_save

            # Interpolate data between planes
            self.interpolate_data_btw_planes()
            
            # Calculate default thresholds based on voxel counts
            self.default_Upper_Threshold = int(np.percentile(self.voxel_counts, 25))
            self.default_Lower_Threshold = int(np.percentile(self.voxel_counts, 10))
            
            # Calculate the volume based on voxelization
            self.calculate_volume_voxelization()
            
            # Create a list of cluster data to save
            list_cluster = [self.point_cloud_to_save, self.id, self.voxel_volume, self.voxel_volume_custome]
            
            # Append the cluster data to the saving list
            self.saving_clusters.append(list_cluster)
            
            # Convert the list to a DataFrame row and append it to self.df_to_csv
            new_row = pd.DataFrame([list_cluster], columns=self.df_to_csv.columns)
            self.df_to_csv = pd.concat([self.df_to_csv, new_row], ignore_index=True)
            
            # Add the current cluster to inactive clusters and disable related buttons
            self.inactive_clusters.append(self.id)
            self.disable_inactive_cluster_buttons()

            # After processing all clusters, update the text boxes with the data
            self.populate_text_boxes()
        
        # Call methods to remove saved clusters and update the arrangement of clusters
        self.remove_clusters()
        self.update_clusters()

    def populate_text_boxes(self):
        
        sublist = self.saving_clusters[-1]
        # Ensure sublist has exactly 3 elements
        if len(sublist) == 4:
            # Insert elements into corresponding text boxes
            self.text_box_clusters.insert(tk.END, f"Cluster {sublist[1]}: {len(sublist[0].points)} points" + "\n")  # 1st element
            self.text_box_clusters_volume_noWeighted.insert(tk.END, str(sublist[2]) + "\n")  # 2nd element
            self.text_box_clusters_volume_Weighted.insert(tk.END, str(sublist[3]) + "\n")  # 3rd element    

    def erase_last_line(self):
        
        # Helper function to remove the last line from a text box
        def remove_last_line(text_box):
            # Get the current content of the text box
            current_text = text_box.get("1.0", tk.END)
            
            # Find the position of the last line break before the last line
            last_newline_index = current_text.strip().rfind("\n")
            
            # If there is at least one newline character
            if last_newline_index != -1:
                # Remove the last line
                new_text = current_text[:last_newline_index + 1]
                
                # Clear the text box and insert the updated content
                text_box.delete("1.0", tk.END)
                text_box.insert(tk.END, new_text)
            else:
                # If no newline found (there's only one line), clear the box
                text_box.delete("1.0", tk.END)
        # Remove the last line from each of the text boxes
        remove_last_line(self.text_box_clusters)
        remove_last_line(self.text_box_clusters_volume_noWeighted)
        remove_last_line(self.text_box_clusters_volume_Weighted)            

    def disable_inactive_cluster_buttons(self): 
        for cluster_id in self.inactive_clusters:    
            if cluster_id in self.cluster_buttons:   
                self.cluster_buttons[cluster_id].config(state=tk.DISABLED)

    def activate_cluster_buttons(self, cluster_indices):
        for cluster_id in cluster_indices:    
            if cluster_id in self.cluster_buttons:   
                self.cluster_buttons[cluster_id].config(state=tk.NORMAL)            

    def convert_point_cloud_list_to_gray(self, point_cloud_list):
        gray_point_clouds = []

        # Iterate over each point cloud in the list
        for pcd in point_cloud_list:
            ...
            # Make the grayscale image an RGB-like array with all channels the same
            gray_rgb = np.stack([grayscale, grayscale, grayscale], axis=1)

            # Create a new point cloud object
            gray_pcd = o3d.geometry.PointCloud()

            # Assign the original XYZ points
            gray_pcd.points = o3d.utility.Vector3dVector(xyz)

            # Assign the grayscale values as the new colors
            gray_pcd.colors = o3d.utility.Vector3dVector(gray_rgb)

            # Append the new grayscale point cloud to the list
            gray_point_clouds.append(gray_pcd)

        return gray_point_clouds  
      
    def save_clusters_to_csv(self):

         # Ask the user to select a folder
        save_folder = filedialog.askdirectory(title="Select Folder to Save Point Clouds and CSV")
        
        # If no folder is selected, return
        if not save_folder:
            return
        
        if not self.df_to_csv.empty:
            # Iterate through each row in the DataFrame
            for index, row in self.df_to_csv.iterrows():
                pointcloud = row['Cluster']
                cluster_id = row['id']
                pointcloud_len = len(pointcloud.points)
                
                # Create the file name for each point cloud
                filename = f"Cluster {cluster_id} - {pointcloud_len} points.pcd"
                file_path = os.path.join(save_folder, filename)
                
                # Save the point cloud to a .pcd file
                #o3d.io.write_point_cloud(file_path, pointcloud)
                df_filtered = BeamImager.pointcloud_to_df (pointcloud)
                merged_df = BeamImager.merging_ping (self.df, df_filtered)
                BeamImager.save_pointcloud_with_raw_intensity_ascii_pcd(merged_df[["x","y","z", "intensity"]], file_path)
                            
            # Save the DataFrame to a CSV file in the same folder
            csv_file_path = os.path.join(save_folder, "clusters_volume.csv")
            self.df_to_csv.to_csv(csv_file_path, index=False)
            
            # Show success message
            messagebox.showinfo("Success", "Point Clouds and CSV saved successfully!")  
        self.activate_cluster_buttons(self.inactive_clusters)    
        self.inactive_clusters = []
        self.empty_result_list()
        self.update_clusters()
        self.df_to_csv = self.df_to_csv.iloc[0:0]
        self.text_box_clusters.delete(1.0, tk.END)
        self.text_box_clusters_volume_noWeighted.delete(1.0, tk.END)
        self.text_box_clusters_volume_Weighted.delete(1.0, tk.END)

    def remove_last_cluster(self):
        print("removing the last cluster from result")
    
        self.result_clusters.append(self.saving_clusters[-1][0])
        print(self.result_clusters)
        self.saving_clusters.pop()
        indice = self.inactive_clusters[-1]
        self.inactive_clusters.pop()
        self.activate_cluster_buttons([indice])
        self.update_clusters()
        # Remove the last row
        self.df_to_csv = self.df_to_csv.drop(self.df_to_csv .index[-1])
        print(self.df_to_csv)
        self.erase_last_line()  # Call this to remove the last line from all three text boxes
    ...
    def interpolate_data_btw_planes (self):
        ...
        self.print_info(self.text)
        self.merged_cloud_interpolated = BeamImager.merge_volumes(self.interpolated_points_list)
        if self.interpolation_var.get() is True:
            o3d.visualization.draw_geometries([self.merged_cloud_interpolated, self.point_cloud])
        self.point_cloud = self.merged_cloud_interpolated
        ## add the point cloud to the point_cloud_history_list
        #self.point_cloud_history.append(self.point_cloud)
    ...
    def calculate_volume_voxelization(self):
        ...
        # Clear and update the Upper Threshold Entry
        self.Upper_Threshold_entry.delete(0, tk.END)
        self.Upper_Threshold_entry.insert(0, str(self.default_Upper_Threshold))

        # Clear and update the Lower Threshold Entry
        self.Lower_Threshold_entry.delete(0, tk.END)
        self.Lower_Threshold_entry.insert(0, str(self.default_Lower_Threshold))
        ...
        self.voxel_volume_custome = BeamImager.custom_voxelization(self.voxel_grid,self.unique_voxels,self.voxel_counts, self.voxel_size, self.threshold_count_up , self.threshold_count_down)
        self.print_info(f"Volume (customized voxelization) of {self.cluster_number}: {self.voxel_volume_custome:.6f} m3")
