...
def getting_values_noping (df_data):
    ...
    return (x_data, y_data, z_data, intensity_data)

def df_to_pointcloud(df): 
    
    ## Creating an open3D point cloud with geometrical and normalized intensity data
    point_cloud = create_pointCloud_object(np.array(df.iloc[:, :3]), np.array(df.iloc[:, -3:]))

    ## Visualizing the point cloud
    o3d.visualization.draw_geometries([point_cloud])

    ## Print number of points
    text = f"Number of points: {len(point_cloud.points)}"
...
def pointcloud_to_df_intensity(point_cloud, I_min, I_max):
    # Extract XYZ coordinates
    point_cloud_data = np.asarray(point_cloud.points)
    
    # Extract RGB color data
    color_data = np.asarray(point_cloud.colors)

    # Get the colormap used for RGB mapping (assuming 'viridis')
    colormap = cm.get_cmap('viridis')

    # Generate a lookup table of RGB values for normalized intensity range [0, 1]
    sample_points = np.linspace(0, 1, 256)
    colormap_rgb = colormap(sample_points)[:, :3]  # RGB values (ignore alpha)

    # Initialize array to store recovered normalized intensities
    normalized_intensity = np.zeros(color_data.shape[0])

    # Find the closest RGB match for each point in the point cloud
    for i, rgb in enumerate(color_data):
        # Calculate the Euclidean distance between the point RGB and colormap RGBs
        distances = np.linalg.norm(colormap_rgb - rgb, axis=1)
        # Find the index of the closest colormap RGB value
        closest_idx = np.argmin(distances)
        # Get the corresponding normalized intensity
        normalized_intensity[i] = sample_points[closest_idx]

    # Denormalize the intensity to get the original intensity
    intensity = normalized_intensity * (I_max - I_min) + I_min

    # Stack XYZ coordinates with intensity into a single array
    combined_data = np.column_stack((point_cloud_data, intensity))

    # Create DataFrame with the correct columns
    columns = ['x', 'y', 'z', 'intensity']
    df_filtered = pd.DataFrame(combined_data, columns=columns)
...
# Initialize global variables for storing the bounds
global_bounds = {
    "min_x": None,
    "max_x": None,
    "min_y": None,
...
initialized = False  # Flag to check if bounds are already initialized

def initialize_global_bounds(clustered_point_clouds):
    global global_bounds, initialized
    
    # Only initialize bounds if not already done
    if initialized:
        return
    ...
    # Calculate global bounds from all clusters
    for cluster_points in clustered_point_clouds:
        cluster_points, intensities, cluster_names = zip(*cluster_points)
        cluster_points = np.array(cluster_points)

        # Update global bounds
        min_x = min(min_x, cluster_points[:, 0].min())
        max_x = max(max_x, cluster_points[:, 0].max())
        min_y = min(min_y, cluster_points[:, 1].min())
        max_y = max(max_y, cluster_points[:, 1].max())
        min_z = min(min_z, cluster_points[:, 2].min())
        max_z = max(max_z, cluster_points[:, 2].max())

    # Store bounds in the global variable
    global_bounds = {
        "min_x": min_x,
        "max_x": max_x,
        "min_y": min_y,
    ...
    # Set flag so bounds aren't recalculated
    initialized = True
...
def visualize_clusters(clustered_point_clouds, cluster_colors):
    ...
    initialize_global_bounds(clustered_point_clouds)

    # Print global bounds for debugging
    print(f"Global bounds: x[{global_bounds['min_x']}, {global_bounds['max_x']}], "
          f"y[{global_bounds['min_y']}, {global_bounds['max_y']}], "
          f"z[{global_bounds['min_z']}, {global_bounds['max_z']}]")

    plt.close('all')
    cluster_clouds = []

    # Convert cluster points into point clouds
    for cluster_points in clustered_point_clouds:
        cluster_points, intensities, cluster_names = zip(*cluster_points)
        cluster_points = np.array(cluster_points)
    ...
    # Adjust the legend to the side (move it outside the plot)
    ...
    return sorted_point_clouds, global_bounds, initialized
...
def save_pointcloud_with_raw_intensity_ascii_pcd(df, file_name):
    # Extract x, y, z, and raw intensity data from the DataFrame
    x_data = df['x'].values
    y_data = df['y'].values
    z_data = df['z'].values
    intensity_data = df['intensity'].values  # Unnormalized intensity
    ...
    with open(file_name, 'w') as f:
        ...
        # Write the points and intensity
        for x, y, z, intensity in zip(x_data, y_data, z_data, intensity_data):
            f.write(f"{x} {y} {z} {intensity}\n")

    print(f"Point cloud with raw intensity saved to {file_name}")
