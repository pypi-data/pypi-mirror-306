    ...
    def from_anthropic_message(cls, anthropic_message: AnthropicMessage,
        ...
        for i in anthropic_message.content:
            if isinstance(i, TextBlock):
                ...
                if not hasattr(i.input, "__dict__"):
                    log.warning(f"Unable to parse")
                    continue
                result.append(
                    cls(
                        creator=creator,
...
class ToolUsedOrFailed(ABC):
    tool_use_decision: Optional[LLMToolUseDecision]
    tool: "AnthropicTool"
...
class NoToolsAvailable:
    """When an Agent was asked to choose a tool to use, but did not have any tools at its disposal"""
