...
def data_generator_patch(extracted_image_path: str,
                         gender: Series, label: Series,
                          train: bool = True) -> tuple[tuple[tf.Tensor, tf.Tensor], tf.Tensor]:
    ...
    if train:
        all_samples = np.random.choice([0, 1, 2], p = [1/3]*3, size = len(index_list)*4)
    ...
    for num, index in enumerate(index_list):
        ...
        if train:
            ...
            processed_patch = [patch[::-1, :, :] if sample == 1 else
                                 patch[:, ::-1, :] if sample == 2 else
                                   patch for patch, sample in zip(processed, sample_step)]
        else:
