...
#     fig.text(0.17, 0.85, "Original Image", ha='center', va='center', fontsize=12)
#     fig.text(0.5, 0.85, "After cropping, resizing and CLAHE", ha='center', va='center', fontsize=12)
#     fig.text(0.83, 0.85, "Patches", ha='center', va='center', fontsize=12) 
...
def plot_processed_image(extracted_image_path: str, num_patches_per_side:int = 3, 
                            crop_list: Dict[str,int] = 
                                    ...
                                    fig_size = [10, 7], augmentations_list = [True, True, False, True]) -> None:
    """
    Plot the original image and a composite image of patches extracted from it.
    ...
    index = int(num_patches_per_side*((num_patches_per_side-1)/2)+ (num_patch_per_side+1)/2)
    ...
    axes_2[0].axis('off')
    axes_2[0].set_title("Original Patch", fontsize=14, pad=20)
    ...
    for img, title in zip(plotted, title_list):
        ...
        axes_2[j].imshow(img)
        axes_2[j].axis('off')
        axes_2[j].set_title(title, fontsize=14, pad=20)
        j += 1
...
def data_generator_patch(extracted_image_path: str,
    ...
    for num, index in enumerate(index_list):
        ...
        if train:
            index_patch = random.sample(range(len(processed)), 1)[0]
