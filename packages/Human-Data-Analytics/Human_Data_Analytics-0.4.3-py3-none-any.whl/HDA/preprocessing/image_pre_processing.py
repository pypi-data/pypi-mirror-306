...
def pre_process(image_n : str, path : str, num_patches_per_side:int = 3) -> np.ndarray:
    '''
...
def display_image_preprocess(extracted_image_path: str, num_patches_per_side:int = 3) -> tuple[np.ndarray, List[np.ndarray]]:
  ...
  processed = pre_process(images_filename, extracted_image_path, num_patches_per_side = num_patches_per_side)
...
def plot_processed_image(extracted_image_path: str, num_patches_per_side:int = 3) -> None:
    """
    ...
    large_image, small_images = display_image_preprocess(extracted_image_path, num_patches_per_side = num_patches_per_side)
...
def data_generator_patch(extracted_image_path: str,
                         gender: Series, label: Series,
                          train: bool = True, admissible_augmentations = [0,3,4],
                             prob: List[int] = [1/3]*3, num_patch_per_side: int = 3) -> tuple[tuple[tf.Tensor, tf.Tensor], tf.Tensor]:
    ...
    if train:
        index_list = np.random.choice(index_list_, size = 16000)
        all_samples = np.random.choice(admissible_augmentations, p = prob, size = len(index_list))
    else:
        index_list = index_list_
        
    for num, index in enumerate(index_list):
        processed = pre_process(images_filenames[index], extracted_image_path, num_patches_per_side = num_patch_per_side)
