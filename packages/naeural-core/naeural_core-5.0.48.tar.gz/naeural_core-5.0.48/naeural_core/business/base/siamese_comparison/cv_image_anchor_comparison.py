"""
This plugin is a template which contains the basic logic for comparing the current frame with an anchor frame.
This plugin is adds support for ATM and non-ATM use cases, and also adds support for forced lower and re-raise of the alerter.
The flow of the plugin is completely described in the `_process` method.
This plugin uses the general detector AI engine for people detection (because we do not want people to influence the comparison).

Implementation of this plugin should be done by the user and should only contain the logic for comparing the current frame with the anchor frame.
Additionally, users can override the methods at the bottom of this file to add custom logic.
"""
...
class CvImageAnchorComparisonPlugin(BasePlugin):
  ...
  def _maybe_save_anchor(self, image, object_detection_inferences):
    """
    Save the anchor if the conditions are met.

    Parameters
    ----------
    image : np.ndarray
        The current frame
    object_detection_inferences : List[dict]
        The object detection inferences of the current frame
    """
  ...
  def _maybe_load_anchor(self):
    """
    Load the anchor if the conditions are met. This method is called at the start of the plugin.
    """
  ...
  def _intersect_tlbrs_with_target_zone(self, object_detector_inferences):
    ...
    object_detector_inferences : List[dict]
        The object detection inferences to be intersected with the target zone
  ...
  def _maybe_update_people_cooldown_counter(self, image, object_detector_inferences):
    """
    Update the people cooldown counter. Decrease the counter if there are no people in the image, otherwise reset the counter.
  ...
  def __validate_image_as_anchor_check_people(self, image, object_detection_inferences):
    """
    Check if the image is valid as an anchor. 
    The image is valid if the sum of the people area is less than a percentage.
    ...
    -------
    bool
        True if the image is valid as an anchor, False otherwise
    """
  ...
  def __validate_image_as_anchor_check_people_cooldown(self):
    """
    Check if the people cooldown is ok. 
    The people cooldown is ok if the counter is less than or equal to 0.

    Returns
    -------
    bool
        True if the people cooldown is ok, False otherwise
    """
  ...
  def __validate_image_as_anchor_check_entropy(self, image):
    """
    Check if the image entropy is ok.
    The image entropy is ok if it is greater than a threshold.
    ...
    -------
    bool
        True if the image entropy is ok, False otherwise
    """
  ...
  def _validate_image_as_anchor(self, image, object_detection_inferences):
    """
    Validate the image as an anchor. 
    The image is valid as an anchor if the people area is less than a percentage,
    the people cooldown is ok, and the entropy is greater than a threshold.
  ...
  def _maybe_force_lower(self):
    ...
    -------
    bool
        True if the alert needs to be force lowered, False otherwise
    """
  ...
  def _custom_alerter_status_changed(self):
    ...
    if is_new_lower:
      # if status changed from (raise or re-raise) to lower
      self._alerter_forced_lower = False
      self._alerter_re_raised = False
      return True, "Lower"
  ...
  def _draw_witness_image(self, img, object_detector_inferences, debug_results, **kwargs):
    """
    Draw the witness image.
    Draw the detection boxes for the people in the image, along with the confidence score for each box.

    Parameters
    ----------
    img : np.ndarray
        The image to draw the witness on
    object_detector_inferences : List[dict]
        The object detection inferences of the current frame
    debug_results : dict
        The debug results of the current frame

    Returns
    -------
    np.ndarray
        The image with the witness drawn on it
    """
    for dct_inference in object_detector_inferences:
  ...
  def _get_payload(self, alerter_status, **kwargs):
    """
    This method extracts the necessary information from the cache and creates the payload.

    Parameters
    ----------
    alerter_status : str
        The current alerter status

    Returns
    -------
    dict
        The payload of the plugin
    """
    ...
    if alerter_status == "Forced Lower":
      # if we force lower the alerter, we need to reset the anchor
      # and then reset the alerter
      self._anchor = None
    ...
    payload = self._create_payload(
      # we send the witness image, the original image, and the anchor
      # the last 2 are used for debugging and further improvements 
  ...
  def __get_person_prc_area(self, image, dct_inference):
    """
    Get the person area as a percentage of the image area.
    ...
    image : np.ndarray
        The image
    dct_inference : dict
        The inference of the person

    Returns
    -------
    float
        The person area as a percentage of the image area
    """
    if self._coords_type == self.ct.COORDS_NONE:
      H, W = image.shape[:2]
      image_area = H * W
  ...
  def __people_areas_prc(self, image, lst_obj_inferences, round_digits=None):
    """
    Get the people areas as a percentage of the image area.

    Parameters
    ----------
    image : np.ndarray
        The image
    lst_obj_inferences : List[dict] 
        The object detection inferences
    round_digits : int, optional
        The number of digits to round the areas to. If None, will not round. By default None

    Returns
    -------
    List[float]
        The people areas as a percentage of the image area
    """
  ...
  def _validate_image_for_analysis(self, image, lst_obj_inferences):
    """
    Validate the image for analysis. 
    The image is valid for analysis if the people areas are smaller than a percentage and
    the users custom image validation is ok.
    ...
    lst_obj_inferences : List[dict]
        The object detection inferences of the image

    Returns
    -------
    bool
        True if the image is valid for analysis, False otherwise
    """
  ...
  def _validate_detections_for_analysis(self, image, lst_obj_inferences):
    """
    Validate the detections for analysis.
    The detections are valid for analysis if all persons are smaller than a percentage or
    if there is a person bigger than a percentage.
    ...
    -------
    bool
        True if the detections are valid for analysis, False otherwise
    """
  ...
  def _prepare_debug_info_for_witness(self, img, object_detector_inferences, debug_results):
    """
    Prepare the debug info for the witness.
    In the debug info we add the following information:
    - The percentage of the people areas in the image
    - The last anchor time
    - The anchor save period
    - The last raise time
    - The re-raise time
    - The force lower time
    ...
    -------
    List[dict]
        The debug info for the witness
    """
  ...
  def _update_cache_witness_image(self, img, object_detector_inferences, debug_info, result, debug_results):
    """
    Update the cache with the current image and the witness information.
    When the alerter status changes, we use this cache to create the witness image
    because we want to use the last valid image before the alerter status changed.
    ...
    debug_info : List[dict]
        The debug info of the current frame
    result : float
        The comparison result of the current frame
    debug_results : dict
        The debug results of the current frame
    """
  ...
  def _update_cache_last_witness_image(self, img, object_detector_inferences, debug_info, debug_results):
    """
    Update the cache with the last witness image.
    This cache is used for the GET_LAST_WITNESS command.
  ...
  def get_last_witness_image(self):
    """
    Get the last witness image. This method is used for the GET_LAST_WITNESS command.
    Extract the data from the cache and compute the last witness.

    Returns
    -------
    np.ndarray
        The last witness image
    """
  ...
  def _print_alert_status_changed(self, current_alerter_status):
    """
    Print the alert status changed message.

    Parameters
    ----------
    current_alerter_status : str
        The current alerter status
    """
  ...
  def _process(self):
    ...
    if result_similar and validation_ok:
      ...
      # so it might not be necessary to validate the image with the if used above
      self._maybe_save_anchor(self.__last_capture_image, object_detector_inferences)
    # endif
