...
def dummy_blur_movie(log, video_path, dst_path, use_good_rotation=True, rotate_before_write=True, blur_method=None):
  ...
  while ret:
    ...
    if ret == True:
      ...
      dq.append(frame)
    if len(dq) > 250 or (not ret and len(dq) > 0):
      len_dq = len(dq)
      for _ in range(len_dq):
        out.write(dq.popleft())
  out.release()
  cap.release()
  log.P(f'Done processing {cnt} frames from movie {video_path}')
  return
...
if __name__ == '__main__':
  ...
  video_path = '/home/bleo/debug/wetransfer_sample-14mb-40s-fullhd-60fps-avi_2023-01-06_1443/sample-236MB-31s-FullHD-60FPS.MP4'
  path_prefix = '/home/bleo/debug/ee_shards'
  output_path_prefix = '/home/bleo/debug/ee_shards_out'
  result_path = f'/home/bleo/debug/final_{time.time()}.mp4'

  DISPLAY_SHARDS = False
  RUN_THREADS = False
  ...
  print(f'Video has in total {_get_number_of_frames(video_path)}')
