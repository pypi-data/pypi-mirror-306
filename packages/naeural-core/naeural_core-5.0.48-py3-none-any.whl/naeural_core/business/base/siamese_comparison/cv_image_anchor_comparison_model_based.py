"""
This plugin is a template which contains the basic logic for comparing the current frame with an anchor frame using model based approaches.
This plugin is inherits CvImageAnchorComparisonPlugin with all its logic.
This plugin assumes the model used is an AI engine that inherits the `basic_th_anchor.py` model. 

Implementation of this plugin should be done by the user and should only contain the logic for extracting, validating and interpreting the inference.
"""
...
class CvImageAnchorComparisonModelBasedPlugin(BasePlugin):
  ...
  def anchor_reload_period(self):
    """
    Get the anchor reload period in seconds.

    Returns
    -------
    int | None
        The anchor reload period in seconds or None if it can't be found.
    """
  ...
  def compare_image_with_anchor(self, image):
    """
    Compare the image with the anchor.
  ...
  def _inference_validation(self):
    """
    Check if the inferences are valid.
  ...
  def _update_anchor_callback(self, serving_serialized_data):
    """
    Update the anchor in the serving cache.
    Save the anchor and its last save time in the serving cache, under `self.str_unique_identification` key.

    Parameters
    ----------
    serving_serialized_data : dict | None
        The serving cache data.

    Returns
    -------
    dict
        The updated serving cache data.
    """
  ...
  def _cleanup_anchor_callback(self, serving_serialized_data):
    """
    Remove the anchor from the serving cache.
  ...
  def custom_save_anchor(self, image):
    """
    Save the anchor in the serving cache.
  ...
  def custom_load_anchor(self):
    """
    Load the anchor from the serving cache.

    Returns
    -------
    anchor : np.ndarray | None
        The anchor image or None if the anchor is not found.
    """
