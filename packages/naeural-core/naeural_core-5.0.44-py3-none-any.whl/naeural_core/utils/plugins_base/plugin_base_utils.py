...
class _UtilsBaseMixin(
  ...
  def git_clone(self, repo_url, repo_dir, target='output', user=None, token=None, pull_if_exists=True):
    ...
    try:
      ...
      self.P(f"git_clone: Error while cloning git repository {repo_url} in {repo_path}: {exc}")
      repo_path = None
    # end try
    return repo_path
  ...
  def git_get_local_commit_hash(self, repo_dir):
    ...
    try:
      ...
      if results is not None:
        ...
        printer(f"git_get_local_commit_hash: Error while retrieving commit hash from remote repository: {results}")
    except Exception as exc:
      printer(f"git_get_local_commit_hash: Error while retrieving commit hash from remote repository: {exc}")
    return commit_hash
  ...
  def git_get_last_commit_hash(self, repo_url, user=None, token=None):
    ...
    try:
      ...
      if results is not None:
        ...
        printer(f"git_get_last_commit_hash: Error while retrieving commit hash from remote repository: {results}")
    except Exception as exc:
      printer(f"git_get_last_commit_hash: Error while retrieving commit hash from remote repository: {exc}")
    return commit_hash
