...
from iclearn.session.machine_learning import MachineLearningSession
from iclearn.output.logging_output_handler import LoggingOutputHandler

# from icflow.output.plotting_output_handler import PlottingOutputHandler
...
def setup_session(dataset_func: Callable, model_func: Callable, config: Config):
    ...
    logger.info("Creating dataset with dir %s", config.dataset_dir)
    ...
    logger.info("Creating ML session")
    session = MachineLearningSession(model, runtime_ctx, config.result_dir, dataset)
    session.output_handlers.append(LoggingOutputHandler(config.result_dir))
    #    session.output_handlers.append(PlottingOutputHandler(cli_args.result_dir))
