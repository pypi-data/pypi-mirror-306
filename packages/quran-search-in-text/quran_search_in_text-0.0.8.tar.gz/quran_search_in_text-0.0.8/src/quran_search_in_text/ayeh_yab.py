...
class AyehYab():
    ...
    def ayeh_find(self, text):
        ...
        '''  حذف شود  '''
        # متن قرآن را از فایل csv گرفته تبدیل به لیست نموده و بعد متن را استاندارد میکند
        # import pandas as pd
        # quran_file_df = pd.read_csv('متن قرآن.csv', sep='@')
        # ayat_list = quran_file_df[quran_file_df['ayehid'] != 0]['ayeh']
        # ayat_list = list(map(lambda text: self.standard_text(text), ayat_list))
        #
        # # ایجاد یک دیکشنری که کلید ayehid است و مقدار آدرس آیه است
        # adress_ayeh_df0 = quran_file_df[quran_file_df['ayehid'] != 0][['ayehid', 'sureh_name', 'ayeh_number']]
        # adress_ayeh_df0['ayeh_adress'] = adress_ayeh_df0['sureh_name'] + adress_ayeh_df0['ayeh_number'].astype(str)
        # adress_ayeh_df = adress_ayeh_df0[['ayehid', 'ayeh_adress']]
        # adress_ayeh_dict = adress_ayeh_df.set_index('ayehid')['ayeh_adress'].to_dict()
        #
        # # ایجاد یک دیکشنری که کلید ayehid است و مقدار آن {'شماره سوره/شماره آیه'} است
        # adress_ayeh_df0 = quran_file_df[quran_file_df['ayehid'] != 0][['ayehid', 'sureh_number', 'ayeh_number']]
        # adress_ayeh_df0['ayeh_link'] = adress_ayeh_df0['sureh_number'].astype(str) + '/' + adress_ayeh_df0[
        #     'ayeh_number'].astype(str)
        # adress_ayeh_df = adress_ayeh_df0[['ayehid', 'ayeh_link']]
        # ayeh_link_dict = adress_ayeh_df.set_index('ayehid')['ayeh_link'].to_dict()
        ...
        surehname_ayehnumber_file = open(os.path.join(current_dir, "packages", "surehname_ayehnumber.txt"), 'r', encoding='utf-8')
        ...
        surehnumber_ayehnumber_file = open(os.path.join(current_dir, "packages", "surehnumber_ayehnumber.txt"), 'r', encoding='utf-8')
        ...
        for index in range(rows_len):
            ...
            for item in output_items[::-1]:
                ...
                link_ayeh = item["ayeh_adress_text"]
