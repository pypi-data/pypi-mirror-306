...
def precompute_bi_encoder_negatives(bi_encoder, tokenizer, candidates, jobs, positive_matches, config, batch_size=1024, num_workers=4):
    """
    ...
    for batch_candidates in tqdm_fn(dataloader, desc="Precomputing negatives in batches"):
        batch_candidate_ids = [candidate['candidate_id'] for candidate in batch_candidates]
...
def generate_hard_negatives_with_dataloader(model, data_samples, tokenizer, negatives, config, candidate_feature_size, job_feature_size, num_workers=4):
    ...
    with torch.no_grad():
        for batch in tqdm_fn(dataloader, desc="Generating hard negatives"):
            batch_candidate_texts = batch['candidate_text']
            batch_negative_job_texts = batch['negative_job_text']
            batch_candidate_ids = batch['candidate_id']
            batch_negative_job_ids = batch['negative_job_id']
            ...
            if use_sparse and candidate_features_tensor is not None and job_features_tensor is not None:
                # Assuming the model accepts features as a combined tensor or separate inputs
                # Modify according to your model's requirements
                logits = model(input_ids=input_ids, attention_mask=attention_mask, candidate_features=candidate_features_tensor, job_features=job_features_tensor)
            else:
    ...
    for cid, neg_id, neg_text, score in zip(all_candidate_ids, all_negative_job_ids, all_negative_job_texts, scores):
        ...
        if use_sparse:
            candidate_negatives[cid]['job_features'] = neg_features  # Assuming you have access to features here
