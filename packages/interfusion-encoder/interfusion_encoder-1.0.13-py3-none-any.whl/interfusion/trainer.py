...
def precompute_bi_encoder_negatives(bi_encoder, tokenizer, candidates, jobs, positive_matches, config, batch_size=10240, num_workers=4):
    """
    ...
    # Compute embeddings using DataLoader
    ...
    for batch_candidates in tqdm_fn(dataloader, desc="Precomputing negatives in batches"):
        # Since DataLoader collates dictionaries into a dict of lists, access fields directly
        batch_candidate_ids = batch_candidates['candidate_id']  # List of candidate IDs
        # Convert candidate IDs to indices
        ...
        top_indices_partial = np.argpartition(-similarities_cpu, start_rank + M - 1, axis=1)[:, :start_rank + M]  # [batch_size, start_rank + M]
...
class CandidateNegativePairDataset(Dataset):
    def __init__(self, data_samples, negatives, use_sparse):
        """
        Dataset of candidate-negative pairs.

        Args:
            data_samples: List of data sample dictionaries.
            negatives: Precomputed negatives dictionary.
            use_sparse: Boolean indicating whether to include features.
        """
        self.pairs = []
        ...
        for sample in data_samples:
            ...
            for neg_text, neg_id, neg_features in zip(neg_job_texts, neg_job_ids, neg_features_list):
                self.pairs.append({
                    'candidate_id': candidate_id,
                    'candidate_text': candidate_text,
                    'negative_job_id': neg_id,
                    'negative_job_text': neg_text,
                    'candidate_features': candidate_features,
                    'negative_job_features': neg_features
                })
...
def generate_hard_negatives_with_dataloader(model, data_samples, tokenizer, negatives, config, candidate_feature_size, job_feature_size, num_workers=4):
    """
    Generate hard negatives from precomputed negatives using DataLoader for efficient batching.

    Args:
        model: The cross-encoder model.
        data_samples: List of data sample dictionaries.
        tokenizer: Tokenizer for the cross-encoder.
        negatives: Precomputed negatives dictionary.
        config: Configuration dictionary.
        candidate_feature_size: Size of candidate features.
        job_feature_size: Size of job features.
        num_workers: Number of worker processes for data loading.

    Returns:
        hard_negatives: Dictionary containing hard negative samples per candidate.
    """
    ...
    # Create Dataset and DataLoader for candidate-negative pairs
    dataset = CandidateNegativePairDataset(data_samples, negatives, use_sparse)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)

    # Prepare lists to collect data
    all_candidate_ids = []
    all_negative_job_ids = []
    all_negative_job_texts = []
    all_candidate_features = []
    all_negative_job_features = []
    scores = []

    with torch.no_grad():
        for batch in tqdm_fn(dataloader, desc="Generating hard negatives"):
            # Access batched data directly
            batch_candidate_texts = batch['candidate_text']  # List of candidate texts
            batch_negative_job_texts = batch['negative_job_text']  # List of negative job texts
            batch_candidate_ids = batch['candidate_id']  # List of candidate IDs
            batch_negative_job_ids = batch['negative_job_id']  # List of negative job IDs
            all_candidate_ids.extend(batch_candidate_ids)
            all_negative_job_ids.extend(batch_negative_job_ids)
            all_negative_job_texts.extend(batch_negative_job_texts)

            if use_sparse:
                batch_candidate_features = batch['candidate_features']
                batch_negative_job_features = batch['negative_job_features']
                # Handle possible None values
                candidate_features_tensor = torch.stack([
                    torch.tensor(cf, dtype=torch.float) if cf is not None else torch.zeros(candidate_feature_size)
                    for cf in batch_candidate_features
                ]).to(device)
                job_features_tensor = torch.stack([
                    torch.tensor(nf, dtype=torch.float) if nf is not None else torch.zeros(job_feature_size)
                    for nf in batch_negative_job_features
                ]).to(device)
            else:
                candidate_features_tensor = None
                job_features_tensor = None
            ...
            if use_sparse and candidate_features_tensor is not None and job_features_tensor is not None:
                # Assuming the model accepts features as separate inputs
                logits = model(input_ids=input_ids, attention_mask=attention_mask, 
                               candidate_features=candidate_features_tensor, job_features=job_features_tensor)
            else:
                logits = model(input_ids=input_ids, attention_mask=attention_mask)
    ...
    for cid, neg_id, neg_text, score in zip(all_candidate_ids, all_negative_job_ids, all_negative_job_texts, scores):
        candidate_negatives[cid].append({
            'job_id': neg_id,
            'job_text': neg_text
        })
        if use_sparse:
            # Since 'job_features' are already processed in the batch, retrieve them accordingly
            # Here, we're assuming that 'neg_features' are included; adjust if necessary
            candidate_negatives[cid][-1]['job_features'] = None  # Replace with actual features if needed
    ...
    for cid in candidate_negatives:
        ...
        if use_sparse:
            job_features = [item.get('job_features', None) for item in hard_neg_list]
        hard_negatives[cid] = {
            'job_ids': job_ids,
            'job_texts': job_texts
        }
