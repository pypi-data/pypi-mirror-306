...
# Helper Dataset Classes for Optimized Data Loading
class HardNegativeDataset(Dataset):
    def __init__(self, data_samples, negatives, tokenizer, config, use_sparse=False, candidate_feature_size=0, job_feature_size=0):
    ...
    def prepare_pairs(self):
        for sample in self.data_samples:
            ...
            c_text = sample['candidate_text']
            negs = self.negatives[cid]['job_texts']
            neg_ids = self.negatives[cid]['job_ids']
            if self.use_sparse:
                c_feats = sample.get('candidate_features', None)
                j_feats = self.negatives['negative_job_features'][cid]
                for jid, j_text, j_feat in zip(negs, neg_ids, j_feats):
                    self.pairs.append((c_text, j_text, c_feats, j_feat))
            else:
                for jid, j_text in zip(negs, neg_ids):
                    self.pairs.append((c_text, j_text, None, None))
...
def precompute_bi_encoder_negatives(bi_encoder, tokenizer, candidates, jobs, positive_matches, config):
    ...
    negatives = defaultdict(dict) if use_sparse else defaultdict(dict)
    ...
    for idx, cid in tqdm(enumerate(candidate_ids), total=len(candidate_ids), desc="Populating negatives"):
        ...
        if use_sparse:
            ...
            negatives['negative_job_features'][cid] = neg_features
...
def generate_hard_negatives(model, data_samples, tokenizer, negatives, config, candidate_feature_size, job_feature_size):
    ...
    hard_neg_dataset = HardNegativeDataset(data_samples, negatives, tokenizer, config, use_sparse, candidate_feature_size, job_feature_size)
    hard_neg_dataloader = DataLoader(hard_neg_dataset, batch_size=batch_size, shuffle=False, num_workers=config['num_workers'], pin_memory=True)

    scores = []
    candidate_ids_batch = []
    negative_job_ids_batch = []
    negative_job_texts_batch = []
    negative_job_features_batch = [] if use_sparse else None

    with torch.no_grad():
        for batch in tqdm(hard_neg_dataloader, desc="Generating hard negatives"):
            c_texts, j_texts, c_feats, j_feats = batch
            ...
            if use_sparse:
                ...
                job_features = torch.stack([
                    torch.tensor(jf, dtype=torch.float) if jf is not None else torch.zeros(job_feature_size)
                    for jf in j_feats
                ]).to(device)
            ...
            # Collect batch information for aggregation
            for i in range(len(c_texts)):
                cid = data_samples[i]['candidate_id']
                neg_jid = negatives[cid]['job_ids'][i]
                neg_jtext = negatives[cid]['job_texts'][i]
                candidate_ids_batch.append(cid)
                negative_job_ids_batch.append(neg_jid)
                negative_job_texts_batch.append(neg_jtext)
                if use_sparse:
                    negative_job_features_batch.append(j_feats[i])

    # Aggregate scores per candidate
    candidate_negatives = defaultdict(list)
    candidate_scores = defaultdict(list)
    for cid, jid, jtext, score in zip(candidate_ids_batch, negative_job_ids_batch, negative_job_texts_batch, scores):
    ...
    # Select top N hard negatives per candidate
    hard_negatives = {}
...
def evaluate(model, tokenizer, candidates_eval, jobs_eval, positive_matches_eval, config, bi_encoder, epoch):
    ...
    for idx, candidate_id in tqdm(enumerate(candidate_ids), total=len(candidate_ids), desc="Bi-Encoder Evaluation"):
    ...
    for idx, candidate_id in tqdm(enumerate(candidate_ids), total=len(candidate_ids), desc="Preparing Cross-Encoder Data"):
    ...
    model.eval()
    with torch.no_grad():
        for batch in tqdm(eval_dataloader, desc="Cross-Encoder Evaluation"):
    ...
    for candidate_id in tqdm(candidate_ids, desc="Calculating Precision@N"):
...
def generate_hard_negatives(model, data_samples, tokenizer, negatives, config, candidate_feature_size, job_feature_size):
    ...
    for cid in candidate_negatives:
        ...
        job_texts = [item['job_text'] for item in hard_neg_list]
        if use_sparse:
            job_features = [neg_list[i].get('job_features', None) for i in top_indices]
            hard_negatives[cid] = {
                'job_ids': job_ids,
                'job_texts': job_texts,
                'job_features': job_features
