...
class HardNegativeDataset(Dataset):
    ...
    def prepare_pairs(self):
        """
        Prepare pairs of (candidate_id, job_text, job_features).
        """
...
def train_model(candidates, jobs, positive_matches, candidates_eval=None, jobs_eval=None, positive_matches_eval=None, user_config=None):
    ...
    hard_negatives = generate_hard_negatives(model, data_samples, tokenizer, negatives, config, jobs, candidate_feature_size, job_feature_size)
...
def generate_hard_negatives(model, data_samples, tokenizer, negatives, config, jobs, candidate_feature_size, job_feature_size):
    """
        ...
        for batch in tqdm_cls(hard_neg_dataloader, desc="Generating hard negatives"):
            ...
            if use_sparse:
                ...
                for cid, jf in zip(candidate_ids, job_features):
                    # Retrieve candidate features if available
                    cf = None
                    for sample in data_samples:
                        if sample['candidate_id'] == cid:
                            cf = sample.get('candidate_features', None)
                            break
                    if cf is not None:
...
def train(model, train_dataset, optimizer, device, config, epoch, scaler, scheduler):
    ...
    for batch in tqdm_cls(train_dataloader, desc=f"Training Epoch {epoch+1}"):
        ...
        # Tokenize the job texts
        inputs = model.tokenizer(job_texts, padding=True, truncation=True, max_length=config['max_length'], return_tensors='pt').to(device)
        ...
        if config['use_sparse']:
            ...
            for cid, jf in zip(candidate_ids, job_features):
                # Retrieve candidate features if available
                cf = None
                for sample in train_dataset.data_samples:
                    if sample['candidate_id'] == cid:
                        cf = sample.get('candidate_features', None)
                        break
                if cf is not None:
