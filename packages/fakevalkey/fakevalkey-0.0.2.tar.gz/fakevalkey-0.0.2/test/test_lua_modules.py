...
async def test_async_asgi_ratelimit_script(async_redis: valkey.Redis):
    script = """
local ruleset = cjson.decode(ARGV[1])

-- Set limits
for i, key in pairs(KEYS) do
    valkey.call('SET', key, ruleset[key][1], 'EX', ruleset[key][2], 'NX')
end

-- Check limits
for i = 1, #KEYS do
    local value = valkey.call('GET', KEYS[i])
...
@pytest.mark.load_lua_modules("cjson")
def test_asgi_ratelimit_script(r: valkey.Redis):
    script = """
local ruleset = cjson.decode(ARGV[1])
