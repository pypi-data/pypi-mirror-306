...
def test_arrlen(r: valkey.Redis):
    r.json().set(
        "arr",
        Path.root_path(),
        [0, 1, 2, 3, 4],
...
def test_arrappend(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
...
def test_arrindex(r: valkey.Redis):
    r.json().set(
        "foo",
        Path.root_path(),
        [0, 1, 2, 3, 4],
...
def test_arrinsert(r: valkey.Redis):
    r.json().set(
        "arr",
        Path.root_path(),
        [0, 4],
...
def test_arrpop(r: valkey.Redis):
    r.json().set(
        "arr",
        Path.root_path(),
        [0, 1, 2, 3, 4],
...
def test_arrtrim(r: valkey.Redis):
    r.json().set(
        "arr",
        Path.root_path(),
        [0, 1, 2, 3, 4],
