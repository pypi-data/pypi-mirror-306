"""
Tests for `fakevalkey-py`'s emulation of Redis's JSON.GET command subset.
"""
...
def test_jsonget(r: valkey.Redis):
    data = {"x": "bar", "y": {"x": 33}}
...
def test_json_setgetdeleteforget(r: valkey.Redis):
    data = {"x": "bar"}
...
def test_json_delete_with_dollar(r: valkey.Redis):
...
def test_json_et_non_dict_value(r: valkey.Redis):
    r.json().set(
        "str",
        Path.root_path(),
        "str_val",
...
def test_jsonset_existential_modifiers_should_succeed(r: valkey.Redis):
    obj = {"foo": "bar"}
...
def test_jsonset_flags_should_be_mutually_exclusive(r: valkey.Redis):
...
def test_json_unknown_param(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        testtools.raw_command(r, "json.set", "obj", "$", json.dumps({"foo": "bar"}), "unknown")


def test_jsonmget(r: valkey.Redis):
...
def test_jsonmget_should_succeed(r: valkey.Redis):
...
def test_jsonclear(r: valkey.Redis):
    r.json().set(
        "arr",
        Path.root_path(),
        [0, 1, 2, 3, 4],
...
def test_jsonclear_dollar(r: valkey.Redis):
    data = {
...
def test_jsonclear_no_doc(r: valkey.Redis):
    # Test missing key
    with pytest.raises(valkey.ResponseError):
        r.json().clear("non_existing_doc", "$..a")


def test_jsonstrlen(r: valkey.Redis):
    data = {"x": "bar", "y": {"x": 33}}
    r.json().set("foo", Path.root_path(), data)
...
def test_toggle(r: valkey.Redis):
    r.json().set("bool", Path.root_path(), False)
...
def test_toggle_dollar(r: valkey.Redis):
    data = {
        "a": ["foo"],
        "nested1": {"a": False},
        "nested2": {"a": 31},
...
def test_json_commands_in_pipeline(r: valkey.Redis):
    p = r.json().pipeline()
...
def test_strappend(r: valkey.Redis):
    # Test single
...
def test_decode_null(r: valkey.Redis):
    assert r.json().get("abc") is None


def test_decode_response_disabaled_null(r: valkey.Redis):
    assert r.json().get("abc") is None


def test_json_get_jset(r: valkey.Redis):
...
def test_nonascii_setgetdelete(r: valkey.Redis):
    assert r.json().set(
        "not-ascii",
        Path.root_path(),
        "hyvää-élève",
...
def test_json_setbinarykey(r: valkey.Redis):
...
def test_set_file(r: valkey.Redis):
...
def test_set_path(r: valkey.Redis):
    # Standard Library Imports
    import json
    import tempfile
...
def test_objlen(r: valkey.Redis):
...
def test_objkeys(r: valkey.Redis):
    obj = {"foo": "bar", "baz": "qaz"}
    r.json().set("obj", Path.root_path(), obj)
...
def test_numincrby(r: valkey.Redis):
    r.json().set("num", Path.root_path(), 1)
...
def test_nummultby(r: valkey.Redis):
    r.json().set("num", Path.root_path(), 1)
...
def test_json_merge(r: valkey.Redis):
    # Test with root path $
    assert r.json().set(
        "person_data",
        "$",
