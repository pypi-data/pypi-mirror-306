...
@pytest.mark.unsupported_server_types("dragonfly")
def test_swapdb_same_db(r: valkey.Redis):
    assert r.swapdb(1, 1)


def test_save(r: valkey.Redis):
    assert r.save()


@pytest.mark.unsupported_server_types("dragonfly")
def test_bgsave(r: valkey.Redis):
    assert r.bgsave()
...
def test_lastsave(r: valkey.Redis):
    assert isinstance(r.lastsave(), datetime)


@fake_only
def test_command(r: valkey.Redis):
...
@fake_only
def test_command_count(r: valkey.Redis):
    assert r.command_count() >= len([cmd for cmd in SUPPORTED_COMMANDS if " " not in cmd])


@pytest.mark.unsupported_server_types("dragonfly")
@pytest.mark.slow
def test_bgsave_timestamp_update(r: valkey.Redis):
...
@pytest.mark.slow
def test_save_timestamp_update(r: valkey.Redis):
    early_timestamp = r.lastsave()
    sleep(1)
    assert r.save()
    late_timestamp = r.lastsave()
...
def test_dbsize(r: valkey.Redis):
    assert r.dbsize() == 0
    r.set("foo", "bar")
    r.set("bar", "foo")
    assert r.dbsize() == 2
...
def test_flushdb_redispy4(r: valkey.Redis):
    r.set("foo", "bar")
    assert r.keys() == [b"foo"]
