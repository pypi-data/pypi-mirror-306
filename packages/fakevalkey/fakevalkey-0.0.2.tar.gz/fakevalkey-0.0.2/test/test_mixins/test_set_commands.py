...
def test_sadd_as_str_type(r: valkey.Redis):
    assert r.sadd("foo", *range(3)) == 3
    assert r.smembers("foo") == {b"0", b"1", b"2"}


def test_sadd_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.sadd("foo", "member2")


def test_scard(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("foo", "member2")
...
def test_scard_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.scard("foo")


def test_sdiff(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_sdiff_one_key(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    assert r.sdiff("foo") == {b"member1", b"member2"}


def test_sdiff_empty(r: valkey.Redis):
    assert r.sdiff("foo") == set()


def test_sdiff_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
        r.sdiff("foo", "bar")
...
def test_sdiffstore(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_sinter(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_sinter_bytes_keys(r: valkey.Redis):
    foo = os.urandom(10)
    bar = os.urandom(10)
    r.sadd(foo, "member1")
    r.sadd(foo, "member2")
...
def test_sinter_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
        r.sinter("foo", "bar")
...
def test_sinterstore(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_sismember(r: valkey.Redis):
    assert not r.sismember("foo", "member1")
    r.sadd("foo", "member1")
    assert r.sismember("foo", "member1")


def test_smismember(r: valkey.Redis):
...
def test_smismember_wrong_type(r: valkey.Redis):
    ...
    with pytest.raises(valkey.DataError, match="Invalid input of type"):
        r.smismember("foo2", [["member1", "member2"]])


def test_sismember_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.sismember("foo", "member")


def test_smembers(r: valkey.Redis):
    assert r.smembers("foo") == set()


def test_smembers_copy(r: valkey.Redis):
    r.sadd("foo", "member1")
    ret = r.smembers("foo")
    r.sadd("foo", "member2")
...
def test_smembers_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.smembers("foo")


def test_smembers_runtime_error(r: valkey.Redis):
    r.sadd("foo", "member1", "member2")
    for member in r.smembers("foo"):
        r.srem("foo", member)


def test_smove(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
...
def test_smove_non_existent_key(r: valkey.Redis):
    assert not r.smove("foo", "bar", "member1")


def test_smove_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
...
def test_spop_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.spop("foo")


def test_srandmember(r: valkey.Redis):
    r.sadd("foo", "member1")
...
def test_srandmember_number(r: valkey.Redis):
...
def test_srandmember_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.srandmember("foo")


def test_srem(r: valkey.Redis):
...
def test_srem_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    with pytest.raises(valkey.ResponseError):
        r.srem("foo", "member")


def test_sunion(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_sunion_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
        r.sunion("foo", "bar")
...
def test_sunionstore(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
def test_empty_set(r: valkey.Redis):
    r.sadd("foo", "bar")
    r.srem("foo", "bar")
    assert not r.exists("foo")


def test_sscan(r: valkey.Redis):
    # Set up the data
    name = "sscan-test"
    for ix in range(20):
        k = "sscan-test:%s" % ix
...
def test_sintercard(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
@pytest.mark.min_server("7")
def test_sintercard_key_doesnt_exist(r: valkey.Redis):
    r.sadd("foo", "member1")
    r.sadd("foo", "member2")
    r.sadd("bar", "member2")
    r.sadd("bar", "member3")
...
@pytest.mark.min_server("7")
def test_sintercard_bytes_keys(r: valkey.Redis):
    foo = os.urandom(10)
    bar = os.urandom(10)
    r.sadd(foo, "member1")
    r.sadd(foo, "member2")
...
@pytest.mark.min_server("7")
def test_sintercard_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
...
@pytest.mark.min_server("7")
def test_sintercard_syntax_error(r: valkey.Redis):
    r.zadd("foo", {"member": 1})
    r.sadd("bar", "member")
    with pytest.raises(valkey.ResponseError):
...
def test_pfadd(r: valkey.Redis):
    key = "hll-pfadd"
    assert r.pfadd(key, "a", "b", "c", "d", "e", "f", "g") == 1
    assert r.pfcount(key) == 7


def test_pfcount(r: valkey.Redis):
    key1 = "hll-pfcount01"
    key2 = "hll-pfcount02"
    key3 = "hll-pfcount03"
...
def test_pfmerge(r: valkey.Redis):
    key1 = "hll-pfmerge01"
    key2 = "hll-pfmerge02"
    key3 = "hll-pfmerge03"
...
@pytest.mark.slow
def test_set_ex_should_expire_value(r: valkey.Redis):
    r.set("foo", "bar")
...
@pytest.mark.slow
def test_set_px_should_expire_value(r: valkey.Redis):
...
@pytest.mark.slow
def test_psetex_expire_value(r: valkey.Redis):
...
@pytest.mark.slow
def test_psetex_expire_value_using_timedelta(r: valkey.Redis):
    with pytest.raises(ResponseError):
