...
def _push_thread(r: valkey.Redis) -> threading.Thread:
    def run():
        sleep(0.5)
        r.rpush("foo", "value1")
        sleep(0.5)
...
def test_lpush_then_lrange_all(r: valkey.Redis):
...
def test_lpush_then_lrange_portion(r: valkey.Redis):
    r.lpush("foo", "one")
    r.lpush("foo", "two")
    r.lpush("foo", "three")
    r.lpush("foo", "four")
...
def test_lrange_negative_indices(r: valkey.Redis):
    r.rpush("foo", "a", "b", "c")
    assert r.lrange("foo", -1, -2) == []
    assert r.lrange("foo", -2, -1) == [b"b", b"c"]


def test_lpush_key_does_not_exist(r: valkey.Redis):
    assert r.lrange("foo", 0, -1) == []


def test_lpush_with_nonstr_key(r: valkey.Redis):
    r.lpush(1, "one")
    r.lpush(1, "two")
    r.lpush(1, "three")
...
def test_lpush_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lpush("foo", "element")


def test_llen(r: valkey.Redis):
    r.lpush("foo", "one")
    r.lpush("foo", "two")
    r.lpush("foo", "three")
    assert r.llen("foo") == 3
...
def test_llen_no_exist(r: valkey.Redis):
    assert r.llen("foo") == 0


def test_llen_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.llen("foo")


def test_lrem_positive_count(r: valkey.Redis):
    r.lpush("foo", "same")
    r.lpush("foo", "same")
...
def test_lrem_negative_count(r: valkey.Redis):
...
def test_lrem_zero_count(r: valkey.Redis):
    r.lpush("foo", "one")
    r.lpush("foo", "one")
    r.lpush("foo", "one")
    r.lrem("foo", 0, "one")
...
def test_lrem_default_value(r: valkey.Redis):
    r.lpush("foo", "one")
    r.lpush("foo", "one")
    r.lpush("foo", "one")
    r.lrem("foo", 0, "one")
...
def test_lrem_does_not_exist(r: valkey.Redis):
    r.lpush("foo", "one")
    r.lrem("foo", 0, "one")
...
def test_lrem_return_value(r: valkey.Redis):
    r.lpush("foo", "one")
...
def test_lrem_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lrem("foo", 0, "element")


def test_rpush(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
    r.rpush("foo", "three")
...
def test_rpush_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.rpush("foo", "element")


def test_lpop(r: valkey.Redis):
...
def test_lpop_empty_list(r: valkey.Redis):
    r.rpush("foo", "one")
    r.lpop("foo")
...
def test_lpop_zero_elem(r: valkey.Redis):
    r.rpush(b"\x00", b"")
    assert r.lpop(b"\x00", 0) == []


def test_lpop_zero_non_existing_list(r: valkey.Redis):
    assert r.lpop(b"", 0) is None


def test_lpop_zero_wrong_type(r: valkey.Redis):
    r.set(b"", b"")
    with pytest.raises(valkey.ResponseError):
        r.lpop(b"", 0)


def test_lpop_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lpop("foo")
...
def test_lpop_count(r: valkey.Redis):
    assert r.rpush("foo", "one") == 1
    assert r.rpush("foo", "two") == 2
    assert r.rpush("foo", "three") == 3
    assert testtools.raw_command(r, "lpop", "foo", 2) == [b"one", b"two"]
    # See https://github.com/valkey/valkey/issues/9680
    raw = testtools.raw_command(r, "rpop", "foo", 0)
    assert raw is None or raw == []  # https://github.com/valkey/valkey/pull/10095


@pytest.mark.min_server("6.2")
def test_lpop_count_negative(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        testtools.raw_command(r, "lpop", "foo", -1)


def test_lset(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
    r.rpush("foo", "three")
    r.lset("foo", 0, "four")
...
def test_lset_index_out_of_range(r: valkey.Redis):
    r.rpush("foo", "one")
    with pytest.raises(valkey.ResponseError):
        r.lset("foo", 3, "three")


def test_lset_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lset("foo", 0, "element")


def test_rpushx(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpushx("foo", "two")
    r.rpushx("bar", "three")
...
def test_rpushx_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.rpushx("foo", "element")


def test_ltrim(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
    r.rpush("foo", "three")
    r.rpush("foo", "four")
...
def test_ltrim_with_non_existent_key(r: valkey.Redis):
    assert r.ltrim("foo", 0, -1)


def test_ltrim_expiry(r: valkey.Redis):
...
def test_ltrim_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.ltrim("foo", 1, -1)


def test_lindex(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
...
def test_lindex_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lindex("foo", 0)


def test_lpushx(r: valkey.Redis):
    r.lpush("foo", "two")
    r.lpushx("foo", "one")
    r.lpushx("bar", "one")
    assert r.lrange("foo", 0, -1) == [b"one", b"two"]
...
def test_lpushx_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.lpushx("foo", "element")


def test_rpop(r: valkey.Redis):
...
def test_rpop_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.rpop("foo")
...
@pytest.mark.min_server("6.2")
def test_rpop_count(r: valkey.Redis):
    assert r.rpush("foo", "one") == 1
    assert r.rpush("foo", "two") == 2
    assert r.rpush("foo", "three") == 3
...
@pytest.mark.min_server("6.2")
def test_rpop_count_negative(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        testtools.raw_command(r, "rpop", "foo", -1)


def test_linsert_before(r: valkey.Redis):
    r.rpush("foo", "hello")
    r.rpush("foo", "world")
...
def test_linsert_after(r: valkey.Redis):
    r.rpush("foo", "hello")
    r.rpush("foo", "world")
...
def test_linsert_bad_command(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        testtools.raw_command(r, "LINSERT", "x", "NOT_BEFORE", "pivot", "val")


def test_linsert_no_pivot(r: valkey.Redis):
    r.rpush("foo", "hello")
    r.rpush("foo", "world")
...
def test_linsert_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.linsert("foo", "after", "bar", "element")


def test_rpoplpush(r: valkey.Redis):
...
def test_rpoplpush_to_nonexistent_destination(r: valkey.Redis):
    r.rpush("foo", "one")
    assert r.rpoplpush("foo", "bar") == b"one"
    assert r.rpop("bar") == b"one"


def test_rpoplpush_expiry(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("bar", "two")
    r.expire("bar", 10)
    r.rpoplpush("foo", "bar")
...
def test_blpop_single_list(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
    r.rpush("foo", "three")
...
def test_blpop_test_multiple_lists(r: valkey.Redis):
    r.rpush("baz", "zero")
...
def test_blpop_allow_single_key(r: valkey.Redis):
...
@pytest.mark.slow
def test_blpop_block(r: valkey.Redis):
...
@pytest.mark.slow
def test_blpop_block_float(r: valkey.Redis):
    thread = _push_thread(r)
    try:
...
@pytest.mark.slow
def test_brpop_block(r: valkey.Redis):
    thread = _push_thread(r)
    try:
...
def test_blpop_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.blpop("foo", timeout=1)


def test_blpop_transaction(r: valkey.Redis):
    p = r.pipeline()
    p.multi()
...
def test_brpop_test_multiple_lists(r: valkey.Redis):
    r.rpush("baz", "zero")
...
def test_brpop_single_key(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("foo", "two")
    assert r.brpop("foo", timeout=1) == (b"foo", b"two")


def test_brpop_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.brpop("foo", timeout=1)


def test_brpoplpush_multi_keys(r: valkey.Redis):
    assert r.lpop("bar") is None
    r.rpush("foo", "one")
    r.rpush("foo", "two")
...
def test_brpoplpush_wrong_type(r: valkey.Redis):
    r.set("foo", "bar")
    r.rpush("list", "element")
    with pytest.raises(valkey.ResponseError):
...
@pytest.mark.slow
def test_blocking_operations_when_empty(r: valkey.Redis):
...
def test_empty_list(r: valkey.Redis):
    r.rpush("foo", "bar")
    r.rpop("foo")
    assert not r.exists("foo")


def test_lmove_to_nonexistent_destination(r: valkey.Redis):
    r.rpush("foo", "one")
    assert r.lmove("foo", "bar", "RIGHT", "LEFT") == b"one"
    assert r.rpop("bar") == b"one"


def test_lmove_expiry(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("bar", "two")
    r.expire("bar", 10)
...
def test_lmove_wrong_type(r: valkey.Redis):
    r.rpush("foo", "one")
    r.rpush("bar", "two")
    with pytest.raises(valkey.ResponseError):
...
def test_blmove(r: valkey.Redis):
...
@testtools.fake_only
def test_lmove_disconnected_raises_connection_error(r: valkey.Redis):
    with pytest.raises(valkey.ConnectionError):
        r.lmove(1, 2, "LEFT", "RIGHT")


def test_lpos(r: valkey.Redis):
...
def test_blmpop(r: valkey.Redis):
    r.rpush("a", "1", "2", "3", "4", "5")
    res = [b"a", [b"1", b"2"]]
