...
def test_hstrlen_missing(r: valkey.Redis):
...
def test_hstrlen(r: valkey.Redis):
    r.hset("foo", "key", "value")
    assert r.hstrlen("foo", "key") == 5


def test_hset_then_hget(r: valkey.Redis):
    assert r.hset("foo", "key", "value") == 1
    assert r.hget("foo", "key") == b"value"


def test_hset_update(r: valkey.Redis):
    assert r.hset("foo", "key", "value") == 1
    assert r.hset("foo", "key", "value") == 0


def test_hset_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hset("foo", "key", "value")


def test_hgetall(r: valkey.Redis):
...
def test_hgetall_empty_key(r: valkey.Redis):
    assert r.hgetall("foo") == {}


def test_hgetall_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hgetall("foo")


def test_hexists(r: valkey.Redis):
    r.hset("foo", "bar", "v1")
...
def test_hexists_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hexists("foo", "key")


def test_hkeys(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    r.hset("foo", "k2", "v2")
...
def test_hkeys_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hkeys("foo")


def test_hlen(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    r.hset("foo", "k2", "v2")
    assert r.hlen("foo") == 2


def test_hlen_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hlen("foo")


def test_hvals(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    r.hset("foo", "k2", "v2")
...
def test_hvals_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hvals("foo")


def test_hmget(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    r.hset("foo", "k2", "v2")
    r.hset("foo", "k3", "v3")
    # Normal case.
...
def test_hmget_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hmget("foo", "key1", "key2")


def test_hdel(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    r.hset("foo", "k2", "v2")
    r.hset("foo", "k3", "v3")
...
def test_hdel_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hdel("foo", "key")


def test_hincrby(r: valkey.Redis):
...
def test_hincrby_with_no_starting_value(r: valkey.Redis):
    assert r.hincrby("foo", "counter") == 1
    assert r.hincrby("foo", "counter") == 2
    assert r.hincrby("foo", "counter") == 3


def test_hincrby_with_range_param(r: valkey.Redis):
    assert r.hincrby("foo", "counter", 2) == 2
    assert r.hincrby("foo", "counter", 2) == 4
    assert r.hincrby("foo", "counter", 2) == 6


def test_hincrby_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hincrby("foo", "key", 2)


def test_hincrbyfloat(r: valkey.Redis):
...
def test_hincrbyfloat_with_no_starting_value(r: valkey.Redis):
    assert r.hincrbyfloat("foo", "counter") == 1.0
    assert r.hincrbyfloat("foo", "counter") == 2.0
    assert r.hincrbyfloat("foo", "counter") == 3.0


def test_hincrbyfloat_with_range_param(r: valkey.Redis):
...
def test_hincrbyfloat_on_non_float_value_raises_error(r: valkey.Redis):
    r.hset("foo", "counter", "cat")
    with pytest.raises(valkey.ResponseError):
        r.hincrbyfloat("foo", "counter")


def test_hincrbyfloat_with_non_float_amount_raises_error(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        r.hincrbyfloat("foo", "counter", "cat")


def test_hincrbyfloat_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hincrbyfloat("foo", "key", 0.1)


def test_hincrbyfloat_precision(r: valkey.Redis):
    x = 1.23456789123456789
    assert r.hincrbyfloat("foo", "bar", x) == x
    assert float(r.hget("foo", "bar")) == x


def test_hsetnx(r: valkey.Redis):
    assert r.hsetnx("foo", "newkey", "v1") == 1
    assert r.hsetnx("foo", "newkey", "v1") == 0
    assert r.hget("foo", "newkey") == b"v1"


def test_hmset_empty_raises_error(r: valkey.Redis):
    with pytest.raises(valkey.DataError):
        r.hmset("foo", {})


@testtools.run_test_if_redispy_ver("lte", "4.6")
def test_hmset_redispy4(r: valkey.Redis):
    r.hset("foo", "k1", "v1")
    assert r.hmset("foo", {"k2": "v2", "k3": "v3"}) is True


def test_hmset_wrong_type(r: valkey.Redis):
    r.zadd("foo", {"bar": 1})
    with pytest.raises(valkey.ResponseError):
        r.hmset("foo", {"key": "value"})


def test_empty_hash(r: valkey.Redis):
    r.hset("foo", "bar", "baz")
    r.hdel("foo", "bar")
    assert not r.exists("foo")


def test_hset_removing_last_field_delete_key(r: valkey.Redis):
    r.hset(b"3L", b"f1", b"v1")
    r.hdel(b"3L", b"f1")
    assert r.keys("*") == []


def test_hscan(r: valkey.Redis):
    # Set up the data
    name = "hscan-test"
    for ix in range(20):
        k = "key:%s" % ix
...
def test_hrandfield(r: valkey.Redis):
