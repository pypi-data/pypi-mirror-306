...
def test_cms_create(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_cms_incrby(r: valkey.Redis):
    assert r.cms().initbydim("cmsDim", 100, 5)
    assert r.cms().initbyprob("cmsProb", 0.01, 0.01)
    ...
    with pytest.raises(valkey.exceptions.ResponseError, match="CMS: key does not exist"):
        r.cms().incrby("noexist", ["foo", "bar"], [3, 4])

    with pytest.raises(valkey.exceptions.ResponseError, match="CMS: Cannot parse number"):
        r.cms().incrby("cmsDim", ["foo", "bar"], [3, "four"])


@pytest.mark.unsupported_server_types("dragonfly")
def test_cms_merge(r: valkey.Redis):
    assert r.cms().initbydim("cmsDim", 100, 5)
    ...
    with pytest.raises(valkey.exceptions.ResponseError, match="CMS: wrong number of keys"):
        r.cms().merge("cms2", 0, ["cmsDim"])

    with pytest.raises(valkey.exceptions.ResponseError, match="wrong number of arguments for '.*' command"):
        r.cms().merge("cms2", 1, [])

    with pytest.raises(valkey.exceptions.ResponseError, match="CMS: wrong number of keys/weights"):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_cms_info(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_cms_merge_fail(r: valkey.Redis):
    assert r.cms().initbydim("A", 1000, 5)
    assert r.cms().initbydim("B", 1000, 5)
    assert r.cms().initbydim("C", 1000, 5)
