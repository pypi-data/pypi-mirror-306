...
def test_add_ts_close(r: valkey.Redis):
    ts1 = r.ts().add(5, "*", 1)
    time.sleep(0.001)
    ts2 = r.ts().add(5, "*", 1)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_key_exist(r: valkey.Redis):
    assert r.ts().create(1)
    with pytest.raises(valkey.ResponseError) as e:
        r.ts().create(1)
    assert str(e.value) == msgs.TIMESERIES_KEY_EXISTS


@pytest.mark.unsupported_server_types("dragonfly")
def test_create_bad_duplicate_policy(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError) as e:
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create(r: valkey.Redis):
    assert r.ts().create(1)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_duplicate_policy(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_alter_diplicate_policy(r: valkey.Redis):
    assert r.ts().create(1)
    info = r.ts().info(1)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_add_before_retention(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_add_before_last(r: valkey.Redis):
    r.ts().create("time-serie-1", retention_msecs=1000)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_add_duplicate_policy(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_madd(r: valkey.Redis):
    r.ts().create("a")
    assert [1, 2, 3] == r.ts().madd([("a", 1, 5), ("a", 2, 10), ("a", 3, 15)])


@pytest.mark.unsupported_server_types("dragonfly")
def test_incrby_decrby(r: valkey.Redis):
    for _ in range(100):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_and_delete_rule(r: valkey.Redis):
    # test rule creation
    time = 100
    r.ts().create(1)
    r.ts().create(2)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_del_range(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError) as e:
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_range_advanced(r: valkey.Redis):
    for i in range(100):
        r.ts().add(1, i, i % 7)
        r.ts().add(1, i + 200, i % 7)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_range_latest(r: valkey.Redis):
    timeseries = r.ts()
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_range_bucket_timestamp(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_range_empty(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.add("t1", 15, 1)
    timeseries.add("t1", 17, 4)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_rev_range(r: valkey.Redis):
    for i in range(100):
        r.ts().add(1, i, i % 7)
    assert 100 == len(r.ts().range(1, 0, 200))
    for i in range(100):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_revrange_latest(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.create("t2")
    timeseries.createrule("t1", "t2", aggregation_type="sum", bucket_size_msec=10)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_revrange_bucket_timestamp(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.add("t1", 15, 1)
    timeseries.add("t1", 17, 4)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_revrange_empty(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.add("t1", 15, 1)
    timeseries.add("t1", 17, 4)
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_mrange(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_multi_range_advanced(r: valkey.Redis):
    r.ts().create(1, labels={"Test": "This", "team": "ny"})
    r.ts().create(2, labels={"Test": "This", "Taste": "That", "team": "sf"})
    for i in range(100):
        r.ts().add(1, i, i % 7)
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_mrange_latest(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_multi_reverse_range(r: valkey.Redis):
    r.ts().create(1, labels={"Test": "This", "team": "ny"})
    r.ts().create(2, labels={"Test": "This", "Taste": "That", "team": "sf"})
    for i in range(100):
        r.ts().add(1, i, i % 7)
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_mrevrange_latest(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.create("t2", labels={"is_compaction": "true"})
    timeseries.create("t3")
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_get_latest(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.create("t2")
    timeseries.createrule("t1", "t2", aggregation_type="sum", bucket_size_msec=10)
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_mget_errors(r: valkey.Redis):
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_mget_latest(r: valkey.Redis):
    timeseries = r.ts()
    timeseries.create("t1")
    timeseries.create("t2", labels={"is_compaction": "true"})
    timeseries.createrule("t1", "t2", aggregation_type="sum", bucket_size_msec=10)
...
@pytest.mark.unsupported_server_types("dragonfly")
def testInfoDuplicatePolicy(r: valkey.Redis):
    r.ts().create(1, retention_msecs=5, labels={"currentLabel": "currentData"})
    info = r.ts().info(1)
    assert info.get("duplicate_policy") is None
...
@pytest.mark.onlynoncluster
@pytest.mark.unsupported_server_types("dragonfly")
def test_query_index(r: valkey.Redis):
    r.ts().create(1, labels={"Test": "This"})
    r.ts().create(2, labels={"Test": "This", "Taste": "That"})
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_uncompressed(r: valkey.Redis):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_rule_green(r: valkey.Redis):
    r.ts().create(1)
    r.ts().create(2)
    r.ts().createrule(1, 2, "avg", 100)
    for i in range(50):
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_rule_bad_aggregator(r: valkey.Redis):
    r.ts().create(1)
    r.ts().create(2)
    with pytest.raises(valkey.ResponseError) as e:
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_rule_key_not_exist(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError) as e:
        r.ts().createrule(1, 2, "avg", 100)
    assert str(e.value) == msgs.TIMESERIES_KEY_DOES_NOT_EXIST
...
@pytest.mark.unsupported_server_types("dragonfly")
def test_create_rule_with_rule_to_dest_key_exists(r: valkey.Redis):
    r.ts().create(1)
    r.ts().create(2)
    r.ts().createrule(1, 2, "avg", 100)
    with pytest.raises(valkey.ResponseError) as e:
