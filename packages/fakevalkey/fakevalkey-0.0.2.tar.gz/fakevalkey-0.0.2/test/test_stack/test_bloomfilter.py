...
def test_bf_add(r: valkey.Redis):
...
def test_bf_madd(r: valkey.Redis):
...
def test_bf_card(r: valkey.Redis):
...
def test_bf_exists(r: valkey.Redis):
    assert r.bf().madd("key", "v1", "v2", "v3") == [1, 1, 1]
...
def test_bf_mexists(r: valkey.Redis):
    assert r.bf().madd("key", "v1", "v2", "v3") == [1, 1, 1]
...
def test_bf_reserve(r: valkey.Redis):
    assert r.bf().reserve("bloom", 0.01, 1000)
    assert r.bf().reserve("bloom_ns", 0.01, 1000, noScale=True)
    with pytest.raises(valkey.exceptions.ResponseError, match=msgs.NONSCALING_FILTERS_CANNOT_EXPAND_MSG):
        assert r.bf().reserve("bloom_e", 0.01, 1000, expansion=1, noScale=True)
    with pytest.raises(valkey.exceptions.ResponseError, match=msgs.ITEM_EXISTS_MSG):
        assert r.bf().reserve("bloom", 0.01, 1000)


@pytest.mark.unsupported_server_types("dragonfly")
def test_bf_insert(r: valkey.Redis):
    ...
    with pytest.raises(valkey.exceptions.ResponseError, match=msgs.NOT_FOUND_MSG):
