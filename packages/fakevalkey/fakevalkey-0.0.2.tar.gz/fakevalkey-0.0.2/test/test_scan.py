...
def test_sscan_delete_key_while_scanning_should_not_returns_it_in_scan(r: valkey.Redis):
    size = 600
    name = "sscan-test"
...
def test_hscan_delete_key_while_scanning_should_not_returns_it_in_scan(r: valkey.Redis):
    size = 600
    name = "hscan-test"
...
def test_scan_delete_unseen_key_while_scanning_should_not_returns_it_in_scan(r: valkey.Redis):
    size = 30
    all_keys_dict = key_val_dict(size=size)
...
# def test_scan_delete_seen_key_while_scanning_should_return_all_keys(r: valkey.Redis):
#     size = 30
#     all_keys_dict = key_val_dict(size=size)
#     assert all(r.set(k, v) for k, v in all_keys_dict.items())
#     assert len(r.keys()) == size
...
def test_scan_add_key_while_scanning_should_return_all_keys(r: valkey.Redis):
    size = 30
    all_keys_dict = key_val_dict(size=size)
    assert all(r.set(k, v) for k, v in all_keys_dict.items())
    assert len(r.keys()) == size
...
def test_scan_single(r: valkey.Redis):
    r.set("foo1", "bar1")
    assert r.scan(match="foo*") == (0, [b"foo1"])


def test_scan_iter_single_page(r: valkey.Redis):
    r.set("foo1", "bar1")
    r.set("foo2", "bar2")
...
def test_scan_multiple_pages_with_count_arg(r: valkey.Redis):
    all_keys = key_val_dict(size=100)
    assert all(r.set(k, v) for k, v in all_keys.items())
    assert set(r.scan_iter(count=1000)) == set(all_keys)


def test_scan_all_in_single_call(r: valkey.Redis):
    all_keys = key_val_dict(size=100)
    assert all(r.set(k, v) for k, v in all_keys.items())
...
@pytest.mark.slow
def test_scan_expired_key(r: valkey.Redis):
...
def test_scan_stream(r: valkey.Redis):
