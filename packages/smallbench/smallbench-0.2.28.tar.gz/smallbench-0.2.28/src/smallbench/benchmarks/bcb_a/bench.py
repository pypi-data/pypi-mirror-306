...
from typing import Any, Callable, Dict, List, Literal, Type, Tuple, Optional
import copy
...
class BCB_AgentBenchmark(AgentBenchmark):
    ...
    async def evaluate_async(
        ...
        if verbose:
            ...
            pbar = tqdm(total=max_agent_steps, desc="Agent steps")
        ...
        for _ in range(max_agent_steps):
            ...
            action_args = None
            try:
                action, action_args = await agent.act_async(aci)
            except Exception as e:
                result = f"Error rendering action and action args: {e}. These were the action and action args: {action}, {action_args}"
...
"""
    ...
    def evaluate_sync(
        ...
        for _ in range(max_agent_steps):
            action, action_args = agent.act_sync(aci)
            try:
                result = aci.accept_delta_sync(action, action_args)
            except Exception as e:
                result = f"""Action failed. Information: 
