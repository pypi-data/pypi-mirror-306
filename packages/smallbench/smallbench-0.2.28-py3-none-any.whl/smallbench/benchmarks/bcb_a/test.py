...
def get_contexts_extremely_hacky_please_fix():
    ...
    contexts = {
        ...
        "constraints": "You will be given a code_prompt_for_answer, which contains imports and the function signature. Your solution must comprise code that can be appended to code_prompt_for_answer and run as a single script. If the action_arg_spec for an action is \{\}, that means there are no action arguments - provide an empty list of argument dictionaries.",
    }
    return contexts
...
def score_agent_sync(
    ...
    agent = SimpleReActLanguageAgent(
        lm=LM(model_name), contexts=contexts_for_agent, multi_threaded=True
    )
...
async def score_agent_async(
    ...
    agent = SimpleReActLanguageAgent(
        lm=LM(model_name), contexts=contexts_for_agent, multi_threaded=False
    )
...
from src.smallbench.benchmarks.bcb_a.bench import VanillaRolloutOrchestrator 

async def score_agent_rollout(
    contexts_for_agent: List[Dict[str, Any]],
    model_name: str,
    index: int = 0,
):
    agent = SimpleReActLanguageAgent(
        lm=LM(model_name,formatting_model_name=model_name, temperature=0.0), contexts=contexts_for_agent, multi_threaded=False
    )
    agent_benchmark = BCB_AgentBenchmark(backend="modal")
    aci = BCBAgentComputerInterface(BCBEngine(BigCodeBenchComplete_Benchmark().train[index], backend="modal", use_persistent_container=False), synchronous=False)
    rollout = VanillaRolloutOrchestrator()
    t0 = time.time()
    result, submission = await rollout.evaluate_async(agent, aci)
    t1 = time.time()
    print(f"Time taken: {t1-t0} seconds")
    print(f"Score for {model_name}: " + str(result))
