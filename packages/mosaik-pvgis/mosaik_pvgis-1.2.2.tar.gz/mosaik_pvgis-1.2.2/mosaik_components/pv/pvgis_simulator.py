...
class PVGISSimulator(mosaik_api_v3.Simulator):
    ...
    def init(self, sid: str, time_resolution: float = 1, step_size: int = STEP_SIZE, sim_params: Dict = {}):
        ...
        self.start_date = arrow.get(sim_params.get('start_date', '2016-01-01 00:00:00'), DATE_FORMAT)
    ...
    def create(self, num: int, model: str, **model_params: Any) -> List[CreateResult]:
        ...
        for n in range(len(self.entities), len(self.entities) + num):
            ...
            production.index = pd.to_datetime(production.index, utc=True) +\
                        pd.offsets.DateOffset(years=self.start_date.year - production.index[0].year) # change history year to current one
    ...
    def get_production(self, eid, attr):
        idx = self.entities[eid][0].index.get_indexer([self.current_date.datetime], method='ffill')[0]
        if attr == 'P[MW]':
    ...
    def step(self, time, inputs, max_advance):
        self.current_date = self.start_date.shift(seconds=time*self.time_resolution)
