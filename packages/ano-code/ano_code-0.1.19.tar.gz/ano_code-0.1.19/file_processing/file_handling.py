...
def process_directory(directory)-> str:
    ...
    fl = {".py", ".js", ".go", ".ts", ".tsx", ".jsx", ".dart", "php"}
    
    code = ""
    for root, dirs, files in os.walk(directory):
        if ".gitignore" in os.path.join(root, filename):
            ig_content = process_file(os.path.join(root, filename))
            folders_to_ignore += ig_content
