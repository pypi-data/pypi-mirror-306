...
class ServiceOptions(BaseModel):
    ...
    max_concurrent_workflow_runs: int = Field(default=3000)
    max_concurrent_function_runs: int = Field(default=1000)

class ServiceInput(BaseModel):
    task_queue: str = Field(default='restack')
    workflows_path: Optional[str] = Field(default=None)
...
class Restack(BaseModel):
    ...
    async def create_service(self, service_input: ServiceInput) -> Worker:
        try:
            ...
            service = Worker(
                ...
                client=client,
                task_queue=service_input.task_queue,
                workflows=service_input.workflows_path or [],
                activities=service_input.functions,
                max_activities_per_second=service_input.options.rate_limit,
                max_concurrent_activities=service_input.options.max_concurrent_function_runs,
                max_concurrent_workflow_tasks=service_input.options.max_concurrent_workflow_runs,
            )
    ...
    async def run_service(self, service: Worker):
        try:
            ...
            logger.info("Service was cancelled. Shutting down gracefully...")
        except Exception as e:
            logger.error("Failed to run service", meta={"error": e})
            raise e
        finally:
    ...
    async def start_service(
        ...
        Parameters:
        - workflows_path (Optional[str]): A list of workflows to be used.
        ...
        service = await self.create_service(service_input)
    ...
    async def send_workflow_event(self, event: Dict[str, Any], workflow: Dict[str, str]):
        if self.client:
            try:
                workflow_id, run_id = workflow['workflowId'], workflow['runId']
                name, input = event['name'], event['input']
                handle = self.client.get_workflow_handle(workflow_id, run_id)
                return await handle.signal(name, input)
            except Exception as e:
