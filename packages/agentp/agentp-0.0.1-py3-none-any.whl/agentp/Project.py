...
## imported for Genotype TWAS class if needed:
...
class Project: 
    ...
    def __init__(self, projectID_dir): 
        self.path = f'{os.getcwd()}/{projectID_dir}'
        self.name = projectID_dir
        if os.path.exists(self.path): 
            msg = f'{MSG}Project already exists. Results will be saved to:\n{TAB}{self.path}'
            print(msg)
        else:
            os.mkdir(self.path)
            os.mkdir(f'{self.path}/input_data')
        ...
        self.output_paths = {'gwas': f'{self.path}/GWAS', \
                             ...
                             'gtwas': f'{self.path}/gTWAS', \
                             'grex': f'{self.path}/GREX', \
                             'webg': f'{self.path}/enrichment'}

        ## get ordered array of subjects (for gTWAS) if exists
        try: 
    ...
    '''
    Init additional paths for Genotype TWAS. 
    '''
    def set_genotype_twas_paths(self, twas_approach, grex_region): 
        ...
        twas_dir = self.set_twas_paths('gtwas', twas_approach, grex_region)
        if twas_approach == 'FUS':
