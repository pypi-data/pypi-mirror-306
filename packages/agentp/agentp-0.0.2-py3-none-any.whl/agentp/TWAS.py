...
## Note: imported in GenotypeTWAS class if needed:
...
class GenotypeTWAS(TWAS):
    def __init__(self, project, twas_approach, grex_region): 
        super().__init__(project, twas_approach, grex_region)
        self.framework = 'genotype'

        paths = project.set_genotype_twas_paths(twas_approach, grex_region)
    ...
    '''
    Run gTWAS on one phenotype. 
    TODO: decide how to communicate TWAS logs
    '''
    def _run_phenotype_twas(self, phen, overwrite):
        output_twas_file = f'{self.output_dir}/{phen}.csv'        
        if os.path.exists(output_twas_file) and (not overwrite): 
            msg = f'{MSG}Skipping gTWAS on "{phen}" using {self.approach} {self.grex_region} models - already exists'
            print(msg) 
            return

        msg = f'{MSG}Running gTWAS on "{phen}" using {self.approach} {self.grex_region} models...'
        print(msg)
    ...
    def run_twas(self, phens, overwrite=False, num_threads=1): 
        if self.covar_cols is None: 
            try:
                ...
                msg = f'{MSG}gTWAS ERROR: Add subject-level covariates to project first'
                sys.exit(msg) 
