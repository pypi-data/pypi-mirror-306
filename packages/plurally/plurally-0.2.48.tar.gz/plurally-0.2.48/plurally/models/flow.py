    ...
    def __del__(self):
        if self.worker:
            ...
            worker = Worker.find_by_key(
                self.worker, connection=Redis.from_url(redis_url)
            )
            if not worker:
        ...
        for job_id in failed_job_ids:
            ...
            if job:
                ...
                if res and res.type == res.Type.FAILED:
                    exc_val = re.search(r"RuntimeError: (.*)", res.exc_string).group(1)
                    try:
                    ...
                    for src_node, tgt_node, attrs in graph.in_edges(item, True):
                        ...
                        if utils.is_list_type(
                            ...
                            os.environ.get("VERBOSE") and print(
                                f"Adding list input for {tgt_handle=}: {src_value}"
                            )
                        else:
                            kwargs[tgt_handle] = src_value
                            os.environ.get("VERBOSE") and print(
                                f"Adding non list input for {tgt_handle=}: {src_value}"
                            )
