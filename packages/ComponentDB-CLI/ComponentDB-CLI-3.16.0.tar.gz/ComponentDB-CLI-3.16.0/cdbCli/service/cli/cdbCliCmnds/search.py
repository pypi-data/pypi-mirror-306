    ...
    while interactive:
        ...
        while True:
            ...
            if item_selection is not None:                
                cdbInfo_result, header_style = cdbInfo_helper(factory=factory, item_id=item_selection.object_id, all=all_opt, format=format)
                if isinstance(cdbInfo_result, Exception):
                    # Standard CLI exception handled and displayed to user. 
                    exit(1)
                cliBase.print_results(result_obj=cdbInfo_result, console=console, format=format, header_style=header_style, pager=pager)
    else:
