...
def pack_data_into_time_slots(
    ...
    if not sorted_data:
        ...
        lengths = (
            backend.sort(lengths)[::-1]
            if backend.type != "torch"
            else backend.sort(lengths, descending=True)[0]
        )
...
def find_dominant_type(lst, raise_error: bool = True):
    ...
    if isinstance(lst, list | tuple):
        curr_val: type[bool] | type[int] | type[float] | type[None] = bool
        for elem in lst:
            ...
            if val is float:
                ...
                break
        return curr_val
    elif isinstance(lst, bool | float | int | None):
        return type(lst)
