...
from . import utils
from .common_functions import ArrayType, flatten
...
# Ops
def abs(input: ArrayType) -> ArrayType:
    return mx.abs(input)
...
def sqrt(input) -> ArrayType:
    return mx.sqrt(input)


def log(input) -> ArrayType:
    return mx.log(input)


def sign(input) -> ArrayType:
...
def variance(
    ...
    *,
    ...
    return mx.var(input, axis=axis, ddof=correction, keepdims=keepdim)
...
def conv2d_bias(
    ...
    *,
    ...
    padding = utils.squeeze_padding(padding)
    return (
