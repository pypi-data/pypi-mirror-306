...
class Model(BaseModel):
        ...
        for pair in pairs:
            ...
            if new_name is None:  # Non-named connections.
                ...
                data: Tensor | Scalar = (
                    ...
                    else Tensor(metadata.data.shape, metadata.data._type, PARAM, None)
                )
    ...
    def _set_value(self, key: ConnectionData, value: MainValueType) -> Updates:
        if (
            isinstance(key.metadata.data, Tensor)
            and value is not DATA
            and value is not PARAM
        ):
    ...
    def _check_multi_write(
        ...
        if isinstance((data := local_connection.metadata.data), Scalar):
            ...
            # TODO: should we include PARAM for the check below
            # e.g. (check_data.value is not PARAM)
            if check_data.value is not DATA and check_data.value != other.value:
    ...
    def _unroll_template(
        ...
        if template.output_connection is None:
            ...
            default_args_dict = {
                key: DATA for key in default_args if key not in template.defaults
            }
            ...
            for local_key, outer_con in zip(
                ...
                if isinstance(outer_con, MainValueType) and outer_con is not DATA:
    ...
    def handle_auto_conversion(
        ...
        if isinstance(connection, MainValueType) and connection is not DATA:
        ...
        # ):
            ...
            is_value_given = not isinstance(
                set_value, ParameterObject | DataObject | NullConnection
            )
            ...
            if is_value_given:
                ...
                data = Tensor(shape_node, set_type, PARAM, None)
            ...
            if isinstance(connection.key, IOKey):
                ...
                elif connection.key._value not in (DATA, NOT_GIVEN):
                    data_type = Scalar
                    is_conn_input = True
        ...
        **kwargs: ConnectionType,
        ...
        for input_key in model._input_keys:
            ...
            if (
                ...
                and input_data.temp_value != ...
                # and not isinstance(input_data.value, Constant)
                and input_data.value is not DATA
            ):
                if (
                    (given_value := kwargs.get(input_key)) is not None
                    and given_value not in (DATA, NOT_GIVEN)
    ...
    def _generate_keys(
        ...
        for key in sorted_inputs:
            ...
            if (
                ...
                assert isinstance(key_origin, str), "key origin must be a string"
            ...
            for model, model_name in name_mappings.items():
                ...
                for inner_key in input_keys + tuple(model.conns.output_keys):
                    ...
                    if (
                        ...
                        and (val := key_data.value) is not DATA
                    ):
                        conn.append(str(val))
