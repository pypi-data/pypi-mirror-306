...
def test_1_set_shapes_bug():
    ...
    shapes = {"input": [120, 120], "w_0": [None, 32], "w_1": [32, 32], "b_1": [None]}
    comp_model = mithril.compile(
...
def test_2_set_shapes_bug():
    ...
    linear1.set_shapes({"input": [120, 120], "w": [None, 32]})
    linear2.set_shapes({"w": [32, 32], "b": [None]})
...
def test_1_solve_constraint_extend():
    model = Model()
    c1 = Convolution2D(3)
    c1.set_shapes({"input": [8, 3, 224, 224], "kernel": [16, 3, None, None]})
    model += c1
...
def test_pickle_empty_backend():
    ...
    comp_model_3 = mithril.compile(model=ctx, backend=torch_backend, safe=False)
    ...
    comp_model_6 = mithril.compile(
        model=ctx, backend=unpickled_torch_backend, safe=False
    )
...
def test_reuse_pickled_registered_backend():
    ...
    class MyAdder(PrimitiveModel):
        ...
        def __call__(self, left, right, output):
...
def test_logical_model_compile_twice():
    ...
    static_keys = {"input": jnp.array([[1.0]]), "target": jnp.array([0])}
    ...
    static_keys = {"input": torch.tensor([[1.0]]), "target": torch.tensor([0])}
...
def test_relational_operators_ignored_1():
    ...
    model = compile(model, NumpyBackend(), safe=False, inference=True)
    assert "yoyoyo" in model.ignore_grad_keys
...
def test_relational_operators_ignored_3():
    ...
    model = compile(model, NumpyBackend(), safe=False, inference=True)
    assert (
        "relational_out" in model.ignore_grad_keys
        and "ignore_this" in model.ignore_grad_keys
    )


def test_arange_primitive():
    backends = [JaxBackend, TorchBackend, NumpyBackend, MlxBackend]
    precisions = [32, 64]
    for backend in backends:
        ...
        for precision in precisions:
            ...
            output = model.evaluate(params, data)
...
def test_flatten_dag0():
    ...
    model += l1(input=IOKey(name="", value=PARAM), w="w_2")
    model += Linear(10)(input=IOKey(name="", value=PARAM))
    model += Linear(10)(input=IOKey(name="", value=PARAM))
    model += Linear(10)(input=IOKey(name="", value=PARAM))
    model += l5(input=IOKey(name="", value=PARAM), output=IOKey(name="output1"))
...
def test_static_concat():
    ...
    assert all(
        pm.evaluate()["output"] == backend.array([0.0, 0.0], dtype=backend.float32)
    )
...
def test_reduce_overlap_shapes_1():
    ...
    relu_model_1_1.set_shapes({"input": ["u1", "u2", ("Var1", ...)]})
...
def test_prune_duplicate_grad():
    ...
    backend.set_seed(10)
    params = model.randomize_params()
    res = model.evaluate_gradients(
        params=params,
        data={"input1": input1, "input2": input2},
        output_gradients={model.output_keys[0]: grads},
    )
...
def test_prune_tensor_match():
    ...
    assert model.data["output1"] == model.data["output2"] == model.data["output3"]
...
def test_arange_1():
    ...
    backends = [TorchBackend, JaxBackend, NumpyBackend, MlxBackend]
    for backend in backends:
        if backend.is_installed:
            backend = backend(precision=32)
            cm = compile(
...
def test_empy_out_grad():
    ...
    params = comp_model.randomize_params()
    target = backend.ones(1, dtype=backend.int32)
...
def geomean_multigpu_test():
    ...
    target1 = backend.ones(40, 6, 8, 16, 32, 32, dtype=backend.float32) / 3.3
    target2 = backend.ones(40, 6, 8, 16, 32, 32, dtype=backend.float32) / 3.3
...
def test_to_tensor():
    ...
    # Test for torch
    pm = compile(model, TorchBackend(precision=64), safe=False)
    result = pm.evaluate({}, {"input": input1})["output"]
    expected = torch.tensor(input1, dtype=torch.float64)
    np.testing.assert_allclose(result, expected, 1e-12)

    result = pm.evaluate({}, {"input": input2})["output"]
    expected = torch.tensor(input2, dtype=torch.bool)
    assert (result == expected).all()

    # Test for Jax
    pm = compile(model, JaxBackend(precision=64), safe=False, jit=False)
    result = pm.evaluate({}, {"input": input1})["output"]
    ...
    if platform.system() == "Darwin":
        pm = compile(model, MlxBackend(precision=32), safe=False)
        result = pm.evaluate({}, {"input": input1})["output"]
        expected = mx.array(input1, mx.float32)
        np.testing.assert_allclose(result, expected, 1e-6)  # type: ignore
    ...
    # Test for Numpy
    pm = compile(model, NumpyBackend(precision=64), safe=False, jit=False)
    result = pm.evaluate({}, {"input": input1})["output"]
    expected = np.array(input1, np.float64)
    np.testing.assert_allclose(result, expected, 1e-12)

    result = pm.evaluate({}, {"input": input2})["output"]
    expected = np.array(input2, dtype=np.bool_)
    assert (result == expected).all()
...
def test_leaky_relu_trainable_slope():
    backend = JaxBackend()
    model = Model()
    model += LeakyRelu(slope=PARAM)(input="input", output="output", slope="slope")
