...
from mithril.framework.common import PARAM, Connection, Table, _get_summary_shapes
...
def test_physical_summary_2():
    ...
    model.set_values({model.canonical_input: PARAM})
    comp_model = mithril.compile(
...
def test_physical_model_summary_5():
    ...
    divide = Divide()
    exp = Power()
    add.set_shapes({"left": ["u1", "u2"], "right": [1]})
    divide.set_shapes({"numerator": ["u3", "u4"], "denominator": [1]})
    exp.set_shapes({"base": ["u3", "u4"], "exponent": [1]})
    model += add
    model += divide
    model += exp
    comp_model = mithril.compile(model=model, backend=JaxBackend(), safe=False)
...
def test_physical_model_summary_8():
    ...
    another_random_model.set_shapes({"input1": ["a", ("Var1", ...), "b"]})
    random_kernel_model += (add1 := Add())(left="input1", right="input2")
    random_kernel_model += (relu1 := Relu())(input=add1.output)
