...
from mithril import PARAM, JaxBackend, compile
...
def test_2():
    ...
    model_1 = Model()
    lin = Linear(2)
    model_1 += lin(input="input", w="w_1", b="b_1")
    model_1 += Linear(4)(input=lin.output, w="w", b="b", output="output")
    model_1.set_outputs(output1=lin.output)
    model_1.set_values({"input": PARAM})

    model_2 = Model()
    model_2 += Linear(2)(input="input", w="w_1", b="b_1", output=IOKey("output1"))
    model_2 += Linear(4)(input=model_2.output1, w="w", b="b", output="output")  # type: ignore
    model_2.set_values({"input": PARAM})
