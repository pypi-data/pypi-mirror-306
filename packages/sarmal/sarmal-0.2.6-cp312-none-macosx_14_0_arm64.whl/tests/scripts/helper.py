...
from mithril import PARAM, Backend, Constant, compile, epsilon_table
from mithril.models import BaseModel, Model, NotAvailable, Scalar, Tensor
...
def evaluate_case(
    ...
    models = []
    m = finalize_model(current_case)
    for key in reference_gradients:
        m.set_values({key: PARAM})
    models.append(m)
    if test_rtt:
        model_dict = model_to_dict(models[0])
        m = dict_to_model(model_dict)
        for key in reference_gradients:
            m.set_values({key: PARAM})
        models.append(m)
        ...
        for data in all_data:
            ...
            if copied_data and copied_data not in unused_data:
                ...
                if isinstance((data_value := data.value), Constant):
                    data_value = epsilon_table[64][data_value]
...
def assert_models_equal(model1: BaseModel, model2: BaseModel):
    ...
    if (
        not isinstance(model1.canonical_input, NotAvailable)
        and not isinstance(model2.canonical_input, NotAvailable)
        and not isinstance(model1.canonical_output, NotAvailable)
        and not isinstance(model2.canonical_output, NotAvailable)
    ):
