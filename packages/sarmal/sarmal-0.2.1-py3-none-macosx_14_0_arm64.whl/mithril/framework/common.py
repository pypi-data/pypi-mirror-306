...
class Variadic:
    ...
    def update_possible_values(self, *possibles: PossibleValues) -> Updates:
        ...
        if self.possibles is None:
            ...
            # prev_pos_info = {
            #     _len: len(pos_val.dnf_list)
            #     for _len, pos_val in self.possibles.items()
            # }
            _possibles = {}
    ...
    def update_possibilities(self, lengths: set[int] | None = None) -> Updates:
        # TODO: remove variadic from uniadics vars_dict if uniadic is removed
        updates = Updates()
        if self.possibles is None:
            return updates
