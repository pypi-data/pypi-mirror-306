...
def relu(input: np.ndarray, cache: CacheType | None = None) -> np.ndarray:
    return common_functions.relu(input)


def leaky_relu(input: np.ndarray, slope: np.ndarray, cache=None):
    return common_functions.leaky_relu(input=input, slope=slope)


def tanh(input: np.ndarray, cache: CacheType | None = None) -> np.ndarray:
    return common_functions.tanh(input)


def sigmoid(input: np.ndarray, cache: CacheType | None = None) -> np.ndarray:
    return common_functions.sigmoid(input=input)


def softplus(input: np.ndarray, cache: CacheType | None = None) -> np.ndarray:
    return common_functions.softplus(input=input)
...
def binary_cross_entropy_with_logits(
    ...
    *,
    ...
    if isinstance(pos_weight, bool):
        _pos_weight = (
            calculate_binary_class_weight(common_functions.sigmoid(target))
            if pos_weight
            else 1.0
        )
    else:
...
def flatten(
    ...
    *,
    ...
    return common_functions.flatten(input, start_dim=start_dim, end_dim=end_dim)
