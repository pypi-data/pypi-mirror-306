...
class CBackend(Backend[PyArray]):
    ...
    def empty(self, *shape: int, dtype: core.dtype | None = None) -> PyArray:
        assert dtype is None, "dtype is not supported in CBackend"
        return array.empty(shape)

    def ones(self, *shape: int, dtype: core.dtype | None = None) -> PyArray:
        assert dtype is None, "dtype is not supported in CBackend"
        return array.ones(shape)

    def zeros(self, *shape: int, dtype: core.dtype | None = None) -> PyArray:
