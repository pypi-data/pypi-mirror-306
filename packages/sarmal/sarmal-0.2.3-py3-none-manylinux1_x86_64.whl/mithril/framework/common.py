...
# TODO: could not be derived from abc.ABC because linter says `SingletonObject` is
# an abstract base class, but it has no abstract method.
...
class NullConnection(SingletonObject):
    """
    A singleton class representing a null connection to indicate that no connection is
    specified.

    It inherits from the SingletonObject class to ensure a singleton pattern.
    """
...
class DataObject(SingletonObject):
    """
    A singleton class representing a state of a connection that will not be trained.
...
class ParameterObject(SingletonObject):
    """
    A singleton class representing a state where a connection is to be trained.
...
type ScalarType = (
    ...
    | type[EllipsisType]
    | type[DataObject]
    # TODO: Do we need to add DataObject here?
    # If yes We should add ParameterObject also.
    | NestedListType
    | type[None]
    | UnionType
    | GenericAlias
...
ReduceType = int | tuple[int, ...] | None | DataObject
MainValueType = (
    int
    | float
    | tuple
...
class BaseData(ABC):
    ...
    @abstractmethod
    def match(self, other: Self):
        return Updates()


class Tensor(BaseData, GenericDataType[DataType]):
    # TODO: Physical Tensor could be a separated class.
    _type: type[float] | type[int] | type[bool] | UnionType
    temp_value: (
        int | float | tuple | list | None | Constant | NestedListType | EllipsisType
    )
    value: DataType | DataObject | ParameterObject
    ...
    def __init__(
        ...
        if not isinstance(value, DataObject | ParameterObject):
    ...
    @property
    def is_non_diff(self) -> bool:
        return self.temp_value != ... or self.value is not PARAM

    @property
    def is_valued(self) -> bool:
        return self.value is not PARAM and self.value is not DATA
    ...
    def match(self, other: Tensor) -> Updates:
        # TODO: Refactor match
        updates = Updates()
        if self != other:
            ...
            elif not self.is_valued and not other.is_valued:
                if PARAM in (self.value, other.value) and self._logical_data:
                    self.set_value(PARAM)
                    other.set_value(PARAM)  # TODO: Do we need to set other's value?
    ...
    def _set_logical_value(self, value: TensorValueType) -> Updates:
        if isinstance(value, DataObject | ParameterObject):
            if self.temp_value != ...:
                raise ValueError(
                    f"Value is set before as {self.temp_value}. Can not be reset."
                )
            # if self.value is DATA and value is None:
            #     raise ValueError(
            ...
            if self.temp_value != ... and self.temp_value != value:
                raise ValueError(
                    f"Value is set before as {self.temp_value}. Can not be reset."
                )
            self.temp_value = value
...
class Scalar(BaseData):
    ...
    def __init__(
        ...
        super().__init__(type=possible_types)
        # Update type if any value is given.
        if not isinstance(value, DataObject | str):
    ...
    def set_value(self, value: MainValueType) -> Updates:
        ...
        if self.value is not DATA and self.value != value and value is not DATA:
            raise ValueError(
    ...
    def match(self, other: Scalar) -> Updates:
        # TODO: Refactor match
        updates = Updates()
        if self != other:
            updates = Updates()
