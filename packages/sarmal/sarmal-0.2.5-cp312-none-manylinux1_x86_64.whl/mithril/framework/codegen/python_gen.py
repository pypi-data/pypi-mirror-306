...
class PythonCodeGen[DataType](CodeGen):
    ...
    def compute_gradients(
        ...
        *,
        ...
        if self.pm.backend.type == "mlx":
            ordered_keys = sorted(params.keys())
            partial_fn = partial(
                self._fn_list_wrapper, keys=ordered_keys, fn=partial_fn
            )
            params_list = [params[key] for key in ordered_keys]
            all_output_gradients_lst = [
                total_output_gradients[key]
                for key in self.pm.output_keys
                if key in total_output_gradients
            ]
            output, input_gradients = self.pm.backend.vjp(  # type: ignore
                partial_fn, params_list, all_output_gradients_lst
            )
            input_gradients = {
                key: input_gradients[idx] for idx, key in enumerate(ordered_keys)
            }
            output = {key: output[idx] for idx, key in enumerate(self.pm.output_keys)}
        else:
            output, vjp_fn, aux = self.pm.backend.vjp(partial_fn, params, has_aux=True)  # type: ignore
            (input_gradients,) = vjp_fn(total_output_gradients)
            output |= aux

        if include_output:
            # TODO: Fix vjp return type
            return output, input_gradients  # type: ignore
        else:
    ...
    def _fn_list_wrapper(self, *inputs, keys: list[str], fn: Callable):
        # Used for MLX to convert inputs to list
        input_dict = {}
        for key, input in zip(keys, inputs, strict=False):
            input_dict[key] = input

        output, aux = fn(input_dict)
        return [
            output[key] if key in output else self.pm.backend.stop_gradient(aux[key])
            for key in self.pm.output_keys
        ]
