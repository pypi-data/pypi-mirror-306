...
class BaseModel(abc.ABC):
    ...
    def set_types(
        self,
        types: dict[str | Connection, type | UnionType]
        | dict[Connection, type | UnionType]
        | dict[str, type | UnionType],
    ):
        """
    ...
    def _match_hyper_edges(self, left: IOHyperEdge, right: IOHyperEdge) -> Updates:
        ...
        # if (
        #     isinstance(left.data, Tensor)
        #     and isinstance(right.data, Tensor)
        #     or isinstance(left.data, Scalar)
        #     and isinstance(right.data, Scalar)
        # ):
