...
def dict_to_model(modelparams: dict[str, Any]) -> BaseModel:
    ...
    for m_key, v in submodels.items():
        ...
        for k, conn in connections[m_key].items():
            ...
            #     mappings[k] = static_values[conn] if conn in static_values else conn
            if isinstance(conn, float | int | tuple | list | str):
                mappings[k] = conn
...
def connection_to_dict(
    ...
    for key, connection in connections.items():
        ...
        if (
            ...
            elif default_input.value is DATA:
                static_values[connection.key] = "DATA"
            elif default_input.value is PARAM:
                static_values[connection.key] = "PARAM"
            else:
