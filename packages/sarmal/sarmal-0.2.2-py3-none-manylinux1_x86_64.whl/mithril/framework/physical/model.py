...
class PhysicalModel(GenericDataType[DataType]):
    ...
    def randomize_params(
        ...
        for key in sorted(self._input_keys):
            ...
            # Divide by 10000 causes casting to 32-bit for 16 bit backends
            normalization_factor: int = 100 if self.backend.precision == 16 else 10000
            assert is_list_int(shape)
            if isinstance(self.backend, ParallelBackend):
                device_mesh = shards.get(key, None)
                shapes[key] = (
                    self.backend.randn(*shape, device_mesh=device_mesh)
                    / normalization_factor
                )
            else:
                shapes[key] = self.backend.randn(*shape) / normalization_factor
