...
from mithril.framework.common import NOT_GIVEN, Connection, ConnectionType, Updates
...
def test_slice_item_conversions():
    ...
    model += Add()(left=item, right=slc, output="output")
    model_2 = model
...
def test_type_propagation_8():
    ...
    assert model.output.metadata.data._type is int
...
def test_type_propagation_9():
    ...
    assert add.left.metadata.data._type is int
    assert add.right.metadata.data._type is float
    assert model.output.metadata.data._type is float
...
def test_type_propagation_10():
    ...
    assert add.right.metadata.data._type == int | bool
    assert model.output.metadata.data._type == int | bool
...
def test_type_propagation_floor_divide_3():
    ...
    assert model.output.metadata.data._type == float | int
...
# @pytest.mark.skip(
#     reason=(
#         "Since Connect holds a set in it, model extension order changes, "
#         "so the DAG orders."
#     )
# )
