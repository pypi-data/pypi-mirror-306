...
def convert_sentinel_legacy_data(
    ...
    **kwargs: Any,
    ...
    if product_type_pattern_str.startswith("product/s3-legacy"):
    ...
    elif product_type_pattern_str.startswith("product/s2-legacy"):
    ...
    # Set user attrs and update it with safe attrs
    legacy_attrs = xdt.attrs
    xdt.attrs = kwargs.get("attrs", {})
    xdt.attrs.update(legacy_attrs)

    logger.info(f"open and convert {product_type_pattern} file {url!r} to datatree")
    attrs = AttributeHandler(xdt.attrs)
    attrs.fix()
    return xdt
