...
class InMemorySessionRepository(SessionRepository):
    __slots__ = ("__memory", "__user_repository")

    def __init__(self, user_repository: UserRepository) -> None:
        self.__memory = InMemoryRepository[Session](self.__post_processor)
        self.__user_repository = user_repository

    async def get(self, application_id: UUID) -> Session | None:
        return await self.__memory.get_first(
            lambda session: session.application_id == application_id,
        )

    async def save(self, session: Session) -> None:
        await self.__memory.save(session)

    async def delete(self, application_id: UUID) -> None:
        await self.__memory.delete_by(
            lambda session: session.application_id == application_id
        )
