...
from .exceptions import (
    ...
    BackendCustomError,
    EmbeddingsUploadLimitReachedException,
    HistogramsUploadLimitReachedException,
...
from .file_uploader import ImageUploadProcessor
from .logging_extensions.filters.log_record_types import (
    EMBEDDING_THROTTLING_MSG_EXTRA,
    HISTOGRAM_THROTTLING_MSG_EXTRA,
    IMAGE_THROTTLING_MSG_EXTRA,
)
...
def raise_on_upload_for_backend_error_response(
    ...
    if code == IMAGE_UPLOAD_LIMIT_REACHED:
        ...
        raise EmbeddingsUploadLimitReachedException()
    elif code == HISTOGRAM_LIMIT_REACHED:
        upload_limits_guard.histogram_upload_limit_exceeded()
        raise HistogramsUploadLimitReachedException()
...
def check_upload_limit_not_reached(
    ...
    elif options.upload_type == "embeddings":
        if upload_limits_guard.has_embedding_upload_limit_exceeded():
            LOGGER.debug("Already exceed limit of number embeddings to be uploaded")
            raise EmbeddingsUploadLimitReachedException()

    elif options.upload_type in ["histogram2d", "histogram3d"]:
        if upload_limits_guard.has_histogram_upload_limit_exceeded():
            LOGGER.debug("Already exceed limit of number histograms to be uploaded")
            raise HistogramsUploadLimitReachedException()
...
def upload_file(
    ...
    try:
        ...
        if response.status_code != HTTPStatus.OK:
            ...
            raise_on_upload_for_backend_error_response(response, upload_limits_guard)
        ...
        if isinstance(e, ImagesUploadLimitReachedException):
            LOGGER.error(
                msg=UPLOAD_LIMIT_REACHED.format(asset_type="images"),
                extra=IMAGE_THROTTLING_MSG_EXTRA,
            )
        elif isinstance(e, EmbeddingsUploadLimitReachedException):
            LOGGER.error(
                msg=UPLOAD_LIMIT_REACHED.format(asset_type="embeddings"),
                extra=EMBEDDING_THROTTLING_MSG_EXTRA,
            )
        elif isinstance(e, HistogramsUploadLimitReachedException):
            LOGGER.error(
                msg=UPLOAD_LIMIT_REACHED.format(asset_type="histograms"),
                extra=HISTOGRAM_THROTTLING_MSG_EXTRA,
            )
        elif (
