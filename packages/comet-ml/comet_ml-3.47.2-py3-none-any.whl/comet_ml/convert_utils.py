...
from ._typing import AudioNormalization, Number, StringResult
...
def validate_single_3d_point(single_3d_point: Any) -> Optional[List[Number]]:
    try:
...
def validate_and_convert_3d_points(points: Any) -> List[List[Number]]:
    if points is None:
        return []
...
def validate_single_3d_box(
    validator: Any, box: Dict[str, Any]
...
def validate_and_convert_3d_boxes(boxes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if boxes is None:
        return []
...
def image_data_to_file_like_object(
    ...
    if hasattr(image_data, "numpy"):  # pytorch tensor
        from .integration.pytorch.pytorch_helpers import is_tensor_on_gpu
