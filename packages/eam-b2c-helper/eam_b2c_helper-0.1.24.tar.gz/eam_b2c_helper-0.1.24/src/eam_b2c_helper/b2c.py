...
def build_local_user_object(user_details: dict, b2c_prefix: str, extension_id: str) -> dict:
    ...
    return {
        ...
        "department": user_details['company_name'],
...
class B2CHelper:
    ...
    def filter_feat_branch(self, users: list, branch_id: int) -> list:
        """Filters out users with the by branch id'"""
        if branch_id != 'master':

            filtered_users = []
            for user in users:
                first_split = user['identities'][0]['issuerAssignedId'].split(
                    '+')[-1]

                branch_id_extract = first_split.split('@')[0]

                if branch_id_extract == str(branch_id):
    ...
    def get_users(self, company_id: str = None, role_type: str = None, branch_id: int = None) -> list:
        ...
        users = self.filter_feat_branch(users, branch_id)
