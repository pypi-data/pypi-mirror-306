...
async def run(
    *,
    ...
    while True:
        ...
        initial_src_balance = await balances.get_balance(
        ...
        dest_w3 = await client_utility.make_w3(dest_token.chain.id)
        ...
        start_dest_balance = await balances.get_balance(
            dest_w3, dest_token, account.address
        )
        ...
        if count >= config.max_polls:
            ...
            src_token, src_w3, src_order_book_contract = (
                dest_token,
                dest_w3,
                client_utility.make_order_book_contract(dest_w3, dest_token),
            )
