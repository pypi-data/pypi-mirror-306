...
class Retrievable(Resource):
    ...
    @classmethod
    def retrieve_live_keys(cls, id: str, env: str) -> Resource:
        if cls._resource != "organizations":
            raise Exception("Resource not allowed")
        """Retrieve a resource given its ID
        ...
        """
        response = client.put(f'/{cls._resource}/{id}/apikeys/{env}',None)
        return response

    @classmethod
    def _tax_info_validation(cls, id: str) -> Resource:
        """Retrieve a resource given its ID
        ...
        """
        response = client.get(f'/{cls._resource}/{id}/tax-info-validation')
        return cls._from_dict(response)
...
class Creatable(Resource):
    ...
    @classmethod
    def _createPayment(cls, **data) -> Resource:
        """Create a resource
...
class Updatable(Resource):
    ...
    @classmethod
    def _update_logo(cls, id: str, files:Any) -> Resource:
        """Update an specific resource with new data.
        ...
        """
        response = client.logo(f'/{cls._resource}/{id}/logo', files)
        return cls._from_dict(response)
...
class Deletable(Resource):
    ...
    @classmethod
    def _delete(cls, resource_id: str, **query_params) -> Resource:
        ...
        Returns:
            Resource: The deleted or cancelled resource.
        """
        # Codificar los parámetros en la URL
        query_string = urlencode(query_params)
        # Construir la URL completa para la solicitud DELETE
        url = f'/{cls._resource}/{resource_id}?{query_string}'
