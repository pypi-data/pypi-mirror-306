...
def sensi_config_is_valid(sensi_config):
    ...
    if not required_headers.issubset(set(sensi_config_header)):
        logger.error(
            "Sensi config is missing required headers. " 
            f"Expected {SENSI_CONFIG_HEADER}, got {sensi_config_header}"
        )
        raise SensiIOError(
            "Sensitivity configuration file is missing required headers. " 
            f"Expected {SENSI_CONFIG_HEADER}, got {sensi_config_header}"
        )

    # Checking if the sensi config has the correct number
    # of columns using the '_count_sep' colum    
    ...
    logger.debug(f'Checking sensi config values in "Description" if it is present')
    if 'Description' in sensi_config_header:
        ...
        if not description_check.all():
            ...
            logger.error(
                "Sensi config has the wrong values in 'Description'. "
                f"Rows with wrong values: {rows_with_wrong_description}"
            )
            raise SensiIOError(
                "Sensitivity configuration file has the wrong values in 'Description'. "
                f"Rows with wrong values: {rows_with_wrong_description}"
            )
        else:
