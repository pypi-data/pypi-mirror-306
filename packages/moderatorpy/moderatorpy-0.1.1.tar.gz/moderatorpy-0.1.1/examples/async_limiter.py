...
from moderatorpy.async_ import AsyncLimiterTransport, RedisBucket
...
async def main():
    transport = AsyncLimiterTransport(
        per_second=10,
        per_minute=100,
        bucket_class=RedisBucket,
        bucket_kwargs={"redis_url": "redis://localhost:6379/0"},
    )
    async with AsyncClient(base_url=DEFAULT_BASE_URL, transport=transport) as client:
