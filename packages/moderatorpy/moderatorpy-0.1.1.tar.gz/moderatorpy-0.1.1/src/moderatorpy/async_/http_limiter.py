...
class AsyncNameBucketFactory(BucketFactory):
    """Async version of the NameBucketFactory"""
...
class AsyncLimiterMixin(AsyncBaseTransport if TYPE_CHECKING else object):
    """Async version of the LimiterMixin"""
    ...
    def __init__(  # noqa: PLR0913
        ...
        bucket_class: type[AbstractBucket] = InMemoryBucket,
        bucket_kwargs: dict | None = None,
        bucket_factory: type[BucketFactory] = AsyncNameBucketFactory,
        limiter: Limiter | None = None,
        raise_when_fail: bool = True,
        max_delay: Union[int, float, None] = None,
        clock: type[AbstractClock] = TimeClock,
        ...
        **kwargs,
        ...
        # If the superclass is an AsyncBaseTransport subclass,
        # pass along any valid keyword arguments
        session_kwargs = get_valid_kwargs(super().__init__, kwargs)
        super().__init__(**session_kwargs)

    async def handle_request(self, request: Request, **kwargs) -> Response:
        """Send a request with rate-limiting asynchronously."""
        await self.limiter.try_acquire(self._name(request))
        response = await super().handle_request(request, **kwargs)
        if response.status_code in self.limit_statuses:
            await self._fill_bucket(request)
        return response
    ...
    async def _fill_bucket(self, request: Request):
        """Async version of bucket filling"""
        logger.info(f"Rate limit exceeded for {request.url}; filling limiter bucket")
        item = await self.bucket_factory.wrap_item(self._name(request))
        bucket = await self.bucket_factory.get(item)

        rate = bucket.rates[0]
        item_count = rate.limit - bucket.count()

        filler_item = await self.bucket_factory.wrap_item(
            self._name(request), item_count
        )
        await bucket.put(filler_item)
...
class AsyncLimiterTransport(AsyncLimiterMixin, AsyncHTTPTransport):
    """Async version of LimiterTransport that works with httpx's AsyncClient"""

    # Inherits all documentation from LimiterTransport
    pass
