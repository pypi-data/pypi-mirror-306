...
class Patcher(object):
    ...
    def auto(self, executable_path=None, force=False, version_main=None, _=None):
        ...
        package_path = self.fetch_package()
        self.unzip_package(package_path)
    ...
    def fetch_release_number(self):
        ...
        :rtype: LooseVersion
        """
        # Determine the filename based on the version context
        if self.is_old_chromedriver:
            path = f"/latest_release_{self.version_main}"
            path = path.upper()
            cache_file = os.path.join(self.cache_dir, f"release_{self.version_main}.txt")
            logger.debug("Getting release number from %s" % path)
            ...
            if not self.version_main:
                # Fetch the latest version
                path = "/last-known-good-versions-with-downloads.json"
                cache_file = os.path.join(self.cache_dir, "latest_version.json")
                logger.debug("Getting release number from %s" % path)
            else:
                # Fetch the latest minor version of the major version provided
                path = "/latest-versions-per-milestone-with-downloads.json"
                cache_file = os.path.join(self.cache_dir, f"versions_{self.version_main}.json")
                logger.debug("Getting release number from %s" % path)
        ...
        if os.path.exists(cache_file):
            with open(cache_file, 'r') as f:
                data = f.read()
            if self.is_old_chromedriver:
                return LooseVersion(data.strip())
            else:
                # If the cache file is JSON, parse it appropriately
                major_versions = json.loads(data)
                if not self.version_main:
                    return LooseVersion(major_versions["channels"]["Stable"]["version"])
                else:
                    return LooseVersion(major_versions["milestones"][str(self.version_main)]["version"])

        # Perform the download if the cache file does not exist
        with urlopen(self.url_repo + path) as conn:
            response = conn.read().decode()
        ...
        if self.is_old_chromedriver:
            return LooseVersion(response.strip())
        else:
            last_versions = json.loads(response)
            if not self.version_main:
                return LooseVersion(last_versions["channels"]["Stable"]["version"])
            else:
                return LooseVersion(last_versions["milestones"][str(self.version_main)]["version"])
    ...
    def fetch_package(self):
        ...
        # 
        package_path = os.path.join(self.cache_dir, zip_name)
        
        # create bak dir
        if not os.path.exists(os.path.join(self.cache_dir, "bak")):
            os.mkdir(os.path.join(self.cache_dir, "bak"))

        # if zip file in bak dir, copy over
        bak_file_path = os.path.join(self.cache_dir, "bak", zip_name)
        if os.path.exists(bak_file_path):
            shutil.copy(bak_file_path, package_path)
        
        if not os.path.exists(package_path):
            urlretrieve(download_url, package_path)  # Download and save to cache
            shutil.copy(package_path, bak_file_path)
        
        # print("Zip found at: ", package_path)
        return package_path
    
    def load_cached_package(self, package_path):
        
        # Load the object from the package (assumed to be a zip file)
        # print("PACKAGE PATH: ", package_path)
        with zipfile.ZipFile(package_path, 'r') as zf:
            #print("Contents of the zip file:", zf.namelist())  # List all files in the zip
            
            # Assuming you want to extract some specific data from the zip
            # Replace 'file_inside_zip.txt' with the actual name of the file you want to read
            specific_file_name = zf.filename  # Change this to the actual file you need

            # Specify the file you want to read from the zip archive
            specific_file_name = 'chromedriver-mac-x64/chromedriver'  # Update this to the correct path

            # print("SPECIFIC FILENAME: ", specific_file_name)
            # print("PLATFORM: ", self.platform)

            if specific_file_name in zf.namelist():
                with zf.open(specific_file_name) as file:
                    data = file.read()  # Read the content of the file
                    # Process the data as needed, e.g., deserialize, parse, etc.
                    return data  # Return the loaded data or processed object
            else:
                raise FileNotFoundError(f"The file '{specific_file_name}' was not found in the archive.")
