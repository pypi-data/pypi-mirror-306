...
from yta_general_utils.file.checker import is_folder, is_file
...
class GoogleOauthAPI:
    ...
    def __init__(self, client_secret_filename: str, token_files_abspath: str):
        ...
        """
        if not client_secret_filename or not is_file(client_secret_filename) or not file_exists(client_secret_filename):
            raise Exception('GoogleOauthAPI could not be instantiated due to the token was not found.')
        
        if not token_files_abspath or not is_abspath(token_files_abspath) or not is_folder(token_files_abspath):
