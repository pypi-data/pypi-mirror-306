...
def measure_elapsed_time():
    ...
    def get_elapsed_time():
        ...
        return f"{hours} hrs {minutes} mins {remaining_seconds:.2f} secs"
...
class VideoToPLYConverter:
    ...
    def convert_video_to_ply(self, video_path, fps, skip_ba):
        ...
        if self.method == 'pycolmap':
            self._convert_using_pycolmap(video_path, ply_path, fps, skip_ba)
        # elif self.method ==
        else:
    ...
    # def _manage_files(self, source_path):
    ...
    def _convert_using_pycolmap(self, video_path, ply_path, fps, skip_ba):
