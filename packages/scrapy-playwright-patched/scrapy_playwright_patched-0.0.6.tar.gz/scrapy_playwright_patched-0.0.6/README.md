# scrapy-playwright-patched: Undetected-Playwright integration for Scrapy


A [Scrapy](https://github.com/scrapy/scrapy) Download Handler which performs requests using
[Rebrowser-Playwright for Python](https://github.com/rebrowser/rebrowser-playwright-python).
...
`scrapy-playwright-patched` is available on PyPI and can be installed with `pip`:

```
pip install scrapy-playwright-patched
```

`rebrowser-playwright` is defined as a dependency so it gets installed automatically,
...
# settings.py
DOWNLOAD_HANDLERS = {
    "http": "scrapy_rebrowser_playwright.handler.ScrapyPlaywrightDownloadHandler",
    "https": "scrapy_rebrowser_playwright.handler.ScrapyPlaywrightDownloadHandler",
}
```
...
`scrapy-playwright-patched` uses `Page.route` & `Page.unroute` internally, avoid using
...
scrapy-playwright-patched package provides a replacement extension which also considers
...
from the scrapy-playwright-patched package:
...
scrapy-playwright-patched download handler.
...
Be sure to include which versions of Scrapy, Playwright and scrapy-playwright-patched you are using:
...
- [Eugenio Lacuesta](https://github.com/elacuesta) for the original [scrapy-playwright](https://github.com/scrapy-plugins/scrapy-playwright) project.
- [Rebrowser](https://github.com/rebrowser/rebrowser-playwright-python) for the original [rebrowser-playwright](https://github.com/rebrowser/rebrowser-playwright-python) project.
