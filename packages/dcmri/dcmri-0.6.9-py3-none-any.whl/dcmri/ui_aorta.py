...
import dcmri.pk_kidney as pk_kidney
...
class AortaKidneys(ui.Model):
    ...
    def _conc_kidneys(self, sum=True):
        ...
        if self.kidneys == '2CF':
            ...
            Nlk = pk_kidney.conc_kidney(ca_lk, RPF_lk, Tp_lk, GFR_lk, self.Tt_lk,
                                 t=self.t, dt=self.dt, sum=sum, kinetics='2CF')
            Nrk = pk_kidney.conc_kidney(ca_rk, RPF_rk, Tp_rk, GFR_rk, self.Tt_rk,
                                 t=self.t, dt=self.dt, sum=sum, kinetics='2CF')
        if self.kidneys == 'HF':
            Nlk = pk_kidney.conc_kidney(ca_lk, self.vp_lk*self.vol_lk, GFR_lk,
                                 self.Tt_lk, t=self.t, dt=self.dt, sum=sum, kinetics='HF')
            Nrk = pk_kidney.conc_kidney(ca_rk, self.vp_rk*self.vol_rk, GFR_rk,
