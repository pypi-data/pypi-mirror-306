...
if config_file == None or config_file == 'help':
    ...
    print('mode_type: copy_table, copy_data, copy_index, copy_constraint, copy_sequence, compare_data_number, compare_index, compare_constraint, copy_from_sql')
...
def get_table_metadata(src_db, table_map, global_scn):
    try:
        ...
        tables_data_num[report_table_id] = [table_scn, None, None]
        ...
        if object_type == 'table':
            ...
            # 单表
                ...
                if max_page_num == 1:
                    ...
                    log.info('获取源端数据量成功', src_schema + '.' + src_table, str(src_num))
                    tables_data_num[report_table_id] = [table_scn, src_num, None]
                    page_num = ceil(src_num / 1000)
...
def get_data_from_oracle(table_map, mess, table_scn, table_metadata=None, page=None, page_rn=None):
    ...
    try:
        ...
        for col in cols_for_copy:
            ...
            if 'CHAR' in str(col[1]):
                ...
                for sub_idx in range(1, col_split_num + 1):
                    cols_str = cols_str + 'nvl(replace(to_char(substr(' + col[0] + ',' + str(sub_idx - 1) + "*1300+1,1300)),chr(0),chr(32)),''),"
        ...
        log.error('到Oracle获取数据失败', mess, str(_))
    finally:
        src_db.close()
        dst_db.close()
