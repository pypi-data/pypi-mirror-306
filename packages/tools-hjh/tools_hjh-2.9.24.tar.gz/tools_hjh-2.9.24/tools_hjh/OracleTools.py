    ...
    def get_table_metadata(ora_conn, username, table, partition=True):
        ...
        for rs in rss:
            ...
            if c_type == 'U':
                ...
                rs = ora_conn.run(sql, (r_owner.upper(), r_constraint_name.upper())).get_rows(True)[0]
        ...
        # 表分区
        rss = ora_conn.run('select partitioning_type, subpartitioning_type, interval from dba_part_tables where owner = ? and table_name = ?', (username, table)).get_rows(True)
    ...
    def desc(ora_conn, username, table, simple_mode=True, no_fk=False):
        ...
        for col_ in cols_:
            ...
            if col_['virtual'] == 'NO':
                ...
                mess = mess + 'column: ' + col_['name'].lower() + data_default + '\n'
        ...
        for rs in rss:
            ...
            if c_type == 'unique':
                ...
                r_table = rs['r_table']
                constraint_type = ' fk references ' + r_table + '(' + rs['r_cols'].lower() + ')'
    ...
    def get_table_ddl_pg(dba_conn, username, table, forever_number_to_numeric=True):
        ...
        for col in metadata['columns']:
            ...
            if 'VARCHAR' in type_:
                ...
                if not forever_number_to_numeric and '(' in type_ and ',' not in type_:
            ...
            if col['default_value'] != None:
                ...
                if col['virtual'] == 'NO':
                    ...
                    default_value = 'generated always as (' + col['default_value'].lower() + ') stored'
            else:
                default_value = ''
        ...
        if metadata['partition'] != None:
            ...
            create_table_sql = create_table_sql + ' partition by ' + partition_type.lower() + ' (' + partition_columns.lower() + ')'
        sql_list.append(create_table_sql)    
        ...
        if metadata['comments'] != None:
            comments_str = ''
            comments_str = 'comment on table ' + username.lower() + '."' + table.lower() + '"' + " is '" + metadata['comments'].replace('\\u', '\\\\u') + "'"
            sql_list.append(comments_str)
        ...
        for col in metadata['columns']:
            ...
            comments = col['comments']
            if comments != None: 
                comments_str = comments_str + 'comment on column ' + username.lower() + '."' + table.lower() + '"."' + name.lower() + '"' + " is '" + comments.replace('\\u', '\\\\u') + "'"
                sql_list.append(comments_str)
