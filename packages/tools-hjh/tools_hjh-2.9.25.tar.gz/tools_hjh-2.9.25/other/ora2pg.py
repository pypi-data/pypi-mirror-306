...
run_mode : ↓
copy_table, copy_data, copy_index, copy_constraint, copy_sequence, copy_from_sql
...
copy_table，会提前drop table，如果表存在外键关联可能删不掉，等关联表重建后重复运行一次copy_table即可
copy_table，rowid类型会转为varchar(18)，而不是oid，转为oid数据迁移不过去
copy_data，oracle中的chr(0)会被强行替换为chr(32)，也就是空格
copy_data，oracle中date和timestamp类型中的0000年会被强行替换为0001年，00月替换为01月，00日替换为01日
copy_data，收集元数据时，会count表，且强制走全表，避免索引与表不同步的情况
copy_index，会先drop同名索引
copy_index，如果索引名已被占用（被其他对象占用），create会失败
copy_index，对于分区表的唯一索引，会自动加入分区字段
copy_constraint，不会同步外键
copy_constraint，使用匿名模式
copy_constraint，不会先删除已存在的键
copy_constraint，对于分区表的主键和唯一键，会自动加入分区字段
compare_data_number，只会比较两端每个表行数是否一致
compare_index、compare_constraint，只会比较两端每个表的索引数量、键数量是否一致，而不会去比较被索引或被键的字段
copy_from_sql，不支持并行
某些情况失败率较高，目前已发现的有：被索引字段含有函数、默认值含有函数
'''
    print(help_mess)
    sys.exit()
...
def get_table_metadata(src_db, table_map, global_scn):
    ...
    def fenye(table_map, mess, table_scn, table_metadata):
        if max_page_num == 1:
            ...
            page_num = ceil(src_num / once_num_normal)
    ...
    try:
        ...
        for col in cols_for_copy:
            ...
            if 'CHAR' in str(col[1]):
                ...
                merge_col[idx] = [idx2]
                idx2 = idx2 + 1
                cols_str = cols_str + '''translate(translate(translate(to_char("''' + col[0] + '''",'yyyy-mm-dd hh24:mi:ss'),'0000-','0001-'),'-00-','-01-'),'-00','-01'),'''
            elif 'TIMESTAMP' in str(col[1]):
                merge_col[idx] = [idx2]
                idx2 = idx2 + 1
                cols_str = cols_str + '''translate(translate(translate(to_char("''' + col[0] + '''",'yyyy-mm-dd hh24:mi:ss.ff9'),'0000-','0001-'),'-00-','-01-'),'-00','-01'),'''
    ...
    def copy_one_index(src_db, dst_db, table_map):
        ...
        for sql in sqls:
            if sql.startswith('create table '):
                ...
                drop_sql = ''
                index_name = dst_schema + '.' + sql.split(' on ')[0].split(' ')[-1]
                drop_sql = 'drop index if exists ' + index_name 
                try:
...
def copy_table(run_mode):
    ...
    def copy_one_table(src_db, dst_db, table_map, run_mode):
        ...
        if run_mode == 'copy_table' and is_suc: 
            log.info('PG创建表成功', dst_schema + '.' + dst_table)
...
def copy_constraint():
    ...
    def copy_pk(src_db, dst_db, table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    def copy_fk(src_db, dst_db, table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_pk, (src_db, dst_db, table_map))
    tp.wait()
    
    for table_map in table_map_list:
        copy_fk(src_db, dst_db, table_map)
...
def compare_constraint():
    ...
    def compare_constraint_one(src_sql, table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
