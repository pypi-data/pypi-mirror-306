...
class UrlsFileParam(BaseModel):
    urls_file: str = Field(**URLS_FILE_FIELD_KWARGS)

    @model_validator(mode="after")
    def input_group(self):
        return validate_input_group(self)

    @validator("urls_file", pre=True)
    def validate_urls_file(cls, value):
        if re.match(_URL_PATTERN, value):
            return value
        elif os.path.isfile(value):
            return value
        else:
            raise ValueError("The 'urls_file' must be a valid URL or an existing local file path.")
    ...
    if urls_file:
        if os.path.isfile(urls_file):
            with open(urls_file, 'r') as file:
                urls = load_url_list(file.read())
            spider.logger.info(f"Loaded {len(urls)} initial URLs from local file {urls_file}.")
        else:
