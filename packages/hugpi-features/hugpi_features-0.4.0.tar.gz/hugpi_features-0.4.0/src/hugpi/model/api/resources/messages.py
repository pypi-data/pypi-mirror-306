...
class Messages:
    ...
    def stream(
            ...
            conversation: Union[bool, None] = None,
            **kwargs):
        user_prompt = self._update_dependencies(model_idx=AVAILABLE_MODEL_LIST.index(model),
                                                messages=messages,
                                                conversation=conversation)
    ...
    def _update_dependencies(self, model_idx, messages, conversation):
        ...
        if not conversation:
            ...
            self.llm.new_conversation(modelIndex=model_idx if model_idx else 0,
