...
class CommonValidationRules:
    ...
    PHONE_RULE = ValidationRule(
        pattern=ValidationPatterns.PHONE_INTERNATIONAL
    )
    ...
    DATE_RULE = ValidationRule(
        pattern=ValidationPatterns.DATE_ISO
    )
    ...
    ID_RULE = ValidationRule(
        pattern=ValidationPatterns.ID_NUMBER['generic'],
        min_length=5,
        max_length=12
    )
    
    FLIGHT_RULE = ValidationRule(
        pattern=ValidationPatterns.FLIGHT_NUMBER
    )
    
    BOOKING_RULE = ValidationRule(
        pattern=ValidationPatterns.BOOKING_REFERENCE,
        min_length=6,
        max_length=6
    )
...
class Field:
    ...
    def __post_init__(self):
        ...
        if self.field_type == FieldType.LIST:
            if not self.array_item_type:
                raise ValueError("LIST field_type requires array_item_type to be specified")
            # Allow dict_fields for LIST type when array_item_type is DICT
            if self.dict_fields and self.array_item_type != FieldType.DICT:
                raise ValueError("dict_fields can only be specified for LIST fields with DICT array_item_type")
    ...
    def validate_value(self, value: Any) -> bool:
        ...
        if self.field_type == FieldType.LIST:
            ...
            return all(self._validate_basic_type(item, self.array_item_type) for item in value)
    ...
    def _validate_basic_type(self, value: Any, field_type: FieldType) -> bool:
        ...
        type_checks = {
            ...
            FieldType.DATE: lambda x: isinstance(x, str) and bool(re.match(ValidationPatterns.DATE_ISO, x)),
            FieldType.LIST: lambda x: isinstance(x, (list, tuple)),
            FieldType.DICT: lambda x: isinstance(x, dict),
            FieldType.EMAIL: lambda x: isinstance(x, str) and bool(re.match(ValidationPatterns.EMAIL, x)),
            FieldType.PHONE: lambda x: isinstance(x, str) and bool(re.match(ValidationPatterns.PHONE_INTERNATIONAL, x)),
            FieldType.URL: lambda x: isinstance(x, str)  # Add URL validation pattern if needed
        }
