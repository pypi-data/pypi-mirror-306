...
class OstFormEntry(models.Model):
    ...
    def get_form_entry_values(self):
        ...
        return self.ostformentryvalues_set.exclude(
            field_id=OsTicketCustomization.get_int("osticket_tool_matching_field_id") or None
        )
...
class OstFormEntryValues(models.Model):
    ...
    def get_list_item(self) -> Optional[OstListItems]:
        try:
            value_dict = json.loads(self.value)
            return OstListItems.objects.filter(id=next(iter(value_dict))).first()
        except Exception:
            pass
        return None

    def display_value(self):
        if self.field.is_list_type():
            list_item = self.get_list_item()
            return list_item.value if list_item else self.value
        else:
            return self.value
...
class OstTicket(models.Model):
    ...
    @property
    def ostthread_set(self):
        return OstThread.objects.filter(object_id=self.ticket_id, object_type=OstObjectType.TICKET)
    ...
    @property
    def ostformentry_set(self):
        return OstFormEntry.objects.filter(object_id=self.ticket_id, object_type=OstObjectType.TICKET)
    ...
    def get_absolute_url(self):
        ...
        if osticket_service.get("available", False) and url:
            return url + "/ost/tickets.php?id={}".format(self.ticket_id)
        return None
...
def get_osticket_form_fields(exclude_matching=True) -> List[OstFormField]:
