...
def ten_most_recent_past_comments_and_tasks(request, tool_id):
    user: User = request.user
    tickets = get_tickets_for_tool(get_object_or_404(Tool, pk=tool_id))[:10]
...
def search_tickets(tool: Tool, search: str, start: datetime = None, end: datetime = None) -> List[OstTicket]:
...
def get_tickets_for_tool(tool: Tool, only_open=True) -> List[OstTicket]:
...
def filter_tickets_for_tool(tool: Tool, ticket_ids=None) -> List[int]:
    ...
    if list_item:
        ...
        valid_ticket_ids = form_entry_values.filter(
            value=OsTicketCustomization.get_tool_matching_nemo_property(tool)
        ).values_list("entry__object_id", flat=True)
    return valid_ticket_ids


def get_preloaded_os_tickets(ticket_ids: List[int], start: datetime = None, end: datetime = None) -> List[OstTicket]:
    tickets = []
    threads = list(
