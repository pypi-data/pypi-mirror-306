...
cdef cudaError_t _cudaGetDriverEntryPoint(const char* symbol, void** funcPtr, unsigned long long flags) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = <cudaError_t>cydriver._cuGetProcAddress(symbol, funcPtr, m_global._CUDART_VERSION, flags)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGetExportTable(const void** ppExportTable, cudaUUID_t* pExportTableId) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGetExportTable(ppExportTable, <cydriver.CUuuid*>pExportTableId)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphKernelNodeGetParams(cudaGraphNode_t node, cudaKernelNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    cdef cydriver.CUDA_KERNEL_NODE_PARAMS _driver_pNodeParams

    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphKernelNodeGetParams(<cydriver.CUgraphNode>node, &_driver_pNodeParams)
...
cdef cudaError_t _cudaProfilerInitialize(const char* configFile, const char* outputFile, cudaOutputMode_t outputMode) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuProfilerInitialize(configFile, outputFile, <cydriver.CUoutput_mode>outputMode)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _getLocalRuntimeVersion(int* runtimeVersion) except ?cudaErrorCallRequiresNewerDriver nogil:
    # Load
    handle = dlfcn.dlopen('libcudart.so.11.0', dlfcn.RTLD_NOW)
    if handle == NULL:
        with gil:
            raise RuntimeError(f'Failed to dlopen libcudart.so.11.0')

    __cudaRuntimeGetVersion = dlfcn.dlsym(handle, 'cudaRuntimeGetVersion')

    if __cudaRuntimeGetVersion == NULL:
        with gil:
            raise RuntimeError(f'Function "cudaRuntimeGetVersion" not found in libcudart.so.11.0')
...
cdef cudaError_t _cudaThreadSynchronize() except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    if err != cudaSuccess:
        return err
...
cdef cudaError_t _cudaThreadSetLimit(cudaLimit limit, size_t value) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    if err != cudaSuccess:
        return err
...
cdef cudaError_t _cudaThreadGetLimit(size_t* pValue, cudaLimit limit) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    if err != cudaSuccess:
        return err
...
cdef cudaError_t _cudaLaunchHostFunc(cudaStream_t stream, cudaHostFn_t fn, void* userData) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuLaunchHostFunc(<cydriver.CUstream>stream, <cydriver.CUhostFn>fn, userData)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphAddKernelNode(cudaGraphNode_t* pGraphNode, cudaGraph_t graph, const cudaGraphNode_t* pDependencies, size_t numDependencies, const cudaKernelNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    err = m_global.lazyInitContextState()
    cdef cydriver.CUDA_KERNEL_NODE_PARAMS _driver_pNodeParams
    _driver_pNodeParams.func = <cydriver.CUfunction>pNodeParams[0].func
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphAddKernelNode(<cydriver.CUgraphNode*>pGraphNode, <cydriver.CUgraph>graph, <cydriver.CUgraphNode*>pDependencies, numDependencies, &_driver_pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphKernelNodeSetParams(cudaGraphNode_t node, const cudaKernelNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphKernelNodeSetParams(<cydriver.CUgraphNode>node, &_driver_pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphAddHostNode(cudaGraphNode_t* pGraphNode, cudaGraph_t graph, const cudaGraphNode_t* pDependencies, size_t numDependencies, const cudaHostNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphAddHostNode(<cydriver.CUgraphNode*>pGraphNode, <cydriver.CUgraph>graph, <cydriver.CUgraphNode*>pDependencies, numDependencies, <cydriver.CUDA_HOST_NODE_PARAMS*>pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphHostNodeSetParams(cudaGraphNode_t node, const cudaHostNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphHostNodeSetParams(<cydriver.CUgraphNode>node, <cydriver.CUDA_HOST_NODE_PARAMS*>pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphInstantiate(cudaGraphExec_t* pGraphExec, cudaGraph_t graph, cudaGraphNode_t* pErrorNode, char* pLogBuffer, size_t bufferSize) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphInstantiate_v2(<cydriver.CUgraphExec*>pGraphExec, <cydriver.CUgraph>graph, <cydriver.CUgraphNode*>pErrorNode, pLogBuffer, bufferSize)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphExecKernelNodeSetParams(cudaGraphExec_t hGraphExec, cudaGraphNode_t node, const cudaKernelNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphExecKernelNodeSetParams(<cydriver.CUgraphExec>hGraphExec, <cydriver.CUgraphNode>node, &_driver_pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphExecHostNodeSetParams(cudaGraphExec_t hGraphExec, cudaGraphNode_t node, const cudaHostNodeParams* pNodeParams) except ?cudaErrorCallRequiresNewerDriver nogil:
    ...
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphExecHostNodeSetParams(<cydriver.CUgraphExec>hGraphExec, <cydriver.CUgraphNode>node, <cydriver.CUDA_HOST_NODE_PARAMS*>pNodeParams)
    if err != cudaSuccess:
        _setLastError(err)
    return err
...
cdef cudaError_t _cudaGraphExecUpdate(cudaGraphExec_t hGraphExec, cudaGraph_t hGraph, cudaGraphNode_t* hErrorNode_out, cudaGraphExecUpdateResult* updateResult_out) except ?cudaErrorCallRequiresNewerDriver nogil:
    cdef cudaError_t err = cudaSuccess
    err = m_global.lazyInitContextState()
    if err != cudaSuccess:
        return err
    err = <cudaError_t>cydriver._cuGraphExecUpdate(<cydriver.CUgraphExec>hGraphExec, <cydriver.CUgraph>hGraph, <cydriver.CUgraphNode*>hErrorNode_out, <cydriver.CUgraphExecUpdateResult*>updateResult_out)
    if err != cudaSuccess:
        _setLastError(err)
    return err
