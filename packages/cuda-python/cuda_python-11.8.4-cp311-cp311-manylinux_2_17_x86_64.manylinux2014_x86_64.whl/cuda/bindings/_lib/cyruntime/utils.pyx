...
cdef class cudaPythonGlobal:
    ...
    cdef cudaPythonDevice* getDeviceFromDriver(self, cydriver.CUdevice driverDevice) nogil:
        for i in range(self._numDevices):
            if self._deviceList[i].driverDevice == driverDevice:
                return &self._deviceList[i]
        return NULL

    cdef cudaPythonDevice* getDeviceFromPrimaryCtx(self, cydriver.CUcontext context) nogil:
        if context == NULL:
            return NULL
        for i in range(self._numDevices):
...
cdef cudaError_t streamGetCaptureInfoCommon(
  ...
  size_t *numDependencies_out,
    ...
    if graph_out or dependencies_out or numDependencies_out:
        ...
        err = <cudaError_t>cydriver._cuStreamGetCaptureInfo(stream, &driverCaptureStatus, <cydriver.cuuint64_t*>id_out)
        if err != cudaSuccess:
            return err
...
cdef cydriver.CUDA_MEMCPY3D_v2 memCopy3DInit(cydriver.CUmemorytype_enum dstType, cydriver.CUmemorytype_enum srcType) nogil:
...
cdef cydriver.CUDA_MEMCPY2D_v2 memCopy2DInit(cydriver.CUmemorytype_enum dstType, cydriver.CUmemorytype_enum srcType) nogil:
...
cdef void toDriverMemsetNodeParams(const cudaMemsetParams *pRuntimeParams, cydriver.CUDA_MEMSET_NODE_PARAMS *pDriverParams) nogil:
