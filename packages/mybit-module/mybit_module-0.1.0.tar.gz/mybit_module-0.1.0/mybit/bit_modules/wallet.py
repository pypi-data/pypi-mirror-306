...
class PrivateKey(BaseKey):
    ...
    def segwit_address_native(self):
        """The public segwit nested in p2wpkh address you share with others to
        receive funds."""
        # Only make segwit address if public key is compressed
        if self._segwit_address_native is None and self.is_compressed():
            self._segwit_address_native = public_key_to_p2wpkh_address(self._public_key, version=self.version)
    ...
    def can_sign_unspent(self, unspent):
        ...
        elif self.segwit_address_native:
            segwit_script = bytes_to_hex(address_to_scriptpubkey(self.segwit_address_native))
            return unspent.script == script or unspent.script == segwit_script
        else:
    ...
    def get_unspents(self):
        ...
        if self.segwit_address_native:
            self.unspents += list(map(lambda u: u.set_type('p2wpkh'), NetworkAPI.get_unspent(self.segwit_address_native)))
...
class PrivateKeyTestnet(BaseKey):
    ...
    def get_unspents(self):
        ...
        if self.segwit_address_native:
            self.unspents += list(
                map(lambda u: u.set_type('p2wpkh'), NetworkAPI.get_unspent_testnet(self.segwit_address_native))
            )
