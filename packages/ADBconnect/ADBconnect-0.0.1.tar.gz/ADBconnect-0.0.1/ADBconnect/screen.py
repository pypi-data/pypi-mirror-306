...
def screenshot(filename="screen.png") -> str:
    """Takes a screenshot and saves it to disk."""
    adb_command("exec-out screencap -p > {}".format(filename))
    return filename


def search_image(image_file, attempts=1, wait_time=0) -> tuple:
    """
    ...
    for attempt in range(attempts):
        screenshot_path = screenshot('find.png')
        coordinates = _find_image(screenshot_path, image_file)
        os.remove('find.png')
...
def _find_image(screenshot_path, image_path):
