...
class Context(BaseModel):
    ...
    json_data: Optional[dict] = Field(default=None, exclude=True)

class CustomException(Exception):
    def __init__(self, message="Custom exception", status_code=400, company_id:str=None, agent_email:str=None, chat_id:str=None, details:str=None, json_data:dict=None):
...
class ChatNotFoundError(CustomException):
...
class CompanyNotFound(CustomException):
...
class UnknownMetaNotificationType(CustomException):
    """
    Received a meta notification we don't know how to process
    """
