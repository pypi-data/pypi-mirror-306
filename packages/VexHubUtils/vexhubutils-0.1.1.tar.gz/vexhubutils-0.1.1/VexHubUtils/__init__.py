...
import pyautogui, colorsys, curses, shutil, numpy as np, base64, json
...
class Console:
    ...
    def clear():
        return os.system("cls" if Global.windows else "clear")
...
class Logger:
    @staticmethod
    def _log_template(symbol: str, color: str, label: str, message: str, details: str = "") -> None:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_message = (f' [{ColorUtils.hex_color("#6c757d")}{timestamp}{ColorUtils.hex_color("#FFFFFF")}] '
                       f'{ColorUtils.hex_color("#6c757d")}- {ColorUtils.hex_color("#FFFFFF")}[ {ColorUtils.hex_color(color)}{symbol}{ColorUtils.hex_color("#FFFFFF")} ] '
                       f'{ColorUtils.hex_color(color)}{label}: {ColorUtils.hex_color("#00ccff")}{message}')
        if details:
            log_message += f' {ColorUtils.hex_color("#FFFFFF")}({ColorUtils.hex_color("#00ccff")}{details}{ColorUtils.hex_color("#FFFFFF")})'
        print(log_message)

    @staticmethod
    def success(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="+", color="#00FF00", label=label, message=message, details=details)

    @staticmethod
    def failed(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="-", color="#FF0000", label=label, message=message, details=details)

    @staticmethod
    def captcha(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="$", color="#fcf300", label=label, message=message, details=details)

    @staticmethod
    def info(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="*", color="#f77f00", label=label, message=message, details=details)

    @staticmethod
    def warning(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="!", color="#ffd60a", label=label, message=message, details=details)

    @staticmethod
    def pretty(label: str, message: str, details: str = "") -> None:
        Logger._log_template(symbol="^", color="#5e548e", label=label, message=message, details=details)

    @staticmethod
    def custom(*args) -> None:
        if len(args) == 1:
            message = args[0]
            color, symbol, label, details = "#3498db", "?", "Custom", "" 
        elif len(args) == 5:
            color, symbol, label, message, details = args
        else:
            raise ValueError("Invalid arguments. Use either 1 or 5 arguments.")
        
        Logger._log_template(symbol=symbol, color=color, label=label, message=message, details=details)
