...
class database:
    ...
    def query_table_with_time_diff(self, table_name, filter_time, limit=20, max_minutes=30, exact_filter=None, fields=None):
        """
        查询表中的数据，按照时间差过滤并返回结果，支持精确过滤条件。

        :param db_path: 数据库文件路径
        :param table_name: 表名称
        :param filter_time: 过滤的时间（字符串格式：'YYYY-MM-DD HH:MM:SS'）
        :param limit: 返回的最大记录数，默认为20
        :param max_minutes: 允许的最大时间差（单位：分钟），默认为30分钟
        :param fields: 要查询的字段列表，如果为 None，则查询所有字段
        :param exact_filter: 精确过滤条件，字典形式，如 {id: 123}
        ...
        SELECT {fields_str}
        FROM {table_name}
        WHERE ABS(JULIANDAY(time) - JULIANDAY(?)) * 24 * 60 <= ?
        """
