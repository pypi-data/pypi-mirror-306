...
class ContractService(Component):
    ...
    def create(self, contract):
        power_station = self.env['photovoltaic.power.station'].browse(
            contract.power_plant)

        products = self.env['product.template'].sudo().search([('photovoltaic_power_station_id', '=', power_station.id), 
        ...
        contract = self.env['contract.participation'].sudo().create([{
            'name': self._compute_contract_name(contract, power_station),
            'partner_id': self.env.user.partner_id.id,
    ...
    def _compute_contract_name(self, contract, power_station):
        contract_number = power_station.contracts_count + 1
        name = f'NUEVO - {power_station.name} - {contract_number}'
        while (self.env['contract.participation'].search([('name', '=', name)])):
            contract_number += 1
            name = f'NUEVO - {power_station.name} - {contract_number}'
    ...
    def _to_pydantic(self, contract):
        generated_power, tn_co2_avoided, eq_family_consumption = self._calculate_production_data(
            contract)
