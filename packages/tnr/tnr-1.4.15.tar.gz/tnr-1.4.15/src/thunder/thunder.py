...
def init():
    global INSIDE_INSTANCE, INSTANCE_ID
...
if ENABLE_RUN_COMMANDS:
    ...
    def device(device_type, ngpus, raw):
        ...
        if device_type is None:
            ...
            if gpu_count == 1:
                ...
                console.print(f'[bold cyan]📖 Current GPUs:[/][white] {gpu_count} x {device.upper()}[/]')
        ...
        if device_type.lower() == "cpu":
            ...
            click.echo(
                click.style(f"✅ Device set to {gpu_count} x {device_type.upper()}", fg="green")
            )
    ...
    def run(args):
        raise click.ClickException(
            "tnr run is only supported on Linux based systems. Please switch to a linux machine or create a Thunder Compute instance with 'tnr create' and connect using 'tnr connect <INSTANCE ID>'"
        )
    ...
    def device(device_type, ngpus, raw):
        raise click.ClickException(
            "tnr device is only supported on Linux based systems. Please switch to a linux machine or create a Thunder Compute instance with 'tnr create' and connect using 'tnr connect <INSTANCE ID>'"
        )
...
@cli.command(
    help="Deletes a Thunder Compute instance. This action is not reversible.",
    hidden=INSIDE_INSTANCE,
)
...
def connect(instance_id):
    ...
    raise click.ClickException(f"Unable to find instance {instance_id}")
...
@cli.command(
    help="Logs out of Thunder Compute and deletes the saved login token",
    hidden=INSIDE_INSTANCE,
)
def logout():
    auth.logout()
