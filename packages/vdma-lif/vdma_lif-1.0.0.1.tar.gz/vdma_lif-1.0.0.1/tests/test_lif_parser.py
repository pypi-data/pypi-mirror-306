...
from vdma_lif.models import LIFSchema
import json
...
class TestLIFParser(unittest.TestCase):
    ...
    def test_from_file_valid_json(self):
        ...
        schema = LIFParser.from_file(self.sample_file)
    ...
    def test_to_file_valid_json(self):
        ...
        with tempfile.NamedTemporaryFile(delete_on_close=False) as fp:
            LIFParser.to_file(schema, fp.name)
            schema_from_file = LIFParser.from_file(fp.name)
            self.assert_schema(schema_from_file)


    def assert_schema(self, schema: LIFSchema):
        self.assertIsInstance(schema, LIFSchema)

        # Check Meta Information
        self.assertEqual(
            schema.meta_information.project_identification, "Sample Project")
        self.assertEqual(schema.meta_information.creator, "Sample Creator")
        self.assertEqual(schema.meta_information.export_timestamp.isoformat(
        ), "2069-07-21T00:37:33+00:00")
        self.assertEqual(schema.meta_information.lif_version, "1.0.0")

        # Check Layouts
        self.assertEqual(len(schema.layouts), 2)

        # First Layout
        layout1 = schema.layouts[0]
