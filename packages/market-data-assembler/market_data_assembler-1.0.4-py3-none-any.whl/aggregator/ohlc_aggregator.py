...
class OHLCAggregator:
    ...
    def _aggregate_candles(self, end_time):
        ...
        for candle in self.current_window_candles:
            if 'trades' in candle and isinstance(candle['trades'], (list, np.ndarray)) and len(candle['trades']) > 0:
        ...
        aggregated_books = []
        for candle in self.current_window_candles:
            if 'book_depth' in candle and isinstance(candle['book_depth'], (list, np.ndarray)) and len(
                    candle['book_depth']) > 0:
                aggregated_books.extend(candle['book_depth'])

        if aggregated_books:
            aggregated_books.sort(key=lambda book_depth: book_depth['t'])
    ...
    def aggregate(self, new_candle):
        ...
        if candle_time >= window_end_time:
            if candle_time >= window_end_time_x2_check or not self.current_window_candles:
                raise ValueError("Out of aggregation window range or nothing to aggregate.")
