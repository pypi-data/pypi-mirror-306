...
def _build_chunk_offsets(filename, start, end, worker_id, n_workers, check=False):
    offset = start
    bounds = []
            ...
            if check:
                try:
                    ...
                    if check_legacy(data) or check_message(data) or check_preference(data):
                        ...
                        logger.error(f"worker: {worker_id}/{n_workers}, offset: {offset}, invalid format, line: {line}")
                        raise ValueError("invalid format")
                except Exception as e:
                    logger.error(f"worker: {worker_id}/{n_workers}, offset: {offset}, line: {line}, error: {e}")
                    raise e

    return bounds


def build_index(filename, output=None, num_workers=None, check=False):
    ...
    chunks = [(i * chunk_size, (i + 1) * chunk_size, i + 1, num_workers, check) for i in range(num_workers)]
    chunks[-1] = (chunks[-1][0], file_size, chunks[-1][2], num_workers, check)
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="check data format")

    args = parser.parse_args()

    build_index(args.filename, args.output, args.num_workers, args.check)
