...
class TableJoins(StrEnum):
    ...
    JOBS = "JOIN jobs j ON f.jobid = j.jobid"
    CONFIGURATIONS = "JOIN configurations c ON j.configurationid = c.configurationid"
    PRODUCTIONSCONTAINER = "JOIN productionscontainer cont ON j.production = cont.production"
    ...
    TMP_PRODUCTION = "JOIN LHCB_DIRACBOOKKEEPING.temp_query_production tmp_p ON j.production = tmp_p.production"
