...
from ipfabric.models.security import Machine, Rule
...
class GraphDevice(BaseModel):
    ...
    securityModelRule: Optional[Rule] = None
...
class Diagram(BaseModel):
                    ...
                    for event in chain.events:
                        if event.type == "security check":
                            ...
                            if event.securityType == "zoneFw":
                                sec_model = new_device.device.get_security_model()
                                machine_zones = sec_model.security.machineZones
                                new_device.securityModelRule = self.find_matching_rule(
                                    machine_zones,
                                    event.decidingPolicyName,
                                    event.decidingRule
                                )
                            list_for_return.append(new_device)
    ...
    @staticmethod
    def find_matching_rule(data: Machine, chain_name: str, index_path: List[int]) -> Optional[Rule]:
        """
        ...
        Args:
            ...
            index_path (list[int]): The path of indices to navigate to the desired rule.

        Returns:
            Rule: The matching rule if found, otherwise None.
        """
        rule_chain = data.ruleChains.get(chain_name)
        current_level = rule_chain.rules if rule_chain else None
        for index in index_path:
            if current_level and 0 <= index < len(current_level):
                current_level = current_level[index]
                if isinstance(current_level, Rule) and current_level.test and hasattr(current_level.test, 'items'):
