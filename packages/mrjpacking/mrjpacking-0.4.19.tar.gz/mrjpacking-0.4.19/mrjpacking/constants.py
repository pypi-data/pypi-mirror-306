    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if version.parse(latest_version) > version.parse(current_version):
                print(f"\nĐã có phiên bản mới {latest_version}. Đang cập nhật...")
                update_package()
            # else:
...
def update_package():
    try:
        # Thực hiện cập nhật qua pip
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "mrjpacking"])
        print("\nĐã cập nhật phiên bản mới thành công!")
        
        # Tắt chương trình hiện tại và chạy lại chương trình
        os.execv(sys.executable, ['python'] + sys.argv)
    except Exception as e:
        print("\nCó lỗi xảy ra khi cập nhật:", e)
