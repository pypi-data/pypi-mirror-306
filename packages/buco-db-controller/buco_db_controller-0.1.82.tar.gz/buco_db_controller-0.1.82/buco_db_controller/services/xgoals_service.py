...
class XGoalsService:
    ...
    def get_h2h_xgoals(self, team1_id, team2_id, league_id, season) -> list[XGoals]:
        ...
        xgoals_data = {
            self.fbref: [XGoals.from_dict(x) for x in self.fbref_xgoals_repository.get_many_xgoals(h2h_fixture_ids)],
            self.understat: [XGoals.from_dict(x) for x in self.understat_xgoals_repository.get_many_xgoals(h2h_fixture_ids)],
            self.flashscore: [XGoals.from_dict(x) for x in self.flashscore_xgoals_repository.get_many_xgoals(h2h_fixture_ids)],
        }
