...
class TeamService:
    ...
    def count_teams_consecutive_years_in_league(self, ht_id: int, at_id: int, league_id: int, current_season: int) -> dict:
        seasons_to_check = list(range(2014, current_season + 1))

        def get_years_in_league(team_id):
            _consecutive_years_in_league = 0
            ...
            for season in seasons_to_check:
                ...
                if fixtures and fixtures['data'][0]['league']['id'] == league_id:
                    _consecutive_years_in_league += 1
                else:
                    _consecutive_years_in_league = 0

            return _consecutive_years_in_league

        ht_consecutive_years_in_league = get_years_in_league(ht_id)
        at_consecutive_years_in_league = get_years_in_league(at_id)

        years_in_league = {
            'ht': ht_consecutive_years_in_league,
            'at': at_consecutive_years_in_league
        }
        return years_in_league
