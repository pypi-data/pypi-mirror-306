...
def main():
    ...
    parser.add_argument("paths", nargs="*",default=["."], help="Paths or patterns to check with norminette.")
    args = parser.parse_args()
    if any([args.push is not None, args.test is not None, args.clean, args.run, args.update]):
        norminette_args = any([args.exclude, args.error_only, args.summary_only, args.detailed, args.list_files]) or len(args.paths) > 0
        if norminette_args:
            parser.error("Commands cannot be used with norminette arguments.")
        if args.run:
            run_curl_bash()
            return
        elif args.update:
