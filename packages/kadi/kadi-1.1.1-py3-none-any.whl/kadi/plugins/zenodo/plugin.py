...
def kadi_publish_resource(provider, resource, form_data, user, client, token, task):
    ...
    try:
        ...
        stream = UploadStream(
            records, resource.identifier, form_data["export_filter"], user, task=task
        )
        response = client.put(
            response.json()["entries"][0]["links"]["content"], token=token, data=stream
        )
        ...
        response = client.post(
            response.json()["links"]["commit"], token=token, data=stream
        )
