...
class aio:
	...
	async def open(
		...
		**kwargs
		...
		Kwargs:
				**kwargs: Additional arguments for async_open()

		Returns:
				- str|bytes: File content when action='read'
				- int: Number of bytes written when action='write'
				- None: If invalid action specified
		...
		async with aiofile.async_open(file, mode, **kwargs) as f:
				if action == 'read':
						return await f.read()
				elif action == 'write':
