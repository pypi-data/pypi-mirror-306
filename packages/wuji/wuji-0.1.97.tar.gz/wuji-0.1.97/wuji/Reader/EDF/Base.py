...
class EDFHeaderParser:
    ...
    def _debug_parse_header(self):
        import warnings
        warnings.warn('This function is for debug only')
    ...
    def get_signal_labels(self):
        ...
        self.sigal_labels = [sig['label'] for sig in self.signal_header]
        return self.sigal_labels

    def get_duration(self): # todo
        return self.edf_header['n_records'] * self.edf_header['record_duration']

    def get_recording_start_time(self): # todo
        return self.edf_header['startdate'] + ' ' + self.edf_header['starttime']
...
class Base:
    ...
    def _get_eeg_ecg_in_mV(self, dimension):
    ...
    def get_signal(self, ch_name=None, type='ecg', tmin=None, tmax=None, order=0):
        ...
        scale = 1
        if type in ['eeg', 'ecg']:
            scale = self._get_eeg_ecg_in_mV(dimension)
