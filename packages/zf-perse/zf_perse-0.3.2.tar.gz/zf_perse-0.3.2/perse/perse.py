...
def extract_json_fields(content: Union[str, BeautifulSoup], PydanticModel: Any, model: str = default_model) -> Dict[str, Any]:
    """
    ...
    if isinstance(content, BeautifulSoup):
        content = content.prettify()
    if not issubclass(PydanticModel, BaseModel):
        raise ValueError("PydanticModel must be a subclass of BaseModel")
