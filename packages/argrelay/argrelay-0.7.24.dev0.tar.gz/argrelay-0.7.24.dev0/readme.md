...
The aim is to augment two-way &#10231; interaction by **indexed** reference data:
*   &#10230; Human inputs some **remembered** args (via `Tab`-auto-completion) in **relaxed order**.
*   &#10229; Machine provides feedback (via `Alt+Shift+Q` query) on input progress:
    *   What args machine already **matched** with server data according to **command schema**.
    *   What **else** machine needs from human to select as remaining command args.

This broadens applicability of CLI input as a slim alternative to graphical user interface (GUI)
competing in **convenience** for apps especially for developers who can automate CLI IO = "doing more with less".

*   For sizeable and inter-dependent data, need for perf, relaxed syntax, keyword search, try [`argrelay`][argrelay_org].
*   For simpler cases (when your args are instantly and directly queryable), [`argcomplete`][argcomplete_github] may fit.
