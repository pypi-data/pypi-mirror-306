...
def handle_output_filename(output_filename: Union[None, str], expected_extension: Union[None, AudioFileExtension, VideoFileExtension, ImageFileExtension, FileType]):
    """
    ...
    if expected_extension:
        ...
        if not PythonValidator.is_class(expected_extension, str) and not PythonValidator.is_class(expected_extension, AudioFileExtension) and not PythonValidator.is_instance(AudioFileExtension) and not PythonValidator.is_class(expected_extension, ImageFileExtension) and not PythonValidator.is_instance(expected_extension, ImageFileExtension) and not PythonValidator.is_class(VideoFileExtension) and not PythonValidator.is_instance(VideoFileExtension):
            raise Exception(f'The provided parameter "expected_extension" is not valid.')
        
        if PythonValidator.is_class(expected_extension, str):
            expected_extension = Enum.parse_as_enum(expected_extension, [AudioFileExtension, ImageFileExtension, VideoFileExtension])

        # TODO: Here 'expected_extension' is a YTAEnum class or instance
    ...
    if expected_extension:
        if PythonValidator.is_a_class(expected_extension):
            ...
            default_exception = expected_extension.default()
