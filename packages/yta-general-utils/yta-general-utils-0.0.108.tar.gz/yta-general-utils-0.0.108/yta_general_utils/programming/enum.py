...
class YTAEnum(Enum):
    ...
    def get_value_if_accepted(self, value):
        ...
        Enum value, so it will return an Enum or a single item.
        """
    ...
    @staticmethod
    def parse_as_enum(value, enums: list):
        """
        Try to parse the provided 'value' as the also provided
        'enums' classes and return the corresponding Enum instance
        if found and valid.
        """
        if not PythonValidator.is_list(enums):
            raise Exception('The provided "enums" parameter is not a list.')
        
        if any(not PythonValidator.is_enum_class(enum) for enum in enums):
            raise Exception('Not all elements in the provided "enums" list are YTAEnum instances.')
