...
pub struct KoloProfiler {
    ...
    frames: GILProtected<RefCell<HashMap<String, Vec<SerializedFrame>>>>,
...
impl KoloProfiler {
    ...
    pub fn new_from_python(py: Python, py_profiler: &Bound<'_, PyAny>) -> Result<Self, PyErr> {
        ...
        Ok(Self {
            ...
            use_threading: config.get_or(py, "use_threading", false)?,
    ...
    fn build_trace_inner(&self, py: Python) -> Result<Py<PyBytes>, PyErr> {
        ...
        // thread_frames are legacy and will be refactored shortly
        let thread_frames = self.frames.get(py).take();
        let trace_id = self.trace_id.get(py).borrow().clone();
        utils::build_trace(
            py,
            frames_of_interest,
            thread_frames,
            HashMap::new(), // No threading support in profiler
            &trace_id,
            &self.source,
    ...
    fn push_frames(
        ...
        let thread_id = utils::get_current_thread_id(py)?;
    ...
    fn push_frame_data(
        ...
        if !self.use_threading || thread_id.clone() == self.current_thread_id {
