...
/// a list of frames.
fn write_thread_frames(buf: &mut Vec<u8>, frames: HashMap<String, Vec<SerializedFrame>>) {
...
fn write_threads(
    ...
    for (thread_id, thread) in threads.iter() {
        ...
        let ident: Option<usize> = thread.getattr(py, "ident")
            .expect("Failed to get 'ident' attribute")
            .extract(py)
            .expect("Failed to extract 'ident' as Option<usize>");
        write_int_pair(buf, "ident", ident);
        
        let native_id: Option<usize> = thread.getattr(py, "native_id")
            .expect("Failed to get 'native_id' attribute")
            .extract(py)
            .expect("Failed to extract 'native_id' as Option<u64>");
...
pub fn build_trace(
    ...
    write_frames_of_interest(&mut buf, frames_of_interest);
    write_thread_frames(&mut buf, HashMap::new()); // empty frames dictionary 
