...
def default_process_config_fn(config: Dict, route_config: Optional[Dict], request: Any, acc_path: list[str]):
    if not route_config:
        return None
...
def genContextDict(
    ...
    process_config_fn: Callable[[Dict, Optional[Dict], Any, list[str]], None] = default_process_config_fn,
) -> Dict[str, Any]:
    config = deepcopy(compiled_config)
    path_segs = request.path.strip("/").split("/")
    routes = config.pop(Node.ROUTES, {})
    acc_path = []
    ...
    while path_segs and routes:
        ...
        for route, route_config in routes.copy().items():
            ...
            if matched:
                ...
                process_config_fn(config, route_config, request, acc_path.copy())
                break

        if not matched and match_rest_config:
            matched = True
            routes = match_rest_config.pop(Node.ROUTES, {})
            process_config_fn(config, match_rest_config, request, acc_path.copy())
    ...
    if matched and not path_segs:
        match_complete_config = routes.get(Node.MATCH_COMPLETE, None)
    process_config_fn(config, match_complete_config, request, acc_path)
